...
DO $$ DECLARE b uuid; BEGIN
  ...
  IF (SELECT count(*) FROM migration_batch_item WHERE batch_id=b AND domain='attendance_punch_event' AND extracted_count=1 AND valid_count=0 AND loaded_count=0 AND rejected_count=1 AND status='quarantined')<>1 OR (SELECT count(*) FROM migration_error WHERE batch_id=b AND error_code='ATTENDANCE_PUNCH_PERSON_UNMAPPED')<>1 OR (SELECT count(*) FROM migration_check WHERE batch_id=b AND check_code='T3_ATTENDANCE_PUNCH_QUARANTINE_CONSERVATION' AND passed)<>1 OR (SELECT count(*) FROM migration_rollback_point WHERE batch_id=b AND rollback_code='T3_ATTENDANCE_PUNCH_QUARANTINE')<>1 THEN RAISE EXCEPTION 'attendance quarantine rollback accounting drift'; END IF;
