...
test("committed PostgreSQL driver rejects T4/T5 and stops T1/T2 before unproved dictionary writes", async () => {
  ...
  const credentialRoot = join(sandbox, database, "credentials"), auditRoot = join(sandbox, database, "audit"), runtimeRoot = join(sandbox, database, "runtime"), etlEnvFile = join(credentialRoot, "etl.env"), sourceBackupPath = join(sandbox, "source.bak");
  ...
  const config = {
    ...
    source: { readOnly: true, sourceBackupSha256: sourceSnapshotHash, sourceBackupPath, databaseAlias: "YuzhouHR_Lab_driver01", etlEnvFile, sourceContainer: "jinhu_yuzhou_migration_lab-sqlserver-1" },
  ...
  const adapters = await createCoreT0T3Adapters(config, { commandRunner: () => assert.fail("no command may run for a rejected phase") });
  assert.throws(() => adapters.executePhase({ domain: "T4", phase: "extract" }), /CORE_FORBIDDEN_DOMAIN_REACHABLE/u);
  assert.throws(() => adapters.executePhase({ domain: "T0", phase: "extract" }), /CORE_SOURCE_RESTORE_BINDING_REQUIRED/u);
