...
test("u_assessmentvalue audits an authorized empty result and closes on empty-result audit failure", async () => {
  const empty = harness(null);
  assert.deepEqual(
    await empty.service.assessmentValueQuery(
      scope,
      actor(HR_PERMISSIONS.HR_PERFORMANCE_READ),
      query,
    ),
...
test("u_assessmentvalue rejects a direct-call actor from another tenant or park", async () => {
  for (const principal of [
    ...
    assert.deepEqual(
      await foreign.service.assessmentValueQuery(scope, principal, query),
      { items: [], total: 0, page: 2, page_size: 25 },
    );
