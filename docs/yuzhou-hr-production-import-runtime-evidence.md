...
运行镜像版本的实际采集入口见 [只读运行镜像版本观测](deployment/runtime-image-revision-observation.md)。
它核验正在运行的 API/Web 容器及不可变镜像的构建版本，仅提供运行镜像事实；
仍须独立核验当前/合并提交、生产目标、范围及有效期限，不能直接替代本节的最终运行回执。
