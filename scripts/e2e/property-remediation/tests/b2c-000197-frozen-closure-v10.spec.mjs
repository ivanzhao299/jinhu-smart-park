...
import { authoritativeFileRowsV10 } from "../track-b2c-000197-preliminary-orchestrator-v10.mjs";
import { resolveFormalExecutionClosureV10 } from "../track-b2c-000197-closure-resolver-v10.mjs";

const root = process.cwd();
const manifestPath = resolve(root,
  ".trellis/tasks/07-30-pr192-b-domain-integrations/research/b2c-000197-preliminary-v10-input-manifest-20260802.grammar");
const implementationPaths = [
  "scripts/e2e/property-remediation/track-b2c-000197-failure-cases-v10.mjs",
  "scripts/e2e/property-remediation/track-b2c-000197-preliminary-executor-v10.mjs",
  "scripts/e2e/property-remediation/track-b2c-000197-preliminary-orchestrator-v10.mjs",
  "scripts/e2e/property-remediation/track-b2c-000197-closure-resolver-v10.mjs",
];
const sha256 = (value) => createHash("sha256").update(value).digest("hex");

function rows() {
  const lines = readFileSync(manifestPath, "utf8").trimEnd().split("\n");
  assert.equal(lines.shift(), "b2c-000197-preliminary-input-manifest-v10");
...
test("v10 implementation owns complete fault capability and imports no v3-v7 implementation or authority", () => {
  ...
  const cases = failureInjectionCasesV10();
...
test("manifest is exact omission/excess/size/SHA closure with no node_modules", () => {
  ...
  const expected = authoritativeFileRowsV10();
...
test("every recursive execution dependency is represented and governance outputs avoid self-hash", () => {
  const actual = new Set(rows().map(({ path }) => path)); const closure = resolveFormalExecutionClosureV10(root);
  ...
  assert.equal(actual.has(".trellis/tasks/07-30-pr192-b-domain-integrations/research/b2c-000197-preliminary-v10-input-manifest-20260802.grammar"), false);
  assert.equal(actual.has(".trellis/tasks/07-30-pr192-b-domain-integrations/research/b2c-000197-preliminary-executor-v10-review-handoff-20260802.md"), false);
  assert.equal([...actual].some((path) => path.includes("b2c-000197-v10-complete-preflight-evidence-b2c197_prelim_20260802e")), false);
});
