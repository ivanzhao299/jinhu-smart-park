...
test("u_printassessment receives exact semantic parity credit while bs_ass_compute remains visibly pending API integration", () => {
  assert.deepEqual(
...
test("evidence drift and premature bs_ass_compute promotion fail closed", () => {
  ...
  promoted.routines[0].review = { status: "approved", evidenceSha256: "a".repeat(64) };
  assert.throws(
    () => verifyLegacyPerformanceCalculationPrintParity({ contract: promoted, repositoryRoot: root }),
    error => error instanceof LegacyPerformanceRoutineParityError
      && error.code === "PERFORMANCE_ROUTINE_COMPLETION_CREDIT_INVALID",
  );
});
