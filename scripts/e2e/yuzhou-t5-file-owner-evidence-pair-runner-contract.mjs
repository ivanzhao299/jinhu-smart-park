...
const result = (rehearsal, kind = "document") => ({ status: "CONTRACT_PASS", rehearsal, sourceSnapshotSha256: a.triple.sourceSnapshotHash, sourceRestoreReceiptSha256: a.source.sourceRestoreReceiptSha256, [kind === "document" ? "documentStageSha256" : "photoStageSha256"]: "7".repeat(64), receipts: ["load", "reload"].map(() => ({ load: { status: "succeeded", source: kind === "document" ? 1003 : 2155, loaded: kind === "document" ? 989 : 2155, quarantined: kind === "document" ? 14 : 0 }, rollback: { status: "rolled_back" } })), cleanupState: "cleaned", residualCount: 0, productionImport: "HOLD" });
...
test("T5 photo owner pair uses the same serial receipt contract", async () => {
  const output = await runT5FileOwnerEvidencePair({ kind: "photo", configAPath: aPath, configBPath: bPath, stageAPath: join(root, "photo-stage-a"), stageBPath: join(root, "photo-stage-b") }, { runner: async ({ configPath }) => result(configPath === aPath ? "A" : "B", "photo") });
  assert.equal(output.comparison.receipts[0].load.source, 2155); assert.equal(output.comparison.receipts[1].load.quarantined, 0); assert.equal(output.productionImport, "HOLD");
});
