...
permissions(code, name, resource, action, permission_type, perm_type, api_method, api_path, frontend_route, sort_no) AS (
  VALUES
    ('safety_hazard:to_emergency', '隐患转应急事件', 'biz.safety_hazard', 'to_emergency', 'api', 40, 'POST', '/api/v1/safety/hazards/:id/to-emergency', NULL, 615)
),
updated_permissions AS (
  ...
  SET park_id = seed_scope.park_id,
      ...
      is_builtin = true,
      is_deleted = false,
      remark = 'S5-B hazard to emergency permission seed',
      update_time = now()
  FROM permissions
...
inserted_permissions AS (
  ...
  SELECT
    ...
    'enabled', true, true, true, 'S5-B hazard to emergency permission seed'
  FROM permissions
  CROSS JOIN seed_scope
  WHERE NOT EXISTS (
    SELECT 1
...
role_permissions(role_code, permission_code) AS (
  VALUES
    ('SUPER_ADMIN', 'safety_hazard:to_emergency'),
    ('OPERATIONS_OWNER', 'safety_hazard:to_emergency'),
    ('SAFETY_MANAGER', 'safety_hazard:to_emergency'),
    ('PROPERTY_MANAGER', 'safety_hazard:to_emergency')
)
...
SELECT seed_scope.tenant_id,
       ...
       false,
       1,
       'S5-B hazard to emergency role permission seed'
FROM seed_scope
...
target_type AS (
  ...
  WHERE tenant_id = (SELECT tenant_id FROM seed_scope)
    ...
    AND dict_code = 'safety_hazard_status'
...
updated_items AS (
  ...
  SET item_label = dict_items.item_label,
      ...
      remark = 'S5-B hazard to emergency status dictionary seed',
      update_time = now()
  FROM dict_items
...
SELECT target_type.tenant_id,
       ...
       'enabled',
       dict_items.tag_type,
       'S5-B hazard to emergency status dictionary seed'
FROM dict_items
JOIN target_type ON true
WHERE NOT EXISTS (
  SELECT 1
...
target_type AS (
  ...
  WHERE tenant_id = (SELECT tenant_id FROM seed_scope)
    AND park_id = (SELECT park_id FROM seed_scope)
    AND dict_code = 'safety_emergency_source_type'
    AND is_deleted = false
),
dict_items(item_label, item_value, sort_order, tag_type) AS (
  VALUES
    ('重大隐患', 'hazard', 20, 'danger')
),
updated_items AS (
  UPDATE sys_dict_item existing
  SET item_label = dict_items.item_label,
      ...
      status = 'enabled',
      tag_type = dict_items.tag_type,
      remark = 'S5-B hazard emergency source dictionary seed',
      update_time = now()
  FROM dict_items
  JOIN target_type ON true
  WHERE existing.tenant_id = target_type.tenant_id
...
SELECT target_type.tenant_id,
       ...
       'enabled',
       dict_items.tag_type,
       'S5-B hazard emergency source dictionary seed'
FROM dict_items
JOIN target_type ON true
WHERE NOT EXISTS (
  SELECT 1
