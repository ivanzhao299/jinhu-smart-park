...
本阶段只做设计，不新增脚本、不修改快照脚本、不修改 baseline、不修改 seed、不修改业务代码、不接入 CI。
...
当前尚未创建这些固定样本。后续需要一个可复制、幂等、只在本地 / 测试环境手动运行的 bootstrap 方案。本阶段只设计该方案，不实现 `scripts/e2e/bootstrap-api-snapshot-data.mjs`。

## 3. 当前现状

当前没有发现 snapshot bootstrap 脚本。
...
建议后续新增 `scripts/e2e/bootstrap-api-snapshot-data.mjs`，但本阶段不实现。

bootstrap 应优先采用 API 方式，手动运行，幂等准备 `SNAPSHOT-WO-001` 和 `SNAPSHOT-UNIT-001`。初版不应修改 production seed、dev seed、baseline、CI 或 package 配置。固定样本准备完成并复核稳定后，再评估是否引入 snapshot 只读账号、手动 workflow 或 release-smoke label。
