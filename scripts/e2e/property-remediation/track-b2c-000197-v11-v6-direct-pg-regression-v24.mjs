...
const prefix = "b2c-000197-v11-v6-direct-pg-regression-v24";
const fixed = Object.freeze({
  ...
  spec: resolve(root, "scripts/e2e/property-remediation/tests/b2c-000197-v11-v6-direct-pg-regression-v24.spec.mjs"),
});
const sha = (value) => createHash("sha256").update(value).digest("hex");
const hex64 = /^[0-9a-f]{64}$/u;
const runIdPattern = /^b2c197_v11v6_direct_v24_[a-z0-9_]{8,40}$/u;
const containerPattern = /^jinhu-b2c197-v11v6-direct-v24-[a-z0-9-]{8,40}$/u;
const databasePattern = /^jinhu_b2c197_v11v6_direct_v24_[a-z0-9_]{4,30}$/u;
...
function sealed(path) {
  if (!path.startsWith(`${research}${sep}`) && path !== fixed.runner && path !== fixed.spec) throw new Error("b2c-v24-fixed-path");
  if (!existsSync(path) || lstatSync(path).isSymbolicLink() || realpathSync(path) !== path) throw new Error("b2c-v24-sealed-input");
  let descriptor;
  try { descriptor = openSync(path, constants.O_RDONLY | constants.O_NOFOLLOW); const metadata = fstatSync(descriptor); if (!metadata.isFile() || (metadata.mode & 0o777) !== 0o444) throw new Error("b2c-v24-sealed-input"); return readFileSync(descriptor); }
...
function recursiveFiles(directory) {
  ...
  for (const entry of readdirSync(directory, { withFileTypes: true })) {
    ...
    if (entry.isSymbolicLink()) throw new Error("b2c-v24-formal-source-symlink");
...
function formalSourcePlan() {
  const paths = [migration197Path, approvalGateLibPath, resolve(apiRoot, "package.json"), resolve(apiRoot, "tsconfig.json"), resolve(root, "packages/shared/package.json"),
  ...
  const plan = [...new Set(paths)].sort().map((path) => {
    if (!existsSync(path) || lstatSync(path).isSymbolicLink() || !statSync(path).isFile()) throw new Error("b2c-v24-formal-source-path");
  ...
  if (plan.length !== FORMAL_SOURCE_COUNT || sha(JSON.stringify(manifest)) !== FORMAL_SOURCE_MANIFEST_SHA256) throw new Error("b2c-v24-formal-source-manifest-drift");
  const migration197 = plan.find(({ path }) => path === migration197Path);
  if (!migration197 || migration197.raw_sha256 !== MIGRATION_000197_SHA256) throw new Error("b2c-v24-migration-000197-drift");
...
function verifyFormalSources(frozen) {
  for (const entry of frozen.plan) if (sha(readFileSync(entry.path)) !== entry.raw_sha256) throw new Error(`b2c-v24-formal-source-toctou:${entry.relative}`);
}
...
function baselinePlan() {
  ...
  if (!fixture || fixture.raw_sha256 !== FIXTURE_SHA256) throw new Error("b2c-v24-fixture-drift");
  const manifest = plan.map(({ kind, filename, raw_sha256 }) => ({ kind, filename, raw_sha256 }));
  if (plan.length !== 195 || sha(JSON.stringify(manifest)) !== BASELINE_MANIFEST_SHA256) throw new Error("b2c-v24-baseline-manifest-drift");
  return Object.freeze(plan.map(Object.freeze));
}
function sealedSource(path) { if (!existsSync(path) || lstatSync(path).isSymbolicLink() || realpathSync(path) !== path) throw new Error("b2c-v24-migration-path-drift"); let descriptor; try { descriptor = openSync(path, constants.O_RDONLY | constants.O_NOFOLLOW); const metadata = fstatSync(descriptor); if (!metadata.isFile()) throw new Error("b2c-v24-migration-path-drift"); return readFileSync(descriptor); } finally { if (descriptor !== undefined) closeSync(descriptor); } }
function migrationBytesImmediatelyBeforeExecution(entry) { const raw = sealedSource(entry.path); if (sha(raw) !== entry.raw_sha256) throw new Error(`b2c-v24-migration-byte-drift:${entry.filename}`); return raw.toString("utf8"); }

function validateRegistry(value) {
  exactObject(value, ["schema_version", "resources"], "b2c-v24-registry-schema"); exactString(value.schema_version, "b2c-v24-resource-registry-v1", "b2c-v24-registry-schema");
  if (!Array.isArray(value.resources) || value.resources.length !== 17) throw new Error("b2c-v24-registry-count");
  const labels = new Set(), identities = new Set();
  for (const entry of value.resources) { exactObject(entry, ["label", "run_id", "container", "container_id", "database", "volume"], "b2c-v24-registry-entry"); if (!/^(A|B|C|D|E|F|G|H|v4|v15|v17|v18|v19|v20|v21|v22|v23)$/u.test(entry.label) || labels.has(entry.label)) throw new Error("b2c-v24-registry-label"); labels.add(entry.label); for (const key of ["run_id", "container", "container_id", "database", "volume"]) { if (typeof entry[key] !== "string" || entry[key].trim() === "") throw new Error("b2c-v24-registry-identity"); identities.add(entry[key]); } }
  if (labels.size !== 17) throw new Error("b2c-v24-registry-labels"); return identities;
}
function staticIntake() {
  ...
  const candidate = sealedJson(fixed.candidate, candidateKeys, "b2c-v24-candidate-schema"); exactString(candidate.value.schema_version, "b2c-v24-candidate-authority-v1", "b2c-v24-candidate-schema"); exactString(candidate.value.status, "PENDING_REVIEW", "b2c-v24-candidate-status");
  for (const key of ["execution_authorized", "container_create_authorized", "container_execute_authorized", "formal_go", "docker_or_database_command_executed"]) exactBoolean(candidate.value[key], false, `b2c-v24-candidate-flag:${key}`);
  for (const key of ["runner_raw_sha256", "runner_spec_raw_sha256", "registry_raw_sha256", "static_test_record_raw_sha256", "node22_tap_raw_sha256", "node24_tap_raw_sha256", "migration_000197_raw_sha256", "formal_source_manifest_raw_sha256"]) assertHash(candidate.value[key], `b2c-v24-candidate-hash:${key}`);
  const formal = formalSourcePlan();
  const live = { runner_raw_sha256: sha(sealed(fixed.runner)), runner_spec_raw_sha256: sha(sealed(fixed.spec)), registry_raw_sha256: sha(sealed(fixed.registry)), static_test_record_raw_sha256: sha(sealed(fixed.staticRecord)), node22_tap_raw_sha256: sha(sealed(fixed.node22Tap)), node24_tap_raw_sha256: sha(sealed(fixed.node24Tap)), migration_000197_raw_sha256: formal.migration197.raw_sha256, formal_source_manifest_raw_sha256: formal.manifestSha };
  for (const [key, value] of Object.entries(live)) if (candidate.value[key] !== value) throw new Error(`b2c-v24-live-byte-drift:${key}`);
  ...
  const manifest = sealedJson(fixed.manifest, manifestKeys, "b2c-v24-manifest-schema"); exactString(manifest.value.schema_version, "b2c-v24-candidate-manifest-v1", "b2c-v24-manifest-schema"); exactString(manifest.value.status, "PENDING_REVIEW", "b2c-v24-manifest-status");
  exactObject(manifest.value.candidate_flags, ["container_create_authorized", "container_execute_authorized", "execution_authorized", "formal_go"], "b2c-v24-manifest-flags"); for (const value of Object.values(manifest.value.candidate_flags)) exactBoolean(value, false, "b2c-v24-manifest-flags");
  const expectedHashes = { candidate_authority_raw_sha256: candidate.rawSha256, ...live }; for (const [key, value] of Object.entries(expectedHashes)) if (manifest.value[key] !== value) throw new Error(`b2c-v24-manifest-binding:${key}`);
  if (!same(manifest.value.review_chain, ["candidate", "database", "qa", "security", "drain", "provision_authority", "resource_observation", "execution_authority"])) throw new Error("b2c-v24-review-chain");
  exactObject(manifest.value.canonical_paths, Object.keys(fixed).filter((key) => !["runner", "spec", "evidence"].includes(key)), "b2c-v24-canonical-paths"); for (const key of Object.keys(manifest.value.canonical_paths)) exactString(manifest.value.canonical_paths[key], fixed[key].slice(root.length + 1), `b2c-v24-canonical-path:${key}`);
  exactString(manifest.value.trust_root, "repository-fixed-path-procedural-root-v1", "b2c-v24-trust-root");
  const record = sealedJson(fixed.staticRecord, ["schema_version", "runner_raw_sha256", "test_raw_sha256", "node22", "node24", "docker_or_database_command_executed", "coverage"], "b2c-v24-static-record-schema"); exactString(record.value.schema_version, "b2c-v24-static-test-record-v1", "b2c-v24-static-record-schema"); exactBoolean(record.value.docker_or_database_command_executed, false, "b2c-v24-static-record-command");
  if (record.value.runner_raw_sha256 !== live.runner_raw_sha256 || record.value.test_raw_sha256 !== live.runner_spec_raw_sha256) throw new Error("b2c-v24-static-record-binding");
  for (const [key, tapKey] of [["node22", "node22_tap_raw_sha256"], ["node24", "node24_tap_raw_sha256"]]) { exactObject(record.value[key], ["binary", "result", "raw_tap_sha256"], "b2c-v24-static-record-node"); if (typeof record.value[key].binary !== "string" || record.value[key].result !== "pass" || record.value[key].raw_tap_sha256 !== live[tapKey]) throw new Error("b2c-v24-static-record-node"); }
  if (!Array.isArray(record.value.coverage) || record.value.coverage.some((item) => typeof item !== "string")) throw new Error("b2c-v24-static-record-coverage");
  const registry = sealedJson(fixed.registry, ["schema_version", "resources"], "b2c-v24-registry-schema"); const prohibited = validateRegistry(registry.value);
  return Object.freeze({ candidateSha: candidate.rawSha256, manifestSha: manifest.rawSha256, registrySha: registry.rawSha256, staticRecordSha: record.rawSha256, tap22Sha: live.node22_tap_raw_sha256, tap24Sha: live.node24_tap_raw_sha256, migration197Sha: live.migration_000197_raw_sha256, formalSourceManifestSha: live.formal_source_manifest_raw_sha256, formal, prohibited });
}
function reviewIntake(base) {
  const hashes = {}; let prior = {};
  const schema = { databaseReview: "b2c-v24-independent-database-review-v1", qaReview: "b2c-v24-independent-qa-review-v1", securityReview: "b2c-v24-independent-security-review-v1", drainReview: "b2c-v24-independent-old-writer-drain-review-v1" };
  for (const key of reviewOrder) {
    ...
    const review = sealedJson(fixed[key], keys, `b2c-v24-${key}-schema`); exactString(review.value.schema_version, schema[key], `b2c-v24-${key}-schema`); exactString(review.value.decision, "GO", `b2c-v24-${key}-decision`); exactBoolean(review.value.review_approved, true, `b2c-v24-${key}-approved`); exactZero(review.value.open_p0, `b2c-v24-${key}-p0`); exactZero(review.value.open_p1, `b2c-v24-${key}-p1`);
    ...
    for (const [field, value] of Object.entries(expected)) if (review.value[field] !== value) throw new Error(`b2c-v24-${key}-binding:${field}`);
...
function provisionIntake() {
  ...
  const authority = sealedJson(fixed.provisionAuthority, keys, "b2c-v24-provision-schema"); exactString(authority.value.schema_version, "b2c-v24-provision-authority-v1", "b2c-v24-provision-schema"); exactString(authority.value.status, "AUTHORIZED", "b2c-v24-provision-status"); exactBoolean(authority.value.container_create_authorized, true, "b2c-v24-provision-create"); for (const key of ["container_execute_authorized", "execution_authorized", "formal_go"]) exactBoolean(authority.value[key], false, `b2c-v24-provision-flag:${key}`);
  const bindings = { candidate_authority_raw_sha256: base.candidateSha, candidate_manifest_raw_sha256: base.manifestSha, database_review_raw_sha256: reviews.databaseReview, qa_review_raw_sha256: reviews.qaReview, security_review_raw_sha256: reviews.securityReview, drain_review_raw_sha256: reviews.drainReview }; for (const [key, value] of Object.entries(bindings)) if (authority.value[key] !== value) throw new Error(`b2c-v24-provision-binding:${key}`);
  if (!runIdPattern.test(authority.value.run_id) || !containerPattern.test(authority.value.container) || !databasePattern.test(authority.value.database)) throw new Error("b2c-v24-provision-identity"); exactString(authority.value.image_reference, "postgres:16-alpine", "b2c-v24-provision-image"); exactZero(authority.value.host_port_bindings, "b2c-v24-provision-ports"); exactBoolean(authority.value.anonymous_volume, true, "b2c-v24-provision-volume"); exactString(authority.value.mount_type, "volume", "b2c-v24-provision-mount"); exactBoolean(authority.value.mount_rw, true, "b2c-v24-provision-mount"); exactString(authority.value.mount_destination, "/var/lib/postgresql/data", "b2c-v24-provision-mount");
  for (const identity of [authority.value.run_id, authority.value.container, authority.value.database, authority.value.image_reference]) if (base.prohibited.has(identity)) throw new Error("b2c-v24-prohibited-resource-reuse");
...
function inspect(command, target, runCommand, expectedLabels) {
  ...
  if (result.error || result.signal || result.status !== 0) throw new Error(`b2c-v24-${command}-inspect-child`); const rows = String(result.stdout ?? "").trim().split("\n"), mounts = JSON.parse(rows[4] ?? "null");
  const labels = JSON.parse(rows[5] ?? "null");
  if (rows.length !== 6 || !hex64.test(rows[0] ?? "") || rows[1] !== "true" || !/^sha256:[0-9a-f]{64}$/u.test(rows[2] ?? "") || !same(JSON.parse(rows[3] ?? "null"), {}) || !Array.isArray(mounts) || mounts.length !== 1 || mounts[0]?.Type !== "volume" || !hex64.test(mounts[0]?.Name ?? "") || mounts[0]?.RW !== true || mounts[0]?.Destination !== "/var/lib/postgresql/data") throw new Error(`b2c-v24-${command}-identity-drift`);
  const authorityKeys = ["jinhu.b2c.run_id", "jinhu.b2c.provision_authority_sha256"], inheritedKey = "desktop.docker.io/wsl-distro", keys = Object.keys(labels ?? {}).sort(); if (!same(keys, authorityKeys.sort()) && !same(keys, [...authorityKeys, inheritedKey].sort())) throw new Error(`b2c-v24-${command}-labels`); if (labels["jinhu.b2c.run_id"] !== expectedLabels.run_id || labels["jinhu.b2c.provision_authority_sha256"] !== expectedLabels.provision_authority_sha256) throw new Error(`b2c-v24-${command}-labels`);
  const inherited_labels = {}; if (Object.hasOwn(labels, inheritedKey)) { if (typeof labels[inheritedKey] !== "string" || !/^[A-Za-z0-9][A-Za-z0-9._-]{0,63}$/u.test(labels[inheritedKey])) throw new Error(`b2c-v24-${command}-inherited-label`); inherited_labels[inheritedKey] = labels[inheritedKey]; } if (expectedLabels.inherited_labels !== undefined && !same(inherited_labels, expectedLabels.inherited_labels)) throw new Error(`b2c-v24-${command}-inherited-label-drift`);
  return { container_id: rows[0], image_id: rows[2].slice(7), volume_id: mounts[0].Name, inherited_labels };
}
function writeSealed(path, value) { if (!path.startsWith(`${research}${sep}`) || existsSync(path) || lstatSync(dirname(path)).isSymbolicLink() || realpathSync(dirname(path)) !== dirname(path)) throw new Error("b2c-v24-output-path"); writeFileSync(path, `${JSON.stringify(value, null, 2)}\n`, { flag: "wx", mode: 0o444 }); chmodSync(path, 0o444); }

function provisionWithV24(runCommand) {
...
function executionIntake() {
  ...
  const observation = sealedJson(fixed.observation, observationKeys, "b2c-v24-observation-schema"); exactString(observation.value.schema_version, "b2c-v24-resource-observation-v1", "b2c-v24-observation-schema"); exactString(observation.value.status, "OBSERVED", "b2c-v24-observation-status"); exactBoolean(observation.value.execution_authorized, false, "b2c-v24-observation-execute"); exactBoolean(observation.value.formal_go, false, "b2c-v24-observation-formal");
  const expectedObservation = { provision_authority_raw_sha256: intake.authoritySha, candidate_authority_raw_sha256: intake.base.candidateSha, candidate_manifest_raw_sha256: intake.base.manifestSha, run_id: intake.authority.run_id, container: intake.authority.container, database: intake.authority.database, image_reference: "postgres:16-alpine", host_port_bindings: 0, anonymous_volume: true, mount_type: "volume", mount_rw: true, mount_destination: "/var/lib/postgresql/data" }; for (const [key, value] of Object.entries(expectedObservation)) if (!Object.is(observation.value[key], value)) throw new Error(`b2c-v24-observation-binding:${key}`); exactObject(observation.value.labels, ["run_id", "provision_authority_sha256"], "b2c-v24-observation-labels"); if (observation.value.labels.run_id !== observation.value.run_id || observation.value.labels.provision_authority_sha256 !== intake.authoritySha) throw new Error("b2c-v24-observation-labels"); exactObject(observation.value.inherited_labels, Object.keys(observation.value.inherited_labels), "b2c-v24-observation-inherited-labels"); const inheritedKeys = Object.keys(observation.value.inherited_labels); if (inheritedKeys.length > 1 || (inheritedKeys.length === 1 && (inheritedKeys[0] !== "desktop.docker.io/wsl-distro" || typeof observation.value.inherited_labels[inheritedKeys[0]] !== "string" || !/^[A-Za-z0-9][A-Za-z0-9._-]{0,63}$/u.test(observation.value.inherited_labels[inheritedKeys[0]])))) throw new Error("b2c-v24-observation-inherited-labels"); for (const key of ["container_id", "volume_id", "image_id"]) assertHash(observation.value[key], `b2c-v24-observation-identity:${key}`); for (const identity of [observation.value.run_id, observation.value.container, observation.value.container_id, observation.value.database, observation.value.volume_id, observation.value.image_id]) if (intake.base.prohibited.has(identity)) throw new Error("b2c-v24-prohibited-resource-reuse");
...
export function assertSnapshotV24(value) {
  exactObject(value, ["primary_history", "mirror_history", "forbidden_primary", "forbidden_mirror", "approval_rows", "keys", "indexdef", "predicate", "catalog"], "b2c-v24-snapshot-schema");
  const late = expectedLate(); if (!same(value.primary_history, late) || !same(value.mirror_history, late)) throw new Error("b2c-v24-snapshot-history"); for (const key of ["forbidden_primary", "forbidden_mirror", "approval_rows"]) exactZero(value[key], `b2c-v24-snapshot-zero:${key}`); if (!same(value.keys, ["tenant_id", "park_id", "action_id", "source_type", "source_id", "source_expected_version"]) || value.indexdef !== OLD_INDEX_SHA || value.predicate !== OLD_PREDICATE_SHA) throw new Error("b2c-v24-snapshot-contract"); exactObject(value.catalog, ["index_exists", "build_residue"], "b2c-v24-snapshot-catalog"); exactBoolean(value.catalog.index_exists, true, "b2c-v24-snapshot-index"); exactBoolean(value.catalog.build_residue, false, "b2c-v24-snapshot-residue"); return true;
}
export function assertFinalSnapshotV24(value) {
  exactObject(value, ["primary_history", "mirror_history", "forbidden_primary", "forbidden_mirror", "approval_rows", "keys", "indexdef", "predicate", "old_predicate_matches", "catalog"], "b2c-v24-final-snapshot-schema");
  const history = expectedFinalHistory(); if (!same(value.primary_history, history) || !same(value.mirror_history, history)) throw new Error("b2c-v24-final-snapshot-history");
  for (const key of ["forbidden_primary", "forbidden_mirror", "approval_rows", "old_predicate_matches"]) exactZero(value[key], `b2c-v24-final-snapshot-zero:${key}`);
  if (!same(value.keys, ["tenant_id", "park_id", "action_id", "source_type", "source_id", "source_expected_version"]) || value.indexdef !== NEW_INDEX_SHA || value.predicate !== NEW_PREDICATE_SHA) throw new Error("b2c-v24-final-snapshot-contract");
  exactObject(value.catalog, ["index_exists", "build_residue"], "b2c-v24-final-snapshot-catalog"); exactBoolean(value.catalog.index_exists, true, "b2c-v24-final-snapshot-index"); exactBoolean(value.catalog.build_residue, false, "b2c-v24-final-snapshot-residue"); return true;
}
export function assertFaultMarkerV24(output, marker) { const pattern = /^(?:psql:<stdin>:\d+: )?ERROR:\s+P0001:\s*([^\s].*?)\s*$/u; const matches = String(output).split(/\r?\n/u).map((line) => line.match(pattern)).filter(Boolean); if (matches.length !== 1 || matches[0][1] !== marker || !/^v24-injected-(before-create|after-create|after-drop|before-rename)$/u.test(marker)) throw new Error("b2c-v24-fault-marker"); }
...
const faults = Object.freeze([
  { boundary: "before-create", injectionPoint: "after-validation", catalogState: "old-present-build-absent", prefix: "", assertion: catalogAssertion(true, false), marker: "v24-injected-before-create" },
  { boundary: "after-create", injectionPoint: "after-validation", catalogState: "old-present-build-present", prefix: createBuildIndex, assertion: catalogAssertion(true, true), marker: "v24-injected-after-create" },
  { boundary: "after-drop", injectionPoint: "immediately-after-ddl", catalogState: "old-absent-build-present", prefix: `${createBuildIndex} DROP INDEX public.uq_biz_property_approval_request_active_source;`, assertion: catalogAssertion(false, true), marker: "v24-injected-after-drop" },
  { boundary: "before-rename", injectionPoint: "after-validation", catalogState: "old-absent-build-present", prefix: `${createBuildIndex} DROP INDEX public.uq_biz_property_approval_request_active_source;`, assertion: catalogAssertion(false, true), marker: "v24-injected-before-rename" },
].map(Object.freeze));
...
function approvalPortGate(evidence, target, run) {
  const gate = createRequire(import.meta.url)(approvalGateLibPath), expectedNames = gate.APPROVAL_PORT_PG_REQUIRED_TEST_NAMES;
  if (!Array.isArray(expectedNames) || expectedNames.length !== 7) throw new Error("b2c-v24-approval-names");
  const ipResult = evidence.child("approval-container-ip", "/usr/bin/docker", ["inspect", "--format", "{{range .NetworkSettings.Networks}}{{.IPAddress}}{{end}}", target.container], "", run);
  const ip = ipResult.stdout.trim(); if (!/^[0-9a-f:.]+$/iu.test(ip)) throw new Error("b2c-v24-approval-ip");
  const approvalRunId = sha(target.run_id).slice(0, 32), env = { PATH: `${dirname(process.execPath)}:${process.env.PATH ?? ""}`, PROPERTY_APPROVAL_PORT_PG_URL: `postgresql://postgres@${ip}:5432/${target.database}`, PROPERTY_APPROVAL_PORT_PG_RUN_ID: approvalRunId, PROPERTY_APPROVAL_PORT_PG_EXTERNAL_FIXTURE: "yes" };
  const cli = (phase) => evidence.child(`approval-${phase}`, process.execPath, ["--require", "ts-node/register", approvalCliPath, phase], "", run, true, { cwd: apiRoot, env });
  const requirePhase = (phase, result) => { let parsed; try { parsed = JSON.parse(result.stdout.trim()); } catch { throw new Error(`b2c-v24-approval-${phase}`); } if (result.error || result.signal || result.status !== 0 || parsed?.phase !== phase || parsed?.runId !== approvalRunId || parsed?.status !== "pass") throw new Error(`b2c-v24-approval-${phase}`); };
  let summary, primary;
  try {
    requirePhase("probe", cli("probe")); requirePhase("setup", cli("setup"));
    const testResult = evidence.child("approval-seven-test-suite", process.execPath, ["--test", "--test-reporter=tap", "--require", "ts-node/register", approvalSpecPath], "", run, true, { cwd: apiRoot, env });
    if (testResult.error || testResult.signal || testResult.status !== 0) throw new Error("b2c-v24-approval-suite");
...
function executeWithV24(runCommand) {
  ...
  try {
    const actual = inspect("execute", target, (command, argv, options) => evidence.child("inspect", command, argv, options.input, runCommand), { ...target.labels, inherited_labels: target.inherited_labels }); if (actual.container_id !== target.container_id || actual.volume_id !== target.volume_id || actual.image_id !== target.image_id) throw new Error("b2c-v24-execute-identity-drift");
    if (!/^16[0-9]{4}$/u.test(psql(evidence, target, "postgres-16", "SHOW server_version_num;", runCommand).stdout.trim())) throw new Error("b2c-v24-version-drift");
    ...
    const sql = snapshotSql(expectedLate()), initial = JSON.parse(psql(evidence, target, "initial-snapshot", sql, runCommand).stdout.trim()); assertSnapshotV24(initial); const results = [];
    for (const fault of faults) { const before = JSON.parse(psql(evidence, target, `before-${fault.boundary}`, sql, runCommand).stdout.trim()); assertSnapshotV24(before); const failed = psql(evidence, target, `fault-${fault.boundary}`, faultSql(fault), runCommand, true); if (failed.status === 0) throw new Error(`b2c-v24-fault-success:${fault.boundary}`); assertFaultMarkerV24(`${failed.stdout}${failed.stderr}`, fault.marker); const after = JSON.parse(psql(evidence, target, `after-${fault.boundary}`, sql, runCommand).stdout.trim()); assertSnapshotV24(after); if (!same(before, after)) throw new Error(`b2c-v24-rollback-drift:${fault.boundary}`); results.push({ boundary: fault.boundary, injection_point: fault.injectionPoint, catalog_state: fault.catalogState, marker: fault.marker, sqlstate: "P0001", snapshot_exact: true }); }
    const rollbackFinal = JSON.parse(psql(evidence, target, "rollback-final-snapshot", sql, runCommand).stdout.trim()); assertSnapshotV24(rollbackFinal); if (!same(initial, rollbackFinal)) throw new Error("b2c-v24-final-drift");
    ...
    const committedSql = finalSnapshotSql(), committed = JSON.parse(psql(evidence, target, "committed-snapshot", committedSql, runCommand).stdout.trim()); assertFinalSnapshotV24(committed);
    ...
    const rerun = JSON.parse(psql(evidence, target, "rerun-snapshot", committedSql, runCommand).stdout.trim()); assertFinalSnapshotV24(rerun); if (!same(committed, rerun)) throw new Error("b2c-v24-rerun-drift");
    verifyFormalSources(intake.base.formal); const approval = approvalPortGate(evidence, target, runCommand);
    const postApproval = JSON.parse(psql(evidence, target, "post-approval-snapshot", committedSql, runCommand).stdout.trim()); assertFinalSnapshotV24(postApproval); if (!same(committed, postApproval)) throw new Error("b2c-v24-approval-residue");
    ...
    evidence.terminal("success", { direct_baseline: "fixed-fixture-plus-000185-000195-original-bytes", faults: results, migration_000197: migration, approval_port_pg: approval, final_snapshot_exact: true }, provenance); return Object.freeze({ faults: results, migration_000197: migration, approval_port_pg: approval, final_snapshot_exact: true });
  } catch (error) { evidence.terminal("failure", { failure: { code: "B2C_V24_EXECUTION_FAILED", stage: "execution", name: "ExecutionError" } }, provenance); throw error; }
}

/** Formal phase two API: no caller-controlled command, path, hash, target, run ID, or evidence option. */
export function executeV24() { return executeWithV24(spawnSync); }

/** Test-only facade. It has no path parameter and is unavailable from the canonical repository root. */
export function createIsolatedV24TestHarness(runCommand) {
  if (typeof runCommand !== "function" || !/^\/tmp\/b2c-v24-test-[^/]+$/u.test(root) || realpathSync(root) !== root || !research.startsWith(`${root}${sep}`)) throw new Error("b2c-v24-test-harness-root");
  return Object.freeze({ provision: () => provisionWithV24(runCommand), execute: () => executeWithV24(runCommand) });
}

export function candidateV24() { return Object.freeze({ status: "PENDING_REVIEW", execution_authorized: false, container_create_authorized: false, container_execute_authorized: false, formal_go: false, docker_or_database_command_executed: false, trust_root: "repository-fixed-path-procedural-root-v1", review_chain: ["candidate", "database", "qa", "security", "drain", "provision_authority", "resource_observation", "execution_authority"] }); }
