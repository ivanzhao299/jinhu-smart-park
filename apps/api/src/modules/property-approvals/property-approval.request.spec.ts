...
test("all 11 approval actions derive the selected v2 ordered 18-kind stable-line contract", () => {
  const id = (n: number) => `30000000-0000-4000-8000-${String(n).padStart(12, "0")}`;
  const cases = [
    ["homestay.bookings.cancel.request", {
      bookingId: id(1), occupancy: null, credentials: [],
      roomWaiverAmount: "1.00", cancellationFeeAmount: "2.00", currency: "CNY"
    }, [
      ["homestay.booking.cancel", `booking:${id(1)}`],
      ["homestay.ledger.waiver", `ledger:waiver:booking:${id(1)}:room-cancellation`],
      ["homestay.ledger.charge", `ledger:charge:booking:${id(1)}:cancellation-fee`]
    ]],
    ["homestay.finance.refund-or-waive.request", { lines: [
      { entryType: "refund", sourceLedgerEntryId: id(2), amount: "1.00", currency: "CNY" },
      { entryType: "waiver", sourceLedgerEntryId: id(3), amount: "2.00", currency: "CNY" }
    ] }, [
    ...
    ["housing.leases.checkout.request", { leaseId: id(6), occupancyId: null }, [
    ...
    ["housing.handovers.complete-move-out-financial.request", {
      handoverId: id(10), checkoutReceivableId: id(11), checkoutReceivableAmount: "2.00",
      currency: "CNY", deductions: [{ itemId: id(10), amount: "1.00", currency: "CNY" }]
    }, [
      ["housing.handover.complete.financial", `handover:${id(10)}`],
      ["housing.receivable.checkout", `receivable:checkout:${id(11)}`],
      ["housing.ledger.deduction", `deduction:${id(10)}`]
    ]],
    ...
    ["housing.purchases.transfer.request", {
      items: [{ purchaseItemId: id(13) }], targetReceivableId: id(16),
      aggregateDeltaAmount: "4.00", currency: "CNY"
    }, [
      ["housing.purchase.transfer", `item:${id(13)}`],
      ["housing.receivable.purchase.transfer", `receivable:purchase-transfer:${id(16)}`]
    ]],
  ...
  const authority: Record<string, [string, string, number]> = {
    "homestay.booking.cancel": ["biz_homestay_booking_action_log", "uq_homestay_action_approval_line", 2],
    ...
    "homestay.ledger.charge": ["biz_homestay_ledger_entry", "uq_homestay_ledger_approval_line", 1],
    "housing.lease.approve": ["biz_housing_lease", "biz_housing_lease_pkey", 1],
    "housing.lease.void": ["biz_housing_lease_effect_audit", "uq_housing_lease_effect_audit_approval_line", 2],
    "housing.lease.checkout": ["biz_housing_lease_effect_audit", "uq_housing_lease_effect_audit_approval_line", 2],
    ...
    "housing.handover.complete.financial": ["biz_housing_lease_effect_audit", "uq_housing_lease_effect_audit_approval_line", 3],
    "housing.receivable.checkout": ["biz_housing_receivable", "biz_housing_receivable_pkey", 1],
    "housing.ledger.deduction": ["biz_housing_ledger_entry", "uq_housing_ledger_approval_line", 1],
    "housing.purchase.lifecycle": ["biz_housing_purchase_effect_audit", "uq_housing_purchase_effect_audit_approval_line", 2],
    "housing.purchase.transfer": ["biz_housing_purchase_transfer_effect_audit", "uq_housing_purchase_transfer_effect_audit_approval_line", 2],
    "housing.receivable.purchase.transfer": ["biz_housing_receivable", "biz_housing_receivable_pkey", 1],
...
test("canonical array order rejects missing, extra, reordered and duplicate identities", () => {
  ...
  const line = (n: number) => ({
    ...
    sourceLedgerEntryId: id(n), amount: "1.00", currency: "CNY"
  });
  ...
  assert.notDeepEqual(canonicalEffectLines("homestay.finance.refund-or-waive.request", {
    lines: [line(1), line(2), { ...line(1), sourceLedgerEntryId: id(3) }]
  }), original);
  assert.notDeepEqual(canonicalEffectLines("homestay.finance.refund-or-waive.request", {
    lines: [line(2), line(1)]
  }), original);
...
test("housing checkout frozen policy requires cardinality 2 or 3 from occupancyId", () => {
  const leaseId = "30000000-0000-4000-8000-000000000001";
  const occupancyId = "30000000-0000-4000-8000-000000000002";
  const stages = [{
    stageCode: "housing-domain-approval",
    stageOrdinal: 1,
    eligibilityPolicySnapshot: {},
    eligibilityPolicyVersion: 1,
    eligibilityPolicyHash: "b".repeat(64),
  ...
  const policy = (payload: Record<string, unknown>, expectedCardinality: number) => ({
    policyId: "40000000-0000-4000-8000-000000000001",
    policyVersion: 1,
    policyHash: "a".repeat(64),
    stages,
    exclusions: [],
    effects: [frozenEffect({
      effectKind: "housing.lease.checkout",
      effectOrdinal: 0,
  ...
  const withoutOccupancy = { leaseId, occupancyId: null };
  const withOccupancy = { leaseId, occupancyId };

  assert.doesNotThrow(() => validateFrozenPolicy(
    "housing.leases.checkout.request",
    policy(withoutOccupancy, 2),
    null,
    null,
    withoutOccupancy
  ));
...
test("homestay cancellation cardinality is derived from frozen occupancy and credential snapshots", () => {
  const bookingId = "30000000-0000-4000-8000-000000000021";
  const occupancyId = "30000000-0000-4000-8000-000000000022";
  const credentialId = "30000000-0000-4000-8000-000000000023";
  const payload = {
    bookingId,
  ...
  const effect = frozenEffect({
    effectKind: "homestay.booking.cancel",
    effectOrdinal: 0,
    effectLineKey: `booking:${bookingId}`,
  ...
  const policy = {
    policyId: "40000000-0000-4000-8000-000000000021",
    policyVersion: 1,
    policyHash: "a".repeat(64),
    stages: [{
      stageCode: "homestay-cancellation-approval",
      stageOrdinal: 1,
      eligibilityPolicySnapshot: {},
      eligibilityPolicyVersion: 1,
      eligibilityPolicyHash: "b".repeat(64),
  ...
  assert.doesNotThrow(() => validateFrozenPolicy(
    "homestay.bookings.cancel.request", policy, null, null, payload
  ));
  assert.throws(() => validateFrozenPolicy(
    "homestay.bookings.cancel.request",
    { ...policy, effects: [{ ...effect, expectedCardinality: 3,
      invariantHash: canonicalEffectInvariantHash({ ...effect, expectedCardinality: 3 }, payload) }] },
    null, null, payload
  ), ConflictException);
  assert.throws(() => validateFrozenPolicy(
    "homestay.bookings.cancel.request", policy, null, null,
    { bookingId, occupancyId, credentialIds: [credentialId],
      roomWaiverAmount: "0.00", cancellationFeeAmount: "0.00" }
  ), ConflictException);
});
...
test("financial policies allow frozen refund-only, waiver-only and repeated stable lines", () => {
  ...
  const policy = (effects: ReturnType<typeof line>[]) => {
    const canonicalPayload = { lines: effects.map((effect) => ({
      ...
      sourceLedgerEntryId: effect.effectLineKey.split(":")[2],
...
test("financial canonical lines reject malformed payload money and bind each ordinal exactly", () => {
  ...
  for (const line of [
    { entryType: "refund", sourceLedgerEntryId: receivable(1), amount: 1, currency: "CNY" },
    { entryType: "refund", sourceLedgerEntryId: receivable(1), currency: "CNY" },
    { entryType: "refund", sourceLedgerEntryId: receivable(1), amount: "1.001", currency: "CNY" },
    { entryType: "refund", sourceLedgerEntryId: receivable(1), amount: "01.00", currency: "CNY" },
    { entryType: "refund", sourceLedgerEntryId: receivable(1), amount: "1.00", currency: "cny" }
  ]) assert.throws(() => canonicalEffectLines(
    "homestay.finance.refund-or-waive.request", { lines: [line] }
  ), ConflictException);

  const payload = { lines: [
    { entryType: "refund", sourceLedgerEntryId: receivable(1), amount: "6.00", currency: "CNY" },
    { entryType: "waiver", sourceLedgerEntryId: receivable(2), amount: "4.00", currency: "CNY" }
  ] };
  const definitions = [
