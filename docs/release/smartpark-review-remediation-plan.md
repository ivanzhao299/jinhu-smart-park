...
分支：`docs/smartpark-review-remediation-plan`
...
| 3 | R2 | 稳定 `infra/docker/Dockerfile.api` 的 apt 源、重试、缓存或基础镜像 | API 镜像可构建 |
...
| 注意事项 | 完整 regression 依赖 API/DB，可能写业务测试数据 |
...
| 主要任务 | 稳定 API Docker build 依赖源；增加 apt retry/cache；支持本地端口覆盖；完成 API health/login smoke |
...
| 注意事项 | 会启动 Docker、写 DB、产生日志；本地默认 5432 可能冲突 |
...
| 主要任务 | 新增 `test:unit`；接入 API `.spec.ts`；明确测试脚本语义；新增 coverage 入口和 artifact；CI verify 加 unit |
| 验收命令 | `pnpm lint`；`pnpm typecheck`；`pnpm build`；`pnpm test:unit`；`pnpm test:coverage` |
| 注意事项 | 当前 coverage 脚本不存在，需先新增后再纳入 CI |
...
| 验收命令 | `pnpm --filter @jinhu/api test:unit -- auth`；`node scripts/e2e/auth-security-smoke.mjs` |
...
| 验收命令 | `pnpm --filter @jinhu/api test:unit -- module data-scope field-policy`；`node scripts/e2e/permission-data-scope-matrix.mjs` |
| 注意事项 | 需要与首发菜单、模块、权限、seed contract 对齐 |
...
| 验收命令 | `pnpm --filter @jinhu/api test:unit -- money idempotency leasing`；`node scripts/e2e/financial-concurrency-smoke.mjs`；`node scripts/e2e/first-release-leasing.mjs` |
...
| 验收命令 | `pnpm --filter @jinhu/api test:unit -- files`；`node scripts/e2e/files-security-smoke.mjs`；`node scripts/e2e/first-release-files.mjs` |
...
| 验收命令 | `pnpm --filter @jinhu/api test:unit -- iot websocket external-http`；`node scripts/e2e/iot-security-smoke.mjs`；`node scripts/e2e/robots-external-call-security-smoke.mjs` |
...
| coverage | PR / main，可先非阻断 | coverage artifact | 生成 coverage 文件 |
...
3. 新增 `test:coverage` 并上传 artifact。
