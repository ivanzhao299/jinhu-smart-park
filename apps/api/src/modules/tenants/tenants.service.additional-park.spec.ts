...
test("new tenant and additional park provisioning copy dictionary baselines", () => {
  ...
  assert.match(migration, /tenant\.expire_time IS NULL OR tenant\.expire_time > now\(\)/);
  ...
  assert.match(productionSeed, /tenant\.expire_time IS NULL OR tenant\.expire_time > now\(\)/);
