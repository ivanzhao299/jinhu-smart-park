...
const database = "jinhu_hr_migration_lab_core_photoownertest", project = join(root, database), runtime = join(project, "runtime"), credentials = join(project, "credentials");
...
await assert.rejects(() => runT5PhotoOwnerEvidenceLab({ configPath, photoOwnerStage: stage }, { coreRunner, spawn: recoverySpawn, head: () => head }), /T5_FILE_CHILD_FAILED/);
