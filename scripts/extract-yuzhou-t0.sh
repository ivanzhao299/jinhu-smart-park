...
query_json departments.raw.json "SET NOCOUNT ON; SELECT department AS legacyCode,departmentname AS orgName,rating,master AS legacyManagerValue,defpersons AS plannedHeadcount,tel AS contactPhone,myorder AS sortOrder,id AS legacySourceId FROM dbo.departmentcode ORDER BY department FOR JSON PATH,INCLUDE_NULL_VALUES;"
query_json positions.raw.json "SET NOCOUNT ON; SELECT job AS legacyCode,jobname AS positionName,department AS departmentCode,parentjob AS parentPositionCode,upto AS legacyUptoCode,jobgrade,salarygrade,authority,qualify AS qualification,respos AS responsibilities,defpersons AS headcountLimit,manual AS positionManual,rating,myorder AS sortOrder,id AS legacySourceId FROM dbo.job ORDER BY job FOR JSON PATH,INCLUDE_NULL_VALUES;"
