...
export interface HrPerformanceLegacyTemplate {id:string;sourceAssessment:number;sourceAssessmentName:string|null;sourceDepartment:string|null;sourceMPercent:number|null;sourceTPercent:number|null;sourceXPercent:number|null;sourceCPercent:number|null;sourceSPercent:number|null;sourceTimekeep:boolean|null;sourceBonus:boolean|null;sourceMaster:boolean|null;targetTemplateId:string|null;targetTemplateVersionId:string|null;}
export interface HrPerformanceLegacyLevel {id:string;sourceAssGrade:string|null;sourceDescription:string|null;sourceMyOrder:number|null;sourceAssessmentId:number|null;sourceMinValue:number|null;sourceMaxValue:number|null;legacyTemplateProfileId:string|null;targetTemplateVersionId:string|null;targetLevelId:string|null;}
export interface HrPerformanceLegacyDimension {id:string;sourceItemId:number;sourceAssessmentId:number|null;sourceItemName:string|null;sourceFullValue:string|null;sourceMyOrder:number|null;legacyTemplateProfileId:string|null;targetTemplateVersionId:string|null;targetDimensionId:string|null;}
export interface HrPerformanceLegacyGuide {id:string;sourceGuideId:number;sourceItemId:number|null;sourceGrade:string|null;sourceDescription:string|null;sourceMinValue:number|null;sourceMaxValue:number|null;sourceMyOrder:number|null;legacyDimensionProfileId:string|null;legacyLevelRuleId:string|null;targetTemplateVersionId:string|null;targetDimensionId:string|null;targetLevelId:string|null;}
export interface HrPerformanceLegacyResult {id:string;sourceDetailId:number;sourceSessionId:number|null;sourcePersonCode:string|null;sourceItemId:number|null;sourceSelfValue:string|null;sourceMItemValue:string|null;sourceItemValue:string|null;sourceXItemValue:string|null;sourceCItemValue:string|null;sourceSelfGrade:string|null;sourceAssGrade:string|null;sourceAppraisal:string|null;legacyDimensionProfileId:string|null;targetCycleEmployeeId:string|null;targetTemplateVersionId:string|null;targetDimensionId:string|null;}
export interface HrPerformanceLegacyRubric {sourceAssessmentId:number;levels:Array<{sourceAssGrade:string;sourceDescription:string|null;sourceMyOrder:string|null;sourceMinValue:number|null;sourceMaxValue:number|null}>;items:Array<{sourceItemId:number;sourceItemName:string|null;sourceFullValue:string|null;sourceMyOrder:number|null;descriptions:Record<string,string|null>}>;}
export interface HrPerformanceLegacyMaster {id:string;sourceMasterId:number;sourceSessionId:number|null;sourcePersonCode:string|null;sourceSelfGrade:string|null;sourceAssGrade:string|null;sourceSelfValue:string|null;sourceItemValue:string|null;sourceMItemValue:string|null;sourceXItemValue:string|null;sourceCItemValue:string|null;sourceMasterValue:string|null;sourceTimekeepValue:string|null;sourceBonusValue:string|null;sourceTotalValue:string|null;sourceSelfAppraisal:string|null;sourceAppraisal:string|null;sourcePay:string|null;sourceAssessmentPerson:string|null;sourceRecordedAt:string|null;sourceOperatorCode:string|null;sourceDescription:string|null;legacyTemplateProfileId:string|null;targetCycleEmployeeId:string|null;targetTemplateVersionId:string|null;calculatedTotal:string|null;expectedAssGrade:string|null;winningMinValue:number|null;winningCandidateCount:number;parityStatus:"MATCH"|"MISMATCH"|"TOTAL_UNAVAILABLE"|"NO_ELIGIBLE_GRADE"|"AMBIGUOUS_TOP_THRESHOLD";}
...
export interface HrPayrollCatalogItem {id:string;bookId:string;itemCode:string;displayName:string;valueType:string;itemCategory:string;decimalScale:number|null;sortNo:number;taxable:boolean|null;printEnabled:boolean|null;enabled:boolean;legacyPrintWidth:number|null;legacyDecimalLength:number|null;legacyItemTitle:string|null;legacyLongDescription:string|null;suppressDecimals:boolean|null;legacyMetadataReviewRequired:boolean;}
...
export interface HrAttendanceRequest {id:string;requestNo:string;requestType:string;startAt:string|null;endAt:string|null;attendanceDate:string|null;durationMinutes:number;legacyDeclaredDays:number|null;leavePlannedMinutes:number;leaveEffectiveMinutes:number;leaveDayCount:number;reason:string;status:string;submittedAt:string|null;reviewedAt:string|null;reviewComment:string|null;organizationName:string|null;isSelf:boolean;employeeId?:string;employeeCode?:string;employeeName?:string;}
...
export interface HrInsurancePeriod {id:string;employeeId?:string;employeeCode?:string;employeeName?:string;periodYear:number;periodMonth:number;needsReview:boolean;reviewReasonCode:string|null;legacyCompatibility?:{legacyFlags?:Record<string,string|number|boolean|null>;fieldPresence?:Record<string,boolean>};employeeAmount?:string;supplementAmount?:string;itemCount:number;employerAmount?:string;totalAmount?:string;items?:HrInsuranceItem[];}
...
export const hrApi={
 ...
 ,contracts:(token?:string,page=1,pageSize=20,filters:HrContractListFilters={},selfOnly=false,signal?:AbortSignal)=>{const query=new URLSearchParams({page:String(page),page_size:String(pageSize)});if(filters.employeeId)query.set("employee_id",filters.employeeId);if(filters.keyword)query.set("keyword",filters.keyword);if(filters.status)query.set("status",filters.status);if(filters.expiryFrom)query.set("expiry_from",filters.expiryFrom);if(filters.expiryTo)query.set("expiry_to",filters.expiryTo);return unwrap(apiRequest<PaginatedResult<HrContract>>(`/hr/contracts${selfOnly?"/me":""}?${query.toString()}`,{token,signal}));}
 ...
 ,attendanceRequest:(id:string,token?:string)=>unwrap(apiRequest<HrAttendanceRequest>(`/hr/attendance/requests/${id}`,{token}))
 ...
 ,performanceLegacyTemplates:(token?:string,page=1,pageSize=20,signal?:AbortSignal)=>unwrap(apiRequest<PaginatedResult<HrPerformanceLegacyTemplate>>(`/hr/performance-legacy/templates?page=${page}&page_size=${pageSize}`,{token,signal}))
 ,performanceLegacyLevels:(token?:string,page=1,pageSize=20,signal?:AbortSignal)=>unwrap(apiRequest<PaginatedResult<HrPerformanceLegacyLevel>>(`/hr/performance-legacy/levels?page=${page}&page_size=${pageSize}`,{token,signal}))
 ,performanceLegacyDimensions:(token?:string,page=1,pageSize=20,signal?:AbortSignal)=>unwrap(apiRequest<PaginatedResult<HrPerformanceLegacyDimension>>(`/hr/performance-legacy/dimensions?page=${page}&page_size=${pageSize}`,{token,signal}))
 ,performanceLegacyGuides:(token?:string,page=1,pageSize=20,signal?:AbortSignal)=>unwrap(apiRequest<PaginatedResult<HrPerformanceLegacyGuide>>(`/hr/performance-legacy/guides?page=${page}&page_size=${pageSize}`,{token,signal}))
 ,performanceLegacyRubric:(sourceAssessmentId:number,token?:string,signal?:AbortSignal)=>unwrap(apiRequest<HrPerformanceLegacyRubric>(`/hr/performance-legacy/rubric?source_assessment_id=${sourceAssessmentId}`,{token,signal}))
 ,performanceLegacyResults:(token?:string,page=1,pageSize=20,sourceSessionId?:number,signal?:AbortSignal)=>{const query=new URLSearchParams({page:String(page),page_size:String(pageSize)});if(sourceSessionId!==undefined)query.set("source_session_id",String(sourceSessionId));return unwrap(apiRequest<PaginatedResult<HrPerformanceLegacyResult>>(`/hr/performance-legacy/results?${query.toString()}`,{token,signal}));}
 ,performanceLegacyMasters:(token?:string,page=1,pageSize=20,sourceSessionId?:number,signal?:AbortSignal)=>{const query=new URLSearchParams({page:String(page),page_size:String(pageSize)});if(sourceSessionId!==undefined)query.set("source_session_id",String(sourceSessionId));return unwrap(apiRequest<PaginatedResult<HrPerformanceLegacyMaster>>(`/hr/performance-legacy/masters?${query.toString()}`,{token,signal}));}
 ...
 ,payrollHistoryTaxRules:(token?:string,page=1,pageSize=20,signal?:AbortSignal)=>unwrap(apiRequest<PaginatedResult<HrPayrollTaxRule>>(`/hr/payroll/history-tax-rules?page=${page}&page_size=${pageSize}`,{token,signal}))
