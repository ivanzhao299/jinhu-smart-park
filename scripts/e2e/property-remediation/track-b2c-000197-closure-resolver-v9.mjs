...
export const FORMAL_CHILD_ENTRIES_V9 = Object.freeze([
  "scripts/e2e/property-remediation/tests/b2c-000197-preliminary-executor-v9.spec.mjs",
  "scripts/e2e/property-remediation/tests/b2c-000197-preliminary-orchestrator-v9.spec.mjs",
  "scripts/e2e/property-remediation/tests/b2c-000197-capability-closure-v9.spec.mjs",
  "scripts/e2e/property-remediation/tests/b2c-000197-recursive-closure-v9.spec.mjs",
  "scripts/e2e/property-remediation/tests/b2c-000197-frozen-closure-v9.spec.mjs",
...
export const TYPECHECK_GOVERNANCE_FILES_V9 = Object.freeze([
...
function tsconfig(root) {
  ...
  if (loaded.error) throw new Error(`v9-tsconfig-read:${JSON.stringify(loaded.error)}`);
  const parsed = ts.parseJsonConfigFileContent(loaded.config, ts.sys, dirname(path), { noEmit: true }, path);
  if (parsed.errors.length) throw new Error(`v9-tsconfig-parse:${JSON.stringify(parsed.errors)}`);
  return parsed;
}

export function compilerRepositoryFilesV9(rootInput = process.cwd()) {
...
export function parseTscListFilesV9(stdout, rootInput = process.cwd()) {
...
export function classifySpecifierV9(importerInput, specifier, rootInput = process.cwd()) {
...
export function resolveFormalExecutionClosureV9(rootInput = process.cwd()) {
  const root = realpathSync(resolve(rootInput)); const parsed = tsconfig(root);
  const repository = new Set([...compilerRepositoryFilesV9(root), ...FORMAL_CHILD_ENTRIES_V9,
    ...TYPECHECK_GOVERNANCE_FILES_V9].map((path) => normalize(root, realpathSync(resolve(root, path)))));
  ...
  if (unresolved.length) {
    throw new Error(`b2c-000197-v9-unresolved-repository-specifiers:${JSON.stringify(unresolved)}`);
  }
  ...
  return Object.freeze({ repositoryFiles: Object.freeze([...repository].sort()),
    compilerFiles: compilerRepositoryFilesV9(root), builtin: Object.freeze(classify(builtin)),
    external: Object.freeze(classify(external)), unresolved: Object.freeze([]) });
}

export function assertTscFileListMatchesV9(stdout, rootInput = process.cwd()) {
  const compiler = compilerRepositoryFilesV9(rootInput); const actual = parseTscListFilesV9(stdout, rootInput);
  if (JSON.stringify(compiler) !== JSON.stringify(actual)) {
    const expected = new Set(compiler); const observed = new Set(actual);
    throw new Error(`b2c-000197-v9-tsc-file-list-drift:${JSON.stringify({
