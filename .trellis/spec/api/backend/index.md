...
- An inactive park retains an enabled `system` assignment plus only `park:read` and `park:update` as recovery capabilities, even when the selected plan/module set omitted `system`. Park list/detail/update use the system module; park create/delete and all building/floor/unit/property operations remain asset-gated.
...
- Tenant reactivation is transactional: before the enabled status commits, every active park with a currently enabled and time-valid asset assignment runs the same projection/runtime-control provisioning primitive. Dormant scopes intentionally skipped by production seed therefore cannot become active in a partial state.
