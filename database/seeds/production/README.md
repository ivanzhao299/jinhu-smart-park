...
- The reviewed `DEPARTMENT_MANAGER` HR role receives the `hr:employees` page plus the atomic `hr:employee:team_read` and masked `hr:employee_profile:team_read` permissions; employee rows remain constrained by the API managed-organization-tree policy and the seed does not grant broad employee/profile, compensation, or payroll read permissions.
