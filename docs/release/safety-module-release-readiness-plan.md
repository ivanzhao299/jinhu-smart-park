...
- 巡检任务列表；`/safety/my-inspect-tasks` 仅作为 API 兼容观察项，菜单入口仍暂缓。
- 开始、打卡、提交巡检结果。
- 异常巡检结果自动创建隐患。
- 人工创建隐患。
...
- 隐患转工单，并验证重复转工单。
- 重大隐患转应急，并验证重复转应急不会重复生成应急事件。
- 超期重算、超期列表、重大隐患升级。
- 安全统计接口。
- 无安全权限账号访问巡检点、隐患、统计等关键 API 被拒绝。
- 最小跨 `tenant_id` / `park_id` 隔离验证：通过 `safety-smoke` 标识的临时异 scope 巡检点和隐患，验证当前登录 scope 不可读取、不可修改，并在脚本结束时软删除临时行。
