...
const prefix = "b2c-000197-v11-v6-direct-pg-regression-v22";
const fixed = Object.freeze({
  ...
  spec: resolve(root, "scripts/e2e/property-remediation/tests/b2c-000197-v11-v6-direct-pg-regression-v22.spec.mjs"),
});
const sha = (value) => createHash("sha256").update(value).digest("hex");
const hex64 = /^[0-9a-f]{64}$/u;
const runIdPattern = /^b2c197_v11v6_direct_v22_[a-z0-9_]{8,40}$/u;
const containerPattern = /^jinhu-b2c197-v11v6-direct-v22-[a-z0-9-]{8,40}$/u;
const databasePattern = /^jinhu_b2c197_v11v6_direct_v22_[a-z0-9_]{4,30}$/u;
...
function sealed(path) {
  if (!path.startsWith(`${research}${sep}`) && path !== fixed.runner && path !== fixed.spec) throw new Error("b2c-v22-fixed-path");
  if (!existsSync(path) || lstatSync(path).isSymbolicLink() || realpathSync(path) !== path) throw new Error("b2c-v22-sealed-input");
  let descriptor;
  try { descriptor = openSync(path, constants.O_RDONLY | constants.O_NOFOLLOW); const metadata = fstatSync(descriptor); if (!metadata.isFile() || (metadata.mode & 0o777) !== 0o444) throw new Error("b2c-v22-sealed-input"); return readFileSync(descriptor); }
...
function baselinePlan() {
  ...
  if (!fixture || fixture.raw_sha256 !== FIXTURE_SHA256) throw new Error("b2c-v22-fixture-drift");
  const manifest = plan.map(({ kind, filename, raw_sha256 }) => ({ kind, filename, raw_sha256 }));
  if (plan.length !== 195 || sha(JSON.stringify(manifest)) !== BASELINE_MANIFEST_SHA256) throw new Error("b2c-v22-baseline-manifest-drift");
  return Object.freeze(plan.map(Object.freeze));
}
function sealedSource(path) { if (!existsSync(path) || lstatSync(path).isSymbolicLink() || realpathSync(path) !== path) throw new Error("b2c-v22-migration-path-drift"); let descriptor; try { descriptor = openSync(path, constants.O_RDONLY | constants.O_NOFOLLOW); const metadata = fstatSync(descriptor); if (!metadata.isFile()) throw new Error("b2c-v22-migration-path-drift"); return readFileSync(descriptor); } finally { if (descriptor !== undefined) closeSync(descriptor); } }
function migrationBytesImmediatelyBeforeExecution(entry) { const raw = sealedSource(entry.path); if (sha(raw) !== entry.raw_sha256) throw new Error(`b2c-v22-migration-byte-drift:${entry.filename}`); return raw.toString("utf8"); }

function validateRegistry(value) {
  exactObject(value, ["schema_version", "resources"], "b2c-v22-registry-schema"); exactString(value.schema_version, "b2c-v22-resource-registry-v1", "b2c-v22-registry-schema");
  if (!Array.isArray(value.resources) || value.resources.length !== 15) throw new Error("b2c-v22-registry-count");
  const labels = new Set(), identities = new Set();
  for (const entry of value.resources) { exactObject(entry, ["label", "run_id", "container", "container_id", "database", "volume"], "b2c-v22-registry-entry"); if (!/^(A|B|C|D|E|F|G|H|v4|v15|v17|v18|v19|v20|v21)$/u.test(entry.label) || labels.has(entry.label)) throw new Error("b2c-v22-registry-label"); labels.add(entry.label); for (const key of ["run_id", "container", "container_id", "database", "volume"]) { if (typeof entry[key] !== "string" || entry[key].trim() === "") throw new Error("b2c-v22-registry-identity"); identities.add(entry[key]); } }
  if (labels.size !== 15) throw new Error("b2c-v22-registry-labels"); return identities;
}
function staticIntake() {
  ...
  const candidate = sealedJson(fixed.candidate, candidateKeys, "b2c-v22-candidate-schema"); exactString(candidate.value.schema_version, "b2c-v22-candidate-authority-v1", "b2c-v22-candidate-schema"); exactString(candidate.value.status, "PENDING_REVIEW", "b2c-v22-candidate-status");
  for (const key of ["execution_authorized", "container_create_authorized", "container_execute_authorized", "formal_go", "docker_or_database_command_executed"]) exactBoolean(candidate.value[key], false, `b2c-v22-candidate-flag:${key}`);
  for (const key of ["runner_raw_sha256", "runner_spec_raw_sha256", "registry_raw_sha256", "static_test_record_raw_sha256", "node22_tap_raw_sha256", "node24_tap_raw_sha256"]) assertHash(candidate.value[key], `b2c-v22-candidate-hash:${key}`);
  ...
  for (const [key, value] of Object.entries(live)) if (candidate.value[key] !== value) throw new Error(`b2c-v22-live-byte-drift:${key}`);
  ...
  const manifest = sealedJson(fixed.manifest, manifestKeys, "b2c-v22-manifest-schema"); exactString(manifest.value.schema_version, "b2c-v22-candidate-manifest-v1", "b2c-v22-manifest-schema"); exactString(manifest.value.status, "PENDING_REVIEW", "b2c-v22-manifest-status");
  exactObject(manifest.value.candidate_flags, ["container_create_authorized", "container_execute_authorized", "execution_authorized", "formal_go"], "b2c-v22-manifest-flags"); for (const value of Object.values(manifest.value.candidate_flags)) exactBoolean(value, false, "b2c-v22-manifest-flags");
  const expectedHashes = { candidate_authority_raw_sha256: candidate.rawSha256, ...live }; for (const [key, value] of Object.entries(expectedHashes)) if (manifest.value[key] !== value) throw new Error(`b2c-v22-manifest-binding:${key}`);
  if (!same(manifest.value.review_chain, ["candidate", "database", "qa", "security", "drain", "provision_authority", "resource_observation", "execution_authority"])) throw new Error("b2c-v22-review-chain");
  exactObject(manifest.value.canonical_paths, Object.keys(fixed).filter((key) => !["runner", "spec", "evidence"].includes(key)), "b2c-v22-canonical-paths"); for (const key of Object.keys(manifest.value.canonical_paths)) exactString(manifest.value.canonical_paths[key], fixed[key].slice(root.length + 1), `b2c-v22-canonical-path:${key}`);
  exactString(manifest.value.trust_root, "repository-fixed-path-procedural-root-v1", "b2c-v22-trust-root");
  const record = sealedJson(fixed.staticRecord, ["schema_version", "runner_raw_sha256", "test_raw_sha256", "node22", "node24", "docker_or_database_command_executed", "coverage"], "b2c-v22-static-record-schema"); exactString(record.value.schema_version, "b2c-v22-static-test-record-v1", "b2c-v22-static-record-schema"); exactBoolean(record.value.docker_or_database_command_executed, false, "b2c-v22-static-record-command");
  if (record.value.runner_raw_sha256 !== live.runner_raw_sha256 || record.value.test_raw_sha256 !== live.runner_spec_raw_sha256) throw new Error("b2c-v22-static-record-binding");
  for (const [key, tapKey] of [["node22", "node22_tap_raw_sha256"], ["node24", "node24_tap_raw_sha256"]]) { exactObject(record.value[key], ["binary", "result", "raw_tap_sha256"], "b2c-v22-static-record-node"); if (typeof record.value[key].binary !== "string" || record.value[key].result !== "pass" || record.value[key].raw_tap_sha256 !== live[tapKey]) throw new Error("b2c-v22-static-record-node"); }
  if (!Array.isArray(record.value.coverage) || record.value.coverage.some((item) => typeof item !== "string")) throw new Error("b2c-v22-static-record-coverage");
  const registry = sealedJson(fixed.registry, ["schema_version", "resources"], "b2c-v22-registry-schema"); const prohibited = validateRegistry(registry.value);
...
function reviewIntake(base) {
  ...
  const schema = { databaseReview: "b2c-v22-independent-database-review-v1", qaReview: "b2c-v22-independent-qa-review-v1", securityReview: "b2c-v22-independent-security-review-v1", drainReview: "b2c-v22-independent-old-writer-drain-review-v1" };
  for (const key of reviewOrder) {
    ...
    const review = sealedJson(fixed[key], keys, `b2c-v22-${key}-schema`); exactString(review.value.schema_version, schema[key], `b2c-v22-${key}-schema`); exactString(review.value.decision, "GO", `b2c-v22-${key}-decision`); exactBoolean(review.value.review_approved, true, `b2c-v22-${key}-approved`); exactZero(review.value.open_p0, `b2c-v22-${key}-p0`); exactZero(review.value.open_p1, `b2c-v22-${key}-p1`);
    ...
    for (const [field, value] of Object.entries(expected)) if (review.value[field] !== value) throw new Error(`b2c-v22-${key}-binding:${field}`);
...
function provisionIntake() {
  ...
  const authority = sealedJson(fixed.provisionAuthority, keys, "b2c-v22-provision-schema"); exactString(authority.value.schema_version, "b2c-v22-provision-authority-v1", "b2c-v22-provision-schema"); exactString(authority.value.status, "AUTHORIZED", "b2c-v22-provision-status"); exactBoolean(authority.value.container_create_authorized, true, "b2c-v22-provision-create"); for (const key of ["container_execute_authorized", "execution_authorized", "formal_go"]) exactBoolean(authority.value[key], false, `b2c-v22-provision-flag:${key}`);
  const bindings = { candidate_authority_raw_sha256: base.candidateSha, candidate_manifest_raw_sha256: base.manifestSha, database_review_raw_sha256: reviews.databaseReview, qa_review_raw_sha256: reviews.qaReview, security_review_raw_sha256: reviews.securityReview, drain_review_raw_sha256: reviews.drainReview }; for (const [key, value] of Object.entries(bindings)) if (authority.value[key] !== value) throw new Error(`b2c-v22-provision-binding:${key}`);
  if (!runIdPattern.test(authority.value.run_id) || !containerPattern.test(authority.value.container) || !databasePattern.test(authority.value.database)) throw new Error("b2c-v22-provision-identity"); exactString(authority.value.image_reference, "postgres:16-alpine", "b2c-v22-provision-image"); exactZero(authority.value.host_port_bindings, "b2c-v22-provision-ports"); exactBoolean(authority.value.anonymous_volume, true, "b2c-v22-provision-volume"); exactString(authority.value.mount_type, "volume", "b2c-v22-provision-mount"); exactBoolean(authority.value.mount_rw, true, "b2c-v22-provision-mount"); exactString(authority.value.mount_destination, "/var/lib/postgresql/data", "b2c-v22-provision-mount");
  for (const identity of [authority.value.run_id, authority.value.container, authority.value.database, authority.value.image_reference]) if (base.prohibited.has(identity)) throw new Error("b2c-v22-prohibited-resource-reuse");
...
function inspect(command, target, runCommand, expectedLabels) {
  ...
  if (result.error || result.signal || result.status !== 0) throw new Error(`b2c-v22-${command}-inspect-child`); const rows = String(result.stdout ?? "").trim().split("\n"), mounts = JSON.parse(rows[4] ?? "null");
  const labels = JSON.parse(rows[5] ?? "null");
  if (rows.length !== 6 || !hex64.test(rows[0] ?? "") || rows[1] !== "true" || !/^sha256:[0-9a-f]{64}$/u.test(rows[2] ?? "") || !same(JSON.parse(rows[3] ?? "null"), {}) || !Array.isArray(mounts) || mounts.length !== 1 || mounts[0]?.Type !== "volume" || !hex64.test(mounts[0]?.Name ?? "") || mounts[0]?.RW !== true || mounts[0]?.Destination !== "/var/lib/postgresql/data") throw new Error(`b2c-v22-${command}-identity-drift`);
  const authorityKeys = ["jinhu.b2c.run_id", "jinhu.b2c.provision_authority_sha256"], inheritedKey = "desktop.docker.io/wsl-distro", keys = Object.keys(labels ?? {}).sort(); if (!same(keys, authorityKeys.sort()) && !same(keys, [...authorityKeys, inheritedKey].sort())) throw new Error(`b2c-v22-${command}-labels`); if (labels["jinhu.b2c.run_id"] !== expectedLabels.run_id || labels["jinhu.b2c.provision_authority_sha256"] !== expectedLabels.provision_authority_sha256) throw new Error(`b2c-v22-${command}-labels`);
  const inherited_labels = {}; if (Object.hasOwn(labels, inheritedKey)) { if (typeof labels[inheritedKey] !== "string" || !/^[A-Za-z0-9][A-Za-z0-9._-]{0,63}$/u.test(labels[inheritedKey])) throw new Error(`b2c-v22-${command}-inherited-label`); inherited_labels[inheritedKey] = labels[inheritedKey]; } if (expectedLabels.inherited_labels !== undefined && !same(inherited_labels, expectedLabels.inherited_labels)) throw new Error(`b2c-v22-${command}-inherited-label-drift`);
  return { container_id: rows[0], image_id: rows[2].slice(7), volume_id: mounts[0].Name, inherited_labels };
}
function writeSealed(path, value) { if (!path.startsWith(`${research}${sep}`) || existsSync(path) || lstatSync(dirname(path)).isSymbolicLink() || realpathSync(dirname(path)) !== dirname(path)) throw new Error("b2c-v22-output-path"); writeFileSync(path, `${JSON.stringify(value, null, 2)}\n`, { flag: "wx", mode: 0o444 }); chmodSync(path, 0o444); }

function provisionWithV22(runCommand) {
...
function executionIntake() {
  ...
  const observation = sealedJson(fixed.observation, observationKeys, "b2c-v22-observation-schema"); exactString(observation.value.schema_version, "b2c-v22-resource-observation-v1", "b2c-v22-observation-schema"); exactString(observation.value.status, "OBSERVED", "b2c-v22-observation-status"); exactBoolean(observation.value.execution_authorized, false, "b2c-v22-observation-execute"); exactBoolean(observation.value.formal_go, false, "b2c-v22-observation-formal");
  const expectedObservation = { provision_authority_raw_sha256: intake.authoritySha, candidate_authority_raw_sha256: intake.base.candidateSha, candidate_manifest_raw_sha256: intake.base.manifestSha, run_id: intake.authority.run_id, container: intake.authority.container, database: intake.authority.database, image_reference: "postgres:16-alpine", host_port_bindings: 0, anonymous_volume: true, mount_type: "volume", mount_rw: true, mount_destination: "/var/lib/postgresql/data" }; for (const [key, value] of Object.entries(expectedObservation)) if (!Object.is(observation.value[key], value)) throw new Error(`b2c-v22-observation-binding:${key}`); exactObject(observation.value.labels, ["run_id", "provision_authority_sha256"], "b2c-v22-observation-labels"); if (observation.value.labels.run_id !== observation.value.run_id || observation.value.labels.provision_authority_sha256 !== intake.authoritySha) throw new Error("b2c-v22-observation-labels"); exactObject(observation.value.inherited_labels, Object.keys(observation.value.inherited_labels), "b2c-v22-observation-inherited-labels"); const inheritedKeys = Object.keys(observation.value.inherited_labels); if (inheritedKeys.length > 1 || (inheritedKeys.length === 1 && (inheritedKeys[0] !== "desktop.docker.io/wsl-distro" || typeof observation.value.inherited_labels[inheritedKeys[0]] !== "string" || !/^[A-Za-z0-9][A-Za-z0-9._-]{0,63}$/u.test(observation.value.inherited_labels[inheritedKeys[0]])))) throw new Error("b2c-v22-observation-inherited-labels"); for (const key of ["container_id", "volume_id", "image_id"]) assertHash(observation.value[key], `b2c-v22-observation-identity:${key}`); for (const identity of [observation.value.run_id, observation.value.container, observation.value.container_id, observation.value.database, observation.value.volume_id, observation.value.image_id]) if (intake.base.prohibited.has(identity)) throw new Error("b2c-v22-prohibited-resource-reuse");
...
export function assertSnapshotV22(value) {
  exactObject(value, ["primary_history", "mirror_history", "forbidden_primary", "forbidden_mirror", "approval_rows", "keys", "indexdef", "predicate", "catalog"], "b2c-v22-snapshot-schema");
  const late = expectedLate(); if (!same(value.primary_history, late) || !same(value.mirror_history, late)) throw new Error("b2c-v22-snapshot-history"); for (const key of ["forbidden_primary", "forbidden_mirror", "approval_rows"]) exactZero(value[key], `b2c-v22-snapshot-zero:${key}`); if (!same(value.keys, ["tenant_id", "park_id", "action_id", "source_type", "source_id", "source_expected_version"]) || value.indexdef !== OLD_INDEX_SHA || value.predicate !== OLD_PREDICATE_SHA) throw new Error("b2c-v22-snapshot-contract"); exactObject(value.catalog, ["index_exists", "build_residue"], "b2c-v22-snapshot-catalog"); exactBoolean(value.catalog.index_exists, true, "b2c-v22-snapshot-index"); exactBoolean(value.catalog.build_residue, false, "b2c-v22-snapshot-residue"); return true;
}
export function assertFaultMarkerV22(output, marker) { const pattern = /^(?:psql:<stdin>:\d+: )?ERROR:\s+P0001:\s*([^\s].*?)\s*$/u; const matches = String(output).split(/\r?\n/u).map((line) => line.match(pattern)).filter(Boolean); if (matches.length !== 1 || matches[0][1] !== marker || !/^v22-injected-(before-create|after-create|after-drop|before-rename)$/u.test(marker)) throw new Error("b2c-v22-fault-marker"); }
...
function catalogAssertion(oldPresent, buildPresent) {
  ...
  const build = buildPresent ? "IF NOT EXISTS (SELECT 1 FROM pg_index i WHERE i.indexrelid='public.uq_biz_property_approval_request_active_source_v2_build'::regclass AND i.indisunique=true AND i.indisvalid=true AND i.indisready=true AND (SELECT array_agg(a.attname ORDER BY k.ordinal) FROM unnest(i.indkey::smallint[]) WITH ORDINALITY k(attnum,ordinal) JOIN pg_attribute a ON a.attrelid=i.indrelid AND a.attnum=k.attnum)=ARRAY['tenant_id','park_id','action_id','source_type','source_id','source_expected_version']::text[] AND encode(public.digest(convert_to(pg_get_expr(i.indpred,i.indrelid,false),'UTF8'),'sha256'),'hex')='24ef911486d5274d6c439d63de6aa253b289241ac2b75317b1f98bc93a5a8fda') THEN RAISE EXCEPTION 'build catalog drift'; END IF;" : "IF to_regclass('public.uq_biz_property_approval_request_active_source_v2_build') IS NOT NULL THEN RAISE EXCEPTION 'unexpected build index'; END IF;";
  return `DO $assert$ DECLARE actual_index text; actual_predicate text; BEGIN ${old} ${build} END $assert$;`;
}
const faults = Object.freeze([
  { boundary: "before-create", injectionPoint: "after-validation", catalogState: "old-present-build-absent", prefix: "", assertion: catalogAssertion(true, false), marker: "v22-injected-before-create" },
  { boundary: "after-create", injectionPoint: "after-validation", catalogState: "old-present-build-present", prefix: createBuildIndex, assertion: catalogAssertion(true, true), marker: "v22-injected-after-create" },
  { boundary: "after-drop", injectionPoint: "immediately-after-ddl", catalogState: "old-absent-build-present", prefix: `${createBuildIndex} DROP INDEX public.uq_biz_property_approval_request_active_source;`, assertion: catalogAssertion(false, true), marker: "v22-injected-after-drop" },
  { boundary: "before-rename", injectionPoint: "after-validation", catalogState: "old-absent-build-present", prefix: `${createBuildIndex} DROP INDEX public.uq_biz_property_approval_request_active_source;`, assertion: catalogAssertion(false, true), marker: "v22-injected-before-rename" },
].map(Object.freeze));
...
function executeWithV22(runCommand) {
  ...
  try {
    const actual = inspect("execute", target, (command, argv, options) => evidence.child("inspect", command, argv, options.input, runCommand), { ...target.labels, inherited_labels: target.inherited_labels }); if (actual.container_id !== target.container_id || actual.volume_id !== target.volume_id || actual.image_id !== target.image_id) throw new Error("b2c-v22-execute-identity-drift");
    if (!/^16[0-9]{4}$/u.test(psql(evidence, target, "postgres-16", "SHOW server_version_num;", runCommand).stdout.trim())) throw new Error("b2c-v22-version-drift");
    ...
    const sql = snapshotSql(expectedLate()), initial = JSON.parse(psql(evidence, target, "initial-snapshot", sql, runCommand).stdout.trim()); assertSnapshotV22(initial); const results = [];
    for (const fault of faults) { const before = JSON.parse(psql(evidence, target, `before-${fault.boundary}`, sql, runCommand).stdout.trim()); assertSnapshotV22(before); const failed = psql(evidence, target, `fault-${fault.boundary}`, faultSql(fault), runCommand, true); if (failed.status === 0) throw new Error(`b2c-v22-fault-success:${fault.boundary}`); assertFaultMarkerV22(`${failed.stdout}${failed.stderr}`, fault.marker); const after = JSON.parse(psql(evidence, target, `after-${fault.boundary}`, sql, runCommand).stdout.trim()); assertSnapshotV22(after); if (!same(before, after)) throw new Error(`b2c-v22-rollback-drift:${fault.boundary}`); results.push({ boundary: fault.boundary, injection_point: fault.injectionPoint, catalog_state: fault.catalogState, marker: fault.marker, sqlstate: "P0001", snapshot_exact: true }); }
    const final = JSON.parse(psql(evidence, target, "final-snapshot", sql, runCommand).stdout.trim()); assertSnapshotV22(final); if (!same(initial, final)) throw new Error("b2c-v22-final-drift"); evidence.terminal("success", { direct_baseline: "fixed-fixture-plus-000185-000195-original-bytes", faults: results }, provenance); return Object.freeze({ faults: results });
  } catch (error) { evidence.terminal("failure", { failure: { code: "B2C_V22_EXECUTION_FAILED", stage: "execution", name: "ExecutionError" } }, provenance); throw error; }
}

/** Formal phase two API: no caller-controlled command, path, hash, target, run ID, or evidence option. */
export function executeV22() { return executeWithV22(spawnSync); }

/** Test-only facade. It has no path parameter and is unavailable from the canonical repository root. */
export function createIsolatedV22TestHarness(runCommand) {
  if (typeof runCommand !== "function" || !/^\/tmp\/b2c-v22-test-[^/]+$/u.test(root) || realpathSync(root) !== root || !research.startsWith(`${root}${sep}`)) throw new Error("b2c-v22-test-harness-root");
  return Object.freeze({ provision: () => provisionWithV22(runCommand), execute: () => executeWithV22(runCommand) });
}

export function candidateV22() { return Object.freeze({ status: "PENDING_REVIEW", execution_authorized: false, container_create_authorized: false, container_execute_authorized: false, formal_go: false, docker_or_database_command_executed: false, trust_root: "repository-fixed-path-procedural-root-v1", review_chain: ["candidate", "database", "qa", "security", "drain", "provision_authority", "resource_observation", "execution_authority"] }); }
