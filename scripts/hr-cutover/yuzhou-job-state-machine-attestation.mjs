...
import { canonicalDecisionHash, verifyYuzhouJobStateDecisionArtifact } from "./yuzhou-job-state-decision-artifact-lib.mjs";
...
export class YuzhouJobStateMachineAttestationError extends Error {
  constructor(code, detail) { super(`${code}: ${detail}`); this.name = "YuzhouJobStateMachineAttestationError"; this.code = code; }
}
const fail = (code, detail) => { throw new YuzhouJobStateMachineAttestationError(code, detail); };
const plainObject = value => value !== null && typeof value === "object" && !Array.isArray(value) && Object.getPrototypeOf(value) === Object.prototype;
const exactKeys = (value, keys, code) => { if (!plainObject(value) || JSON.stringify(Object.keys(value).sort()) !== JSON.stringify([...keys].sort())) fail(code, "unexpected object shape"); };
const requireSha = (value, code, detail) => { if (typeof value !== "string" || !SHA256.test(value)) fail(code, detail); };
const sha = value => createHash("sha256").update(value).digest("hex");
const canonicalForHash = value => Array.isArray(value) ? `[${value.map(canonicalForHash).join(",")}]` : value && typeof value === "object" ? `{${Object.keys(value).sort().map(key => `${JSON.stringify(key)}:${canonicalForHash(value[key])}`).join(",")}}` : JSON.stringify(value);
const canonicalPrivatePayloadHash = value => sha(`${canonicalForHash(value)}\n`);
...
export const computeYuzhouJobStateCheckpointArtifactHash = (kind, payload) => sha(`yuzhou-job-state-preload-artifact-v2\0${kind}\0${canonicalYuzhouJobStateMachineJson(payload)}`);
export const computeYuzhouJobStateCheckpointRoot = checkpoint => sha(`yuzhou-job-state-preload-package-v2\0${canonicalYuzhouJobStateMachineJson({ trustedCheckpointRootSha256: checkpoint.trustedCheckpointRootSha256, triple: checkpoint.triple, bindings: checkpoint.bindings })}`);
export const computeYuzhouJobStateAttestationIntegrity = attestation => {
  ...
  return sha(`yuzhou-job-state-machine-attestation-v2\0${canonicalYuzhouJobStateMachineJson(body)}`);
};
export const computeYuzhouJobStateMachineAttestationSha256 = attestation => sha(`yuzhou-job-state-machine-attestation-artifact-v2\0${canonicalYuzhouJobStateMachineJson(attestation)}`);
...
function verifyDecision(decision, triple, trustedCheckpointRootSha256) {
...
function verifyPrivatePayload(payload, decision, triple) {
  exactKeys(payload, ["formatVersion", "kind", "canonicalDecisionSha256", "payloadSha256", "dictionaryVersionId", "expectedDatabaseItemsSha256", "csm", "t0Binding", "scope", "dictionaryEvidenceSha256", "machineActor", "items", "productionImport"], "YUZHOU_JOB_STATE_MACHINE_PRIVATE_PAYLOAD_INVALID");
  if (payload.formatVersion !== 2 || payload.kind !== "yuzhou-job-state-private-materialization" || payload.productionImport !== "HOLD" || payload.canonicalDecisionSha256 !== decision.canonicalDecisionSha256) fail("YUZHOU_JOB_STATE_MACHINE_PRIVATE_PAYLOAD_INVALID", "native v2 payload identity or decision binding");
  ...
  exactKeys(payload.machineActor, ["id", "kind", "verifiedAt"], "YUZHOU_JOB_STATE_MACHINE_ACTOR_INVALID");
  if (!UUID.test(payload.dictionaryVersionId ?? "") || !UUID.test(payload.machineActor.id ?? "") || payload.machineActor.kind !== "machine_policy_engine" || !UTC.test(payload.machineActor.verifiedAt ?? "") || Number.isNaN(Date.parse(payload.machineActor.verifiedAt))) fail("YUZHOU_JOB_STATE_MACHINE_ACTOR_INVALID", "a machine-only UUID actor and UTC verification time are required");
...
export function compileYuzhouJobStateMachineAttestation(checkpoint, { expectedCheckpointRootSha256 } = {}) {
  ...
  exactKeys(checkpoint, ["formatVersion", "kind", "trustedCheckpointRootSha256", "triple", "decisionArtifact", "privatePayload", "t0Evidence", "bindings", "packageRootSha256"], "YUZHOU_JOB_STATE_MACHINE_CHECKPOINT_INVALID");
  if (checkpoint.formatVersion !== 2 || checkpoint.kind !== "yuzhou-job-state-preload-package") fail("YUZHOU_JOB_STATE_MACHINE_CHECKPOINT_INVALID", "checkpoint identity");
  ...
  if (checkpoint.trustedCheckpointRootSha256 !== expectedCheckpointRootSha256) fail("YUZHOU_JOB_STATE_MACHINE_TRUST_ROOT_MISMATCH", "package does not bind the external checkpoint root");
  ...
  const packageRootSha256 = computeYuzhouJobStateCheckpointRoot(checkpoint);
  if (checkpoint.packageRootSha256 !== packageRootSha256) fail("YUZHOU_JOB_STATE_MACHINE_PACKAGE_ROOT_MISMATCH", "declared package root drift");
  const decision = verifyDecision(checkpoint.decisionArtifact, checkpoint.triple, expectedCheckpointRootSha256);
  ...
  const attestation = {
    formatVersion: 2, artifactKind: "yuzhou_job_state_machine_attestation", attestationVersion: "v2", status: "PASS", reasonCodes: [], triple: checkpoint.triple,
    trustedCheckpointRootSha256: expectedCheckpointRootSha256, jobStatePackageRootSha256: packageRootSha256,
    decisionArtifactSha256: checkpoint.bindings.decisionArtifactSha256, canonicalDecisionSha256: decision.recomputed,
    privatePayloadArtifactSha256: checkpoint.bindings.privatePayloadArtifactSha256, privatePayloadSha256: payload.recomputed,
    t0EvidenceArtifactSha256: checkpoint.bindings.t0EvidenceArtifactSha256, t0BindingSha256: payload.t0BindingSha256,
    semanticLedger: { sourceDistinctStateCount: 7, sourceRecordCount: 2949, mappedStateCount: decision.result.mapped, quarantinedStateCount: decision.result.quarantined, mappedRecordCount: decision.result.mappedRecordCount, quarantinedRecordCount: decision.result.quarantinedRecordCount },
    assertionMode: "trusted_root_job_state_machine_semantics", humanSignature: false, humanIdentityAsserted: false, productionImport: "HOLD"
  };
  return { ...attestation, integrityDigest: computeYuzhouJobStateAttestationIntegrity(attestation) };
}

export function verifyYuzhouJobStateMachineAttestation(attestation, { expectedCheckpointRootSha256 } = {}) {
  requireSha(expectedCheckpointRootSha256, "YUZHOU_JOB_STATE_MACHINE_TRUST_ROOT_REQUIRED", "external expectedCheckpointRootSha256 is required");
  exactKeys(attestation, ["formatVersion", "artifactKind", "attestationVersion", "status", "reasonCodes", "triple", "trustedCheckpointRootSha256", "jobStatePackageRootSha256", "decisionArtifactSha256", "canonicalDecisionSha256", "privatePayloadArtifactSha256", "privatePayloadSha256", "t0EvidenceArtifactSha256", "t0BindingSha256", "semanticLedger", "assertionMode", "humanSignature", "humanIdentityAsserted", "productionImport", "integrityDigest"], "YUZHOU_JOB_STATE_MACHINE_ATTESTATION_INVALID");
  verifyTriple(attestation.triple);
  if (attestation.formatVersion !== 2 || attestation.artifactKind !== "yuzhou_job_state_machine_attestation" || attestation.attestationVersion !== "v2" || attestation.status !== "PASS" || !Array.isArray(attestation.reasonCodes) || attestation.reasonCodes.length !== 0 || attestation.assertionMode !== "trusted_root_job_state_machine_semantics" || attestation.humanSignature !== false || attestation.humanIdentityAsserted !== false || attestation.productionImport !== "HOLD") fail("YUZHOU_JOB_STATE_MACHINE_ATTESTATION_INVALID", "fixed assertion fields differ");
  for (const key of ["trustedCheckpointRootSha256", "jobStatePackageRootSha256", "decisionArtifactSha256", "canonicalDecisionSha256", "privatePayloadArtifactSha256", "privatePayloadSha256", "t0EvidenceArtifactSha256", "t0BindingSha256", "integrityDigest"]) requireSha(attestation[key], "YUZHOU_JOB_STATE_MACHINE_ATTESTATION_INVALID", key);
