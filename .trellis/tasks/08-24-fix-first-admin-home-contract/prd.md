...
- 生产 `bootstrap-admin.sh` 不回写 `contact_user_id`，初次创建、幂等确保与密码重置分别写入三个受脚本控制的 bootstrap remark。
...
- API 仅在用户持有当前园区的有效 `TENANT_ADMIN` 角色，且满足下列之一时返回 `true`：
  ...
  2. 为兼容旧生产 bootstrap 链，租户指针为空，且用户同时具有 `create_by IS NULL` 创建来源与脚本 create/ensure/password-reset 三种精确 remark 之一。
