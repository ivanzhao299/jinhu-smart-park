...
try {
  ...
  const eventFor = (legacyId, employeeCode, legacyState = "1") => {
    const source = { legacyId, legacyEventNo: `EV-${legacyId}`, employeeCode, legacyEventType: "就职", sourceEffectiveAt: "2026-01-01 08:30:00", beforeOrgCode: null, afterOrgCode: "ORG", beforePositionCode: null, afterPositionCode: "P", legacyEmployeeState: null, legacyState, departmentflag: null, jobflag: null, payflag: null, otherflag: null, reason: null };
  ...
  const events = Array.from({ length: 6887 }, (_, index) => eventFor(index + 1, index === 6886 ? "E-MISSING" : "E-001", index === 6885 ? "0" : "1"));
  ...
  const stateItems = [
    { id: randomUUID(), sourceCode: null, sourceName: null, sourceValue: "0", sourceIdentitySha256: sha("dbo.readjust.state\0" + "0"), sourceRowSha256: sha("fixture-state-0"), decision: "map", targetDomain: "migration_decision", targetValue: "needs_review", reasonCode: "SOURCE_STATE_UNCONFIRMED" },
    { id: randomUUID(), sourceCode: null, sourceName: null, sourceValue: "1", sourceIdentitySha256: sha("dbo.readjust.state\0" + "1"), sourceRowSha256: sha("fixture-state-1"), decision: "map", targetDomain: "migration_decision", targetValue: "accepted", reasonCode: "EFFECTIVE_SOURCE_STATE" },
  ];
  privateFile(statePath, `${JSON.stringify({ formatVersion: 1, kind: "yuzhou_core_non_t0_machine_dictionary_package", triple, trustedRootSha256: "4".repeat(64), machineActor: { id: randomUUID(), kind: "machine_policy_engine", verifiedAt: "2026-01-01T00:00:00Z" }, evidence: {}, dictionaries: [{ dictionaryCode: "employment_event_state", sourceTable: "dbo.readjust", sourceSnapshotSha256: "5".repeat(64), items: stateItems }], productionImport: "HOLD" })}\n`);
  ...
  const reviewState = artifact.records.find(row => row.sourceIdentitySha256 === sha("dbo.readjust\0" + "6886"));
  assert.equal(reviewState.candidateDisposition, "insert");
  assert.equal(reviewState.targetFields.migration_decision, "needs_review");
