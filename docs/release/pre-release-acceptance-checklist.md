...
- 仅开放当前菜单白名单中的安全巡检、隐患整改和我的巡检路径
- 安全模块上线前人工验收和归档记录见 [安全巡检与隐患整改模块上线验收记录](safety-module-release-acceptance-record.md)
- `/safety/my-inspect-tasks` 已完成权限适配并纳入开放范围，预发验收需确认现场角色仅可访问本人任务
- 不自动开放应急、作业许可等扩展菜单
...
| 安全核心菜单开放 | 检查安全模块白名单 | 显示安全巡检与隐患整改核心菜单 | 待填写 | 待填写 |
| 安全暂缓菜单隐藏 | 检查应急、作业许可菜单 | 不显示应急、作业许可等暂缓菜单 | 待填写 | 待填写 |
