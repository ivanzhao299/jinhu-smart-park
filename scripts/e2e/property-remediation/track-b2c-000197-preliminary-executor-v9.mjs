...
export const V9_RUN_ID = "b2c197_prelim_20260802d";
export const V9_EVIDENCE_POLICY = "b2c-000197-child-evidence-v9";
...
export function discoverSecretsV9(value, target = [], key = "") {
  if (value == null) return target;
  if (Buffer.isBuffer(value)) return discoverSecretsV9(value.toString("utf8"), target, key);
  if (Array.isArray(value)) {
    for (const entry of value) discoverSecretsV9(entry, target, ARG_CONTAINER.test(key) ? "" : key);
    return target;
  }
  if (typeof value === "object") {
    for (const [entryKey, entryValue] of Object.entries(value)) discoverSecretsV9(entryValue, target, entryKey);
    return target;
  }
...
export function redactEvidenceTextV9(value, secrets = []) {
  let output = String(value ?? ""); discoverSecretsV9(output, secrets);
...
function stageName(value) {
  ...
  if (!stage) throw new Error("b2c-000197-v9-stage-required");
  return stage;
}

function serializeError(error, secrets) {
  if (!error) return null;
  return { name: redactEvidenceTextV9(error.name ?? "Error", secrets),
    message: redactEvidenceTextV9(error.message ?? String(error), secrets),
    code: error.code == null ? null : redactEvidenceTextV9(error.code, secrets) };
}

function assertSanitized(payload, secrets) {
  const encoded = JSON.stringify(payload);
  for (const secret of secrets) if (secret && encoded.includes(secret)) throw new Error("b2c-000197-v9-secret-leak");
  if (DATABASE_URL.test(encoded)) throw new Error("b2c-000197-v9-database-url-leak");
  DATABASE_URL.lastIndex = 0;
}

function environmentEnvelope(env, allowlist, secrets) {
  discoverSecretsV9(env, secrets);
  ...
  for (const entry of allowlist) {
    if (!entry || typeof entry.name !== "string" || !["value", "redacted"].includes(entry.persist)) {
      throw new Error("b2c-000197-v9-env-allowlist-malformed");
    }
    if (!(entry.name in env)) throw new Error(`b2c-000197-v9-env-allowlist-missing:${entry.name}`);
    ...
    } else recorded[entry.name] = redactEvidenceTextV9(value, secrets);
  }
  return { childEnv, recorded, redactedFields };
}

export class PhaseExecutionErrorV9 extends Error {
  constructor(stage, result, cause = null) {
    super(`b2c-000197-v9-phase-failed:${stage}:exit=${result?.status ?? "none"}:signal=${result?.signal ?? "none"}:${cause?.message ?? "child"}`);
    this.name = "PhaseExecutionErrorV9"; this.stage = stage; this.result = result; this.cause = cause;
  }
}

export class PreliminaryEvidenceRecorderV9 {
  constructor({ evidenceRoot, runId = V9_RUN_ID, spawn = spawnSync, writeFile = writeFileSync,
    mkdir = mkdirSync, now = () => new Date().toISOString(), secrets = [] }) {
    if (runId !== V9_RUN_ID) throw new Error("b2c-000197-v9-run-id-drift");
    ...
    if (realpathSync(this.root) !== this.root || !statSync(this.root).isDirectory()) throw new Error("v9-evidence-root");
  }

  writeImmutable(filename, payload, stage) {
    const path = resolve(this.root, filename);
    if (dirname(path) !== this.root || basename(path) !== filename) throw new Error(`v9-path-escape:${stage}`);
    discoverSecretsV9(payload, this.secrets);
    const sanitized = JSON.parse(redactEvidenceTextV9(JSON.stringify(payload), this.secrets));
    ...
    if ((observed.mode & 0o777) !== 0o444 || observed.size !== content.byteLength) throw new Error("v9-mode-size-drift");
  ...
  runChild({ stage, command, args = [], cwd, env = {}, envAllowlist = [], input = null,
    ...
    discoverSecretsV9({ command, args, env }, this.secrets);
    ...
    const intent = this.writeImmutable(`${prefix}-intent.json`, {
      schema_version: "b2c-000197-child-intent-v9", evidence_policy_version: V9_EVIDENCE_POLICY,
    ...
    discoverSecretsV9([stdout, stderr, child.error], this.secrets);
    const resultEntry = this.writeImmutable(`${prefix}-result.json`, {
      schema_version: "b2c-000197-child-result-v9", evidence_policy_version: V9_EVIDENCE_POLICY,
      formal_run_id: this.runId, sequence, stage: normalized, recorded_at_utc: this.now(),
    ...
    if (parser) {
      ...
      this.writeImmutable(`${prefix}-parse.json`, {
        schema_version: "b2c-000197-child-parse-v9", evidence_policy_version: V9_EVIDENCE_POLICY,
        formal_run_id: this.runId, sequence, stage: normalized, recorded_at_utc: this.now(),
    ...
    if (!allowFailure && (child.error || child.status !== 0 || child.signal != null)) throw new PhaseExecutionErrorV9(normalized, result);
    if (parseError && !allowParseFailure) throw new PhaseExecutionErrorV9(normalized, result, parseError);
    return result;
  }

  writeTerminal(kind, payload) {
    if (!["success", "failure"].includes(kind)) throw new Error("b2c-000197-v9-terminal-kind");
    discoverSecretsV9(payload, this.secrets);
    const artifact = this.writeImmutable(`${kind}-${this.runId}.json`, {
      schema_version: `b2c-000197-preliminary-${kind}-v9`, evidence_policy_version: V9_EVIDENCE_POLICY,
    ...
    const manifest = this.writeImmutable(`${kind}-${this.runId}.manifest.json`, {
      schema_version: `b2c-000197-preliminary-${kind}-manifest-v9`, evidence_policy_version: V9_EVIDENCE_POLICY,
...
function phaseFailure(stage, result) {
  if (result.error || result.status !== 0 || result.signal != null) return new PhaseExecutionErrorV9(stage, result);
  if (result.parseError) return new PhaseExecutionErrorV9(stage, result, result.parseError);
  return null;
}

export function runPhasedGateV9(recorder, { phases, cleanupPhases }) {
...
export function executeWithEvidenceV9({ evidenceRoot, operation, recorderOptions = {}, successPayload = {} }) {
  const recorder = new PreliminaryEvidenceRecorderV9({ evidenceRoot, ...recorderOptions });
  try {
...
export function parseTapSummaryV9(stdout, { expectedTests, expectedNames = [] }) {
  const count = (label) => { const matches = [...stdout.matchAll(new RegExp(`^# ${label} (\\d+)$`, "gmu"))];
    if (matches.length !== 1) throw new Error(`v9-tap-${label}`); return Number(matches[0][1]); };
  ...
  if (result.tests !== expectedTests || result.suites !== 0 || result.pass !== expectedTests || result.fail !== 0
      || result.cancelled !== 0 || result.skipped !== 0 || result.todo !== 0) throw new Error("v9-tap-count-drift");
  ...
  if (expectedNames.length && (actualNames.length !== expectedNames.length
      || actualNames.some((name, index) => name !== expectedNames[index]))) throw new Error("v9-tap-name-drift");
