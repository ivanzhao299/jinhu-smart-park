...
export const FORMAL_CHILD_ENTRIES_V10 = Object.freeze([
  "scripts/e2e/property-remediation/tests/b2c-000197-preliminary-executor-v10.spec.mjs",
  "scripts/e2e/property-remediation/tests/b2c-000197-preliminary-orchestrator-v10.spec.mjs",
  "scripts/e2e/property-remediation/tests/b2c-000197-capability-closure-v10.spec.mjs",
  "scripts/e2e/property-remediation/tests/b2c-000197-recursive-closure-v10.spec.mjs",
  "scripts/e2e/property-remediation/tests/b2c-000197-frozen-closure-v10.spec.mjs",
...
export const TYPECHECK_GOVERNANCE_FILES_V10 = Object.freeze([
...
function tsconfig(root) {
  ...
  if (loaded.error) throw new Error(`v10-tsconfig-read:${JSON.stringify(loaded.error)}`);
  const parsed = ts.parseJsonConfigFileContent(loaded.config, ts.sys, dirname(path), { noEmit: true }, path);
  if (parsed.errors.length) throw new Error(`v10-tsconfig-parse:${JSON.stringify(parsed.errors)}`);
  return parsed;
}

export function compilerRepositoryFilesV10(rootInput = process.cwd()) {
...
export function parseTscListFilesV10(stdout, rootInput = process.cwd()) {
...
export function classifySpecifierV10(importerInput, specifier, rootInput = process.cwd()) {
...
export function resolveFormalExecutionClosureV10(rootInput = process.cwd()) {
  const root = realpathSync(resolve(rootInput)); const parsed = tsconfig(root);
  const repository = new Set([...compilerRepositoryFilesV10(root), ...FORMAL_CHILD_ENTRIES_V10,
    ...TYPECHECK_GOVERNANCE_FILES_V10].map((path) => normalize(root, realpathSync(resolve(root, path)))));
  ...
  if (unresolved.length) {
    throw new Error(`b2c-000197-v10-unresolved-repository-specifiers:${JSON.stringify(unresolved)}`);
  }
  ...
  return Object.freeze({ repositoryFiles: Object.freeze([...repository].sort()),
    compilerFiles: compilerRepositoryFilesV10(root), builtin: Object.freeze(classify(builtin)),
    external: Object.freeze(classify(external)), unresolved: Object.freeze([]) });
}

export function assertTscFileListMatchesV10(stdout, rootInput = process.cwd()) {
  const compiler = compilerRepositoryFilesV10(rootInput); const actual = parseTscListFilesV10(stdout, rootInput);
  if (JSON.stringify(compiler) !== JSON.stringify(actual)) {
    const expected = new Set(compiler); const observed = new Set(actual);
    throw new Error(`b2c-000197-v10-tsc-file-list-drift:${JSON.stringify({
