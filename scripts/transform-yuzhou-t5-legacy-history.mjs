...
import { readMaterializationKeyFile, validateMaterializationKeyBytes } from "./hr-cutover/materialization-key-contract.mjs";
import { buildLegacyCustomFieldDefinitions,LEGACY_DEFINITION_SOURCE_COLUMNS,LEGACY_PERSON_CUSTOM_FIELDS } from "./hr-cutover/legacy-custom-field-parity.mjs";
import { validateLegacyEmployeeProfilePhysicalMeasurementsGapContract } from "./hr-cutover/legacy-employee-profile-physical-measurements-gap.mjs";
import { buildLegacyProfessionalTitleDictionary } from "./hr-cutover/legacy-professional-title-materialization.mjs";
import { createT5ProtectedValueMaterializer,materializeT5NonfileRecord } from "./hr-cutover/t5-nonfile-field-projection.mjs";
...
const materializationSeed=keyFile?readMaterializationKeyFile(keyFile):validateMaterializationKeyBytes(process.env.PARTY_DATA_ENCRYPTION_KEY??"");
const mappingContractPath=resolve(import.meta.dirname,"hr-cutover/contracts/legacy-employee-profile-materialization-reviewed-v1.json"),mappingContractBytes=readFileSync(mappingContractPath),mappingContractSha256=sha(mappingContractBytes),mappingContract=JSON.parse(mappingContractBytes);
if(mappingContractSha256!=="d44b0f904fb3240d45a52b8dc8a3510ce5622ecb6f7f41356fbe6e48fa53b7e0")throw Error("reviewed employee mapping contract drift");
if(mappingContract.formatVersion!==2||JSON.stringify(mappingContract.personCustomFieldMapping?.sourceDefinitionColumns)!==JSON.stringify(LEGACY_DEFINITION_SOURCE_COLUMNS)||JSON.stringify(mappingContract.personCustomFieldMapping?.fields?.map(field=>[field.sourceColumn,field.valueType,field.mappingOrdinal]))!==JSON.stringify(LEGACY_PERSON_CUSTOM_FIELDS.map(field=>[field.code,field.valueType,field.mappingOrdinal])))throw Error("reviewed employee custom-field mapping contract invalid");
const physicalMeasurementGapContract=validateLegacyEmployeeProfilePhysicalMeasurementsGapContract(JSON.parse(readFileSync(resolve(import.meta.dirname,"hr-cutover/contracts/legacy-employee-profile-physical-measurements-gap-v1.json"),"utf8")));
if(physicalMeasurementGapContract.reviewedMappingContractSha256!==mappingContractSha256)throw Error("physical measurement gap reviewed mapping binding drift");
const protect=createT5ProtectedValueMaterializer(materializationSeed, { identityHashSeed: process.env.PARTY_DATA_IDENTITY_HASH_KEY });
let professionalTitleDictionary;
const materialize=(name,row)=>materializeT5NonfileRecord(name,row,{protect,customFieldDefinitions,professionalTitleDictionary});
...
const definitionSource=read("defs.raw.json"),customFieldDefinitions=buildLegacyCustomFieldDefinitions(definitionSource);
const professionalTitleSource=read("assignment.raw.json");
professionalTitleDictionary=buildLegacyProfessionalTitleDictionary(professionalTitleSource);
// dbo.defs may contain legacy SQL expressions. Preserve the exact source only
// inside the mode-0600 private stage for later capability review; never execute
// or print those expressions, and expose only presence/hash metadata elsewhere.
const definitionEvidence=customFieldDefinitions.map(definition=>({code:definition.code,valueType:definition.valueType,baseClassification:definition.baseClassification,legacyDefinitionId:definition.legacyDefinitionId,legacyDatatype:definition.legacyDatatype,legacyGroupId:definition.legacyGroupId,legacySortOrder:definition.legacySortOrder,legacyNullable:definition.legacyNullable,legacyRuleClassification:definition.legacyRuleClassification,sourceIdentitySha256:definition.sourceIdentitySha256,sourceRowSha256:definition.sourceRowSha256,legacyLogicCoverage:definition.legacyLogicCoverage})),definitionEvidenceFile="defs.safe-evidence.jsonl";
const privateDefinitionSource={sourceObject:"dbo.defs",rows:definitionSource.length,file:"defs.private.jsonl",fileSha256:write("defs.private.jsonl",definitionSource.map(row=>identity("defs",row.ID,row,{privateOnly:true,execution:"forbidden"}))),safeEvidenceFile:definitionEvidenceFile,safeEvidenceFileSha256:write(definitionEvidenceFile,definitionEvidence),logicColumnDenominator:customFieldDefinitions.length*10,logicColumnPresentCount:customFieldDefinitions.reduce((sum,definition)=>sum+definition.legacyLogicCoverage.presentCount,0)};
...
const requiredCatalogTables=["accept","assignment","bonuscode","bonusrecord","compact","compact_c","compacttypecode","course","defs","docs","family","his","jobstatecode","jobtrain","knowhow","person","person_user","person_user_item","readjust","readjustitem","ticket","train","trainhis"];
...
const assignmentCatalog=catalog.find(item=>item.schema==="dbo"&&item.table==="assignment");
if(!Number.isSafeInteger(assignmentCatalog.rows)||assignmentCatalog.rows!==professionalTitleSource.length||JSON.stringify(assignmentCatalog.columns.map(item=>item.name).sort())!==JSON.stringify(["assignment","assignmentname"]))throw Error("assignment dictionary catalog drift");
const domains={};
...
const professionalTitleDictionarySha256=sha(canonical(professionalTitleSource));
const business={formatVersion:1,catalogSha256,mappingContractSha256,professionalTitleDictionary:{sourceObject:"dbo.assignment",rows:professionalTitleSource.length,sourceSha256:professionalTitleDictionarySha256,semantics:"professional_title_not_position"},domains};
const businessSha256=sha(canonical(business));
const manifest={...business,businessSha256,privateDefinitionSource,generatedAt:new Date().toISOString(),sensitive:true,payloadSanitization:"nul_to_literal_escape_v1",productionImport:"HOLD"};
