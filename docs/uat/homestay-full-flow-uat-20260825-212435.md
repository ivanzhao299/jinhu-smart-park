...
| P-FOUNDATION-DATA-04 fixture 的真实 UI 建链 | SOP 要求先建房源经营资格再建民宿数据 | 未闭合 | 隔离环境 `/assets/property-operations` 实测为空且没有新增入口；`biz_unit` 编辑面板亦不提供短租经营配置创建 | `GAP-FOUNDATION-04`：无法从空环境经 UI 建成 `short_stay/enabled`；本轮只在隔离库插入全新 RUN_ID 行用于验证下游，不修改 baseline；角色/住客身份等无 UI fixture 链的流程降级 Blocked |
...
| P-FINANCE-01 财务矩阵/余额 | 闭合：无 settlement 状态；余额=`charges-payments+refunds-waivers`；状态/类型矩阵稳定拒绝非法组合 | 已实现 | `homestay-finance.policy.ts:27-82`; `000177_homestay_mvp.sql:185` | 部分结算仅用 balance 断言；不测试物理删除（无删除入口） |
...
| P-FINANCE-03 前端退款权限 | 共享/API 契约允许 register 或 waive 相应变体；Web refund 要求 register+waive | 偏离设计 | `HomestayFinanceEntryPanel.tsx:76-85`; `access-manifest.ts:431-462`; `homestay.controller.ts:420` | `GAP-RBAC-01`：窄财务角色 refund 入口可能被过度隐藏；对应窄权限 UI Case 进入 gap/BLOCKED，不当作产品 FAIL |
...
- 不进入 PASS 浏览器矩阵的设计/实现差距：`GAP-DESIGN-01` 缺失设计源；`GAP-FOUNDATION-01` usage_type/资格边界；`GAP-FOUNDATION-02` 候选关键词未接线；`GAP-FOUNDATION-03` 离线草稿仅部分适配；`GAP-FOUNDATION-04` 空环境缺经营配置 UI 建链；`GAP-RBAC-01` refund Web 权限过严；`GAP-RBAC-02` asset dependency API 装饰器不统一；`GAP-RBAC-03` task operator bundle 缺 task read。
...
|---|---|---|---|---|---|
| C00-A | FLOW-00 | 管理员/当前园区 | 创建面板仅列 short_stay/enabled；选择后创建；DB booking.unit_id 等于选中 unit | desktop | PASS |
| C00-B | FLOW-00 | 受限 unit 岗 | short_stay/enabled 可见，long_rent/disabled 不可见；输入无关 `ZZ` 仍返回 SHORT | desktop | GAP/BLOCKED（GAP-FOUNDATION-02、04；无受限账号） |
| C01-A | FLOW-01 | 运营 | rates 必填校验→保存 base 688/取消 24h/固定费 50/需检查→14 日显示 688→创建 draft；保存与创建快速双击各仅 1 次写请求/1 行 | desktop | PASS（override 未执行） |
| C01-B | FLOW-01 | 运营 | confirm→active occupancy+688 charge；详情、finance、dashboard、availability、tasks 同步 | desktop+narrow | PASS；但见 FAIL-02 房态语义 |
...
| C02-B | FLOW-02 | 前台 | 无实名入住拒绝且状态不变 | desktop | PASS：页面返回 `Every declared guest must be registered and verified before check-in`，DB 仍 confirmed |
...
| C03-A | FLOW-03 | 财务 | confirmed payment 300→已收 300、余额 388；DB charge/payment 各一条；快速双击仅一条 payment | desktop+narrow | PASS（部分结算）；全额结算未执行 |
...
| C03-E | FLOW-03 | 窄财务岗 | register-only/waive-only 前端入口与 API 契约对照 | desktop | GAP-RBAC-01 / BLOCKED |
...
| C05-A | FLOW-05 | 管理员 | dashboard 与列表/DB 一致性 | desktop+narrow | FAIL：到店=1、已收=300 与事实一致；confirmed 被统计为“在住房间/occupied”，见 FAIL-02 |
| C05-B | FLOW-05 | 业务岗 | 深链/返回上下文 | desktop | PASS（booking/stay/tasks/finance returnTo）；筛选分页数据量不足 |
| C05-C | FLOW-05 | 业务/审批 | taskId/requestId 深链 | desktop+narrow | BLOCKED：无 approval projection |
| C06-A | FLOW-06 | 管理员代业务 | 真实登录→菜单/动作→真实登出 | desktop | PASS（管理员）；独立业务岗未覆盖 |
...
- 执行统计按 28 个冻结业务 Case 计：PASS 7、FAIL 1、BLOCKED 17、GAP/BLOCKED 3；另 C-UX 为 PARTIAL，不计入 28 个业务 Case。共识别 2 个产品 FAIL（其中 rate 404 为跨 Case Console 失败）。结论：**FAIL / 不可发布为“民宿全流程已通过”**。
...
截图工具拒绝将 CDP 截图写入仓库 local-only 根（报告为 workspace-root 限制）；没有伪造或改存截图。`artifacts/homestay-uat-20260825-212435/` 保持空目录且不入 Git。证据由真实浏览器 accessibility snapshot、network/console 与 DB 只读查询交叉形成；这是本轮证据限制。
