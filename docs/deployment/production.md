...
Web logout first attempts `POST /api/v1/auth/logout` with the Bearer access token and cookie credentials. New Web sessions do not write a body `refreshToken`, but if an old session still has a legacy `jinhu_refresh_token` in JS-readable storage, logout sends it once as a body fallback before local cleanup so the server-side token can be revoked during the compatibility window. It then calls public `POST /api/v1/auth/logout-cookie` as a cookie cleanup fallback, which covers expired access-token sessions. Non-refresh 401 session resets also call `logout-cookie` before redirecting to login; 401 responses from `POST /api/v1/auth/token/refresh` are excluded so a stale refresh retry cannot clear a newer rotated cookie. Local access token, user, and legacy refresh-token storage are cleared after the logout attempt.
