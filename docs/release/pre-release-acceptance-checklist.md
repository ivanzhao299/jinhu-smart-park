...
菜单开放范围不再由历史首发白名单统一定义。每个本批次目标角色都必须依据 `/users/me` 返回的角色、权限、启用模块和菜单上下文，核对 Web 实际渲染结果；未列入本批次版本清单或未通过 UAT 的模块不得仅因代码或静态菜单存在而开放。

执行要求：

- 使用至少一个管理员角色、一个本批次标准业务角色和一个缺权或模块禁用角色
- 保存 `/users/me` 的脱敏角色、权限、启用模块和菜单摘要
- 核对实际渲染侧边栏，包括前端规范菜单与非空但不完整 API 菜单树合并后出现的授权链接
- 对实际渲染的每个目标页面执行浏览器检查；本地/UAT 可使用 `pnpm go-live:uat-browser`
- 为标准角色直接访问至少一个缺权路由，为模块禁用角色直接访问至少一个对应模块路由
- 安全模块开放前仍需完成 [安全巡检与隐患整改模块上线验收记录](safety-module-release-acceptance-record.md)，并确认 `/safety/my-inspect-tasks` 仅暴露本人任务

说明：

- 菜单隐藏不代表页面代码删除，也不能替代后端或路由权限拒绝
- 历史 `FIRST_RELEASE_MENU_PATHS` 与 `first-release-menu-whitelist.mjs` 仅作兼容性观察，不作为现行 Go / No-Go
- 具体角色的可见菜单可能不同，不以管理员菜单代替标准角色验收

| 验收项 | 方法 | 预期 | 实际 | 结果 |
|---|---|---|---|---|
| `/users/me` 上下文正确 | 核对角色、权限、启用模块和菜单摘要 | 与本批次批准的角色授权一致 | 待填写 | 待填写 |
| 管理员运行时菜单正确 | 登录并检查实际渲染侧边栏 | 仅显示其权限和启用模块允许的入口 | 待填写 | 待填写 |
| 标准角色运行时菜单正确 | 登录并检查实际渲染侧边栏 | 批准且有权的入口可见，缺权入口不可见 | 待填写 | 待填写 |
| 规范菜单合并覆盖 | 对比 API 菜单与实际渲染菜单 | API 树非空但不完整时，合并新增的授权链接也被纳入页面检查 | 待填写 | 待填写 |
| 缺权路由拒绝 | 标准角色直接打开已确认缺权的 URL | 重定向、403、禁用页或等价拒绝 | 待填写 | 待填写 |
| 禁用模块路由拒绝 | 模块禁用角色直接打开对应 URL | 菜单不可见且路由不可用 | 待填写 | 待填写 |
| 菜单页面浏览器检查 | 执行 `pnpm go-live:uat-browser` 或归档等价证据 | 实际渲染菜单页面无登录跳转、403、空白页或运行时错误 | 待填写 | 待填写 |
