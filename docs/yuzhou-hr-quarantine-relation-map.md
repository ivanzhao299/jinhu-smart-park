...
当前候选生成本身已达到 `READY_FOR_REVIEW`，但 T0 决策物化被稳定阻断为 `PRODUCTION_IMPORT_T0_DECISION_JOB_STATE_INVALID`：现有状态机包仍绑定旧代码 SHA，必须先用当前源状态字典重建受信任的机器候选包，再以新的只读目标库存重跑 T0。该阻断不改变 47 条异常的源表归属，也不允许写入生产库。
