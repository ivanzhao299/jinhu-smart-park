...
export function buildCoreNonT0DictionaryPackage(config, paths) {
  ...
  const dictionaries = [
    { dictionaryCode: "employment_event_type", sourceTable: "dbo.readjustitem", sourceSnapshotSha256: canonicalHash({ kind: "employment_event_type", source: evidence.t1Types }),
      items: t1Types.value.map(row => { const name = String(row.legacyType).trim(); return item({ sourceName: name, sourceTable: "dbo.readjustitem", sourceKey: name, targetDomain: "employment_event_type", targetValue: eventTypeTargets.get(name), reasonCode: "DETERMINISTIC_COMPATIBILITY_MAPPING" }); }) },
