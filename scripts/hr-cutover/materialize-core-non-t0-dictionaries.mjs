...
export function buildCoreNonT0DictionaryPackage(config, paths) {
  if (!config?.triple || !SHA256.test(config?.machineAttestation?.trustedRootSha256 ?? "") || !/^jinhu_hr_migration_lab_(?:core|full)_[a-z0-9_]{6,48}$/u.test(config?.target?.database ?? "")) fail("CORE_DICTIONARY_CONFIG_INVALID");
