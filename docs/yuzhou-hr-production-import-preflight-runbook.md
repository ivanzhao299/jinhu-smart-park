...
`diagnose-yuzhou-hr-production-source-manifest` 是独立的 `ops-only` 只读路径。它只将三份已版本化的程序文件临时同步至生产宿主机：来源封存器、只读恢复回执校验器和诊断包装器；不会同步备份、阶段文件、私有配置、凭据或业务数据。

宿主机上的私有配置必须为常规 `0600` JSON 文件，并由数据保管环境在本机准备。它只指向受控备份、只读恢复回执、T0～T3 分期目录、映射合同指纹和私有输出根；这些输入的路径和值都不进入 GitHub、日志、工件或交接材料。包装器只接受固定的 `report` 动作，拒绝软链接或权限不符合要求的配置。
