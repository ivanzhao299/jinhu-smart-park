...
import { formalOperationV8, formalStaticV8 } from "../track-b2c-000197-preliminary-orchestrator-v8.mjs";
...
test("formal executable callgraph invokes every hard capability on both targets", () => {
  ...
  const result=formalOperationV8({}, { inputs:mark("inputs"),authorities:mark("authorities",{reviews:2,drain:true}),
...
test("formal static child receives frozen mode as recorded benign env",()=>{const calls=[];formalStaticV8({runChild:(x)=>{calls.push(x);return{};}});
 const stage=calls.find((x)=>x.stage==="static-v8-orchestrator");assert.equal(stage.env.B2C_000197_V8_STATIC_MODE,"frozen");
 assert.deepEqual(stage.envAllowlist.find((x)=>x.name==="B2C_000197_V8_STATIC_MODE"),{name:"B2C_000197_V8_STATIC_MODE",persist:"value"});});

test("authoritative manifest closure excludes returned artifacts and matches declared local dependencies",()=>{
 const source=readFileSync(resolve(process.cwd(),"scripts/e2e/property-remediation/track-b2c-000197-preliminary-orchestrator-v8.mjs"),"utf8");
 ...
 for(const dependency of ["track-b2c-000197-failure-cases-v8.mjs","track-b2c-000197-preliminary-executor-v8.mjs",
   "track-b2c-000197-closure-resolver-v8.mjs","static-v8-capability","static-v8-recursive-closure",
