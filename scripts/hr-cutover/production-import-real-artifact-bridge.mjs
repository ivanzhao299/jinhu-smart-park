...
function readPhaseArtifact(input, expectedTriple) {
  const artifact = explicitBytes(input, "phaseArtifact");
  exactKeys(artifact.parsed, PHASE_WRAPPER_KEYS, "PRODUCTION_IMPORT_REAL_PHASE_ARTIFACT_INVALID", "phaseArtifact");
  ...
  return { ...artifact, phase: artifact.parsed.phase, records: structuredClone(artifact.parsed.records) };
}
...
export function bridgeProductionImportRealArtifacts(input, { model: modelInput = DEFAULT_PRODUCTION_IMPORT_TARGET_MODEL } = {}) {
  ...
  const phases = input.phaseArtifacts.map(row => readPhaseArtifact(row, expectedTriple));
  ...
  const presentTables = [...new Set(phases.flatMap(row => row.records.map(record => record?.targetTable)).filter(value => typeof value === "string"))].sort();
