...
assert.match(transform,/materializeT5NonfileRecord\(name,row,\{protect,customFieldDefinitions,professionalTitleDictionary\}\)/);
assert.match(projection,/materializeLegacyEmployeeCustomFields\(row,customFieldDefinitions\)/);
assert.match(transform,/buildLegacyProfessionalTitleDictionary\(professionalTitleSource\)/);
assert.match(projection,/materializeLegacyProfessionalTitle\(row\.assignment,professionalTitleDictionary\)/);
