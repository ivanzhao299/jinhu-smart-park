...
- [ ] 将员工详情、任职办理、敏感档案和附件组织为选中后工作区。
- [ ] 处理选择变化、刷新、空列表与数据级 403。

## 5. 验证与发布门禁

- [ ] 运行 HR/布局定向测试、Web lint/typecheck/build、全仓必要门禁和 diff check。
- [ ] 本地浏览器验证桌面和 390px：首页、员工目录、导航打开/关闭、无横向溢出。
- [ ] 复核原子权限、敏感投影、严格审计和玉舟兼容测试无回归。
- [ ] 提交前重新 fetch，判断远端并行提交，安全合并主线后再推送独立 PR。
- [ ] CI 全绿后合并部署，核对部署 SHA、迁移无意外、健康检查、Docker 清理及三角色只读生产验收。
