...
DO $$DECLARE b uuid;BEGIN
 ...
 IF EXISTS(SELECT 1 FROM hr_legacy_employee_materialization_gap x JOIN legacy_record_map m ON m.batch_id=b AND m.source_identity_sha256=x.source_identity_sha256 AND m.source_row_sha256=x.source_row_sha256)
 OR EXISTS(SELECT 1 FROM hr_employee_credential x JOIN legacy_record_map m ON m.batch_id=b AND m.source_table='dbo.ticket' AND m.source_identity_sha256=x.legacy_source_identity_sha256 AND m.source_row_sha256=x.legacy_source_row_sha256)
 OR EXISTS(SELECT 1 FROM hr_employee_skill x JOIN legacy_record_map m ON m.batch_id=b AND m.source_table='dbo.knowhow' AND m.source_identity_sha256=x.legacy_source_identity_sha256 AND m.source_row_sha256=x.legacy_source_row_sha256)
 OR EXISTS(SELECT 1 FROM hr_employee_family x JOIN legacy_record_map m ON m.batch_id=b AND m.source_table='dbo.family' AND m.source_identity_sha256=x.legacy_source_identity_sha256 AND m.source_row_sha256=x.legacy_source_row_sha256)
 OR EXISTS(SELECT 1 FROM hr_employee_profile x JOIN legacy_record_map m ON m.batch_id=b AND m.source_table='dbo.person.core_residue' AND m.source_identity_sha256=x.legacy_source_identity_sha256 AND m.source_row_sha256=x.legacy_source_row_sha256) THEN RAISE EXCEPTION'T5 employee materialization rollback residual';END IF;
END$$;
