...
test("new contract drafts continue directly into the unit-linking step", () => {
  ...
  assert.match(page, /CONTRACT_UNIT_REQUIRED_MESSAGE = "提交前必须先在“合同房源”中添加至少一个房源。"/);
  assert.match(page, /openContractDrawer\(row, "units", CONTRACT_UNIT_REQUIRED_MESSAGE\)/);
  assert.match(page, /setContractDrawerMessage\(CONTRACT_UNIT_REQUIRED_MESSAGE\)/);
  assert.match(page, /showForm && editing\?\.id === row\.id/);
  assert.match(page, /setContractDetailTab\("units"\)/);
...
test("unit-linking step explains missing candidate units instead of leaving an empty selector", () => {
  ...
  assert.match(page, /unitOptionsRequestSeq = useRef\(0\)/);
  assert.match(page, /const isCurrentRequest = \(\) => unitOptionsRequestSeq\.current === requestSeq/);
  ...
  assert.match(page, /contractUnitNoticeStyle: CSSProperties/);
  assert.match(page, /whiteSpace: "normal"/);
  assert.match(page, /overflowWrap: "anywhere"/);
  assert.match(page, /style=\{contractUnitNoticeStyle\}/);
