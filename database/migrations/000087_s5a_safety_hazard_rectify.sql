...
permissions(code, name, resource, action, permission_type, perm_type, api_method, api_path, frontend_route, sort_no) AS (
  VALUES
    ('safety_hazard:assign_rectify', '下达隐患整改', 'biz.safety_hazard', 'assign_rectify', 'api', 40, 'POST', '/api/v1/safety/hazards/:id/assign-rectify', NULL, 540),
    ('safety_hazard:rectify', '提交隐患整改', 'biz.safety_hazard', 'rectify', 'api', 40, 'POST', '/api/v1/safety/hazards/:id/rectify', NULL, 550)
),
updated_permissions AS (
  ...
  SET park_id = seed_scope.park_id,
      ...
      is_builtin = true,
      is_deleted = false,
      remark = 'S5-A safety hazard rectification permission seed',
      update_time = now()
  FROM permissions
...
inserted_permissions AS (
  ...
  SELECT
    ...
    true,
    true,
    'S5-A safety hazard rectification permission seed'
  FROM permissions
  CROSS JOIN seed_scope
  WHERE NOT EXISTS (
    SELECT 1
...
updated_items AS (
  ...
  SET item_label = status_items.item_label,
      ...
      remark = 'S5-A safety hazard rectification status seed',
      update_time = now()
  FROM status_items
...
SELECT dict_type.tenant_id,
       ...
       'S5-A safety hazard rectification status seed'
FROM status_items
JOIN dict_type ON true
WHERE NOT EXISTS (
  SELECT 1
...
SELECT seed_scope.tenant_id,
       ...
       false,
       1,
       'S5-A safety hazard rectification role permission seed'
FROM seed_scope
