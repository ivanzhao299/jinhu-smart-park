...
| 阶段 | Issue | PR / main commit | main CI | Deploy | 结果 |
| --- | --- | --- | --- | --- | --- |
| F01 密钥治理 | 父队列 #509 | #510 / `a2d6aac0` | `33361438968` | `33361438970` | PASS |
| F02/F03 同意与留存 | #511 | #512 / `1e888d8e` | `33384573249` | `33384573281` | PASS |
| F04 reveal 审计 | #513 | #514 / `7045c318`；契约 #516 / `4aecb6a7`；归档 #517 / `42ab2d7d` | `33403442390`、`33408123320` | `33403442373`、`33408123283` | PASS；#514 后续由 #516 修正契约门禁 |
| F05 住房实名门槛 | #518 | #519 / `b6934fec`；归档 #520 / `034b7317` | `33415348798`、`33420189278` | `33415348718`、`33420190321` | PASS |
| 全链 UAT | #521 | #522 / `2ea2c25e` | `33427457727` | `33427457714` | PASS |

PR #522 hosted review 在第 1/3 轮完成且无具体建议；PR CI 的 Detect 与 Lint/Typecheck/Build 均通过，Release Smoke 因纯文档/Trellis scope 按设计跳过。#522 squash merge 后，Issue #521 自动关闭，main CI 与 Deploy 均成功。

## 分期决策：住宿业专项继续 HOLD

- P0-02 住宿证件目录/有效期：仅在产品与法务确认上线经营地、客源证件目录、各证件字段/有效期/异常处理规则后另立任务。
- P0-03 公安住宿登记/报送：仅在确定上线地区、取得属地公安接口与 sandbox/联调条件，并批准重复、超时、补报、对账及线下降级 SOP 后另立任务。
- P0-04 未成年人住宿“五必须”：仅在法务与属地公安确认本地细则、监护/关系/询问/可疑报告流程及敏感证据保留期后另立任务。

上述三项不因内部身份底座 PASS 自动解锁，也不得以当前 UAT 结论宣称大陆住宿业生产合规完成；触发条件满足后必须分别建立范围、迁移、权限、审计、联调与 UAT 门禁。
