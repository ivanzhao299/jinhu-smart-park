...
function coreDomainFacts(config, run, domain, tables) {
  ...
  const stableProjection = "COALESCE((SELECT jsonb_object_agg(key,value ORDER BY key) FROM jsonb_each(to_jsonb(x)) WHERE key NOT IN ('id','create_time','update_time','create_by','update_by') AND key !~ '(^id$|_id$)'),'{}'::jsonb)::text";
