...
- 默认仍按首发白名单隐藏
- 如本次发布纳入安全巡检与隐患整改模块，必须先完成 [安全模块上线整改计划](safety-module-release-readiness-plan.md) 的 P0 项
- 完成 P0 后，仅开放已验收的安全巡检与隐患整改核心菜单，不自动开放应急、作业许可等扩展菜单
...
| 非首发一级菜单隐藏 | 检查 IoT / 能耗 / 视频安防 / 机器人 | 不显示 | 待填写 | 待填写 |
| 安全管理菜单开放条件 | 检查安全模块 P0 验收记录和白名单 | 未纳入发布时不显示；纳入发布时仅显示已验收安全核心菜单 | 待填写 | 待填写 |
