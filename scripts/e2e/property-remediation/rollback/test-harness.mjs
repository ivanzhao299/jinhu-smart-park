...
export function buildUnattestedTopLevelFixture({ root, profile, runId, finalSha, now = new Date() }) {
  ...
  for (const input of caseInputs) {
    ...
    const baselineSpecs = [{ ...specs.find(({ id }) => id === "shared-build"), id: "baseline-shared-build" }, { ...specs.find(({ id }) => id === "api-build"), id: "baseline-api-build" }, { ...specs.find(({ id }) => id === "web-clean-production-build"), id: "baseline-web-clean-production-build" }, { ...specs.find(({ id }) => id === "flags-artifact-runtime-proof"), id: "baseline-flags-proof", args: specs.find(({ id }) => id === "flags-artifact-runtime-proof").args.map((value) => value === "false" ? "true" : value) }, { ...specs.find(({ id }) => id === "rollback-service-smoke"), id: "baseline-service-smoke", args: specs.find(({ id }) => id === "rollback-service-smoke").args.map((value) => value === "rollback" ? "baseline" : value) }];
