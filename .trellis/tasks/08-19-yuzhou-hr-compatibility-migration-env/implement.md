# 实施计划：以 M0-M5 与 P0-P4 最终验收为出口的 HR 产品重写
...
## 初始阶段的后续分片记录（历史，不限制父任务范围）

以下是当时 T0 阶段的分工记录；这些业务现纳入父任务 M0-M5 全量范围，并须满足上方 P0-P4 验收。不得将本节用作跳过后续开发的依据，也不得把旧演练计划当成重新全量抽取或装载的指令；实际缺项以当前证据为准。
