...
('hr:contract_reminder:park_read','读取园区劳动合同提醒','park_read',671),
('hr:contract_reminder:team_read','读取团队劳动合同提醒','team_read',672),
('hr:contract_reminder:self_read','读取本人劳动合同提醒','self_read',673),
('hr:contract_reminder:ack','确认劳动合同提醒','ack',674),
('hr:contract_reminder:manage','管理劳动合同提醒','manage',675),
('hr:contract_reminder:run','运行劳动合同提醒','run',676);
...
('HR_MANAGER','hr:contract_reminder:park_read'),('HR_MANAGER','hr:contract_reminder:ack'),('HR_MANAGER','hr:contract_reminder:manage'),('HR_MANAGER','hr:contract_reminder:run'),
('DEPARTMENT_MANAGER','hr:contract_reminder:team_read'),('DEPARTMENT_MANAGER','hr:contract_reminder:ack'),
('EMPLOYEE_SELF_SERVICE','hr:contract_reminder:self_read'),('EMPLOYEE_SELF_SERVICE','hr:contract_reminder:ack');
...
INSERT INTO hr_contract_reminder_policy(tenant_id,park_id,reminder_kind,window_days,recipient_scope,recipient_role_code)
SELECT p.tenant_id,p.park_id,k.kind,w.days,r.scope,CASE WHEN r.scope='hr' THEN 'HR_MANAGER' END
FROM biz_park p
...
ON CONFLICT(tenant_id,park_id,reminder_kind,window_days,recipient_scope)WHERE enabled
DO UPDATE SET recipient_role_code=EXCLUDED.recipient_role_code,enabled=true,update_time=now();
DO $$BEGIN
 IF EXISTS(SELECT 1 FROM rel_role_perm rp JOIN sys_role r ON(r.id,r.tenant_id,r.park_id)=(rp.role_id,rp.tenant_id,rp.park_id) JOIN sys_permission p ON(p.id,p.tenant_id,p.park_id)=(rp.permission_id,rp.tenant_id,rp.park_id) WHERE rp.tenant_id='10000001' AND rp.park_id='20000001' AND rp.is_deleted=false AND ((r.code='DEPARTMENT_MANAGER' AND p.code NOT IN('hr:contract_reminder:team_read','hr:contract_reminder:ack')) OR (r.code='EMPLOYEE_SELF_SERVICE' AND p.code NOT IN('hr:contract_reminder:self_read','hr:contract_reminder:ack'))) AND p.code LIKE 'hr:contract_reminder:%')THEN RAISE EXCEPTION 'Contract reminder exact-set permission drift';END IF;
END$$;
COMMIT;
