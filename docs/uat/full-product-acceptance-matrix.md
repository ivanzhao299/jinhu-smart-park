...
- `https://park.cnjinhu.com` 已是正式 Production；矩阵中的正式状态按模块记录。环境已启用不自动表示所有模块均已开放，只有绑定当前 commit、权限范围、UAT 与发布证据后才能将对应模块标记为 `production_enabled`。

状态定义见 [当前产品范围](../product/current-product-scope.md)。

## 2. 当前矩阵

| 领域 | 核心验收范围 | 开发现状 | 当前 UAT 状态 | 模块正式开放状态 | 主要证据/下一步 |
|---|---|---|---|---|---|
...
| BIM 与数字孪生 | 当前仅验收空间与孪生导航总览及跨模块入口；不包含三维模型或数字孪生运行能力 | 独立导航总览页已实现；完整 BIM/数字孪生能力待建设 | `uat_pending` | 未启用 | 核对 `apps/web/app/bim/overview/page.tsx` 与 `/bim/overview` 的桌面/390px、角色菜单、链接目标和权限；完整 BIM 能力另行规划 |
...
| 民宿管理 | 日价、房态、预订、入住、退房、人工收退款、保洁 | MVP 开发完成，菜单为“民宿管理 → 民宿运营 → `/homestay`” | `uat_pending` | 未启用 | [专项验证证据](./homestay-mvp-evidence.md)：最终候选 `8012b854` 的单测/类型/构建、Chrome DevTools MCP 多视口 UAT 与 PostgreSQL 16 民宿专用真实 API E2E 已通过；main `619b8d20` 的 CI、Release Smoke、生产健康/清理和公开生产保护账号校验已通过。仍待完整真人泳道：隔离 UAT 环境与合成数据、角色账号批准分发、同一候选下任务卡/样本/阈值冻结、每岗位至少 5 名真人代表各执行 4 项任务并覆盖桌面/390px、观察/缺陷台账，以及产品/运营、业务、财务、安全、技术/运维和发布负责人的具名签署与发布批准；继续保持 `uat_pending/awaiting_human_gate` |
| 长租经营 | 个人/企业租客、住宅/办公长租租约、周期费用、交割、报修工单、采购成本 | MVP 开发完成，菜单为“长租经营 → 运营看板 → `/housing/dashboard`” | `uat_pending` | 未启用 | [专项验证证据](./housing-rental-mvp-evidence.md)：迁移、类型检查、单元测试、隔离库 SQL 回归、真实 API 全链路和桌面/390px 视觉验收已通过；待目标角色菜单/页面权限、模块启停、文件、数据范围和真实业务数据验收 |
