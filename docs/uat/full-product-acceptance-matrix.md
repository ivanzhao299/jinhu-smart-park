...
| 民宿管理 | 日价、房态、预订、入住、退房、人工收退款、保洁 | MVP 开发完成，菜单为“民宿管理 → 民宿运营 → `/homestay`” | `uat_pending` | 未启用 | [专项验证证据](./homestay-mvp-evidence.md)：迁移、类型检查、单元测试、隔离库业务回归和本地桌面/390px 视觉验收已通过；待民宿专用真实 API E2E，以及目标角色菜单/页面权限、模块启停、文件、数据范围和真实业务数据验收 |
