...
用户创建组织关系和完整替换关系也获取同一把锁，并在持锁后校验有效组织/岗位。这样关系替换之间是 last-writer-wins，且关系校验不会与组织停用/删除交错提交。
...
新建用户通过 `GET /users/org-candidates?tenantId=&parkId=` 获取目标作用域目录；普通管理员仍被固定到当前作用域，超级管理员才可解析显式目标作用域。编辑继续根据目标用户实体获取目录。

## Bootstrap

`bootstrap-admin` 检测同一用户是否已有其他活动主组织：若有则保留该主组织，并将根组织关系恢复为活动非主关系；若没有则根组织作为主组织。重复运行不会触发单主组织唯一索引冲突。
