...
- `true` requires an active `TENANT_ADMIN` role in the requested park and either `sys_tenant.contact_user_id = sys_user.id`, or pointer-less production-bootstrap provenance `sys_user.create_by IS NULL`. Remarks are mutable user metadata and are never identity authority.
...
- API: pointer match, later admin, ordinary user, pointer-less bootstrap create/rerun/edited-remark states, API-created copied-marker rejection, and requested-park role filtering.
