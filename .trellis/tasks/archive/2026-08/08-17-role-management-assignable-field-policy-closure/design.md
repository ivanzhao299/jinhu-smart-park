...
- 旧 endpoint `/roles/:id/field-permissions` 处理策略：
  - 禁写并返回明确 deprecated 错误；GET 可保留只读审计或迁移核查能力。
  - 所有生产 UI 和模块调用必须使用 `/field-policies/role-bindings/:roleId` 或 `/roles/:id/field-policies` 新接口。
- 旧数据迁移映射：
  - `none -> hidden`
  - `mask -> masked`
  - `read -> readonly`
...
- 旧 `/roles/:id/field-permissions` POST 直接禁写并返回 deprecated 错误，不做自动转换。
- 旧字段权限 `accessMode = "read"` 迁移为新字段策略 `readonly`。
