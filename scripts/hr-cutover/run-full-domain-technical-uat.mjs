...
COMMIT;`;
 ...
 try{
  ...
  const fixture=JSON.parse(psql(config,vars,`SELECT json_build_object('otherWorkReportId',(SELECT id FROM hr_work_report WHERE tenant_id=:'tenant' AND park_id=:'park' AND remark='Yuzhou technical UAT '||:'run' ORDER BY id LIMIT 1));`));
  ...
  writePrivate(observationsPath,{formatVersion:1,parentRunId:config.runId,apiMatrixSha256:apiMatrixIdentity.sha256,observedChecks:workReportMatrix.observations.length,observations:workReportMatrix.observations,productionImport:"HOLD"});registryFile(config,observationsPath);
  const summaryPath=resolve(config.target.evidenceRoot,"technical-uat-summary.json");
  writePrivate(summaryPath,{formatVersion:1,parentRunId:config.runId,status:"PASS",roleTypes:["hr_manager","department_manager","employee_self_service"],actors:["hr_maker","hr_reviewer","department_manager","employee_self_service"],apiChecks:18,negativeAuthorizationChecks:4,webRouteChecks:3,legacyTaskCard:{sha256:taskCardIdentity.sha256,apiMatrixSha256:apiMatrixIdentity.sha256,apiMatrixChecks:apiMatrixIdentity.checkCount,observedChecks:workReportMatrix.observations.length,status:"HOLD_PENDING_REMAINING_MATRIX"},humanUat:"HOLD",productionImport:"HOLD"});registryFile(config,summaryPath);
  ...
  return {state:"uat_ready",technicalUat:"PASS",apiChecks:18,negativeAuthorizationChecks:4,webRouteChecks:3,legacyObservedChecks:workReportMatrix.observations.length,legacyScorePromotion:"HOLD_PENDING_REMAINING_MATRIX",manifestSha256:record.sha256,productionImport:"HOLD"};
 }finally{
