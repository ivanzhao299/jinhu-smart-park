...
function stage(path) {
  ...
  if (manifest.artifactKind !== "yuzhou_t5_photo_owner_stage" || manifest.productionImport !== "HOLD"
    ...
    || domain?.file !== "photo-owner-evidence.jsonl" || ![manifest.sourceSnapshotSha256, manifest.sourceRestoreReceiptSha256, domain.fileSha256].every(value => SHA256.test(value ?? ""))) fail("T5_FILE_STAGE_INVALID", "photo owner manifest boundary");
...
export async function runT5PhotoOwnerEvidenceLab({ configPath, photoOwnerStage, durationMinutes = MIN_DURATION_MINUTES, pollSeconds = 1 }, { coreRunner = runCoreT0T3ContinuousLab, spawn = spawnSync, head = currentHead } = {}) {
  ...
  try {
    ...
    return { status: "CONTRACT_PASS", t0RunId, receipts, cleanupState: cleanup.state, residualCount: cleanup.residualCount, productionImport: "HOLD" };
