...
BEGIN; SET LOCAL lock_timeout='10s'; SET LOCAL statement_timeout='10min'; SELECT set_config('yuzhou.t5_rollback',:'run',true),set_config('yuzhou.t5_db',:'db',true);
DO $$DECLARE b uuid;BEGIN
 ...
 IF EXISTS(SELECT 1 FROM legacy_record_map WHERE batch_id=b AND is_active AND (target_table NOT IN('hr_legacy_t5_record','hr_legacy_t5_quarantine') OR (target_table='hr_legacy_t5_quarantine' AND (target_id IS NOT NULL OR mapping_status<>'quarantined')))) THEN RAISE EXCEPTION 'unexpected nonfile rollback target'; END IF;
...
DO $$DECLARE b uuid;BEGIN SELECT id INTO b FROM migration_batch WHERE run_id=current_setting('yuzhou.t5_rollback'); IF EXISTS(SELECT 1 FROM legacy_record_map m WHERE m.batch_id=b AND m.is_active AND m.target_table='hr_legacy_t5_record' AND EXISTS(SELECT 1 FROM hr_legacy_t5_record x WHERE x.id=m.target_id)) THEN RAISE EXCEPTION 'nonfile record rollback residual'; END IF; END$$;
