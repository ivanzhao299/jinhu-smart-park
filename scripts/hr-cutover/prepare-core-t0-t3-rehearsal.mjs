...
export function parseCorePrepareArgs(argv) {
  const args = {}, allowed = new Set(["--rehearsal", "--suffix", "--postgres-port", "--api-port", "--web-port", "--control-root", "--etl-env", "--source-container", "--source-backup", "--source-restore-receipt", "--machine-attestation-root", "--event-type-package", "--event-state-package", "--contract-type-package", "--contract-state-package"]);
  ...
  for (const key of ["rehearsal", "suffix", "postgresPort", "apiPort", "webPort", "controlRoot", "etlEnv", "sourceContainer", "sourceBackup", "sourceRestoreReceipt", "machineAttestationRoot", "eventTypePackage", "eventStatePackage", "contractTypePackage", "contractStatePackage"]) if (!args[key]) fail("CORE_PREPARE_ARGUMENT_MISSING", key);
...
export function prepareCoreConfig(argsInput, { codeSha, mappingContractHash = computeCoreT0T3MappingContractHash() }) {
  ...
  const dictionaryPreflight = verifyCoreDictionaryPreflight(dictionaryPackages);
  ...
  const config = {
    ...
    source: { readOnly: true, sourceBackupSha256: sourceSnapshotHash, sourceBackupPath: sourceBackup, sourceRestoreReceiptPath: sourceRestoreReceipt, sourceRestoreReceiptSha256, databaseAlias: etl.YUZHOU_SQLSERVER_DATABASE, etlEnvFile: etlCopy, sourceContainer: args.sourceContainer, dictionaryPackages },
