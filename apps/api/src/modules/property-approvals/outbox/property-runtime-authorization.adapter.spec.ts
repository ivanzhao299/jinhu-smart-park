...
type FixtureOptions = {
  moduleState?: "active" | "missing" | "disabled" | "expired";
  permissions?: string[];
  ...
  const manager = {
    query: async (sql: string, params: unknown[] = []) => {
      ...
      if (sql.includes("FROM rel_tenant_module")) {
        const state = options.moduleState ?? "active";
        if (state === "active") return [{ id: "module-assignment" }];
        // These fixtures model distinct persisted rows. The adapter query must
        // filter disabled and expired rows out, just as it filters a missing row.
        if (state === "disabled") {
...
describe("DatabasePropertyRuntimeAuthorizationAdapter", () => {
  ...
  it("fails each module, permission and assigned-scope dimension independently", async () => {
    ...
    const cases: FixtureOptions[] = [
      { moduleState: "missing", permissions: exact },
  ...
  it("rejects missing, disabled and expired module assignments with the full predicate", async () => {
    ...
    for (const moduleState of ["missing", "disabled", "expired"] as const) {
      const { adapter, statements } = fixture({ moduleState, permissions });
      await assert.rejects(adapter.authorize({
        scope,
        actor,
        surface: "event-delivery",
        operation: "read"
      }), ForbiddenException, moduleState);
