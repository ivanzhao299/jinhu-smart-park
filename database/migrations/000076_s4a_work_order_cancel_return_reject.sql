...
permissions(code, name, resource, action, api_path) AS (
  VALUES
    ('workorder:cancel', '工单取消', 'biz.work_order', 'cancel', '/api/v1/work-orders/:id/cancel'),
    ('workorder:return', '工单退回', 'biz.work_order', 'return', '/api/v1/work-orders/:id/return'),
    ('workorder:reject', '工单驳回', 'biz.work_order', 'reject', '/api/v1/work-orders/:id/reject')
)
...
role_permissions(role_code, permission_code) AS (
  VALUES
    ...
    ('PROPERTY_MANAGER', 'workorder:return'),
