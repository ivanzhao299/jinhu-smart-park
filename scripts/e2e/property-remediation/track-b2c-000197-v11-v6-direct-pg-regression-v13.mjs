...
const prefix = "b2c-000197-v11-v6-direct-pg-regression-v13";
const fixed = Object.freeze({
  ...
  spec: resolve(root, "scripts/e2e/property-remediation/tests/b2c-000197-v11-v6-direct-pg-regression-v13.spec.mjs"),
});
const sha = (value) => createHash("sha256").update(value).digest("hex");
const hex64 = /^[0-9a-f]{64}$/u;
const runIdPattern = /^b2c197_v11v6_direct_v13_[a-z0-9_]{8,40}$/u;
const containerPattern = /^jinhu-b2c197-v11v6-direct-v13-[a-z0-9-]{8,40}$/u;
const databasePattern = /^jinhu_b2c197_v11v6_direct_v13_[a-z0-9_]{4,30}$/u;
...
function sealed(path) {
  if (!path.startsWith(`${research}${sep}`) && path !== fixed.runner && path !== fixed.spec) throw new Error("b2c-v13-fixed-path");
  if (!existsSync(path) || lstatSync(path).isSymbolicLink() || realpathSync(path) !== path) throw new Error("b2c-v13-sealed-input");
  let descriptor;
  try { descriptor = openSync(path, constants.O_RDONLY | constants.O_NOFOLLOW); const metadata = fstatSync(descriptor); if (!metadata.isFile() || (metadata.mode & 0o777) !== 0o444) throw new Error("b2c-v13-sealed-input"); return readFileSync(descriptor); }
...
function baselinePlan() {
  ...
  if (plan.length !== 194 || sha(JSON.stringify(manifest)) !== BASELINE_MANIFEST_SHA256) throw new Error("b2c-v13-baseline-manifest-drift");
  return Object.freeze(plan.map(Object.freeze));
}
function sealedSource(path) { if (!existsSync(path) || lstatSync(path).isSymbolicLink() || realpathSync(path) !== path) throw new Error("b2c-v13-migration-path-drift"); let descriptor; try { descriptor = openSync(path, constants.O_RDONLY | constants.O_NOFOLLOW); const metadata = fstatSync(descriptor); if (!metadata.isFile()) throw new Error("b2c-v13-migration-path-drift"); return readFileSync(descriptor); } finally { if (descriptor !== undefined) closeSync(descriptor); } }
function migrationBytesImmediatelyBeforeExecution(entry) { const raw = sealedSource(entry.path); if (sha(raw) !== entry.raw_sha256) throw new Error(`b2c-v13-migration-byte-drift:${entry.filename}`); return raw.toString("utf8"); }

function validateRegistry(value) {
  exactObject(value, ["schema_version", "resources"], "b2c-v13-registry-schema"); exactString(value.schema_version, "b2c-v13-resource-registry-v1", "b2c-v13-registry-schema");
  if (!Array.isArray(value.resources) || value.resources.length !== 9) throw new Error("b2c-v13-registry-count");
  const labels = new Set(), identities = new Set();
  for (const entry of value.resources) { exactObject(entry, ["label", "run_id", "container", "container_id", "database", "volume"], "b2c-v13-registry-entry"); if (!/^(A|B|C|D|E|F|G|H|v4)$/u.test(entry.label) || labels.has(entry.label)) throw new Error("b2c-v13-registry-label"); labels.add(entry.label); for (const key of ["run_id", "container", "container_id", "database", "volume"]) { if (typeof entry[key] !== "string" || !entry[key] || identities.has(entry[key])) throw new Error("b2c-v13-registry-identity"); identities.add(entry[key]); } }
  if (labels.size !== 9) throw new Error("b2c-v13-registry-labels"); return identities;
}
function staticIntake() {
  ...
  const candidate = sealedJson(fixed.candidate, candidateKeys, "b2c-v13-candidate-schema"); exactString(candidate.value.schema_version, "b2c-v13-candidate-authority-v1", "b2c-v13-candidate-schema"); exactString(candidate.value.status, "PENDING_REVIEW", "b2c-v13-candidate-status");
  for (const key of ["execution_authorized", "container_create_authorized", "container_execute_authorized", "formal_go", "docker_or_database_command_executed"]) exactBoolean(candidate.value[key], false, `b2c-v13-candidate-flag:${key}`);
  for (const key of ["runner_raw_sha256", "runner_spec_raw_sha256", "registry_raw_sha256", "static_test_record_raw_sha256", "node22_tap_raw_sha256", "node24_tap_raw_sha256"]) assertHash(candidate.value[key], `b2c-v13-candidate-hash:${key}`);
  ...
  for (const [key, value] of Object.entries(live)) if (candidate.value[key] !== value) throw new Error(`b2c-v13-live-byte-drift:${key}`);
  ...
  const manifest = sealedJson(fixed.manifest, manifestKeys, "b2c-v13-manifest-schema"); exactString(manifest.value.schema_version, "b2c-v13-candidate-manifest-v1", "b2c-v13-manifest-schema"); exactString(manifest.value.status, "PENDING_REVIEW", "b2c-v13-manifest-status");
  exactObject(manifest.value.candidate_flags, ["container_create_authorized", "container_execute_authorized", "execution_authorized", "formal_go"], "b2c-v13-manifest-flags"); for (const value of Object.values(manifest.value.candidate_flags)) exactBoolean(value, false, "b2c-v13-manifest-flags");
  const expectedHashes = { candidate_authority_raw_sha256: candidate.rawSha256, ...live }; for (const [key, value] of Object.entries(expectedHashes)) if (manifest.value[key] !== value) throw new Error(`b2c-v13-manifest-binding:${key}`);
  if (!same(manifest.value.review_chain, ["candidate", "database", "qa", "security", "drain", "provision_authority", "resource_observation", "execution_authority"])) throw new Error("b2c-v13-review-chain");
  exactObject(manifest.value.canonical_paths, Object.keys(fixed).filter((key) => !["runner", "spec", "evidence"].includes(key)), "b2c-v13-canonical-paths"); for (const key of Object.keys(manifest.value.canonical_paths)) exactString(manifest.value.canonical_paths[key], fixed[key].slice(root.length + 1), `b2c-v13-canonical-path:${key}`);
  exactString(manifest.value.trust_root, "repository-fixed-path-procedural-root-v1", "b2c-v13-trust-root");
  const record = sealedJson(fixed.staticRecord, ["schema_version", "runner_raw_sha256", "test_raw_sha256", "node22", "node24", "docker_or_database_command_executed", "coverage"], "b2c-v13-static-record-schema"); exactString(record.value.schema_version, "b2c-v13-static-test-record-v1", "b2c-v13-static-record-schema"); exactBoolean(record.value.docker_or_database_command_executed, false, "b2c-v13-static-record-command");
  if (record.value.runner_raw_sha256 !== live.runner_raw_sha256 || record.value.test_raw_sha256 !== live.runner_spec_raw_sha256) throw new Error("b2c-v13-static-record-binding");
  for (const [key, tapKey] of [["node22", "node22_tap_raw_sha256"], ["node24", "node24_tap_raw_sha256"]]) { exactObject(record.value[key], ["binary", "result", "raw_tap_sha256"], "b2c-v13-static-record-node"); if (typeof record.value[key].binary !== "string" || record.value[key].result !== "pass" || record.value[key].raw_tap_sha256 !== live[tapKey]) throw new Error("b2c-v13-static-record-node"); }
  if (!Array.isArray(record.value.coverage) || record.value.coverage.some((item) => typeof item !== "string")) throw new Error("b2c-v13-static-record-coverage");
  const registry = sealedJson(fixed.registry, ["schema_version", "resources"], "b2c-v13-registry-schema"); const prohibited = validateRegistry(registry.value);
...
function reviewIntake(base) {
  ...
  const schema = { databaseReview: "b2c-v13-independent-database-review-v1", qaReview: "b2c-v13-independent-qa-review-v1", securityReview: "b2c-v13-independent-security-review-v1", drainReview: "b2c-v13-independent-old-writer-drain-review-v1" };
  for (const key of reviewOrder) {
    ...
    const review = sealedJson(fixed[key], keys, `b2c-v13-${key}-schema`); exactString(review.value.schema_version, schema[key], `b2c-v13-${key}-schema`); exactString(review.value.decision, "GO", `b2c-v13-${key}-decision`); exactBoolean(review.value.review_approved, true, `b2c-v13-${key}-approved`); exactZero(review.value.open_p0, `b2c-v13-${key}-p0`); exactZero(review.value.open_p1, `b2c-v13-${key}-p1`);
    ...
    for (const [field, value] of Object.entries(expected)) if (review.value[field] !== value) throw new Error(`b2c-v13-${key}-binding:${field}`);
...
function provisionIntake() {
  ...
  const authority = sealedJson(fixed.provisionAuthority, keys, "b2c-v13-provision-schema"); exactString(authority.value.schema_version, "b2c-v13-provision-authority-v1", "b2c-v13-provision-schema"); exactString(authority.value.status, "AUTHORIZED", "b2c-v13-provision-status"); exactBoolean(authority.value.container_create_authorized, true, "b2c-v13-provision-create"); for (const key of ["container_execute_authorized", "execution_authorized", "formal_go"]) exactBoolean(authority.value[key], false, `b2c-v13-provision-flag:${key}`);
  const bindings = { candidate_authority_raw_sha256: base.candidateSha, candidate_manifest_raw_sha256: base.manifestSha, database_review_raw_sha256: reviews.databaseReview, qa_review_raw_sha256: reviews.qaReview, security_review_raw_sha256: reviews.securityReview, drain_review_raw_sha256: reviews.drainReview }; for (const [key, value] of Object.entries(bindings)) if (authority.value[key] !== value) throw new Error(`b2c-v13-provision-binding:${key}`);
  if (!runIdPattern.test(authority.value.run_id) || !containerPattern.test(authority.value.container) || !databasePattern.test(authority.value.database)) throw new Error("b2c-v13-provision-identity"); exactString(authority.value.image_reference, "postgres:16-alpine", "b2c-v13-provision-image"); exactZero(authority.value.host_port_bindings, "b2c-v13-provision-ports"); exactBoolean(authority.value.anonymous_volume, true, "b2c-v13-provision-volume"); exactString(authority.value.mount_type, "volume", "b2c-v13-provision-mount"); exactBoolean(authority.value.mount_rw, true, "b2c-v13-provision-mount"); exactString(authority.value.mount_destination, "/var/lib/postgresql/data", "b2c-v13-provision-mount");
  for (const identity of [authority.value.run_id, authority.value.container, authority.value.database]) if (base.prohibited.has(identity)) throw new Error("b2c-v13-prohibited-resource-reuse");
...
function inspect(command, target, runCommand, expectedLabels) {
  ...
  if (result.error || result.signal || result.status !== 0) throw new Error(`b2c-v13-${command}-inspect-child`); const rows = String(result.stdout ?? "").trim().split("\n"), mounts = JSON.parse(rows[4] ?? "null");
  const labels = JSON.parse(rows[5] ?? "null");
  if (rows.length !== 6 || !hex64.test(rows[0] ?? "") || rows[1] !== "true" || !hex64.test(rows[2] ?? "") || !same(JSON.parse(rows[3] ?? "null"), {}) || !Array.isArray(mounts) || mounts.length !== 1 || mounts[0]?.Type !== "volume" || !hex64.test(mounts[0]?.Name ?? "") || mounts[0]?.RW !== true || mounts[0]?.Destination !== "/var/lib/postgresql/data" || !labels || typeof labels !== "object" || labels["jinhu.b2c.run_id"] !== expectedLabels.run_id || labels["jinhu.b2c.provision_authority_sha256"] !== expectedLabels.provision_authority_sha256) throw new Error(`b2c-v13-${command}-identity-drift`);
  return { container_id: rows[0], image_id: rows[2], volume_id: mounts[0].Name };
}
function writeSealed(path, value) { if (!path.startsWith(`${research}${sep}`) || existsSync(path) || lstatSync(dirname(path)).isSymbolicLink() || realpathSync(dirname(path)) !== dirname(path)) throw new Error("b2c-v13-output-path"); writeFileSync(path, `${JSON.stringify(value, null, 2)}\n`, { flag: "wx", mode: 0o444 }); chmodSync(path, 0o444); }

/** Phase one: sealed fixed-path intake authorizes only the exact pre-create intent. */
export function provisionV13({ runCommand = spawnSync } = {}) {
  ...
  try {
    ...
    createdId = String(created.stdout ?? "").trim(); if (created.error || created.signal || created.status !== 0 || !hex64.test(createdId)) throw new Error("b2c-v13-provision-child");
    const labels = { run_id: intent.run_id, provision_authority_sha256: intake.authoritySha }; const actual = inspect("provision", { container: intent.container }, runCommand, labels); if (actual.container_id !== createdId) throw new Error("b2c-v13-provision-container-id");
    const observation = { schema_version: "b2c-v13-resource-observation-v1", status: "OBSERVED", execution_authorized: false, formal_go: false, provision_authority_raw_sha256: intake.authoritySha, candidate_authority_raw_sha256: intake.base.candidateSha, candidate_manifest_raw_sha256: intake.base.manifestSha, run_id: intent.run_id, container: intent.container, container_id: actual.container_id, database: intent.database, volume_id: actual.volume_id, image_id: actual.image_id, image_reference: intent.image_reference, host_port_bindings: 0, anonymous_volume: true, mount_type: "volume", mount_rw: true, mount_destination: "/var/lib/postgresql/data", labels };
...
function executionIntake() {
  ...
  const observation = sealedJson(fixed.observation, observationKeys, "b2c-v13-observation-schema"); exactString(observation.value.schema_version, "b2c-v13-resource-observation-v1", "b2c-v13-observation-schema"); exactString(observation.value.status, "OBSERVED", "b2c-v13-observation-status"); exactBoolean(observation.value.execution_authorized, false, "b2c-v13-observation-execute"); exactBoolean(observation.value.formal_go, false, "b2c-v13-observation-formal");
  const expectedObservation = { provision_authority_raw_sha256: intake.authoritySha, candidate_authority_raw_sha256: intake.base.candidateSha, candidate_manifest_raw_sha256: intake.base.manifestSha, run_id: intake.authority.run_id, container: intake.authority.container, database: intake.authority.database, image_reference: "postgres:16-alpine", host_port_bindings: 0, anonymous_volume: true, mount_type: "volume", mount_rw: true, mount_destination: "/var/lib/postgresql/data" }; for (const [key, value] of Object.entries(expectedObservation)) if (!Object.is(observation.value[key], value)) throw new Error(`b2c-v13-observation-binding:${key}`); exactObject(observation.value.labels, ["run_id", "provision_authority_sha256"], "b2c-v13-observation-labels"); if (observation.value.labels.run_id !== observation.value.run_id || observation.value.labels.provision_authority_sha256 !== intake.authoritySha) throw new Error("b2c-v13-observation-labels"); for (const key of ["container_id", "volume_id", "image_id"]) assertHash(observation.value[key], `b2c-v13-observation-identity:${key}`); for (const identity of [observation.value.container_id, observation.value.volume_id, observation.value.image_id]) if (intake.base.prohibited.has(identity)) throw new Error("b2c-v13-prohibited-resource-reuse");
...
export function assertSnapshotV13(value) {
  exactObject(value, ["primary_history", "mirror_history", "forbidden_primary", "forbidden_mirror", "approval_rows", "keys", "indexdef", "predicate", "catalog"], "b2c-v13-snapshot-schema");
  const late = expectedLate(); if (!same(value.primary_history, late) || !same(value.mirror_history, late)) throw new Error("b2c-v13-snapshot-history"); for (const key of ["forbidden_primary", "forbidden_mirror", "approval_rows"]) exactZero(value[key], `b2c-v13-snapshot-zero:${key}`); if (!same(value.keys, ["tenant_id", "park_id", "action_id", "source_type", "source_id", "source_expected_version"]) || value.indexdef !== OLD_INDEX_SHA || value.predicate !== OLD_PREDICATE_SHA) throw new Error("b2c-v13-snapshot-contract"); exactObject(value.catalog, ["index_exists", "build_residue"], "b2c-v13-snapshot-catalog"); exactBoolean(value.catalog.index_exists, true, "b2c-v13-snapshot-index"); exactBoolean(value.catalog.build_residue, false, "b2c-v13-snapshot-residue"); return true;
}
export function assertFaultMarkerV13(output, marker) { const lines = String(output).split(/\r?\n/u).filter((line) => /^ERROR:\s+P0001:/u.test(line)); const match = lines.length === 1 ? lines[0].match(/^ERROR:\s+P0001:\s*([^\s].*?)\s*$/u) : null; if (!match || match[1] !== marker || !/^v13-injected-(before-create|after-create|after-drop|before-rename)$/u.test(marker)) throw new Error("b2c-v13-fault-marker"); }
...
const faults = Object.freeze([
  { boundary: "before-create", prefix: "", assertion: catalogAssertion(true, false), marker: "v13-injected-before-create" },
  { boundary: "after-create", prefix: createBuildIndex, assertion: catalogAssertion(true, true), marker: "v13-injected-after-create" },
  { boundary: "after-drop", prefix: `${createBuildIndex} DROP INDEX public.uq_biz_property_approval_request_active_source;`, assertion: catalogAssertion(false, true), marker: "v13-injected-after-drop" },
  { boundary: "before-rename", prefix: `${createBuildIndex} DROP INDEX public.uq_biz_property_approval_request_active_source;`, assertion: catalogAssertion(false, true), marker: "v13-injected-before-rename" },
].map(Object.freeze));
...
/** Phase two: a distinct fixed-path authority binds the immutable observation and authorizes execute only. */
export function executeV13({ runCommand = spawnSync } = {}) {
  ...
  try {
    const actual = inspect("execute", target, (command, argv, options) => evidence.child("inspect", command, argv, options.input, runCommand), target.labels); if (actual.container_id !== target.container_id || actual.volume_id !== target.volume_id || actual.image_id !== target.image_id) throw new Error("b2c-v13-execute-identity-drift");
    if (!/^16[0-9]{4}$/u.test(psql(evidence, target, "postgres-16", "SHOW server_version_num;", runCommand).stdout.trim())) throw new Error("b2c-v13-version-drift");
    ...
    const sql = snapshotSql(expectedLate()), initial = JSON.parse(psql(evidence, target, "initial-snapshot", sql, runCommand).stdout.trim()); assertSnapshotV13(initial); const results = [];
    for (const fault of faults) { const before = JSON.parse(psql(evidence, target, `before-${fault.boundary}`, sql, runCommand).stdout.trim()); assertSnapshotV13(before); const failed = psql(evidence, target, `fault-${fault.boundary}`, faultSql(fault), runCommand, true); if (failed.status === 0) throw new Error(`b2c-v13-fault-success:${fault.boundary}`); assertFaultMarkerV13(`${failed.stdout}${failed.stderr}`, fault.marker); const after = JSON.parse(psql(evidence, target, `after-${fault.boundary}`, sql, runCommand).stdout.trim()); assertSnapshotV13(after); if (!same(before, after)) throw new Error(`b2c-v13-rollback-drift:${fault.boundary}`); results.push({ boundary: fault.boundary, marker: fault.marker, sqlstate: "P0001", snapshot_exact: true }); }
    const final = JSON.parse(psql(evidence, target, "final-snapshot", sql, runCommand).stdout.trim()); assertSnapshotV13(final); if (!same(initial, final)) throw new Error("b2c-v13-final-drift"); evidence.terminal("success", { direct_baseline: "000185-000195-original-bytes", faults: results }, provenance); return Object.freeze({ faults: results });
  } catch (error) { evidence.terminal("failure", { failure_stage: stage(error.stage ?? "runner"), error: { name: String(error.name), message: String(error.message) } }, provenance); throw error; }
}

export function candidateV13() { return Object.freeze({ status: "PENDING_REVIEW", execution_authorized: false, container_create_authorized: false, container_execute_authorized: false, formal_go: false, docker_or_database_command_executed: false, trust_root: "repository-fixed-path-procedural-root-v1", review_chain: ["candidate", "database", "qa", "security", "drain", "provision_authority", "resource_observation", "execution_authority"] }); }
