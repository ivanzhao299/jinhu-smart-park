...
async function insertMaps(tx, batchId, inserted, quarantined) {
...
export async function writeT5NonfilePrivateStage(input) {
  ...
  await insertMaps(input.tx, batchId, inserted, quarantined);
  ...
  return { phase: "T5", migrationBatchId: batchId, counts, records: [...inserted.map(({ record, targetId }) => ({ sourceIdentitySha256: record.sourceIdentitySha256, disposition: "insert", targetTable: record.targetTable, targetId })), ...quarantined.map(record => ({ sourceIdentitySha256: record.sourceIdentitySha256, disposition: "quarantine", targetTable: record.targetTable, reasonCode: record.quarantineReason }))] };
}
