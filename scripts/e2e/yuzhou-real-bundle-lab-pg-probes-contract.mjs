...
import {createYuzhouRealBundleLabPgProbes} from "../hr-cutover/yuzhou-real-bundle-lab-pg-probes.mjs";
...
const scope={tenantId:"fixture",parkId:"fixture"};scope.scopeSha256=computeProductionImportTargetScopeHash(scope);
const base={expectedDatabase:"jinhu_hr_migration_lab_fixture",targetScope:scope,runId:"fixture-run",codeSha:"a".repeat(40),sourceSnapshotHash:"b".repeat(64),
 baselineCounts:Object.fromEntries(Object.keys(model.targetTables).map(t=>[t,t==="sys_org"?14:t==="hr_contract_type"?3:0])),
 phaseCounts:Object.fromEntries(["T0","T1","T2","T3"].map(p=>[p,{records:2,inserted:1,quarantined:1}])),getHttpFixtureIds:async()=>({userIds:[],roleIds:[]})};
...
test("residual checks restored physical counts, reversed ledger and registered fixture dependencies",async()=>{const probe=createYuzhouRealBundleLabPgProbes(base),baseline={tableCounts:base.baselineCounts};assert.equal((await probe.verifyResidual({...fixture({reversed:true}),baseline})).baselineRestored,true);for(const options of [{fixtureLeak:true},{residual:true},{applied:true}])await assert.rejects(probe.verifyResidual({...fixture(options),baseline}),/LAB_PG_PROBE_GUARD/);});
test("unsafe scope/database/table expectations and malformed fixture IDs fail closed",async()=>{for(const change of [{expectedDatabase:"production"},{targetScope:{...scope,tenantId:"x';DROP"}},{baselineCounts:{...base.baselineCounts,"evil;DROP":0}}])assert.throws(()=>createYuzhouRealBundleLabPgProbes({...base,...change}),/LAB_PG_PROBE_GUARD/);const probe=createYuzhouRealBundleLabPgProbes({...base,getHttpFixtureIds:async()=>({userIds:["unsafe"],roleIds:[]})});await assert.rejects(probe.verifyResidual({...fixture(),baseline:{tableCounts:base.baselineCounts}}),/LAB_PG_PROBE_GUARD/);});
