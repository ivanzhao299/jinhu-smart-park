...
候选总入口已经把 `000311 → 000308 → 000310` 串接，并用运行时三个真实回执驱动逆序回滚。合成隔离 PostgreSQL 总链已验证 66 条非空配置、非空周期/人员关系、190 条映射自动晋级、实际服务查询、回滚后查询清空及失败零残留；精确执行字节和覆盖边界见 `docs/yuzhou-hr-performance-fact-identity-integration.md`。HTTP/前端、非空结果业务等价以及真实生产闭环仍未完成验收；当前真实空集不能替代未来找到权威非空快照后的业务 UAT。
