  ...
  async function openCreate() {
    ...
    if (selectedTenant) {
      ...
      try {
        ...
        await Promise.all([
          loadOrgCatalog(undefined, { tenantId: selectedTenant.tenantId, parkId: selection.parkId }),
          loadRoleCatalog(undefined, { tenantId: selectedTenant.tenantId, parkId: selection.parkId })
        ]);
      } catch (error) {
        if (requestId === orgCatalogRequest.current) setOrgCatalogLoading(false);
        throw error;
  ...
  async function saveUser(event: FormEvent<HTMLFormElement>) {
    ...
    if (canAssignRoles && (!wasEditing || rolesChanged)) {
      try {
        ...
        if (!wasEditing) {
          setEditingUser({ ...savedUser, roles: [] });
          setShowCreate(false);
          setLoadedRoleIds([]);
        }
        throw new Error(`${wasEditing ? "用户资料已保存" : "用户已创建"}，但角色配置失败，请在当前窗口重试：${error instanceof Error ? error.message : "未知错误"}`);
      }
    }
    setEditingUser(null);
    setShowCreate(false);
    ...
    clearOrgCatalog();
    clearRoleCatalog();
    const successMessage = wasEditing ? "用户更新成功" : "用户创建成功";
    ...
    <main className="page-container">
      <header className="page-header">
        <div className="header-title">
          <strong>用户管理</strong>
          <span>维护账号、角色、所属租户、默认园区、可访问园区和登录上下文状态</span>
        </div>
          ...
          {data.items.map((item) => (
            <article className="ds-mobile-record" key={item.id}>
              ...
              <div className="task-item">
                ...
                <PermissionButton permission={SYSTEM_PERMISSIONS.USER_UPDATE} type="button" title="编辑用户" onClick={() => void openEdit(item).catch((error: Error) => setMessage(error.message))}>
                  <Edit3 size={16} />编辑
                </PermissionButton>
              </div>
            </article>
      ...
      {(showCreate || editingUser) ? (
        <Drawer size="lg" onClose={closeUserDrawer}>
          <DrawerHeader
            ...
            description="维护用户账号、角色、登录上下文与可访问园区。"
          ...
          <DrawerForm onSubmit={(event) => { setDrawerError(""); void saveUser(event).catch((error: Error) => setDrawerError(error.message)); }}>
                ...
                <select
                  ...
                  onChange={(event) => {
                    ...
                    void loadLoginSettings(
                      ...
                      return Promise.all([
                        loadOrgCatalog(undefined, { tenantId: nextTenantId, parkId: selection.parkId }),
                        loadRoleCatalog(undefined, { tenantId: nextTenantId, parkId: selection.parkId })
                      ]);
            ...
            {canAssignRoles ? (
              <DrawerFormGrid single>
                <div className="field">
                      ...
                      return (
                        <label key={role.id} className="checkbox-row">
                          <input
                            ...
                            disabled={Boolean(editingUser && (formTenantId !== editingUser.tenantId || formParkId !== editingUser.parkId))}
                  ...
                  <span className="muted-text">角色所包含的功能权限、数据权限和字段策略请在“角色管理”中维护。</span>
                </div>
              </DrawerFormGrid>
            ) : null}
            <DrawerFormGrid single>
            ...
            <DrawerFooter>
              ...
              <button className="primary-button" type="submit" disabled={loginSettingsLoading || orgCatalogLoading || (canAssignRoles && roleCatalogLoading) || !formParkId}><CheckCircle2 size={16} />{orgCatalogLoading || roleCatalogLoading ? "配置加载中…" : "保存"}</button>
            </DrawerFooter>
          </DrawerForm>
        </Drawer>
      ) : null}
...
function formatRoleNames(roles: UserRoleOption[]): string {
  return roles.length > 0 ? roles.map((role) => role.name).join("、") : "未配置";
}
