...
import {
  assertTscFileListMatchesV9, classifySpecifierV9, resolveFormalExecutionClosureV9,
...
test("recursive formal closure includes critical PG, authorization, principal and shared dependencies", () => {
  const closure = resolveFormalExecutionClosureV9(root); const files = new Set(closure.repositoryFiles);
...
test("builtin and external dependencies are explicit while repository misses fail closed", () => {
  ...
  assert.deepEqual(classifySpecifierV9(importer, "node:test", root), { kind: "builtin", value: "node:test" });
  assert.equal(classifySpecifierV9(importer, "typeorm", root).kind, "external");
  assert.deepEqual(classifySpecifierV9(importer, "./missing-v9-owned-module", root),
    { kind: "unresolved", value: "./missing-v9-owned-module" });
  assert.equal(classifySpecifierV9(importer, "@jinhu/shared", root).kind, "repository");
  const closure = resolveFormalExecutionClosureV9(root);
...
test("TypeScript Program repository files exactly match real tsc listFilesOnly", () => {
  ...
  const result = assertTscFileListMatchesV9(child.stdout, root);
