...
const prefix = "b2c-000197-v11-v6-direct-pg-regression-v26";
const fixed = Object.freeze({
  ...
  spec: resolve(root, "scripts/e2e/property-remediation/tests/b2c-000197-v11-v6-direct-pg-regression-v26.spec.mjs"),
});
const sha = (value) => createHash("sha256").update(value).digest("hex");
const hex64 = /^[0-9a-f]{64}$/u;
const runIdPattern = /^b2c197_v11v6_direct_v26_[a-z0-9_]{8,40}$/u;
const containerPattern = /^jinhu-b2c197-v11v6-direct-v26-[a-z0-9-]{8,40}$/u;
const databasePattern = /^jinhu_b2c197_v11v6_direct_v26_[a-z0-9_]{4,30}$/u;
...
export function compareCanonicalPathV26(left, right) {
...
function sealed(path) {
  if (!path.startsWith(`${research}${sep}`) && path !== fixed.runner && path !== fixed.spec) throw new Error("b2c-v26-fixed-path");
  if (!existsSync(path) || lstatSync(path).isSymbolicLink() || realpathSync(path) !== path) throw new Error("b2c-v26-sealed-input");
  let descriptor;
  try { descriptor = openSync(path, constants.O_RDONLY | constants.O_NOFOLLOW); const metadata = fstatSync(descriptor); if (!metadata.isFile() || (metadata.mode & 0o777) !== 0o444) throw new Error("b2c-v26-sealed-input"); return readFileSync(descriptor); }
...
function recursiveFiles(directory) {
  ...
  for (const entry of readdirSync(directory, { withFileTypes: true })) {
    ...
    if (entry.isSymbolicLink()) throw new Error("b2c-v26-formal-source-symlink");
...
function formalSourcePlan() {
  ...
  const plan = [...new Set(paths)].sort(compareCanonicalPathV26).map((path) => {
    if (!existsSync(path) || lstatSync(path).isSymbolicLink() || !statSync(path).isFile()) throw new Error("b2c-v26-formal-source-path");
  ...
  if (plan.length !== FORMAL_SOURCE_COUNT || sha(JSON.stringify(manifest)) !== FORMAL_SOURCE_MANIFEST_SHA256) throw new Error("b2c-v26-formal-source-manifest-drift");
  const migration197 = plan.find(({ path }) => path === migration197Path);
  if (!migration197 || migration197.raw_sha256 !== MIGRATION_000197_SHA256) throw new Error("b2c-v26-migration-000197-drift");
...
function verifyFormalSources(frozen) {
  for (const entry of frozen.plan) if (sha(readFileSync(entry.path)) !== entry.raw_sha256) throw new Error(`b2c-v26-formal-source-toctou:${entry.relative}`);
}
...
function packageRootFromEntry(entry, expectedName) {
  ...
  throw new Error(`b2c-v26-runtime-package-root:${expectedName}`);
}
function resolveRuntimePackage(name, fromPackageJson) {
  const resolver = createRequire(fromPackageJson); let entry;
  try { entry = resolver.resolve(name); } catch { try { entry = resolver.resolve(`${name}/package.json`); } catch { throw new Error(`b2c-v26-runtime-package-missing:${name}`); } }
  return isAbsolute(entry) ? packageRootFromEntry(realpathSync(entry), name) : null;
}
function runtimePackageEntries(packageRoot) {
  ...
  const walk = (directory) => {
    for (const item of readdirSync(directory, { withFileTypes: true })) {
      ...
      if (!path.startsWith(`${root}${sep}`)) throw new Error("b2c-v26-runtime-path-escape");
      if (item.isSymbolicLink()) { const target = realpathSync(path); if (!target.startsWith(`${packageRoot}${sep}`)) throw new Error("b2c-v26-runtime-symlink-escape"); entries.push({ kind: "symlink", path: relative, target: target.slice(root.length + 1).split(sep).join("/") }); }
...
function runtimeDependencyPlan() {
  ...
  while (queue.length) {
    const request = queue.shift(); let packageRoot; try { packageRoot = resolveRuntimePackage(request.name, request.from); } catch (error) { if (request.required) throw error; continue; }
    if (packageRoot === null) { topology.push({ kind: "builtin", request: request.name, requested_by: request.requestedBy }); continue; }
    ...
    topology.push({ kind: "resolution", request: request.name, requested_by: request.requestedBy, target: relativeRoot });
  ...
  const executable = /^\/tmp\/b2c-v26-test-[^/]+$/u.test(root) ? resolve(root, "runtime-node-fixture") : realpathSync(process.execPath), executableMetadata = statSync(executable);
  const entries = [
    ...
    ...[...packages.keys()].sort(compareCanonicalPathV26).flatMap(runtimePackageEntries),
    ...topology.sort((left, right) => compareCanonicalPathV26(JSON.stringify(left), JSON.stringify(right))),
  ];
...
function verifyRuntimeDependencies(frozen) {
  const live = runtimeDependencyPlan(); if (live.manifestSha !== frozen.manifestSha || !same(live.entries, frozen.entries)) throw new Error("b2c-v26-runtime-dependency-drift");
}

function writeSnapshotFile(snapshotRoot, relative, raw) {
  const path = resolve(snapshotRoot, relative);
  if (!path.startsWith(`${snapshotRoot}${sep}`) || existsSync(path)) throw new Error("b2c-v26-formal-snapshot-path");
...
function sealSnapshotDirectories(directory) {
  for (const entry of readdirSync(directory, { withFileTypes: true })) {
    if (entry.isSymbolicLink()) throw new Error("b2c-v26-formal-snapshot-symlink");
...
function runtimeSnapshotLayout(snapshotRoot, runtime) {
  const topology = runtime.entries.filter(({ kind }) => kind === "resolution"), targets = [...new Set(topology.map(({ target }) => target))].sort(compareCanonicalPathV26), targetDirectories = new Map(targets.map((target) => [target, resolve(snapshotRoot, "runtime-dependencies", sha(target).slice(0, 32))])), map = {};
...
function expectedFormalSnapshot(frozen, runtime, snapshotRoot) {
  ...
  for (const entry of runtime.entries.filter(({ kind }) => kind === "file")) { const target = layout.targets.filter((candidate) => entry.path === candidate || entry.path.startsWith(`${candidate}/`)).sort((left, right) => right.length - left.length)[0]; if (!target) throw new Error("b2c-v26-runtime-entry-owner"); entries.push({ path: `${layout.targetDirectories.get(target).slice(snapshotRoot.length + 1)}/${entry.path.slice(target.length + 1)}`, bytes: entry.bytes, mode: "0444", raw_sha256: entry.raw_sha256 }); }
  entries.push({ path: "resolution-guard.cjs", bytes: Buffer.byteLength(layout.guard), mode: "0444", raw_sha256: layout.guardSha }); entries.sort((left, right) => compareCanonicalPathV26(left.path, right.path));
...
function materializeFormalSnapshot(evidence, frozen, runtime) {
  ...
  for (const entry of frozen.plan) {
    ...
    if (sha(raw) !== entry.raw_sha256) throw new Error(`b2c-v26-formal-source-toctou:${entry.relative}`);
  ...
  for (const entry of runtime.entries.filter(({ kind }) => kind === "file")) {
    ...
    if (!target) throw new Error("b2c-v26-runtime-entry-owner"); const raw = sealedSource(resolve(root, entry.path));
    if (raw.length !== entry.bytes || sha(raw) !== entry.raw_sha256) throw new Error("b2c-v26-runtime-dependency-drift");
  ...
  const snapshotEntries = recursiveFiles(snapshotRoot).sort(compareCanonicalPathV26).map((path) => { const metadata = statSync(path); return { path: path.slice(snapshotRoot.length + 1).split(sep).join("/"), bytes: metadata.size, mode: (metadata.mode & 0o777).toString(8).padStart(4, "0"), raw_sha256: sha(readFileSync(path)) }; }); if (!same(snapshotEntries, expected.entries)) throw new Error("b2c-v26-formal-snapshot-materialization-drift");
  const manifest = { schema_version: "b2c-v26-formal-source-snapshot-v1", formal_source_manifest_raw_sha256: frozen.manifestSha, entries, snapshot_entries: snapshotEntries };
  ...
  return Object.freeze({
    ...
    tsNodeRegisterPath: (() => { const live = realpathSync(createRequire(resolve(root, "package.json")).resolve("ts-node/register")), target = expected.topology.find(({ request, requested_by }) => request === "ts-node" && requested_by === "apps/api/package.json")?.target; if (!target) throw new Error("b2c-v26-ts-node-topology"); return resolve(expected.targetDirectories.get(target), live.slice(resolve(root, target).length + 1)); })(),
...
function verifyFormalSnapshot(snapshot) {
  const live = recursiveFiles(snapshot.root).sort(compareCanonicalPathV26).map((path) => { const metadata = statSync(path); return { path: path.slice(snapshot.root.length + 1).split(sep).join("/"), bytes: metadata.size, mode: (metadata.mode & 0o777).toString(8).padStart(4, "0"), raw_sha256: sha(readFileSync(path)) }; });
  if (!same(live, snapshot.snapshotEntries) || sha(JSON.stringify(live)) !== snapshot.combinedSnapshotSha256) throw new Error("b2c-v26-formal-snapshot-drift");
}
...
function baselinePlan() {
  ...
  if (!fixture || fixture.raw_sha256 !== FIXTURE_SHA256) throw new Error("b2c-v26-fixture-drift");
  const manifest = plan.map(({ kind, filename, raw_sha256 }) => ({ kind, filename, raw_sha256 }));
  if (plan.length !== 195 || sha(JSON.stringify(manifest)) !== BASELINE_MANIFEST_SHA256) throw new Error("b2c-v26-baseline-manifest-drift");
  return Object.freeze(plan.map(Object.freeze));
}
function sealedSource(path) { if (!existsSync(path) || lstatSync(path).isSymbolicLink() || realpathSync(path) !== path) throw new Error("b2c-v26-migration-path-drift"); let descriptor; try { descriptor = openSync(path, constants.O_RDONLY | constants.O_NOFOLLOW); const metadata = fstatSync(descriptor); if (!metadata.isFile()) throw new Error("b2c-v26-migration-path-drift"); return readFileSync(descriptor); } finally { if (descriptor !== undefined) closeSync(descriptor); } }
function migrationBytesImmediatelyBeforeExecution(entry) { const raw = sealedSource(entry.path); if (sha(raw) !== entry.raw_sha256) throw new Error(`b2c-v26-migration-byte-drift:${entry.filename}`); return raw.toString("utf8"); }

function validateRegistry(value) {
  exactObject(value, ["schema_version", "resources"], "b2c-v26-registry-schema"); exactString(value.schema_version, "b2c-v26-resource-registry-v1", "b2c-v26-registry-schema");
  if (!Array.isArray(value.resources) || value.resources.length !== 18) throw new Error("b2c-v26-registry-count");
  const labels = new Set(), identities = new Set();
  for (const entry of value.resources) { exactObject(entry, ["label", "run_id", "container", "container_id", "database", "volume"], "b2c-v26-registry-entry"); if (!/^(A|B|C|D|E|F|G|H|v4|v15|v17|v18|v19|v20|v21|v22|v23|v25)$/u.test(entry.label) || labels.has(entry.label)) throw new Error("b2c-v26-registry-label"); labels.add(entry.label); for (const key of ["run_id", "container", "container_id", "database", "volume"]) { if (typeof entry[key] !== "string" || entry[key].trim() === "") throw new Error("b2c-v26-registry-identity"); identities.add(entry[key]); } }
  if (labels.size !== 18) throw new Error("b2c-v26-registry-labels"); return identities;
}
function staticIntake() {
  ...
  const candidate = sealedJson(fixed.candidate, candidateKeys, "b2c-v26-candidate-schema"); exactString(candidate.value.schema_version, "b2c-v26-candidate-authority-v1", "b2c-v26-candidate-schema"); exactString(candidate.value.status, "PENDING_REVIEW", "b2c-v26-candidate-status");
  for (const key of ["execution_authorized", "container_create_authorized", "container_execute_authorized", "formal_go", "docker_or_database_command_executed"]) exactBoolean(candidate.value[key], false, `b2c-v26-candidate-flag:${key}`);
  for (const key of ["runner_raw_sha256", "runner_spec_raw_sha256", "registry_raw_sha256", "static_test_record_raw_sha256", "node22_tap_raw_sha256", "node24_tap_raw_sha256", "migration_000197_raw_sha256", "formal_source_manifest_raw_sha256", "runtime_dependency_manifest_raw_sha256", "combined_snapshot_raw_sha256", "resolution_guard_raw_sha256"]) assertHash(candidate.value[key], `b2c-v26-candidate-hash:${key}`);
  ...
  for (const [key, value] of Object.entries(live)) if (candidate.value[key] !== value) throw new Error(`b2c-v26-live-byte-drift:${key}`);
  ...
  const manifest = sealedJson(fixed.manifest, manifestKeys, "b2c-v26-manifest-schema"); exactString(manifest.value.schema_version, "b2c-v26-candidate-manifest-v1", "b2c-v26-manifest-schema"); exactString(manifest.value.status, "PENDING_REVIEW", "b2c-v26-manifest-status");
  exactObject(manifest.value.candidate_flags, ["container_create_authorized", "container_execute_authorized", "execution_authorized", "formal_go"], "b2c-v26-manifest-flags"); for (const value of Object.values(manifest.value.candidate_flags)) exactBoolean(value, false, "b2c-v26-manifest-flags");
  const expectedHashes = { candidate_authority_raw_sha256: candidate.rawSha256, ...live }; for (const [key, value] of Object.entries(expectedHashes)) if (manifest.value[key] !== value) throw new Error(`b2c-v26-manifest-binding:${key}`);
  if (!same(manifest.value.review_chain, ["candidate", "database", "qa", "security", "drain", "provision_authority", "resource_observation", "execution_authority"])) throw new Error("b2c-v26-review-chain");
  exactObject(manifest.value.canonical_paths, Object.keys(fixed).filter((key) => !["runner", "spec", "evidence"].includes(key)), "b2c-v26-canonical-paths"); for (const key of Object.keys(manifest.value.canonical_paths)) exactString(manifest.value.canonical_paths[key], fixed[key].slice(root.length + 1), `b2c-v26-canonical-path:${key}`);
  exactString(manifest.value.trust_root, "repository-fixed-path-procedural-root-v1", "b2c-v26-trust-root");
  const record = sealedJson(fixed.staticRecord, ["schema_version", "runner_raw_sha256", "test_raw_sha256", "node22", "node24", "docker_or_database_command_executed", "coverage"], "b2c-v26-static-record-schema"); exactString(record.value.schema_version, "b2c-v26-static-test-record-v1", "b2c-v26-static-record-schema"); exactBoolean(record.value.docker_or_database_command_executed, false, "b2c-v26-static-record-command");
  if (record.value.runner_raw_sha256 !== live.runner_raw_sha256 || record.value.test_raw_sha256 !== live.runner_spec_raw_sha256) throw new Error("b2c-v26-static-record-binding");
  for (const [key, tapKey] of [["node22", "node22_tap_raw_sha256"], ["node24", "node24_tap_raw_sha256"]]) { exactObject(record.value[key], ["binary", "result", "raw_tap_sha256"], "b2c-v26-static-record-node"); if (typeof record.value[key].binary !== "string" || record.value[key].result !== "pass" || record.value[key].raw_tap_sha256 !== live[tapKey]) throw new Error("b2c-v26-static-record-node"); }
  if (!Array.isArray(record.value.coverage) || record.value.coverage.some((item) => typeof item !== "string")) throw new Error("b2c-v26-static-record-coverage");
  const registry = sealedJson(fixed.registry, ["schema_version", "resources"], "b2c-v26-registry-schema"); const prohibited = validateRegistry(registry.value);
...
function reviewIntake(base) {
  ...
  const schema = { databaseReview: "b2c-v26-independent-database-review-v1", qaReview: "b2c-v26-independent-qa-review-v1", securityReview: "b2c-v26-independent-security-review-v1", drainReview: "b2c-v26-independent-old-writer-drain-review-v1" };
  for (const key of reviewOrder) {
    ...
    const review = sealedJson(fixed[key], keys, `b2c-v26-${key}-schema`); exactString(review.value.schema_version, schema[key], `b2c-v26-${key}-schema`); exactString(review.value.decision, "GO", `b2c-v26-${key}-decision`); exactBoolean(review.value.review_approved, true, `b2c-v26-${key}-approved`); exactZero(review.value.open_p0, `b2c-v26-${key}-p0`); exactZero(review.value.open_p1, `b2c-v26-${key}-p1`);
    ...
    for (const [field, value] of Object.entries(expected)) if (review.value[field] !== value) throw new Error(`b2c-v26-${key}-binding:${field}`);
...
function provisionIntake() {
  ...
  const authority = sealedJson(fixed.provisionAuthority, keys, "b2c-v26-provision-schema"); exactString(authority.value.schema_version, "b2c-v26-provision-authority-v1", "b2c-v26-provision-schema"); exactString(authority.value.status, "AUTHORIZED", "b2c-v26-provision-status"); exactBoolean(authority.value.container_create_authorized, true, "b2c-v26-provision-create"); for (const key of ["container_execute_authorized", "execution_authorized", "formal_go"]) exactBoolean(authority.value[key], false, `b2c-v26-provision-flag:${key}`);
  const bindings = { candidate_authority_raw_sha256: base.candidateSha, candidate_manifest_raw_sha256: base.manifestSha, database_review_raw_sha256: reviews.databaseReview, qa_review_raw_sha256: reviews.qaReview, security_review_raw_sha256: reviews.securityReview, drain_review_raw_sha256: reviews.drainReview, runtime_dependency_manifest_raw_sha256: base.runtimeDependencyManifestSha, combined_snapshot_raw_sha256: base.combinedSnapshotSha, resolution_guard_raw_sha256: base.resolutionGuardSha }; for (const [key, value] of Object.entries(bindings)) if (authority.value[key] !== value) throw new Error(`b2c-v26-provision-binding:${key}`);
  if (!runIdPattern.test(authority.value.run_id) || !containerPattern.test(authority.value.container) || !databasePattern.test(authority.value.database)) throw new Error("b2c-v26-provision-identity"); exactString(authority.value.image_reference, "postgres:16-alpine", "b2c-v26-provision-image"); exactZero(authority.value.host_port_bindings, "b2c-v26-provision-ports"); exactBoolean(authority.value.anonymous_volume, true, "b2c-v26-provision-volume"); exactString(authority.value.mount_type, "volume", "b2c-v26-provision-mount"); exactBoolean(authority.value.mount_rw, true, "b2c-v26-provision-mount"); exactString(authority.value.mount_destination, "/var/lib/postgresql/data", "b2c-v26-provision-mount");
  for (const identity of [authority.value.run_id, authority.value.container, authority.value.database, authority.value.image_reference]) if (base.prohibited.has(identity)) throw new Error("b2c-v26-prohibited-resource-reuse");
...
function inspect(command, target, runCommand, expectedLabels) {
  ...
  if (result.error || result.signal || result.status !== 0) throw new Error(`b2c-v26-${command}-inspect-child`); const rows = String(result.stdout ?? "").trim().split("\n"), mounts = JSON.parse(rows[4] ?? "null");
  const labels = JSON.parse(rows[5] ?? "null");
  if (rows.length !== 6 || !hex64.test(rows[0] ?? "") || rows[1] !== "true" || !/^sha256:[0-9a-f]{64}$/u.test(rows[2] ?? "") || !same(JSON.parse(rows[3] ?? "null"), {}) || !Array.isArray(mounts) || mounts.length !== 1 || mounts[0]?.Type !== "volume" || !hex64.test(mounts[0]?.Name ?? "") || mounts[0]?.RW !== true || mounts[0]?.Destination !== "/var/lib/postgresql/data") throw new Error(`b2c-v26-${command}-identity-drift`);
  const authorityKeys = ["jinhu.b2c.run_id", "jinhu.b2c.provision_authority_sha256"], inheritedKey = "desktop.docker.io/wsl-distro", keys = Object.keys(labels ?? {}).sort(); if (!same(keys, authorityKeys.sort()) && !same(keys, [...authorityKeys, inheritedKey].sort())) throw new Error(`b2c-v26-${command}-labels`); if (labels["jinhu.b2c.run_id"] !== expectedLabels.run_id || labels["jinhu.b2c.provision_authority_sha256"] !== expectedLabels.provision_authority_sha256) throw new Error(`b2c-v26-${command}-labels`);
  const inherited_labels = {}; if (Object.hasOwn(labels, inheritedKey)) { if (typeof labels[inheritedKey] !== "string" || !/^[A-Za-z0-9][A-Za-z0-9._-]{0,63}$/u.test(labels[inheritedKey])) throw new Error(`b2c-v26-${command}-inherited-label`); inherited_labels[inheritedKey] = labels[inheritedKey]; } if (expectedLabels.inherited_labels !== undefined && !same(inherited_labels, expectedLabels.inherited_labels)) throw new Error(`b2c-v26-${command}-inherited-label-drift`);
  return { container_id: rows[0], image_id: rows[2].slice(7), volume_id: mounts[0].Name, inherited_labels };
}
function writeSealed(path, value) { if (!path.startsWith(`${research}${sep}`) || existsSync(path) || lstatSync(dirname(path)).isSymbolicLink() || realpathSync(dirname(path)) !== dirname(path)) throw new Error("b2c-v26-output-path"); writeFileSync(path, `${JSON.stringify(value, null, 2)}\n`, { flag: "wx", mode: 0o444 }); chmodSync(path, 0o444); }

function provisionWithV26(runCommand) {
...
function executionIntake() {
  ...
  const observation = sealedJson(fixed.observation, observationKeys, "b2c-v26-observation-schema"); exactString(observation.value.schema_version, "b2c-v26-resource-observation-v1", "b2c-v26-observation-schema"); exactString(observation.value.status, "OBSERVED", "b2c-v26-observation-status"); exactBoolean(observation.value.execution_authorized, false, "b2c-v26-observation-execute"); exactBoolean(observation.value.formal_go, false, "b2c-v26-observation-formal");
  const expectedObservation = { provision_authority_raw_sha256: intake.authoritySha, candidate_authority_raw_sha256: intake.base.candidateSha, candidate_manifest_raw_sha256: intake.base.manifestSha, runtime_dependency_manifest_raw_sha256: intake.base.runtimeDependencyManifestSha, combined_snapshot_raw_sha256: intake.base.combinedSnapshotSha, resolution_guard_raw_sha256: intake.base.resolutionGuardSha, run_id: intake.authority.run_id, container: intake.authority.container, database: intake.authority.database, image_reference: "postgres:16-alpine", host_port_bindings: 0, anonymous_volume: true, mount_type: "volume", mount_rw: true, mount_destination: "/var/lib/postgresql/data" }; for (const [key, value] of Object.entries(expectedObservation)) if (!Object.is(observation.value[key], value)) throw new Error(`b2c-v26-observation-binding:${key}`); exactObject(observation.value.labels, ["run_id", "provision_authority_sha256"], "b2c-v26-observation-labels"); if (observation.value.labels.run_id !== observation.value.run_id || observation.value.labels.provision_authority_sha256 !== intake.authoritySha) throw new Error("b2c-v26-observation-labels"); exactObject(observation.value.inherited_labels, Object.keys(observation.value.inherited_labels), "b2c-v26-observation-inherited-labels"); const inheritedKeys = Object.keys(observation.value.inherited_labels); if (inheritedKeys.length > 1 || (inheritedKeys.length === 1 && (inheritedKeys[0] !== "desktop.docker.io/wsl-distro" || typeof observation.value.inherited_labels[inheritedKeys[0]] !== "string" || !/^[A-Za-z0-9][A-Za-z0-9._-]{0,63}$/u.test(observation.value.inherited_labels[inheritedKeys[0]])))) throw new Error("b2c-v26-observation-inherited-labels"); for (const key of ["container_id", "volume_id", "image_id"]) assertHash(observation.value[key], `b2c-v26-observation-identity:${key}`); for (const identity of [observation.value.run_id, observation.value.container, observation.value.container_id, observation.value.database, observation.value.volume_id, observation.value.image_id]) if (intake.base.prohibited.has(identity)) throw new Error("b2c-v26-prohibited-resource-reuse");
...
const expectedFinalHistory = () => [...expectedLate(), migration197Entry()].sort((left, right) => compareCanonicalPathV26(left.filename, right.filename));
export function assertSnapshotV26(value) {
  exactObject(value, ["primary_history", "mirror_history", "forbidden_primary", "forbidden_mirror", "approval_rows", "keys", "indexdef", "predicate", "catalog"], "b2c-v26-snapshot-schema");
  const late = expectedLate(); if (!same(value.primary_history, late) || !same(value.mirror_history, late)) throw new Error("b2c-v26-snapshot-history"); for (const key of ["forbidden_primary", "forbidden_mirror", "approval_rows"]) exactZero(value[key], `b2c-v26-snapshot-zero:${key}`); if (!same(value.keys, ["tenant_id", "park_id", "action_id", "source_type", "source_id", "source_expected_version"]) || value.indexdef !== OLD_INDEX_SHA || value.predicate !== OLD_PREDICATE_SHA) throw new Error("b2c-v26-snapshot-contract"); exactObject(value.catalog, ["index_exists", "build_residue"], "b2c-v26-snapshot-catalog"); exactBoolean(value.catalog.index_exists, true, "b2c-v26-snapshot-index"); exactBoolean(value.catalog.build_residue, false, "b2c-v26-snapshot-residue"); return true;
}
export function assertFinalSnapshotV26(value) {
  exactObject(value, ["primary_history", "mirror_history", "forbidden_primary", "forbidden_mirror", "approval_rows", "keys", "indexdef", "predicate", "old_predicate_matches", "catalog"], "b2c-v26-final-snapshot-schema");
  const history = expectedFinalHistory(); if (!same(value.primary_history, history) || !same(value.mirror_history, history)) throw new Error("b2c-v26-final-snapshot-history");
  for (const key of ["forbidden_primary", "forbidden_mirror", "approval_rows", "old_predicate_matches"]) exactZero(value[key], `b2c-v26-final-snapshot-zero:${key}`);
  if (!same(value.keys, ["tenant_id", "park_id", "action_id", "source_type", "source_id", "source_expected_version"]) || value.indexdef !== NEW_INDEX_SHA || value.predicate !== NEW_PREDICATE_SHA) throw new Error("b2c-v26-final-snapshot-contract");
  exactObject(value.catalog, ["index_exists", "build_residue"], "b2c-v26-final-snapshot-catalog"); exactBoolean(value.catalog.index_exists, true, "b2c-v26-final-snapshot-index"); exactBoolean(value.catalog.build_residue, false, "b2c-v26-final-snapshot-residue"); return true;
}
export function assertFaultMarkerV26(output, marker) { const pattern = /^(?:psql:<stdin>:\d+: )?ERROR:\s+P0001:\s*([^\s].*?)\s*$/u; const matches = String(output).split(/\r?\n/u).map((line) => line.match(pattern)).filter(Boolean); if (matches.length !== 1 || matches[0][1] !== marker || !/^v26-injected-(before-create|after-create|after-drop|before-rename)$/u.test(marker)) throw new Error("b2c-v26-fault-marker"); }
...
const faults = Object.freeze([
  { boundary: "before-create", injectionPoint: "after-validation", catalogState: "old-present-build-absent", prefix: "", assertion: catalogAssertion(true, false), marker: "v26-injected-before-create" },
  { boundary: "after-create", injectionPoint: "after-validation", catalogState: "old-present-build-present", prefix: createBuildIndex, assertion: catalogAssertion(true, true), marker: "v26-injected-after-create" },
  { boundary: "after-drop", injectionPoint: "immediately-after-ddl", catalogState: "old-absent-build-present", prefix: `${createBuildIndex} DROP INDEX public.uq_biz_property_approval_request_active_source;`, assertion: catalogAssertion(false, true), marker: "v26-injected-after-drop" },
  { boundary: "before-rename", injectionPoint: "after-validation", catalogState: "old-absent-build-present", prefix: `${createBuildIndex} DROP INDEX public.uq_biz_property_approval_request_active_source;`, assertion: catalogAssertion(false, true), marker: "v26-injected-before-rename" },
].map(Object.freeze));
...
function approvalPortGate(evidence, target, snapshot, runtime, run) {
  ...
  verifyExecutableClosure();
  if (!Array.isArray(expectedNames) || expectedNames.length !== 7) throw new Error("b2c-v26-approval-names");
  ...
  const ip = ipResult.stdout.trim(); if (!/^[0-9a-f:.]+$/iu.test(ip)) throw new Error("b2c-v26-approval-ip");
  const approvalRunId = sha(target.run_id).slice(0, 32), env = { PATH: dirname(runtime.executable), NODE_PATH: "", TS_NODE_PROJECT: resolve(snapshot.apiRoot, "tsconfig.json"), PROPERTY_APPROVAL_PORT_PG_URL: `postgresql://postgres@${ip}:5432/${target.database}`, PROPERTY_APPROVAL_PORT_PG_RUN_ID: approvalRunId, PROPERTY_APPROVAL_PORT_PG_EXTERNAL_FIXTURE: "yes" };
  ...
  const requirePhase = (phase, result) => { let parsed; try { parsed = JSON.parse(result.stdout.trim()); } catch { throw new Error(`b2c-v26-approval-${phase}`); } if (result.error || result.signal || result.status !== 0 || parsed?.phase !== phase || parsed?.runId !== approvalRunId || parsed?.status !== "pass") throw new Error(`b2c-v26-approval-${phase}`); };
  let summary, primary;
  try {
    ...
    if (testResult.error || testResult.signal || testResult.status !== 0) throw new Error("b2c-v26-approval-suite");
...
function executeWithV26(runCommand) {
  ...
  try {
    const actual = inspect("execute", target, (command, argv, options) => evidence.child("inspect", command, argv, options.input, runCommand), { ...target.labels, inherited_labels: target.inherited_labels }); if (actual.container_id !== target.container_id || actual.volume_id !== target.volume_id || actual.image_id !== target.image_id) throw new Error("b2c-v26-execute-identity-drift");
    if (!/^16[0-9]{4}$/u.test(psql(evidence, target, "postgres-16", "SHOW server_version_num;", runCommand).stdout.trim())) throw new Error("b2c-v26-version-drift");
    const formalSnapshot = materializeFormalSnapshot(evidence, intake.base.formal, intake.base.runtime); verifyRuntimeDependencies(intake.base.runtime); evidence.write("runtime-dependency-manifest.json", { schema_version: "b2c-v26-runtime-dependency-manifest-v1", runtime_dependency_manifest_raw_sha256: intake.base.runtimeDependencyManifestSha, entries: intake.base.runtime.entries });
    ...
    const sql = snapshotSql(expectedLate()), initial = JSON.parse(psql(evidence, target, "initial-snapshot", sql, runCommand).stdout.trim()); assertSnapshotV26(initial); const results = [];
    for (const fault of faults) { const before = JSON.parse(psql(evidence, target, `before-${fault.boundary}`, sql, runCommand).stdout.trim()); assertSnapshotV26(before); const failed = psql(evidence, target, `fault-${fault.boundary}`, faultSql(fault), runCommand, true); if (failed.status === 0) throw new Error(`b2c-v26-fault-success:${fault.boundary}`); assertFaultMarkerV26(`${failed.stdout}${failed.stderr}`, fault.marker); const after = JSON.parse(psql(evidence, target, `after-${fault.boundary}`, sql, runCommand).stdout.trim()); assertSnapshotV26(after); if (!same(before, after)) throw new Error(`b2c-v26-rollback-drift:${fault.boundary}`); results.push({ boundary: fault.boundary, injection_point: fault.injectionPoint, catalog_state: fault.catalogState, marker: fault.marker, sqlstate: "P0001", snapshot_exact: true }); }
    const rollbackFinal = JSON.parse(psql(evidence, target, "rollback-final-snapshot", sql, runCommand).stdout.trim()); assertSnapshotV26(rollbackFinal); if (!same(initial, rollbackFinal)) throw new Error("b2c-v26-final-drift");
    ...
    const committedSql = finalSnapshotSql(), committed = JSON.parse(psql(evidence, target, "committed-snapshot", committedSql, runCommand).stdout.trim()); assertFinalSnapshotV26(committed);
    ...
    const rerun = JSON.parse(psql(evidence, target, "rerun-snapshot", committedSql, runCommand).stdout.trim()); assertFinalSnapshotV26(rerun); if (!same(committed, rerun)) throw new Error("b2c-v26-rerun-drift");
    const approval = approvalPortGate(evidence, target, formalSnapshot, intake.base.runtime, runCommand);
    const postApproval = JSON.parse(psql(evidence, target, "post-approval-snapshot", committedSql, runCommand).stdout.trim()); assertFinalSnapshotV26(postApproval); if (!same(committed, postApproval)) throw new Error("b2c-v26-approval-residue");
    ...
    if (formalSnapshot.combinedSnapshotSha256 !== intake.base.combinedSnapshotSha || formalSnapshot.resolutionGuardSha256 !== intake.base.resolutionGuardSha) throw new Error("b2c-v26-authorized-snapshot-drift");
  ...
  } catch (error) { evidence.terminal("failure", { failure: { code: "B2C_V26_EXECUTION_FAILED", stage: "execution", name: "ExecutionError" } }, provenance); throw error; }
}

/** Formal phase two API: no caller-controlled command, path, hash, target, run ID, or evidence option. */
export function executeV26() { return executeWithV26(spawnSync); }

/** Test-only facade. It has no path parameter and is unavailable from the canonical repository root. */
export function createIsolatedV26TestHarness(runCommand) {
  if (typeof runCommand !== "function" || !/^\/tmp\/b2c-v26-test-[^/]+$/u.test(root) || realpathSync(root) !== root || !research.startsWith(`${root}${sep}`)) throw new Error("b2c-v26-test-harness-root");
  return Object.freeze({ provision: () => provisionWithV26(runCommand), execute: () => executeWithV26(runCommand) });
}

export function candidateV26() { return Object.freeze({ status: "PENDING_REVIEW", execution_authorized: false, container_create_authorized: false, container_execute_authorized: false, formal_go: false, docker_or_database_command_executed: false, trust_root: "repository-fixed-path-procedural-root-v1", review_chain: ["candidate", "database", "qa", "security", "drain", "provision_authority", "resource_observation", "execution_authority"] }); }
export function runtimeDependencyManifestV26() { return runtimeDependencyPlan().manifestSha; }
export function snapshotAuthorityV26() { const value = expectedFormalSnapshot(formalSourcePlan(), runtimeDependencyPlan(), resolve(fixed.evidence, "formal-source-snapshot")); return Object.freeze({ combined_snapshot_raw_sha256: value.combinedSnapshotSha256, resolution_guard_raw_sha256: value.guardSha }); }
