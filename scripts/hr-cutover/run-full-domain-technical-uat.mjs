...
COMMIT;`;
 const cleanupSql=`BEGIN; UPDATE sys_org SET leader_user_id=NULL WHERE leader_user_id IN(SELECT id FROM sys_user WHERE tenant_id=:'tenant' AND park_id=:'park' AND username IN(:'hrMaker',:'hrReviewer',:'manager',:'employee')); UPDATE hr_employee SET user_id=NULL WHERE user_id IN(SELECT id FROM sys_user WHERE tenant_id=:'tenant' AND park_id=:'park' AND username IN(:'hrMaker',:'hrReviewer',:'manager',:'employee')); DELETE FROM sys_auth_refresh_token WHERE user_id IN(SELECT id FROM sys_user WHERE tenant_id=:'tenant' AND park_id=:'park' AND username IN(:'hrMaker',:'hrReviewer',:'manager',:'employee')); DELETE FROM rel_user_org WHERE user_id IN(SELECT id FROM sys_user WHERE tenant_id=:'tenant' AND park_id=:'park' AND username IN(:'hrMaker',:'hrReviewer',:'manager',:'employee')); DELETE FROM rel_user_park WHERE user_id IN(SELECT id FROM sys_user WHERE tenant_id=:'tenant' AND park_id=:'park' AND username IN(:'hrMaker',:'hrReviewer',:'manager',:'employee')); DELETE FROM rel_user_role WHERE user_id IN(SELECT id FROM sys_user WHERE tenant_id=:'tenant' AND park_id=:'park' AND username IN(:'hrMaker',:'hrReviewer',:'manager',:'employee')); DELETE FROM sys_login_log WHERE username IN(:'hrMaker',:'hrReviewer',:'manager',:'employee') AND tenant_id=:'tenant' AND park_id=:'park'; DELETE FROM sys_user WHERE tenant_id=:'tenant' AND park_id=:'park' AND username IN(:'hrMaker',:'hrReviewer',:'manager',:'employee'); COMMIT;`;
 let api,web;
 try{
  ...
  await request(`${apiBase}/users/me`,{headers:headers[1]});
  await request(`${apiBase}/users/me`,{headers:headers[2]}); await request(`${apiBase}/hr/employees?page=1&page_size=5`,{headers:headers[2]}); await request(`${apiBase}/hr/payroll/history/team-summary?page=1&page_size=5`,{headers:headers[2]}); await request(`${apiBase}/hr/payroll/history?page=1&page_size=5`,{headers:headers[2]},403);
  ...
  const fixture=JSON.parse(psql(config,vars,`SELECT json_build_object('employeeId',(SELECT id FROM hr_employee WHERE tenant_id=:'tenant' AND park_id=:'park' AND employee_code=:'run'||'-employee'),'outsideEmployeeId',(SELECT id FROM hr_employee WHERE tenant_id=:'tenant' AND park_id=:'park' AND employee_code=:'run'||'-outside'),'otherWorkReportId',(SELECT id FROM hr_work_report WHERE tenant_id=:'tenant' AND park_id=:'park' AND remark='Yuzhou technical UAT '||:'run' ORDER BY id LIMIT 1));`));
  ...
  const matrixObservations=[...employeeMatrix.observations,...workReportMatrix.observations];
  ...
  writePrivate(summaryPath,{formatVersion:1,parentRunId:config.runId,status:"PASS",roleTypes:["hr_manager","department_manager","employee_self_service"],actors:["hr_maker","hr_reviewer","department_manager","employee_self_service"],apiChecks:23,negativeAuthorizationChecks:6,webRouteChecks:3,legacyTaskCard:{sha256:taskCardIdentity.sha256,apiMatrixSha256:apiMatrixIdentity.sha256,apiMatrixChecks:apiMatrixIdentity.checkCount,observedChecks:matrixObservations.length,status:"HOLD_PENDING_REMAINING_MATRIX"},humanUat:"HOLD",productionImport:"HOLD"});registryFile(config,summaryPath);
  ...
  const evidence=buildEvidenceIndex(config.target.evidenceRoot,[{kind:"approved_ignored_attestation",relativePath:"cold-archive-scope-attestation.json"},{kind:"global_facts_summary",relativePath:"global-facts-summary.json"},{kind:"technical_uat_api_matrix_observations",relativePath:basename(observationsPath)},{kind:"technical_uat_summary",relativePath:basename(summaryPath)}]);
  ...
  return {state:"uat_ready",technicalUat:"PASS",apiChecks:23,negativeAuthorizationChecks:6,webRouteChecks:3,legacyObservedChecks:matrixObservations.length,legacyScorePromotion:"HOLD_PENDING_REMAINING_MATRIX",manifestSha256:record.sha256,productionImport:"HOLD"};
 }finally{
