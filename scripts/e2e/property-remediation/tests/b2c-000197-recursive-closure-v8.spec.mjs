...
import {
  assertTscFileListMatchesV8, classifySpecifierV8, resolveFormalExecutionClosureV8,
...
test("recursive formal closure includes critical PG, authorization, principal and shared dependencies", () => {
  const closure = resolveFormalExecutionClosureV8(root); const files = new Set(closure.repositoryFiles);
...
test("builtin and external dependencies are explicit while repository misses fail closed", () => {
  ...
  assert.deepEqual(classifySpecifierV8(importer, "node:test", root), { kind: "builtin", value: "node:test" });
  assert.equal(classifySpecifierV8(importer, "typeorm", root).kind, "external");
  assert.deepEqual(classifySpecifierV8(importer, "./missing-v8-owned-module", root),
    { kind: "unresolved", value: "./missing-v8-owned-module" });
  assert.equal(classifySpecifierV8(importer, "@jinhu/shared", root).kind, "repository");
  const closure = resolveFormalExecutionClosureV8(root);
...
test("TypeScript Program repository files exactly match real tsc listFilesOnly", () => {
  ...
  const result = assertTscFileListMatchesV8(child.stdout, root);
