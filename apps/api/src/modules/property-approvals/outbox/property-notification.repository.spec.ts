...
describe("TypeOrmPropertyNotificationStore", () => {
  ...
  it("projects the recipient read version required by the mark-read CAS", async () => {
    const statements: string[] = [];
    const store = new TypeOrmPropertyNotificationStore({
      query: async (sql: string) => {
        statements.push(sql);
        if (sql.includes("FROM rel_property_notification_recipient r")) return [{
          id: notificationId,
          source_event_id: "33333333-3333-4333-8333-333333333333",
          notification_type: "approval-infra-exhausted",
          title: "审批异常",
          summary: "需要处理",
          severity: "critical",
          route_params: { sourceType: "approval", sourceId: notificationId, routeId: notificationId },
    ...
    const detail = await store.detail(scope, recipientId, notificationId);

    assert.equal(detail?.readVersion, 7);
    assert.match(statements[0] ?? "", /r\.read_version/);
  });
