...
DO $$BEGIN
 IF EXISTS(SELECT 1 FROM source_rows WHERE payload->'materialized'->>'kind'='profile' AND jsonb_array_length(COALESCE(payload->'materialized'->'customFields','[]'::jsonb))<>19) THEN RAISE EXCEPTION 'T5 custom field coverage drift'; END IF;
 IF EXISTS(
   SELECT field->>'code' FROM source_rows CROSS JOIN LATERAL jsonb_array_elements(COALESCE(payload->'materialized'->'customFields','[]'::jsonb)) field
   WHERE payload->'materialized'->>'kind'='profile' GROUP BY field->>'code' HAVING count(DISTINCT ROW(field->>'label',field->>'valueType',field->>'group',field->>'sortOrder',field->>'definitionSourceIdentitySha256',field->>'definitionSourceRowSha256'))<>1
 ) THEN RAISE EXCEPTION 'T5 custom field definition drift'; END IF;
END$$;
WITH b AS(SELECT id FROM migration_batch WHERE run_id=:'run'),definitions AS(
 SELECT DISTINCT ON(field->>'code') field FROM source_rows CROSS JOIN LATERAL jsonb_array_elements(payload->'materialized'->'customFields') field
 WHERE payload->'materialized'->>'kind'='profile' ORDER BY field->>'code'
)
INSERT INTO hr_custom_field_definition(tenant_id,park_id,migration_batch_id,field_code,display_label,value_type,field_group,sort_order,sensitivity,origin,source_system,source_table,source_column,source_identity_sha256,source_row_sha256,status,create_by,update_by)
SELECT :'tenant',:'park',b.id,field->>'code',field->>'label',field->>'valueType',field->>'group',(field->>'sortOrder')::int,'restricted','legacy','yuzhou-v10','dbo.defs',field->>'code',field->>'definitionSourceIdentitySha256',field->>'definitionSourceRowSha256','enabled',:'actor',:'actor' FROM definitions CROSS JOIN b;
WITH b AS(SELECT id FROM migration_batch WHERE run_id=:'run')
INSERT INTO hr_employee_custom_value(tenant_id,park_id,migration_batch_id,employee_id,definition_id,text_value,numeric_value,date_value,boolean_value,is_source_null,value_status,origin,source_system,source_table,source_column,source_identity_sha256,source_row_sha256,create_by,update_by)
SELECT :'tenant',:'park',b.id,c.employee_id,d.id,
 CASE WHEN NOT(field->>'valid')::boolean THEN field->>'rawValue' WHEN field->>'valueType'='text' THEN field->>'value' END,
 CASE WHEN (field->>'valid')::boolean AND field->>'valueType'='numeric' THEN NULLIF(field->>'value','')::numeric END,
 CASE WHEN (field->>'valid')::boolean AND field->>'valueType'='date' THEN NULLIF(field->>'value','')::date END,
 CASE WHEN (field->>'valid')::boolean AND field->>'valueType'='boolean' THEN NULLIF(field->>'value','')::boolean END,
 (field->>'isSourceNull')::boolean,
 CASE WHEN (field->>'isSourceNull')::boolean THEN 'null' WHEN (field->>'valid')::boolean THEN 'valid' ELSE 'invalid' END,
 'legacy','yuzhou-v10','dbo.person',field->>'code',
 encode(digest(c.payload->>'sourceIdentitySha256'||chr(31)||field->>'code','sha256'),'hex'),
 encode(digest(c.payload->>'sourceRowSha256'||chr(31)||field->>'code'||chr(31)||coalesce(field->>'value',field->>'rawValue','<null>'),'sha256'),'hex'),:'actor',:'actor'
FROM classified c CROSS JOIN LATERAL jsonb_array_elements(c.payload->'materialized'->'customFields') field
 CROSS JOIN b
JOIN hr_custom_field_definition d ON d.tenant_id=:'tenant' AND d.park_id=:'park' AND d.origin='legacy' AND d.source_column=field->>'code' AND NOT d.is_deleted
WHERE c.quarantine_code IS NULL AND c.payload->'materialized'->>'kind'='profile';
WITH b AS(SELECT id FROM migration_batch WHERE run_id=:'run')
...
SELECT b.id,'yuzhou-v10','dbo.defs','field='||d.source_column,d.source_identity_sha256,d.source_row_sha256,'hr_custom_field_definition',d.id,'loaded'
FROM b JOIN hr_custom_field_definition d ON d.migration_batch_id=b.id AND d.tenant_id=:'tenant' AND d.park_id=:'park' AND d.origin='legacy';
WITH b AS(SELECT id FROM migration_batch WHERE run_id=:'run')
INSERT INTO legacy_record_map(batch_id,source_system,source_table,source_pk_canonical,source_identity_sha256,source_row_sha256,target_table,target_id,mapping_status)
SELECT b.id,'yuzhou-v10','dbo.person','identity='||x.source_identity_sha256||';field='||x.source_column,x.source_identity_sha256,x.source_row_sha256,'hr_employee_custom_value',x.id,'loaded'
FROM b JOIN hr_employee_custom_value x ON x.migration_batch_id=b.id AND x.tenant_id=:'tenant' AND x.park_id=:'park' AND x.origin='legacy';
...
UPDATE hr_legacy_t5_import_batch b SET loaded_row_count=x.loaded,quarantined_row_count=7752-x.loaded,status='staged',update_time=now() FROM(SELECT count(*) FILTER(WHERE mapping_status='loaded' AND target_table='hr_legacy_t5_record') loaded FROM legacy_record_map WHERE batch_id=(SELECT id FROM migration_batch WHERE run_id=:'run'))x WHERE b.batch_code=:'run';
DO $$DECLARE source_count bigint;map_count bigint;error_count bigint;BEGIN
 SELECT count(*) INTO source_count FROM source_rows; SELECT count(*) INTO map_count FROM legacy_record_map WHERE batch_id=(SELECT id FROM migration_batch WHERE run_id=current_setting('yuzhou.t5_run')) AND target_table IN('hr_legacy_t5_record','hr_legacy_t5_quarantine'); SELECT count(*) INTO error_count FROM migration_error WHERE batch_id=(SELECT id FROM migration_batch WHERE run_id=current_setting('yuzhou.t5_run'));
...
SELECT b.id,'T5_LEGACY_HISTORY',jsonb_build_object('runId',:'run','filesExcluded',jsonb_build_array('photo','docs')),jsonb_build_object('strategy','exact_nonfile_record_maps','identityResolutionMode',:'resolution_mode','identityResolutionSha256',NULLIF(:'resolution_hash',''),'targetTables',jsonb_build_array('hr_legacy_employee_materialization_gap','hr_employee_credential','hr_employee_skill','hr_employee_family','hr_employee_custom_value','hr_custom_field_definition','hr_employee_profile','hr_legacy_t5_record')),encode(digest(:'run'||':T5_LEGACY_HISTORY','sha256'),'hex'),now() FROM b;
