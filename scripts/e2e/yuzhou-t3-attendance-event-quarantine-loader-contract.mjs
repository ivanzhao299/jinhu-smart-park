...
test("T3 attendance punch quarantine loader is lab-only, audit-only, and conservation-bound",()=>{
  for(const value of ["ALLOW_YUZHOU_MIGRATION","controlled attendance staging directory required","^jinhu_hr_migration_lab_","^jinhu_hr_migration_lab_core_","wrong PostgreSQL compose project","sourceSnapshotSha256!==snapshot","migration_batch","migration_batch_item","migration_error","migration_check","migration_rollback_point","T3_ATTENDANCE_PUNCH_QUARANTINE_CONSERVATION","delete_migration_audit_only","businessWriteTarget','none'","productionImport','HOLD'"])assert.match(script,new RegExp(value.replace(/[.*+?^${}()|[\]\\]/g,"\\$&")));
