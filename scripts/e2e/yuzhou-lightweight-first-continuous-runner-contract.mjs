...
const config = {
  ...
  source: { readOnly: true, sourceBackupSha256: snapshot, sourceBackupPath: backup, sourceRestoreReceiptPath: receipt, sourceRestoreReceiptSha256: receiptHash, databaseAlias: "YuzhouHR_Lab_lwtest01", etlEnvFile: etl, sourceContainer: "yuzhou-source-lab", dictionaryPackages: {}, dictionaryCaptureReceipt: "" },
...
const candidateConfigPath = join(root, "candidate-config.json"); writePrivate(candidateConfigPath, JSON.stringify(candidateConfig));
...
const t3 = makeStage("t3", { artifactKind: "yuzhou_t3_attendance_insurance_stage", sourceReadOnly: true, sourceSnapshotSha256: snapshot, sourceRestoreReceiptSha256: receiptHash, sourceCatalogSha256: "d".repeat(64), sourceBusinessSha256: "e".repeat(64), mappingContractSha256: "2".repeat(64), productionImport: "HOLD" });
const t4 = makeStage("t4", { sourceBackupSha256: snapshot, sourceRestoreReceiptSha256: receiptHash, mappingContractSha256: "2".repeat(64), businessContentSha256: business, productionImport: "HOLD" });
const candidateT3 = makeStage("candidate-t3", { artifactKind: "yuzhou_t3_attendance_insurance_stage", sourceReadOnly: true, sourceSnapshotSha256: snapshot, sourceRestoreReceiptSha256: candidateReceiptHash, sourceCatalogSha256: "d".repeat(64), sourceBusinessSha256: "e".repeat(64), mappingContractSha256: "2".repeat(64), productionImport: "HOLD" });
...
const firstRun = runLightweightFirstContinuous({ configPath, t5Stage: t5, t3Stage: t3, t4Stage: t4, t5Baseline: testBaselinePath }, { coreRunner: concurrentCoreRunner, technicalUat: async () => ({ status: "PASS", productionImport: "HOLD" }), spawn: successfulSpawn(concurrentCommands), uuid: () => "00000000-0000-4000-8000-000000000010" });
await new Promise(resolve => setImmediate(resolve));
await assert.rejects(() => runLightweightFirstContinuous({ configPath, t5Stage: t5, t3Stage: t3, t4Stage: t4, t5Baseline: testBaselinePath }, { coreRunner, technicalUat: async () => ({ status: "PASS", productionImport: "HOLD" }), spawn, uuid: () => "00000000-0000-4000-8000-000000000011" }), /LIGHTWEIGHT_RUN_CONCURRENT/);
releaseConcurrentCore();
await firstRun;
assert.equal(existsSync(join(root, ".yuzhou-lightweight-first-continuous.lock")), false);
const result = await runLightweightFirstContinuous({ configPath, t5Stage: t5, t3Stage: t3, t4Stage: t4, t5Baseline: testBaselinePath }, { coreRunner, technicalUat: async () => ({ status: "PASS", productionImport: "HOLD" }), spawn, uuid: () => "00000000-0000-4000-8000-000000000001" });
...
assert.equal(commands.at(1).env.YUZHOU_T5_BASELINE_FILE, testBaselinePath);
assert.equal(commands.at(2).env.YUZHOU_SOURCE_RESTORE_RECEIPT_SHA256, receiptHash);
...
assert.equal(commands.at(2).env.YUZHOU_MAPPING_CONTRACT_SHA256, "2".repeat(64));
...
await assert.rejects(() => runLightweightFirstContinuous({ configPath, t5Stage: t5, t3Stage: t3, t4Stage: t4, t5Baseline: testBaselinePath }, { coreRunner, technicalUat: browserRuntimeFailure, spawn, uuid: () => "00000000-0000-4000-8000-000000000007" }), /YUZHOU_UAT_BROWSER_RUNTIME_SURFACE/);
...
await assert.rejects(() => runLightweightFirstContinuous({ configPath, t5Stage: t5, t3Stage: t3, t4Stage: t4, t5Baseline: testBaselinePath }, { coreRunner, technicalUat: async () => ({ status: "PASS", productionImport: "HOLD" }), spawn: rollbackFailure, uuid: () => "00000000-0000-4000-8000-000000000002" }), /LIGHTWEIGHT_T4_ROLLBACK_FAILED/);
...
await assert.rejects(() => runLightweightFirstContinuous({ configPath, t5Stage: t5, t3Stage: t3, t4Stage: t4, t5Baseline: testBaselinePath }, { coreRunner, technicalUat: async () => ({ status: "PASS", productionImport: "HOLD" }), spawn: t4ReceiptFailure, uuid: () => "00000000-0000-4000-8000-000000000004" }), /LIGHTWEIGHT_T4_LOAD_RECEIPT_INVALID/);
...
await assert.rejects(() => runLightweightFirstContinuous({ configPath, t5Stage: t5, t3Stage: t3, t4Stage: t4, t5Baseline: testBaselinePath }, { coreRunner, technicalUat: async () => ({ status: "PASS", productionImport: "HOLD" }), spawn: t5Failure, uuid: () => "00000000-0000-4000-8000-000000000003" }), /LIGHTWEIGHT_T5_NONFILE_TRANSACTION_STATEMENT_TIMEOUT/);
...
await assert.rejects(() => runLightweightFirstContinuous({ configPath, t5Stage: t5, t3Stage: t3, t4Stage: t4, t5Baseline: testBaselinePath }, { coreRunner, technicalUat: async () => ({ status: "PASS", productionImport: "HOLD" }), spawn: t5PreconditionFailure, uuid: () => "00000000-0000-4000-8000-000000000006" }), /LIGHTWEIGHT_T5_NONFILE_TRANSACTION_STATEMENT_TIMEOUT|LIGHTWEIGHT_T5_NONFILE_STAGING_DIR_MODE_INVALID/);
...
const candidateResult = await runLightweightFirstContinuous({ configPath: candidateConfigPath, t5Stage: candidateT5, t3Stage: candidateT3, t4Stage: candidateT4, t5Baseline: candidateBaselinePath }, { coreRunner: candidateCoreRunner, technicalUat: async () => ({ status: "PASS", productionImport: "HOLD" }), spawn: successfulSpawn(candidateCommands), uuid: () => "00000000-0000-4000-8000-000000000005" });
