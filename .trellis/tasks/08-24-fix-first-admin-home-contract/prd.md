...
- 生产 `bootstrap-admin.sh` 不回写 `contact_user_id`，但会写入精确、系统控制的 `remark='bootstrap-admin created'`。
...
- API 仅在用户持有当前园区的有效 `TENANT_ADMIN` 角色，且满足下列之一时返回 `true`：
  ...
  2. 为兼容旧生产 bootstrap 链，租户指针为空，且用户同时具有脚本写入的精确 remark 与 `create_by IS NULL` 创建来源。
