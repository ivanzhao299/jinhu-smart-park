...
const PORT_FINANCIAL_ACTIONS = new Set<TrackBApprovalActionId>([
...
function validateAndNormalizePortCommand(
  ...
  if (
    !pairedMoney
    || !validMoney
    || (PORT_FINANCIAL_ACTIONS.has(command.actionId)
      ? command.amount == null
      : command.amount != null)
  ) throw propertyApprovalError("property-validation-failed");
  return {
...
export function validateFrozenPolicy(
  ...
  if (
    ...
    || policy.effects.some((effect) =>
      ...
      || effect.expectedCardinality !== effectCardinality(effect.effectKind)
    )
  ) throw propertyApprovalError("approval-policy-not-found", {
...
export function canonicalEffectLines(
  ...
  switch (actionId) {
    case "homestay.bookings.cancel.request":
      result = [{ effectKind: "homestay.booking.cancel", effectLineKey: `booking:${uuid("bookingId")}`, ...plain }];
      break;
    case "homestay.finance.refund-or-waive.request":
      result = lines("lines").map((line) => {
        ...
        if ((entryType !== "refund" && entryType !== "waiver")
          || typeof line.sourceReceivableId !== "string"
          || !isUuid(line.sourceReceivableId)) throw propertyApprovalError("approval-policy-not-found");
        return {
          effectKind: `homestay.ledger.${entryType}`,
          effectLineKey: `ledger:${entryType}:${line.sourceReceivableId}`,
          ...financial(line)
        };
      });
      break;
    ...
    case "housing.handovers.complete-move-out-financial.request":
      result = [{
        ...
        effectLineKey: `handover:${uuid("handoverId")}`,
        ...plain
...
const EFFECT_AUTHORITIES: Record<string, {
  ...
  "homestay.booking.cancel": {
    ...
    owningUniqueName: "uq_homestay_booking_action_log_approval_line",
  ...
  "housing.lease.approve": {
    ...
    owningUniqueName: "pk_biz_housing_lease",
    expectedCardinality: 1,
    lineKey: /^lease:[0-9a-f-]{36}$/,
    financial: false
  },
  "housing.lease.void": {
    owningTable: "biz_housing_lease_audit",
    owningUniqueName: "uq_housing_lease_audit_approval_line",
    expectedCardinality: 2,
    lineKey: /^lease:[0-9a-f-]{36}$/,
    financial: false
  },
  "housing.lease.checkout": {
    owningTable: "biz_housing_checkout_audit",
    owningUniqueName: "uq_housing_checkout_audit_approval_line",
    expectedCardinality: 2,
    lineKey: /^lease:[0-9a-f-]{36}$/,
    financial: false
  },
  ...
  "housing.handover.complete.financial": {
    owningTable: "biz_housing_handover",
    owningUniqueName: "pk_biz_housing_handover",
    expectedCardinality: 1,
  ...
  "housing.purchase.lifecycle": {
    owningTable: "biz_housing_purchase_audit",
    owningUniqueName: "uq_housing_purchase_audit_approval_line",
    expectedCardinality: 2,
    lineKey: /^purchase:[0-9a-f-]{36}$/,
    financial: false
  },
  "housing.purchase.transfer": {
    owningTable: "biz_housing_purchase_transfer_audit",
    owningUniqueName: "uq_housing_purchase_transfer_audit_approval_line",
    expectedCardinality: 1,
...
function effectCardinality(effectKind: string): number {
  ...
  if (
    ...
    || manifests.some((manifest) =>
      ...
      || manifest.expectedCardinality
        !== EFFECT_AUTHORITIES[manifest.effectKind]!.expectedCardinality
