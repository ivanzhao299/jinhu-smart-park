...
permissions(code, name, resource, action, permission_type, perm_type, api_method, api_path, frontend_route, sort_no) AS (
  VALUES
    ('iot_alert:create_workorder', 'IoT 告警转工单', 'biz.iot_alert', 'create_workorder', 'api', 40, 'POST', '/api/v1/iot/alerts/:id/create-work-order', NULL, 540)
),
updated_permissions AS (
  ...
  SET park_id = seed_scope.park_id,
      ...
      is_builtin = true,
      is_deleted = false,
      remark = 'S6-A IoT alert create workorder permission seed',
      update_time = now()
  FROM permissions
...
inserted_permissions AS (
  ...
  SELECT seed_scope.tenant_id,
         ...
         true,
         true,
         'S6-A IoT alert create workorder permission seed'
  FROM permissions
  CROSS JOIN seed_scope
  WHERE NOT EXISTS (
    SELECT 1
...
role_permissions(role_code, permission_code) AS (
  VALUES
    ('SUPER_ADMIN', 'iot_alert:create_workorder'),
    ('OPERATIONS_OWNER', 'iot_alert:create_workorder'),
    ('PARK_OPERATOR', 'iot_alert:create_workorder'),
    ('PROPERTY_MANAGER', 'iot_alert:create_workorder'),
    ('SAFETY_MANAGER', 'iot_alert:create_workorder')
)
...
SELECT seed_scope.tenant_id,
       ...
       false,
       1,
       'S6-A IoT alert create workorder role permission seed'
FROM seed_scope
...
updated_items AS (
  ...
  SET item_label = dict_items.item_label,
      ...
      remark = 'S6-A IoT alert workorder source dictionary seed',
      update_time = now()
  FROM dict_items
...
SELECT target_type.tenant_id,
       ...
       'enabled',
       dict_items.tag_type,
       'S6-A IoT alert workorder source dictionary seed'
FROM dict_items
JOIN target_type ON true
WHERE NOT EXISTS (
  SELECT 1
