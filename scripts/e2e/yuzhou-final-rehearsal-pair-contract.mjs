...
test("pair execution is serial and any stage failure invokes scoped recovery without a PASS result",()=>{
  ...
  const result=runFinalPair(configs[0],configs[1],contract,hooks);assert.equal(result.status,"PASS");assert.deepEqual(result.rehearsals.map(x=>x.rehearsal),["A","B"]);assert(calls.indexOf("pair:compare")>calls.indexOf("B:p0"));assert(calls.indexOf("pair:browser-evidence")>calls.indexOf("pair:compare"));assert(calls.indexOf("pair:browser-evidence")<calls.findIndex(row=>row.includes(":rollback")));assert(calls.indexOf("B:full-domain-lifecycle.mjs:rollback")<calls.indexOf("A:full-domain-lifecycle.mjs:rollback"));
