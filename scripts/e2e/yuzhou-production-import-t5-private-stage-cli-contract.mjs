...
test("private-stage CLI turns a verified T5 stage into 0600 private files and a safe receipt", () => {
  ...
  try {
    ...
    privateWrite(join(stage, "manifest.json"), { artifactKind: "yuzhou_t5_nonfile_materialization_stage", sourceSnapshotSha256: source, sourceRestoreReceiptSha256: hash("restore"), nonfileBusinessSha256: hash("business"), domains, filesExcluded: ["photo", "docs"], sourceRows: 1, productionImport: "HOLD" });
    privateWrite(index, [{ employeeCode: "E-001", sourceIdentitySha256: hash("employee") }]);
    privateWrite(triple, { codeSha: "1".repeat(40), sourceSnapshotHash: source, mappingContractHash: hash("mapping") });
    const result = prepareT5ProductionPrivateStage({ stagePath: stage, employeeIndexPath: index, triplePath: triple, outputRoot, runId: "t5private01" });
    ...
    assert.equal(parseT5ProductionPrivateStageArgs(["--stage", stage, "--employee-index", index, "--triple", triple, "--output-root", outputRoot, "--run-id", "t5private01"]).runId, "t5private01");
  } finally {
