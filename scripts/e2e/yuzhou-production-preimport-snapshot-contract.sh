...
grep -Fq 'source_manifest_json' "$workflow"
grep -Fq 'YUZHOU_SOURCE_MANIFEST_JSON' "$workflow"
grep -Fq 'verifyProductionSourceManifest' "$workflow"
grep -Fq 'PRODUCTION_IMPORT_SOURCE_MANIFEST_REQUIRED' "$workflow"
grep -Fq 'reasonCodes = Array.isArray(snapshot.reasonCodes)' "$workflow"
