...
import {
  PhaseExecutionErrorV10, V10_RUN_ID, executeWithEvidenceV10, parseTapSummaryV10, runPhasedGateV10,
} from "./track-b2c-000197-preliminary-executor-v10.mjs";
import { failureInjectionCasesV10 } from "./track-b2c-000197-failure-cases-v10.mjs";
import {
  resolveFormalExecutionClosureV10, TYPECHECK_GOVERNANCE_FILES_V10,
...
const manifestPath = resolve(research, "b2c-000197-preliminary-v10-input-manifest-20260802.grammar");
const handoffPath = resolve(research, "b2c-000197-preliminary-executor-v10-review-handoff-20260802.md");
const resolverPath = new URL("./track-b2c-000197-closure-resolver-v10.mjs", import.meta.url);
...
const expected = Object.freeze({ migration: "a9b98ca82aa4dafc16535085184df838880ef27801f7cd4b225e1ca1a15af059",
  ...
  preflightArtifact: "7480fbd6a0e1d24a8b0ed0a1ed3c6a9bab9993094909a38c3e8de4f407cc6ec7",
  preflightManifest: "f4a112c1dac5066f022c9c65fa9f49a3cc8544e3797aa51b4b5b464dffaae642",
...
const STATIC_INPUTS_V10 = Object.freeze([
  ...
  [".trellis/tasks/07-30-pr192-b-domain-integrations/research/b2c-000197-v10-complete-preflight-evidence-b2c197_prelim_20260802b/success-b2c197_prelim_20260802b.json", expected.preflightArtifact],
  [".trellis/tasks/07-30-pr192-b-domain-integrations/research/b2c-000197-v10-complete-preflight-evidence-b2c197_prelim_20260802b/success-b2c197_prelim_20260802b.manifest.json", expected.preflightManifest],
...
let authoritativeCacheV10 = null;
let authoritativeRowsCacheV10 = null;
export function authoritativeFileRowsV10() {
  if (!authoritativeRowsCacheV10) {
    const closure = resolveFormalExecutionClosureV10(root); const compiler = new Set(closure.compilerFiles);
    const governance = new Set(TYPECHECK_GOVERNANCE_FILES_V10); const staticInputs = new Set(STATIC_INPUTS_V10.map(([path]) => path));
    authoritativeRowsCacheV10 = Object.freeze([...new Set([...closure.repositoryFiles, ...staticInputs])].sort().map((path) =>
      Object.freeze({ path, reason: staticInputs.has(path) ? (path.includes("v10-complete-preflight-evidence")
  ...
  return authoritativeRowsCacheV10;
}
export function authoritativeFilePathsV10() {
  authoritativeCacheV10 ??= Object.freeze(authoritativeFileRowsV10().map(({ path }) => path));
  return authoritativeCacheV10;
}

export const V10_TARGETS = Object.freeze([
...
export function frozenIdentityAuthorityV10() {
  exact(resourceAuthorityPath, expected.resource);
  const lines = readFileSync(resourceAuthorityPath, "utf8").trimEnd().split("\n");
  if (lines.shift() !== "b2c-000197-preliminary-v3-resource-authority-v1") throw new Error("v10-resource-authority-schema");
  const fields = new Map(lines.map((line) => { const [name, ...values] = line.split("\t"); return [name, values]; }));
  const formalRunId = fields.get("formal_run_id")?.[0];
  const targetC = fields.get("target_c"); const targetD = fields.get("target_d");
  if (!formalRunId || !targetC || !targetD) throw new Error("v10-resource-authority-incomplete");
  return { writer_build: "approval-port-v8", runtime_version: "v8", runtime_raw_sha256: expected.runtime,
    formal_run_id: formalRunId, resource_authority_raw_sha256: expected.resource,
    target_c: targetC.join("\t"), target_d: targetD.join("\t"),
    review_schema: "b2c-000197-preliminary-v10-independent-review-v1",
    drain_schema: "b2c-000197-old-writer-drain-v10", sql_raw_sha256: expected.migration,
...
function exact(path, hash) {
  if (!existsSync(path) || lstatSync(path).isSymbolicLink() || realpathSync(path) !== path
      || sha256(readFileSync(path)) !== hash) throw new Error(`b2c-000197-v10-input-drift:${path}`);
}
...
function inputs({ includePreflight = true } = {}) {
  ...
  if (prefix.length !== 1 || prefix[0] !== migrationFilename) throw new Error("b2c-000197-v10-prefix-drift");
  for (const [path, hash] of STATIC_INPUTS_V10) {
    if (!includePreflight && path.includes("v10-complete-preflight-evidence")) continue;
    exact(resolve(root, path), hash);
  }
  const identity = frozenIdentityAuthorityV10();
  if (identity.formal_run_id !== V10_RUN_ID
      || identity.target_c !== [V10_TARGETS[0].topology, V10_TARGETS[0].container, V10_TARGETS[0].containerId,
        V10_TARGETS[0].database, V10_TARGETS[0].volume].join("\t")
      || identity.target_d !== [V10_TARGETS[1].topology, V10_TARGETS[1].container, V10_TARGETS[1].containerId,
        V10_TARGETS[1].database, V10_TARGETS[1].volume].join("\t")) throw new Error("v10-resource-authority-identity-drift");
}
...
function inspect(recorder, target) {
  ...
  if (value !== wanted) throw new Error(`b2c-000197-v10-resource:${target.key}`);
  return value;
}
...
function inspectNoHostPorts(recorder, target) {
  ...
  if (value !== '{}|{"5432/tcp":null}') throw new Error(`b2c-000197-v10-host-ports:${target.key}`);
...
function preflightHealth(recorder, target) {
  ...
  for (const [name, count] of Object.entries(value)) {
    if (count !== 0) throw new Error(`b2c-000197-v10-preflight-health:${target.key}:${name}:${count}`);
  }
  return value;
}
...
export function assertAbsentV10(value, target = { key: "unknown" }) {
  if (value.history_primary !== null || value.history_mirror !== null || value.approval_rows !== 0
    ...
    throw new Error(`b2c-000197-v10-not-dual-absent-empty:${target.key}`);
  }
  return value;
}
...
function history(recorder, target, status, error = null) {
  psql(recorder, target, `history-${target.key}-${status}`, `BEGIN;
    ...
    VALUES (${literal(migrationFilename)},${literal(expected.migration)},${literal(status)},clock_timestamp(),
      ...
      'b2c-000197-v10-executor','${V10_RUN_ID}') ON CONFLICT(filename) DO UPDATE SET checksum=EXCLUDED.checksum,
...
function migrate(recorder, target) {
  const before = snapshot(recorder, target, `before-${target.key}`); assertAbsentV10(before, target); history(recorder, target, "running");
  const result = psql(recorder, target, `migration-${target.key}`, readFileSync(migrationPath), { allowFailure: true });
  if (result.status !== 0 || result.signal || result.error) {
    history(recorder, target, "failed", `${result.stdout}${result.stderr}`); throw new PhaseExecutionErrorV10(`migration-${target.key}`, result);
  }
  ...
  if (after.approval_rows !== 0 || after.indexdef !== expected.newIndex || after.predicate !== expected.newPredicate
      || after.build_residue) throw new Error(`b2c-000197-v10-post-drift:${target.key}`);
  ...
  if (JSON.stringify(after) !== JSON.stringify(rerun)) throw new Error(`b2c-000197-v10-rerun:${target.key}`);
  return { before, after, rerun_exact: true };
}

function faults(recorder, target) {
  return failureInjectionCasesV10().map(({ name, boundary, prefix, assertion }) => {
    const before = snapshot(recorder, target, `fault-${target.key}-${name}-before`); const marker = `v10-injected-${name}`;
    ...
    if (result.status === 0 || !`${result.stdout}${result.stderr}`.includes(marker)
        || JSON.stringify(before) !== JSON.stringify(after)) throw new Error(`b2c-000197-v10-fault:${name}`);
...
function predicateMatrix(recorder, target) {
  const tenant = `v10-matrix-${target.key}-${V10_RUN_ID}`; const park = "p1";
  const cases = [
  ...
  if (result.active !== 7 || result.terminal !== 5 || result.total !== 12) {
    throw new Error(`b2c-000197-v10-predicate-matrix-drift:${target.key}`);
  }
  ...
  if (blocked.status === 0 || !`${blocked.stdout}${blocked.stderr}`.includes("23505")) {
    throw new Error(`b2c-000197-v10-active-duplicate-not-blocked:${target.key}`);
  }
  ...
  if (terminalCount !== 2) throw new Error(`b2c-000197-v10-terminal-duplicate-blocked:${target.key}`);
...
function approval(recorder, target) {
  ...
  if (!password || !ip) throw new Error("b2c-000197-v10-secret-ip");
  ...
  return runPhasedGateV10(recorder, { phases: [
...
function frozenManifest() {
  ...
  if (lines.shift() !== "b2c-000197-preliminary-input-manifest-v10") throw new Error("v10-manifest-schema");
  ...
  if (actualPaths.some((path) => path.includes("b2c-000197-v10-complete-preflight-evidence-b2c197_prelim_20260802e"))) {
    throw new Error("v10-manifest-audit-only-02e-forbidden");
  }
  if (new Set(actualPaths).size !== actualPaths.length
      || JSON.stringify(actualRows) !== JSON.stringify(authoritativeFileRowsV10())) {
    throw new Error("v10-manifest-authoritative-closure");
  }
  for (const line of rows) {
    const [, relative, size, hash, reason] = line.split("\t"); const content = readFileSync(resolve(root, relative));
    if (!reason) throw new Error(`v10-manifest-reason:${relative}`);
    if (content.length !== Number(size) || sha256(content) !== hash) throw new Error(`v10-manifest-drift:${relative}`);
  }
  return sha256(readFileSync(manifestPath));
}

export function assertIndependentReviewV10(text, authority, bindings) {
  const lines = String(text).trimEnd().split("\n");
  if (lines.shift() !== "b2c-000197-preliminary-v10-independent-review-v1") throw new Error("v10-review-schema");
  ...
  for (const [name, value] of Object.entries({ ...bindings, reviewer_authority: authority, decision: "GO" })) {
    if (fields.get(name) !== value) throw new Error(`v10-review:${name}`);
  }
  return Object.fromEntries(fields);
}

export function assertDrainAuthorityV10(text) {
  const identity = frozenIdentityAuthorityV10();
  const lines = String(text).trimEnd().split("\n");
  if (lines.shift() !== identity.drain_schema) throw new Error("v10-drain-schema");
  const fields = new Map(lines.map((line) => { const at = line.indexOf("\t"); return [line.slice(0, at), line.slice(at + 1)]; }));
  for (const [name, value] of Object.entries({ formal_run_id: identity.formal_run_id,
    resource_authority_raw_sha256: identity.resource_authority_raw_sha256,
    decision: "GO", intake: "stopped", in_flight_approval_create_transactions: "0",
    new_writer_build: identity.writer_build })) {
    if (fields.get(name) !== value) throw new Error(`v10-drain:${name}`);
  }
  return Object.fromEntries(fields);
}
...
function authorities() {
  ...
  if (resolver !== expected.resolver) throw new Error("v10-resolver-runtime-drift");
  const bindings = { formal_run_id: V10_RUN_ID, manifest_raw_sha256: manifest,
    handoff_raw_sha256: handoff, resource_authority_raw_sha256: expected.resource,
    executor_raw_sha256: sha256(readFileSync(new URL("./track-b2c-000197-preliminary-executor-v10.mjs", import.meta.url))),
  ...
  for (const [key, authority] of [["A", "independent-database-reviewer"], ["B", "independent-qa-security-reviewer"]]) {
    const path = resolve(root, process.env[`B2C_000197_V10_REVIEW_${key}_PATH`] ?? "");
    exact(path, process.env[`B2C_000197_V10_REVIEW_${key}_SHA`]);
    result[key] = assertIndependentReviewV10(readFileSync(path, "utf8"), authority, bindings);
  }
  const drainPath = resolve(root, process.env.B2C_000197_V10_OLD_WRITER_DRAIN_PATH ?? "");
  exact(drainPath, process.env.B2C_000197_V10_OLD_WRITER_DRAIN_SHA);
  result.drain = assertDrainAuthorityV10(readFileSync(drainPath, "utf8"));
  result.drain_sha256 = process.env.B2C_000197_V10_OLD_WRITER_DRAIN_SHA;
  return result;
}

export function formalStaticV10(recorder) {
  const frozenEnv = childEnv({ B2C_000197_V10_STATIC_MODE: "frozen" });
  const frozenAllow = [...safeEnv(), { name: "B2C_000197_V10_STATIC_MODE", persist: "value" }];
  for (const [stage, cwd, args, count, env, envAllowlist, expectedNames = []] of [
    ["static-v10-evidence", root, ["--test-reporter=tap", "scripts/e2e/property-remediation/tests/b2c-000197-preliminary-executor-v10.spec.mjs"], 8, childEnv(), safeEnv()],
    ["static-v10-orchestrator", root, ["--test-reporter=tap", "scripts/e2e/property-remediation/tests/b2c-000197-preliminary-orchestrator-v10.spec.mjs"], 11, frozenEnv, frozenAllow],
    ["static-v10-capability", root, ["--test-reporter=tap", "scripts/e2e/property-remediation/tests/b2c-000197-capability-closure-v10.spec.mjs"], 3, childEnv(), safeEnv(), [
    ...
    ["static-v10-recursive-closure", root, ["--test-reporter=tap", "scripts/e2e/property-remediation/tests/b2c-000197-recursive-closure-v10.spec.mjs"], 3, childEnv(), safeEnv(), [
    ...
    ["static-v10-closure", root, ["--test-reporter=tap", "scripts/e2e/property-remediation/tests/b2c-000197-frozen-closure-v10.spec.mjs"], 3, childEnv(), safeEnv()],
    ["static-v10-contract", root, ["--test-reporter=tap", "scripts/e2e/property-remediation/tests/b2c-approval-index-forward-fix.spec.mjs"], 8, childEnv(), safeEnv()],
    ["static-v10-lifecycle", apiRoot, ["--test-reporter=tap", "--require", "ts-node/register",
    ...
    parser: (stdout) => parseTapSummaryV10(stdout, { expectedTests: count, expectedNames }) });
}

export function staticV10Candidate({ mode = "auto" } = {}) {
  inputs(); const frozen = existsSync(manifestPath) && existsSync(handoffPath);
  if (mode === "frozen") {
    if (!frozen) throw new Error("b2c-000197-v10-freeze-files-missing"); frozenManifest();
    return { status: "frozen-awaiting-independent-reviews", manifest_frozen: true, execution_authorized: false,
      live_execution: false, formal_run_id: V10_RUN_ID, fixture_run_id: fixtureRunId,
      approval_runtime_v8_raw_sha256: expected.runtime,
      review_schema: "b2c-000197-preliminary-v10-independent-review-v1", resolver_raw_sha256: expected.resolver };
  }
  if (mode === "unfrozen") return { status: "unfrozen-v10-integrated", manifest_frozen: false,
    execution_authorized: false, live_execution: false, formal_run_id: V10_RUN_ID, fixture_run_id: fixtureRunId,
    approval_runtime_v8_raw_sha256: expected.runtime,
    review_schema: "b2c-000197-preliminary-v10-independent-review-v1",
    resolver_raw_sha256: expected.resolver };
  return staticV10Candidate({ mode: frozen ? "frozen" : "unfrozen" });
}

export function executePreflightV10() {
  return executeWithEvidenceV10({ evidenceRoot: resolve(research, `b2c-000197-v10-complete-preflight-evidence-${V10_RUN_ID}`),
    operation: (recorder) => { inputs({ includePreflight: false }); return V10_TARGETS.map((target) => { const identity = inspect(recorder, target);
      const state = snapshot(recorder, target, `preflight-${target.key}`); assertAbsentV10(state, target);
    ...
    successPayload: { scope: "c-and-d-v10-complete-read-only-preflight", execution_authorized: false,
      database_or_container_mutation_performed: false, resources_retained: ["c", "d"] } });
}

export function executeStaticGateV10() {
  return executeWithEvidenceV10({ evidenceRoot: resolve(research, `b2c-000197-v10-static-gate-evidence-${V10_RUN_ID}`),
    operation: (recorder) => { inputs(); frozenManifest(); formalStaticV10(recorder);
      return { formal_static_children: 7, capability_recorded: true, recursive_closure_recorded: true }; },
    successPayload: { scope: "v10-frozen-static-only", execution_authorized: false, live_execution: false,
      ...
      drain_schema: frozenIdentityAuthorityV10().drain_schema,
      drain_required_fields: ["formal_run_id", "resource_authority_raw_sha256", "decision", "intake",
        "in_flight_approval_create_transactions", "new_writer_build"],
      new_writer_build: frozenIdentityAuthorityV10().writer_build } });
}

export function executeFormalV10() {
  if (process.env.B2C_000197_PRELIMINARY_V10_RUN_ID !== V10_RUN_ID) throw new Error("v10-run-id-drift");
  return executeWithEvidenceV10({ evidenceRoot: resolve(research, `b2c-000197-formal-evidence-${V10_RUN_ID}`),
    operation: (recorder) => formalOperationV10(recorder),
    successPayload: { scope: "absent-path-c-and-d-preliminary-only", final_current: false, resources_retained: ["c", "d"] } });
}

export function formalOperationV10(recorder, overrides = {}) {
  const dependency = { inputs, authorities, formalStatic: formalStaticV10, inspect, snapshot,
    assertAbsent: assertAbsentV10, migrate, predicateMatrix, faults, approval, ...overrides };
  dependency.inputs(); const authority = dependency.authorities(); dependency.formalStatic(recorder);
  const preflight = V10_TARGETS.map((target) => { const identity = dependency.inspect(recorder, target);
  ...
  const results = V10_TARGETS.map((target) => ({ key: target.key,
  ...
  return { authority, preflight, results, approval: dependency.approval(recorder, V10_TARGETS[0]) };
}

if (fileURLToPath(import.meta.url) === resolve(process.argv[1] ?? "")) {
  if (process.env.B2C_000197_V10_PREFLIGHT === "1") process.stdout.write(`${JSON.stringify(executePreflightV10(), null, 2)}\n`);
  else if (process.env.B2C_000197_V10_STATIC_GATE === "1") process.stdout.write(`${JSON.stringify(executeStaticGateV10(), null, 2)}\n`);
  else if (process.env.B2C_000197_PRELIMINARY_V10_EXECUTE === "1") process.stdout.write(`${JSON.stringify(executeFormalV10(), null, 2)}\n`);
  else process.stdout.write(`${JSON.stringify(staticV10Candidate({ mode: process.env.B2C_000197_V10_STATIC_MODE ?? "auto" }), null, 2)}\n`);
}
