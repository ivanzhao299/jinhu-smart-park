...
const genericKeys=["formatVersion","artifactKind","sourceSystem","sourceSnapshotSha256","dictionaryCode","sourceObject","sourceRecordCount","decisions","productionImport"];
...
function verifyGenericDictionaryDecision(value, dictionaryCode){
 ...
 if(value.formatVersion!==1||value.artifactKind!=="yuzhou_hr_dictionary_machine_decision"||value.sourceSystem!=="yuzhou-v10"||value.dictionaryCode!==dictionaryCode||typeof value.sourceObject!=="string"||!value.sourceObject||!SHA.test(value.sourceSnapshotSha256??"")||!Number.isSafeInteger(value.sourceRecordCount)||value.sourceRecordCount<1||value.productionImport!=="HOLD"||!Array.isArray(value.decisions)||!value.decisions.length)fail("CORE_DICTIONARY_PACKAGE_INVALID");
