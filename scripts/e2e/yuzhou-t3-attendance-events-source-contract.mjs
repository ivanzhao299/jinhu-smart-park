...
test("T3 attendance event source receipt is aggregate-only, read-only, and holds production import",()=>{
  for(const value of ["dbo.[leave]","dbo.overtime","dbo.attrecord","dbo.timekeeprecord","JOIN dbo.person p ON p.person=a.person","JOIN dbo.person p ON p.person=t.person","linkedPersonRows","sourceReadOnly!==1","YUZHOU_SOURCE_RESTORE_RECEIPT_PATH","YUZHOU_MAPPING_CONTRACT_SHA256","validateSourceRestoreReceipt","sourceRestoreReceiptSha256","sourceCatalogSha256","sourceBusinessSha256","mappingContractSha256","operationMode:\"read_only_aggregate\"","productionImport:\"HOLD\""])assert.match(script,new RegExp(value.replace(/[.*+?^${}()|[\]\\]/g,"\\$&")));
