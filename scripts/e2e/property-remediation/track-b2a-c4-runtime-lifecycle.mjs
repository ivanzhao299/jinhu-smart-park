...
export function reserveRunId({ reservationPath, runId, artifact, manifest, reservedAt }) {
  const value = {
    schema_version: "property-remediation-b2a-c4-runid-reservation-v1",
    run_id: runId,
    artifact,
    manifest,
...
export function publishOutcome({ artifactPath, manifestPath, artifactLabel, outcome }) {
  ...
  const manifestBytes = `property-remediation-b2a-c4-runtime-candidate-v1\n${authority}`
    + `status\t${outcome.status}\ncandidate_admissible\t${outcome.candidate_admissible}\n`
    + "publication_contract\tartifact-and-manifest-both-required\n"
    + `artifact\t${artifactLabel}\t${Buffer.byteLength(bytes)}\t${digest(bytes)}\n`
    + `input_freeze_before\t${outcome.input_freeze_before?.raw_sha256 ?? "unavailable"}\n`
    + `input_freeze_after\t${outcome.input_freeze_after?.raw_sha256 ?? "unavailable"}\n`
    ...
    + (outcome.specs ?? []).map((spec) =>
      `pg_spec\t${spec.path}\t${spec.raw_sha256}\t${spec.tests}\t${spec.passed}\n`).join("");
  ...
  return {
    ...
    artifact_bytes: Buffer.byteLength(bytes),
    manifest_bytes: Buffer.byteLength(manifestBytes)
  };
}
...
export function cleanupExactLifecycle(input) {
  ...
  try {
    containerAbsent = inspectContainer(containerName) === null;
  } catch (error) {
    errors.push(`container-absence-unproven:${error.message}`);
  }
  ...
  if (volumeName && containerAbsent) {
    try {
      ...
      errors.push(`volume-absence-unproven:${error.message}`);
    }
  } else if (!volumeName) {
    errors.push("anonymous-volume-identity-unproven");
  }
  return {
    status: errors.length === 0 && containerAbsent && volumeAbsent ? "passed" : "failed",
    attempted: true,
...
export function readOutcome(path) {
