...
INSERT INTO hr_cutover_side_effect_required VALUES(${q(run)},'sys_user');`);
  ...
  writeFileSync(gate,JSON.stringify({codeSha:`a1234567890123456b${"c".repeat(22)}`,sourceSnapshotHash:`a1234567890123456b${"c".repeat(46)}`})+"\n",{mode:0o600}); chmodSync(gate,0o600); assert.equal(buildEvidenceIndex(evidenceDir,[{kind:"gate",relativePath:"gate.json"}]).length,1,"validated Git and content digests are non-sensitive evidence");
  writeFileSync(gate,'{"phone_390":{"status":"PASS"}}\n',{mode:0o600}); chmodSync(gate,0o600); assert.equal(buildEvidenceIndex(evidenceDir,[{kind:"gate",relativePath:"gate.json"}]).length,1,"fixed viewport labels are non-sensitive evidence");
  writeFileSync(gate,'{"phone_390":"13800138000"}\n',{mode:0o600}); chmodSync(gate,0o600); expect("SECRET_PATTERN_DETECTED",()=>buildEvidenceIndex(evidenceDir,[{kind:"gate",relativePath:"gate.json"}]));
