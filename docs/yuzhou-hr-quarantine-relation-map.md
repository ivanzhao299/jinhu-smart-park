...
`dbo.person.jobstate` 不是导入脚本临时生成的字段：旧 DDL 明确声明它引用
`dbo.jobstatecode.jobstate`，旧系统的统计过程 `u_count_jobstate`、
`u_count_jobstate_age` 和 `u_count_jobstate_payway` 都以这组字典作为人员统计的完整维度。
旧系统分析报告还明确记录了 8 个状态及其在本次源数据中的使用量：

| 旧代码 | 旧名称 | 当前源使用量 | 隔离目标候选 | 证据/风险 |
| --- | --- | ---: | --- | --- |
| `1` | 在职人员 | 254 | `active` | 直接语义映射 |
| `2` | 退休人员 | 60 | `departed` | 现代状态没有退休枚举；需保留旧代码和名称 |
| `3` | 离休人员 | 6 | `departed` | 同上，不能丢失退休/离休差异 |
| `4` | 离职人员 | 2,549 | `departed` | 与旧离职类别链一致 |
| `5` | 内退人员 | 20 | `suspended` | 现代模型可表达“非在岗但未离职” |
| `6` | 试用人员 | 0 | `probation` | 字典存在，但当前源没有实际人员使用 |
| `A` | 临时人员 | 1 | `active` + 保留旧状态 | 现代 `employment_type` 目前没有 `temporary`，不能只改状态而声称完整复现 |
| `B` | 未办退厂手续 | 59 | `suspended` + 保留旧状态 | 旧系统把它作为独立人员状态；不能直接改成 `departed`，否则会绕过现代离职清退流程 |

这组证据说明：那 60 名 `raw_only` 员工确实来自 `dbo.person` 的 59 个 `B` 状态和 1 个 `A` 状态，
不是下游表“凭空制造”的异常。当前机器候选包已经能对 7 个实际使用状态做守恒映射，但其状态为
`MACHINE_CANDIDATE`、生产导入仍为 `HOLD`。本轮已补齐现代模型的 `temporary` 类型以及
`legacy_jobstate_code/name` 兼容字段；下一步是用当前源快照重新物化这份 7 状态字典，并在隔离 T0→T3
链中验证 60 名员工及其下游依赖是否全部收敛，不能直接把旧候选包当成生产批准。
...
## 当前候选重绑定核验（2026-09-08）

用当前代码候选 `b24087cb`、当前源快照和同一目标范围重新生成 T0 阶段后，先对隔离目标做了一次只读库存核对。隔离目标 `jinhu_hr_migration_lab_real_t0t3_20260907a` 返回 `sys_org=14`、`hr_position=0`、`hr_employee=0`，目标身份哈希也不同于此前绑定的库存（此前库存为 `sys_org=16`）。因此旧的目标库存不能通过修改元数据强行绑定到新候选；否则会把目标漂移误报成可导入。

此前候选生成曾被 `PRODUCTION_IMPORT_T0_DECISION_JOB_STATE_INVALID` 阻断，原因是状态机包绑定旧代码 SHA。现已在新的隔离核心目标中用当前代码 `ac991bba` 重建受信任机器包：7 个状态、2,949 条员工状态记录全部守恒，机器断言为 `PASS`，`productionImport=HOLD`。随后用同一目标的当前只读库存重新物化 T0，结果为 3,105 条候选全部 `insert`，无冲突、无隔离，状态 `READY_FOR_FREEZE`。该结果只证明 T0 候选链已恢复，不代表生产授权，也不改变 47 条异常的源表归属。
