...
permissions(code, name, resource, action, permission_type, perm_type, api_method, api_path, frontend_route, sort_no) AS (
  VALUES
    ('safety_emergency_timeline:read', '应急事件时间线读取', 'biz.safety_emergency_timeline', 'read', 'api', 40, 'GET', '/api/v1/safety/emergencies/:id/timeline', NULL, 920),
    ('safety_emergency_timeline:create', '追加应急处置日志', 'biz.safety_emergency_timeline', 'create', 'api', 40, 'POST', '/api/v1/safety/emergencies/:id/timeline', NULL, 930)
),
updated_permissions AS (
  ...
  SET park_id = seed_scope.park_id,
      ...
      is_builtin = true,
      is_deleted = false,
      remark = 'S5-B emergency timeline permission seed',
      update_time = now()
  FROM permissions
...
inserted_permissions AS (
  ...
  SELECT
    ...
    'enabled', true, true, true, 'S5-B emergency timeline permission seed'
  FROM permissions
  CROSS JOIN seed_scope
  WHERE NOT EXISTS (
    SELECT 1
...
role_permissions(role_code, permission_code) AS (
  VALUES
    ('SUPER_ADMIN', 'safety_emergency_timeline:read'),
    ('SUPER_ADMIN', 'safety_emergency_timeline:create'),
    ('OPERATIONS_OWNER', 'safety_emergency_timeline:read'),
    ('OPERATIONS_OWNER', 'safety_emergency_timeline:create'),
    ('SAFETY_MANAGER', 'safety_emergency_timeline:read'),
    ('SAFETY_MANAGER', 'safety_emergency_timeline:create'),
...
SELECT seed_scope.tenant_id,
       ...
       false,
       1,
       'S5-B emergency timeline role permission seed'
FROM seed_scope
