...
- Real-artifact bridge JSON parsing already creates a privately owned graph. Reuse that graph across phase/role/envelope construction instead of whole-graph clones; preserve caller isolation with exact synchronous hash/fatal-UTF-8 decoding and reject shared backing memory. Regression-test offset byte views, caller/output mutation, unchanged generated hashes and invalid UTF-8. Reduced allocation is not a measured real-data memory guarantee; retain the process RSS guard and require a bounded real-data verification before clearing a memory failure.
...
- Payload-bundle hashing streams one record at a time under the original sealed canonical ordering and trailing newline. Do not reuse the artifact hash: the target-model serializer uses JSON.stringify object enumeration, which differs for numeric keys. Compare against an independent pre-change byte/hash oracle including nested numeric keys, Unicode, empty arrays and caller non-mutation; synthetic RSS improvements do not close the full-data memory gate.
