...
test("additional park provisioning creates an independent, atomic tenant scope", () => {
  ...
  assert.match(block, /ensureTenantDictionaries\(manager, sourceScope, targetScope/);
...
test("new tenant and additional park provisioning copy dictionary baselines", () => {
  ...
  const migration = readFileSync(
    resolve(__dirname, "../../../../../database/migrations/000210_tenant_scope_dictionary_provisioning.sql"),
    "utf8"
  );
  const productionSeed = readFileSync(
    resolve(__dirname, "../../../../../database/seeds/production/000015_tenant_scope_dictionary_reconcile.sql"),
    "utf8"
  );
