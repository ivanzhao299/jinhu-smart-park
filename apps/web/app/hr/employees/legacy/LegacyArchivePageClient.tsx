...
export function LegacyArchivePageClient({unclaimed=false}:{unclaimed?:boolean}){
 ...
 const [page,setPage]=useState(1),[rows,setRows]=useState<HrLegacyArchiveRecord[]>([]),[total,setTotal]=useState(0),[loading,setLoading]=useState(true),[message,setMessage]=useState(""),[forbidden,setForbidden]=useState(false),[selected,setSelected]=useState<HrLegacyArchiveRecord|null>(null);
 ...
 const load=useCallback(async()=>{listAbort.current?.abort();const controller=new AbortController();listAbort.current=controller;setLoading(true);setMessage("");setForbidden(false);if(!canRead){setForbidden(true);setRows([]);setLoading(false);return}try{const result=await hrApi.legacyArchive(getAccessToken(),page,PAGE_SIZE,{keyword:keyword.trim(),status,recordType:recordType.trim(),employeeId},unclaimed,controller.signal);if(controller.signal.aborted)return;setRows(result.items);setTotal(result.total);setSelected(current=>current&&result.items.some(row=>row.id===current.id)?current:null)}catch(error){if(controller.signal.aborted)return;if(isForbiddenError(error))setForbidden(true);else setMessage(error instanceof Error?error.message:"加载旧系统资料失败");setRows([]);setTotal(0)}finally{if(listAbort.current===controller)setLoading(false)}},[canRead,employeeId,keyword,page,recordType,status,unclaimed]);
 ...
 return <PermissionGuard module="hr" permission={pagePermission} fallback={fallback}><main className={`content ds-page ${styles.page}`}>
  ...
  <section className="ds-panel"><div className={styles.filters}><label className="form-field"><span>搜索资料标题</span><input type="search" value={keyword} onChange={event=>{setKeyword(event.target.value);setPage(1)}} maxLength={100} placeholder="仅搜索安全标题"/></label><label className="form-field"><span>归属状态</span><select value={status} onChange={event=>{setStatus(event.target.value);setPage(1)}}><option value="">全部状态</option>{Object.entries(statusLabels).filter(([value])=>!unclaimed||value==="archive_only"||value==="quarantine").map(([value,label])=><option key={value} value={value}>{label}</option>)}</select></label><label className="form-field"><span>资料类型</span><input value={recordType} onChange={event=>{setRecordType(event.target.value);setPage(1)}} maxLength={64} placeholder="例如 employee_profile"/></label></div><span className="status-pill">共 {total} 条安全投影</span></section>
  {message?<p className="form-error" role="alert">{message}</p>:null}
  <section className="ds-panel"><div className="ds-mobile-record-list">{loading?<p>正在加载…</p>:rows.length?rows.map(row=><article className="ds-mobile-record" key={row.id}><div className={styles.recordHeading}><strong>{row.displayTitle}</strong><span className="status-pill">{statusLabels[row.mappingStatus]??row.mappingStatus}</span></div><span>{row.recordType} · {row.occurredOn??"日期未登记"}</span><span>{row.employeeId?"已通过稳定源身份归属员工":"未归属员工"}</span><button className="ds-button" type="button" onClick={()=>void showDetail(row)}>查看安全详情</button></article>):<p>{unclaimed?"当前没有待认领资料。":"当前权限范围内暂无旧系统资料。"}</p>}</div>{total>PAGE_SIZE?<nav className={styles.pagination} aria-label="旧系统资料分页"><button className="ds-button ds-button-secondary" type="button" disabled={loading||page<=1} onClick={()=>setPage(value=>Math.max(1,value-1))}>上一页</button><span>第 {page}/{Math.max(1,Math.ceil(total/PAGE_SIZE))} 页</span><button className="ds-button ds-button-secondary" type="button" disabled={loading||page*PAGE_SIZE>=total} onClick={()=>setPage(value=>value+1)}>下一页</button></nav>:null}</section>
