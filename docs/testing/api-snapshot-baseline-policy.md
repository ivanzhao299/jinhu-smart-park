...
first release API snapshot / release gate 总复盘见 `docs/testing/first-release-api-snapshot-release-gate-review.md`。release gate 只允许检查 baseline，不允许自动更新 baseline；如默认 baseline 或 numeric baseline 需要变化，必须单独 PR、说明数据来源和运行顺序，并人工审查 diff。
