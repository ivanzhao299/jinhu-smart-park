...
test("target registration request binds only the attested hashes and preserves the production hold", () => {
  const request = createProductionTargetRegistrationRequest({ attestation: attestation(), attestationSha256: hash("attestation") });
  assert.equal(request.productionImport, "HOLD");
  assert.equal(request.executionReachable, false);
  assert.equal(request.preparedBy, "machine_policy_engine");
  ...
  assert.throws(() => createProductionTargetRegistrationRequest({ attestation: { ...attestation(), validScopeCount: 2 }, attestationSha256: hash("attestation") }), error => error.code === "PRODUCTION_TARGET_REGISTRATION_ATTESTATION_INVALID");
});
...
test("registration CLI accepts only a private attestation and creates a new private held signing input", () => {
  ...
  try {
    ...
    const input = parseProductionTargetRegistrationRequestArgs(["--attestation", attestationPath, "--output-dir", outputDir, "--request-id", "target-registration-01"]);
    ...
    assert.throws(() => parseProductionTargetRegistrationRequestArgs(["--attestation", attestationPath, "--actor-id", "11111111-1111-4111-8111-111111111111", "--output-dir", outputDir, "--request-id", "target-registration-02"]), error => error.code === "PRODUCTION_TARGET_REGISTRATION_ARGUMENT_INVALID");
