...
permissions(code, name, resource, action, permission_type, perm_type, api_method, api_path, frontend_route, sort_no) AS (
  VALUES
    ...
    ('safety_statistics:read', '安全统计', 'biz.safety_statistics', 'read', 'api', 40, 'GET', '/api/v1/safety/statistics', '/safety/dashboard', 620)
),
updated_permissions AS (
  ...
  SET park_id = seed_scope.park_id,
      ...
      is_builtin = true,
      is_deleted = false,
      remark = 'S5-A safety statistics permission and menu seed',
      update_time = now()
  FROM permissions
...
inserted_permissions AS (
  ...
  SELECT
    ...
    true,
    true,
    'S5-A safety statistics permission and menu seed'
  FROM permissions
  CROSS JOIN seed_scope
  WHERE NOT EXISTS (
    SELECT 1
...
WHERE child.tenant_id = parent.tenant_id
  ...
  AND child.code = 'safety:dashboard'
...
touched_permissions AS (
  ...
  WHERE tenant_id = (SELECT tenant_id FROM seed_scope)
    ...
    AND code = 'safety_statistics:read'
...
WHERE child.tenant_id = parent.tenant_id
  AND child.park_id = parent.park_id
  AND child.id = touched.id
  AND parent.code = 'safety:dashboard'
  AND parent.is_deleted = false
  AND child.is_deleted = false;
...
SELECT seed_scope.tenant_id,
       ...
       false,
       1,
       'S5-A safety statistics role permission seed'
FROM seed_scope
