...
function runtime(options: {
  ...
  const service = new PropertyApprovalService(
    ...
    {
      inspect: async () => ({
        effective: options.controlMode === "enforce",
        mode: options.controlMode ?? "enforce",
        version: 1
      }),
      approvalMode: async () => options.controlMode ?? "enforce",
      requireApprovalEnforce: async () => {
        if ((options.controlMode ?? "enforce") !== "enforce") {
