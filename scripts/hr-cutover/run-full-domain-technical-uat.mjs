...
WITH employees AS(SELECT id,row_number()OVER(ORDER BY id) rn FROM hr_employee WHERE tenant_id=:'tenant' AND park_id=:'park' AND remark LIKE '%'||:'run'||'-t0%' AND is_deleted=false LIMIT 3), users AS(SELECT id,row_number()OVER(ORDER BY CASE username WHEN :'hrMaker' THEN 1 WHEN :'manager' THEN 2 ELSE 3 END)rn FROM sys_user WHERE tenant_id=:'tenant' AND park_id=:'park' AND username IN(:'hrMaker',:'manager',:'employee')) UPDATE hr_employee e SET user_id=u.id FROM employees x JOIN users u USING(rn) WHERE e.id=x.id;
...
COMMIT;`;
 ...
 try{
  ...
  return {state:"uat_ready",technicalUat:"PASS",apiChecks:13,negativeAuthorizationChecks:2,webRouteChecks:3,legacyScorePromotion:"HOLD_PENDING_FULL_MATRIX",manifestSha256:record.sha256,productionImport:"HOLD"};
 }finally{
