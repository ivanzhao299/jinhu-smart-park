...
### 隔离社保 HTTP 超时定位（2026-09-09）

配置准备器须使用与落盘配置相同的规范 JSON 键顺序生成执行 manifest 摘要。准备回执和从落盘配置重建的执行 manifest 必须一致；旧准备回执若存在排序前摘要，不覆盖历史文件，应从原受控配置通过原验证器重建实际运行身份，再核对最终回执。修正准备器不构成重新抽取或装载的理由。

真实 lab 的 Nest 实例对社保列表增加隔离进程内分段计时：list、count、page、items、projection、audit。
失败时在 HTTP 截止点截取最多 128 条固定阶段名、状态和毫秒数，经 owner 与最终回执的同一白名单净化后保留。
pending 表示截止时尚未返回，不等于 SQL 已失败；阶段可以嵌套，耗时不能相加为总耗时。
不记录 SQL、参数、响应、异常文本或业务身份，不修改生产服务、权限、审计、15 秒请求预算或回滚要求。
该诊断助手纳入运行器依赖指纹；旧配置不能冒充新诊断代码的执行证据。诊断测试通过不代表真实超时已修复。
