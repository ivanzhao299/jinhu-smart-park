...
把既有玉舟 T0～T5/T4 领域迁移器提升为可审计、可重复、失败即停的企业级全域迁移能力。在同一冻结源事实、同一映射合同和同一候选代码 SHA 下，用两个完全独立目标环境连续执行 T0→T1→T2→T3→T4→T5，证明全域总账、工资双轨、增量冻结、三角色业务流、备份恢复、受控回滚和零残留清理。
...
5. 第二轮后执行最终源冻结和 zero/controlled delta。无稳定键或无法证明删除的表必须在停写后重新全量，不允许猜测增量。
6. T4 必须核对真实 46,092 工资行、711 项目、244 公式、1,431 关账、647 账套成员和 9 税率，运行批准范围内的双轨对账；只算不发，正式 payroll/payslip/payment/bank/tax/message 零写。
...
- [ ] 最终 zero/controlled delta 应用后的候选库与 S1 从空库全量重建具有相同 global/canonical hash；不稳定表只允许冻结后全量。
...
以下事项不阻止 Slice 1～3，也不阻止 Slice 4～8 的 schema、fixture、负向测试、dry-run wrapper 和 fail-closed compiler 工程实施；但对应的真实源抽取/冻结、真实业务差异接受、真人 UAT、正式 RTO/RPO 判定和最终 `GO_CANDIDATE` 必须等待输入：最终停写责任人与窗口、无稳定键表的冻结全量决定、T4 公式批准范围/容差/签署人、三角色真人 UAT 人员、正式 RTO/RPO 目标、生产值班与回退职责。未确定时 compiler 必须给出稳定 reason code 并保持 `NO_GO/productionImport=HOLD`，不得把缺输入误报为工程阻塞或通过。
