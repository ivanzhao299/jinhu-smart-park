...
export default function UnitsPage({ title = "房源管理" }: UnitsPageProps = {}) {
  ...
  const loadLookups = useCallback(async () => {
    ...
    const dictCodes = ["unit_usage_type", "unit_rental_status", "unit_fitting_status"];
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    const body = {
      ...
      refPrice: Number(form.refPrice || 0),
            ...
            <tbody>
              {pageData.items.map((row) => (
                <tr key={row.id}>
                  ...
                  <td>{formatMoney(row.refPrice)}</td>
  ...
  return (
    <section className="login-panel drawer-panel drawer-panel-lg">
      ...
      <div className="form-stack">
        ...
        <DetailItem label="参考租金" value={formatMoney(unit.refPrice)} />
        <DetailItem label="可租日期" value={unit.availableDate ?? "-"} />
        <DetailItem label="照片" value={<button type="button" onClick={() => onOpenAttachments("photos")}>查看照片</button>} />
        ...
        <DetailItem label="备注" value={unit.remark ?? "-"} />
      </div>
