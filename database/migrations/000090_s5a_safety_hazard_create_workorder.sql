...
permissions(code, name, resource, action, permission_type, perm_type, api_method, api_path, frontend_route, sort_no) AS (
  VALUES
    ('safety_hazard:create_workorder', '隐患转工单', 'biz.safety_hazard', 'create_workorder', 'api', 40, 'POST', '/api/v1/safety/hazards/:id/create-work-order', NULL, 610)
),
updated_permissions AS (
  ...
  SET park_id = seed_scope.park_id,
      ...
      is_builtin = true,
      is_deleted = false,
      remark = 'S5-A safety hazard create workorder permission seed',
      update_time = now()
  FROM permissions
...
inserted_permissions AS (
  ...
  SELECT
    ...
    true,
    true,
    'S5-A safety hazard create workorder permission seed'
  FROM permissions
  CROSS JOIN seed_scope
  WHERE NOT EXISTS (
    SELECT 1
...
SELECT seed_scope.tenant_id,
       ...
       false,
       1,
       'S5-A safety hazard create workorder role permission seed'
FROM seed_scope
