...
function bindSignedMergeConflict(fixture) {
  const decision = {
    sourceIdentitySha256: "1".repeat(64),
    strategy: "merge",
    existingTargetIdentitySha256: "2".repeat(64),
    beforeImageSha256: "3".repeat(64),
    legacyRecordMapSha256: "4".repeat(64),
...
test("a detached conflict ledger can authorize an explicit merge without enabling execution", () => {
  const fixture = makeFixture();
  try {
    bindSignedMergeConflict(fixture);
    const result = evaluateProductionImportPreflight(fixture.plan, fixture.options);
...
test("missing, stale, reused and restore authorization artifacts fail closed", () => {
  const cases = [
    ...
    ["reused", fixture => rewriteRole(fixture, "authorization_usage_ledger", value => value.entries.push({ operationId: "yzprod-import-20260828T000000Z-123456abcdef", authorizationArtifactSha256: fixture.authDeclaration.sha256, intent: "production_import", status: "CONSUMED", consumedAt: "2026-08-28T01:00:00.000Z" })), "PRODUCTION_IMPORT_AUTH_REUSED"],
    ["operation-reused", fixture => rewriteRole(fixture, "authorization_usage_ledger", value => value.entries.push({ operationId: OPERATION_ID, authorizationArtifactSha256: "f".repeat(64), intent: "production_import", status: "CONSUMED", consumedAt: "2026-08-28T01:00:00.000Z" })), "PRODUCTION_IMPORT_OPERATION_REUSED"],
