...
| POST | `/api/engineering/rectifications/overdue-scan` | 扫描并标记逾期整改 |
...
- `deadline`：整改期限。
- `attachment_ids`：附件 ID 引用，写入前校验文件属于当前租户/园区。
...
`MARK_OVERDUE` 暂不改变 Issue 状态，只发布逾期事件；逾期扫描已由 Task 017 接入，通知推送后续由 Notification Runtime 接管。
...
```text
POST /api/engineering/rectifications/overdue-scan
```
...
| 字段 | 说明 |
| --- | --- |
| `today` | 可选，按指定日期扫描，格式 `YYYY-MM-DD`；为空时使用服务端当前日期 |

处理规则：

1. 只扫描当前 `tenantId` / `parkId` / DataScope 范围内的整改任务。
2. 候选条件为 `deadline < today`，且状态不是 `PASSED`、`CLOSED`、`OVERDUE`。
3. 每条命中记录必须通过 `EngineeringRectificationStateMachine` 执行 `MARK_OVERDUE`。
4. 每条状态变化写 `ACTION_MARK_OVERDUE` 审计日志。
5. 每条状态变化发布 `EngineeringRectificationOverdueEvent`。
6. `MARK_OVERDUE` 不直接关闭问题，也不自动修改来源 `EngineeringIssue` 状态；后续通知和责任追踪由 Workflow / Notification Runtime 接管。
...
5. Issue 状态同步。
6. 逾期整改扫描 API。
7. RBAC、DataScope、AuditLog、EventBus 入口。

未实现：

1. 定时调度器。
2. 附件上传和预览页面，当前 API 已支持 `attachment_ids` 引用校验。
3. 通知推送。

## 后续任务

- Task 016：实现整改反馈与复查页面，见 [engineering-rectification-ui.md](./engineering-rectification-ui.md)。
- Task 018：实现工程验收模型与 API。
