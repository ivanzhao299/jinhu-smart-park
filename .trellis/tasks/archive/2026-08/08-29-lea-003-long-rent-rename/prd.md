...
> 当前态注记（2026-09-06）：本任务已确立的根业务现名“长租经营”继续有效；housing 与传统 leasing 维持双 bounded context，当前权威入口见 `docs/architecture/property-current-state-index.md`。以下内容保留历史实施事实。
...
- 历史迁移不可修改；远端 main 并发占用 `000283` 后，本任务使用新的未占用迁移号 `000284`，仓库仅保留已知的历史重复 `000136`。
...
- [x] API `/users/me` canonical 菜单与 fallback 菜单根节点显示“长租经营”，子页面 label 与 Web 一致。
- [x] Web 菜单、住房工作台和无权限/不可用边界不再把根业务显示为“住房出租”。
- [x] 房源经营模式、候选 picker 等具体语境正确显示“住宅长租”或“办公长租”。
- [x] shared 权限目录及数据库中既有目标权限显示名更新，所有 code 逐项保持不变。
- [x] `000284` 迁移按租户收敛已存在名称、允许租户权限子集、对重复/更新后漂移目标 fail closed，且不触碰角色权限绑定。
- [x] 迁移逐租户断言、API/Web 菜单契约、权限显示名回归均通过。
- [x] 桌面及 390px 浏览器证据通过，截图/Network/检查结果写入实施记录且不包含敏感信息。
- [x] PR 审查不超过 3 轮，CI 全绿，squash merge 后 main 的 CI 与 Deploy 双绿，Issue #485 关闭。
- [x] Trellis 任务进度、续跑点与最终证据完整记录并归档。
