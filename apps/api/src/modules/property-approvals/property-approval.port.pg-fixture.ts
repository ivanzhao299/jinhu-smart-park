...
export interface ApprovalPortPgRunDataCleanupOptions {
  isolatedImmutableBypass?: boolean;
...
export async function cleanupApprovalPortPgRunData(
  ...
  options: ApprovalPortPgRunDataCleanupOptions = {}
): Promise<unknown[]> {
  if (options.isolatedImmutableBypass) {
    return cleanupApprovalPortPgIsolatedRunData(
      executor, tenantId, parkId, audit, options.expectedDatabase
    );
  }
...
async function cleanupApprovalPortPgIsolatedRunData(
  executor: ApprovalPortPgQueryExecutor,
  tenantId: string,
  parkId: string,
  audit: ApprovalPortPgFixtureAudit,
  ...
  const scopePattern = /^b2c-[0-9a-f]{32}$/u;
  const databasePattern = /^jinhu_b2c197_v11v6_direct_v[0-9]+_[a-z0-9_]{8,40}$/u;
  if (!scopePattern.test(tenantId) || parkId !== tenantId
      || !expectedDatabase || !databasePattern.test(expectedDatabase)) {
    return [new Error("isolated immutable cleanup authority is invalid")];
  }
  audit.cleanup.push("isolated-immutable-cleanup-preflight");
  const authority = await executor.query(
    `SELECT current_database() AS "database",current_user AS "user",
            current_setting('server_version_num') AS "version"`
  ) as Array<{ database: string; user: string; version: string }>;
  if (authority.length !== 1 || authority[0]!.database !== expectedDatabase
      || authority[0]!.user !== "postgres" || !/^16[0-9]{4}$/u.test(authority[0]!.version)) {
    return [new Error("isolated immutable cleanup database authority mismatch")];
  }
  const nonRunUnion = APPROVAL_PORT_PG_DATA_TABLES_IN_DELETE_ORDER.map((table) =>
    `SELECT '${table}'::text AS table_name,count(*)::int AS row_count
       FROM public.${quoteApprovalPortPgIdentifier(table)}
      WHERE tenant_id<>'${tenantId}' OR park_id<>'${parkId}'`
  ).join(" UNION ALL ");
  const nonRun = await executor.query(
    `SELECT table_name AS "tableName",row_count AS "rowCount"
       FROM (${nonRunUnion}) counts WHERE row_count<>0 ORDER BY table_name`
  ) as Array<{ tableName: string; rowCount: number }>;
  if (nonRun.length !== 0) return [new Error("isolated immutable cleanup found non-run data")];
  audit.cleanup.push("isolated-immutable-trigger-bypass");
  const deletes = APPROVAL_PORT_PG_DATA_TABLES_IN_DELETE_ORDER.map((table) => {
    audit.cleanup.push(`delete-run-data:${table}`);
    return `DELETE FROM public.${quoteApprovalPortPgIdentifier(table)} WHERE tenant_id='${tenantId}' AND park_id='${parkId}';`;
  }).join("\n");
  try {
    await executor.query(
      `DO $isolated_cleanup$ BEGIN
         PERFORM set_config('session_replication_role','replica',true);
         ${deletes}
       END $isolated_cleanup$;`
    );
    return [];
  } catch (error) {
    return [error];
