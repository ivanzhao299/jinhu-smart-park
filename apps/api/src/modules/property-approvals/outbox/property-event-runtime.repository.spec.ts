...
describe("TypeOrmPropertyEventRuntimeStore", () => {
  ...
  it("projects the approval execution version required by incident retry CAS", async () => {
    const store = new TypeOrmPropertyEventRuntimeStore({
      query: async (sql: string) => sql.includes("count(*)") ? [{ total: 1 }] : [{
        id,
        action_id: "housing.leases.approve.request",
        source_type: "housing-lease",
        source_id: id,
        execution_version: 9,
        last_error_code: "approval-infra-exhausted",
        infra_exhausted_at: new Date("2026-08-03T01:00:00.000Z"),
        last_retry_at: null,
        updated_at: new Date("2026-08-03T02:00:00.000Z"),
        requester_display_name: "审批申请人",
        submitted_at: new Date("2026-08-03T00:00:00.000Z")
      }]
    } as unknown as DataSource);

    const page = await store.listApprovalIncidents(scope, [id], {});

    assert.equal(page.items[0]?.executionVersion, 9);
  });
