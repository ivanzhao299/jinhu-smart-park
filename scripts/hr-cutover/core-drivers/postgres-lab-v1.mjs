...
function coreDomainFacts(config, run, domain, tables) {
  ...
  // Target UUIDs and audit timestamps differ by isolated run.  Bind the stable
  // business projection to immutable source row identities instead.
  const rows = tables.map(({ table, key }) => `SELECT '${table}' AS target_table,(to_jsonb(x)-ARRAY['id','create_time','update_time','create_by','update_by'])::text AS row_json,m.source_identity_sha256,m.source_row_sha256 FROM ${table} x JOIN legacy_record_map m ON m.target_table='${table}' AND m.target_id=x.${key} JOIN migration_batch b ON b.id=m.batch_id WHERE b.run_id='${childRun}'`).join(" UNION ALL ");
