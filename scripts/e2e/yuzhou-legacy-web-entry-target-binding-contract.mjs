...
test("all 15 Yuzhou Web entries have a mapped target or an explicit gap",()=>{
  assert.deepEqual(verifyLegacyWebEntryTargetBinding(manifest,{root}),{ok:true,entries:15,mapped:14,gaps:1,roleMatrixVerified:false,compatibilityScoreContribution:0,productionImport:"HOLD"});
});
...
test("gaps cannot carry fabricated targets and summary cannot drift",()=>{
  const invented=clone(manifest);invented.entries[6].targetRoute="/account/security";
  expectCode("BINDING_GAP_ENTRY_INVALID",()=>verifyLegacyWebEntryTargetBinding(invented,{root}));
  const swapped=clone(manifest);swapped.entries[6].reasonCode="TARGET_DEDICATED_PHOTO_VIEW_NOT_IMPLEMENTED";
