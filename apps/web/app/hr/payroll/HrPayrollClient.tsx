...
const decimalText=(value:string|null)=>value!==null&&/^-?\d+(?:\.\d+)?$/.test(value)?value:"—";
...
export function HrPayrollClient() {
  ...
  const canReadOnlineDetail = hasPermission(user, HR_PERMISSIONS.HR_PAYROLL_DETAIL_READ);
...
function RuleReview({canAct}:{canAct:boolean}){
  const [page,setPage]=useState(1),[bookPage,setBookPage]=useState(1),[itemPage,setItemPage]=useState(1),[formulaPage,setFormulaPage]=useState(1),[taxPage,setTaxPage]=useState(1);
  const [books,setBooks]=useState<PaginatedResult<HrPayrollBook>>(EMPTY_PAGE),[catalogItems,setCatalogItems]=useState<PaginatedResult<HrPayrollCatalogItem>>(EMPTY_PAGE),[formulas,setFormulas]=useState<PaginatedResult<HrPayrollFormula>>(EMPTY_PAGE),[taxRules,setTaxRules]=useState<PaginatedResult<HrPayrollTaxRule>>(EMPTY_PAGE),[result,setResult]=useState<PaginatedResult<HrPayrollReviewCase>>(EMPTY_PAGE),[state,setState]=useState<ViewState>("loading");
  ...
  const load=useCallback(async()=>{abort.current?.abort();detailAbort.current?.abort();const current=++generation.current,controller=new AbortController();abort.current=controller;setState("loading");setSelected(null);setDetailTarget(null);setDetailState("empty");try{const [b,i,f,t,c]=await Promise.all([hrApi.payrollHistoryBooks(getAccessToken(),bookPage,20,controller.signal),hrApi.payrollHistoryCatalogItems(getAccessToken(),itemPage,20,{},controller.signal),hrApi.payrollHistoryFormulas(getAccessToken(),formulaPage,20,{parseStatus:"manual_review"},controller.signal),hrApi.payrollHistoryTaxRules(getAccessToken(),taxPage,20,controller.signal),hrApi.payrollHistoryReviewCases(getAccessToken(),page,20,{},controller.signal)]);if(current!==generation.current)return;setBooks(b);setCatalogItems(i);setFormulas(f);setTaxRules(t);setResult(c);setState(c.items.length||b.items.length||i.items.length||f.items.length||t.items.length?"ready":"empty");}catch(e){if(current===generation.current&&!(e instanceof DOMException&&e.name==="AbortError"))setState(errorState(e));}},[bookPage,formulaPage,itemPage,page,taxPage]);
  ...
  </div><section className="ds-panel" aria-labelledby="history-tax-rules-heading"><div className={workbenchStyles.sectionHeading}><h2 id="history-tax-rules-heading">历史税率规则</h2><span>{taxRules.total} 条</span></div><p role="note">税率单位、区间边界、舍入和期间语义待复核；仅展示历史规则，不代表旧系统计算等价。</p><div className="ds-mobile-record-list">{taxRules.items.map(rule=><article className="ds-mobile-record" key={`${rule.legacyTaxId}:${rule.versionNo}`}><strong>规则 {rule.legacyTaxId} · 版本 {rule.versionNo}</strong><span>基数原始值 {decimalText(rule.baseAmount)} · 区间原始值 {decimalText(rule.lowerLimit)} ～ {decimalText(rule.upperLimit)}</span><span>税率原始值 {decimalText(rule.taxPercent)} · 偏移原始值 {decimalText(rule.offsetAmount)}</span><small>单位、边界、舍入和期间语义待复核</small></article>)}{taxRules.total===0?<p className={workbenchStyles.emptyState}>暂无历史税率规则。</p>:null}</div><Pager page={taxRules.page} pageSize={taxRules.page_size} total={taxRules.total} onPage={setTaxPage}/></section><section className={`ds-panel ${styles.mobileNotice}`}><strong>公式与复核请使用电脑端</strong><p>手机端仍可分页查看历史税率规则。</p></section></>;
}
