...
export class HrContractReminderService{
 ...
 async action(s:TenantParkScope,a:JwtPrincipal,id:string,d:HrContractReminderActionDto){
  ...
  return this.db.transaction(async m=>{
   ...
   await m.query(`UPDATE hr_contract_reminder SET status=$1,read_at=CASE WHEN $1 IN('read','acknowledged') THEN COALESCE(read_at,now()) ELSE read_at END,read_by=CASE WHEN $1 IN('read','acknowledged') THEN COALESCE(read_by,$2) ELSE read_by END,acknowledged_at=CASE WHEN $1='acknowledged' THEN now() ELSE acknowledged_at END,acknowledged_by=CASE WHEN $1='acknowledged' THEN $2 ELSE acknowledged_by END,resolved_at=CASE WHEN $1='resolved' THEN now() ELSE resolved_at END,resolved_by=CASE WHEN $1='resolved' THEN $2 ELSE resolved_by END,cancelled_at=CASE WHEN $1='cancelled' THEN now() ELSE cancelled_at END,cancelled_by=CASE WHEN $1='cancelled' THEN $2 ELSE cancelled_by END,cancel_reason=CASE WHEN $1='cancelled' THEN 'MANUAL_CANCELLED' ELSE cancel_reason END,update_time=now() WHERE id=$3`,[target,a.sub,id]);
