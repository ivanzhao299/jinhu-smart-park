...
1. 从运行时权威模块授权关系中识别 `homestay` 和 `housing_rental`：
   - `rel_tenant_module.enabled = true`
   - `rel_tenant_module.status = 'enabled'`
   - 未软删除、未过期
   - 关联的 `sys_module.status = 1` 且未软删除
   - 不依赖 `sys_module_registry`，因为标准租户模块开通 API 只维护前述关系
...
`hasPermission` 可继续让超级管理员旁路权限码，但 `hasModule` 必须始终检查
`/users/me.enabled_modules`。移动端登录落点也必须先满足模块启用条件，不能通过
`is_super` 直接跳入受模块控制的终端。
...
超级管理员只改变“页面权限/API 权限”列，不改变“模块”列；模块禁用时结果与普通角色一致。
...
- 页面路由与父子关系正确；
- 角色桥接从已有 API 权限派生；
- 模块范围来自 `rel_tenant_module + sys_module`，并拒绝 registry-only、disabled、expired 范围；
- 没有修改或重新授予业务 API 权限。
...
4. 增加仅标准 `rel_tenant_module` 授权、无 registry 记录的租户/园区，验证四个菜单/页面节点
   和角色桥接均生成。
5. 使用超级管理员抽样模块禁用态，验证菜单、登录落点、页面和 API 不发生旁路。
6. 浏览器检查桌面与 390px：
   - 侧栏入口存在
   - 点击到达正确页面
   - 当前菜单高亮与面包屑正确
