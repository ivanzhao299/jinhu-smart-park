...
`EngineeringEventPublisher.publishProjectStatusChanged` 定义了 `EngineeringProjectStatusChangedEvent` 的稳定 envelope，并写入 `biz_engineering_event_log`。外部消息队列仍保持适配边界，后续平台 EventBus 可从 publisher 内部接出，不需要业务 Service 改造。
