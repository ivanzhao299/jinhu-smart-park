...
后续同步：固定业务标识机制已落地，文档入口为 `docs/testing/api-snapshot-business-key-plan.md`。当前脚本已支持 `SNAPSHOT_WORKORDER_NO`、`SNAPSHOT_UNIT_NO` 和显式 `ALLOW_SNAPSHOT_FALLBACK=true`，但仍未接入 CI，也未新增 seed 或 snapshot 专用账号。
