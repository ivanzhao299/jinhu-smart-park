...
import { authoritativeFileRowsV11 } from "../track-b2c-000197-preliminary-orchestrator-v11.mjs";
import { resolveFormalExecutionClosureV11 } from "../track-b2c-000197-closure-resolver-v11.mjs";

const root = process.cwd();
const manifestPath = resolve(root,
  ".trellis/tasks/07-30-pr192-b-domain-integrations/research/b2c-000197-preliminary-v11-v5-input-manifest-20260802.grammar");
const implementationPaths = [
  "scripts/e2e/property-remediation/track-b2c-000197-failure-cases-v11.mjs",
  "scripts/e2e/property-remediation/track-b2c-000197-preliminary-executor-v11.mjs",
  "scripts/e2e/property-remediation/track-b2c-000197-preliminary-orchestrator-v11.mjs",
  "scripts/e2e/property-remediation/track-b2c-000197-closure-resolver-v11.mjs",
];
const sha256 = (value) => createHash("sha256").update(value).digest("hex");

function rows() {
  const lines = readFileSync(manifestPath, "utf8").trimEnd().split("\n");
  assert.equal(lines.shift(), "b2c-000197-preliminary-input-manifest-v11");
...
test("v11 implementation owns complete fault capability and imports no v3-v7 implementation or authority", () => {
...
test("manifest is exact omission/excess/size/SHA closure with no node_modules", () => {
  ...
  const expected = authoritativeFileRowsV11();
...
test("every recursive execution dependency is represented and governance outputs avoid self-hash", () => {
  const actual = new Set(rows().map(({ path }) => path)); const closure = resolveFormalExecutionClosureV11(root);
  ...
  assert.equal(actual.has(".trellis/tasks/07-30-pr192-b-domain-integrations/research/b2c-000197-preliminary-v11-v5-input-manifest-20260802.grammar"), false);
  assert.equal(actual.has(".trellis/tasks/07-30-pr192-b-domain-integrations/research/b2c-000197-preliminary-executor-v11-v5-review-handoff-20260802.md"), false);
  assert.equal([...actual].some((path) => path.includes("b2c-000197-v11-complete-preflight-evidence-b2c197_prelim_20260802e")), false);
});
