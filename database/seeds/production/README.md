...
- Property role templates from `packages/shared/src/property-business/role-templates.ts`, reconciled by `000015_property_role_template_reconcile.sql` into protected database roles for display and drift detection. These database bindings are not the runtime authority for instantiation; the Roles API instantiates managed property templates from the shared template definition and fails closed on metadata drift.
