...
- Descriptor Buffer/Uint8Array inputs are hashed and fatal-UTF-8 decoded synchronously from their exact byte view, including offset/length, without another whole-file copy. String inputs are UTF-8 encoded. Reject shared backing memory that can mutate between hash and decode; do not mutate caller-owned bytes. This removes redundant allocation, not the existing in-memory JSON/graph boundary or production memory cap.
- Raw scope files follow the existing T0 consumer's exact `{tenantId,parkId}` interface. Accept that or exact `{tenantId,parkId,scopeSha256}`, require nonempty trimmed string IDs, compute the existing canonical scope hash, and reject a supplied hash mismatch or any unknown field with `CANDIDATE_FREEZE_SCOPE_INVALID`. Compare normalized scope with inventory/candidates; retain the untouched original descriptor byte SHA. Test both pure and private-file two-field scope paths and the actual T2 materializer output with this raw scope shape.
- The T2 private materializer alone may append optional `resolutionEvidence` with exactly `dictionaryPackageSha256` (lowercase SHA-256), `changeDecisionsSha256` (lowercase SHA-256 or null), and `approvalClaimed: false`. Reject invalid/unknown nested fields and this extension on other phases. Retain the validated object as `t2ResolutionEvidence` alongside the exact original candidate byte hash; do not promote machine references into approval or rewrite old mapping bindings. The pure T2 assembler remains valid without the extension. Exercise actual nonempty T2 materializer output through the adapter with and without a change-decision artifact; an assembler-only fixture cannot verify the private producer envelope.
...
- Cross-layer producer/consumer regression: choose `scope` when actual executable refs are empty and every table FK is optional, not merely when a table declares no FKs. Root `sys_org` has an optional parent FK and permits scope; a child with a parent stays `record_graph`. Required refs still reject before generation. The previous table-FK-count-only selection emitted a graph root without refs that the sealed validator correctly rejected. Preserve an actual generator → sealed root-org → child-org → employee test, rather than only separately valid generator and hand-built sealed fixtures. There is no generated template mirror for this domain module.
