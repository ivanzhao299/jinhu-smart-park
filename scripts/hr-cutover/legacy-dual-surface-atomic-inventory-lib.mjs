...
export function verifyMaterializedDualSurfaceAtomicInventories(client,groupWeb,previousPair=null){
  ...
  const summary={client:{total:client.records.length,menuEntries:count(client.records,"menu"),tables:count(client.records,"table"),fields:count(client.records,"field"),rules:count(client.records,"rule"),permissions:count(client.records,"permission")},groupWeb:{total:groupWeb.records.length,menuEntries:count(groupWeb.records,"menu"),sourcePaths:count(groupWeb.records,"action"),shortcutCrossReferences:groupWeb.crossReferences.length}};
  if(JSON.stringify(summary.client)!==JSON.stringify({total:3721,menuEntries:68,tables:162,fields:2364,rules:212,permissions:915})||JSON.stringify(summary.groupWeb)!==JSON.stringify({total:417,menuEntries:231,sourcePaths:186,shortcutCrossReferences:15}))fail("DUAL_SURFACE_COUNT_CONSERVATION_FAILED",JSON.stringify(summary));
