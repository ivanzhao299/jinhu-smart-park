...
const insurance=read("insurance.raw.json").map(source=>identity("dbo.person_insure",source.id,source,{source:{id:source.id,year:source.recyear,month:source.recmonth,employeeCode:String(source.person??"").trim()},legacyCompatibility:{legacyFlags:{oldage:source.oldageflag??null,remedy:source.remedyflag??null,losework:source.loseworkflag??null,fund:source.fundflag??null,wound:source.woundflag??null,bear:source.bearflag??null},fieldPresence:{insureaccount:Boolean(source.insureaccount_present),inpatient:Boolean(source.inpatient_present),hurt:Boolean(source.hurt_present),insure:Boolean(source.insure_present),insureEmployer:Boolean(source.insure_e_present),insureEmployee:Boolean(source.insure_p_present),insureSupplement:Boolean(source.insure_pc_present)}},items:kinds.map(kind=>{const base=source[`base_${kind}`]??source.base;return{kind,contributionBase:negative(base)?null:base,totalAmount:source[kind],employerAmount:source[`${kind}_e`],employeeAmount:source[`${kind}_p`],supplementAmount:source[`${kind}_pc`],legacyBaseNegative:negative(base),legacyFlag:source[`${kind}flag`]??null}})}));
