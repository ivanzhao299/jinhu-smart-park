...
export const V10_RUN_ID = "b2c197_prelim_20260802b";
export const V10_EVIDENCE_POLICY = "b2c-000197-child-evidence-v10";
...
export function discoverSecretsV10(value, target = [], key = "") {
  if (value == null) return target;
  if (Buffer.isBuffer(value)) return discoverSecretsV10(value.toString("utf8"), target, key);
  if (Array.isArray(value)) {
    for (const entry of value) discoverSecretsV10(entry, target, ARG_CONTAINER.test(key) ? "" : key);
    return target;
  }
  if (typeof value === "object") {
    for (const [entryKey, entryValue] of Object.entries(value)) discoverSecretsV10(entryValue, target, entryKey);
    return target;
  }
...
export function redactEvidenceTextV10(value, secrets = []) {
  let output = String(value ?? ""); discoverSecretsV10(output, secrets);
...
function stageName(value) {
  ...
  if (!stage) throw new Error("b2c-000197-v10-stage-required");
  return stage;
}

function serializeError(error, secrets) {
  if (!error) return null;
  return { name: redactEvidenceTextV10(error.name ?? "Error", secrets),
    message: redactEvidenceTextV10(error.message ?? String(error), secrets),
    code: error.code == null ? null : redactEvidenceTextV10(error.code, secrets) };
}

function assertSanitized(payload, secrets) {
  const encoded = JSON.stringify(payload);
  for (const secret of secrets) if (secret && encoded.includes(secret)) throw new Error("b2c-000197-v10-secret-leak");
  if (DATABASE_URL.test(encoded)) throw new Error("b2c-000197-v10-database-url-leak");
  DATABASE_URL.lastIndex = 0;
}

function environmentEnvelope(env, allowlist, secrets) {
  discoverSecretsV10(env, secrets);
  ...
  for (const entry of allowlist) {
    if (!entry || typeof entry.name !== "string" || !["value", "redacted"].includes(entry.persist)) {
      throw new Error("b2c-000197-v10-env-allowlist-malformed");
    }
    if (!(entry.name in env)) throw new Error(`b2c-000197-v10-env-allowlist-missing:${entry.name}`);
    ...
    } else recorded[entry.name] = redactEvidenceTextV10(value, secrets);
  }
  return { childEnv, recorded, redactedFields };
}

export class PhaseExecutionErrorV10 extends Error {
  constructor(stage, result, cause = null) {
    super(`b2c-000197-v10-phase-failed:${stage}:exit=${result?.status ?? "none"}:signal=${result?.signal ?? "none"}:${cause?.message ?? "child"}`);
    this.name = "PhaseExecutionErrorV10"; this.stage = stage; this.result = result; this.cause = cause;
  }
}

export class PreliminaryEvidenceRecorderV10 {
  constructor({ evidenceRoot, runId = V10_RUN_ID, spawn = spawnSync, writeFile = writeFileSync,
    mkdir = mkdirSync, now = () => new Date().toISOString(), secrets = [] }) {
    if (runId !== V10_RUN_ID) throw new Error("b2c-000197-v10-run-id-drift");
    ...
    if (realpathSync(this.root) !== this.root || !statSync(this.root).isDirectory()) throw new Error("v10-evidence-root");
  }

  writeImmutable(filename, payload, stage) {
    const path = resolve(this.root, filename);
    if (dirname(path) !== this.root || basename(path) !== filename) throw new Error(`v10-path-escape:${stage}`);
    discoverSecretsV10(payload, this.secrets);
    const sanitized = JSON.parse(redactEvidenceTextV10(JSON.stringify(payload), this.secrets));
    ...
    if ((observed.mode & 0o777) !== 0o444 || observed.size !== content.byteLength) throw new Error("v10-mode-size-drift");
  ...
  runChild({ stage, command, args = [], cwd, env = {}, envAllowlist = [], input = null,
    ...
    discoverSecretsV10({ command, args, env }, this.secrets);
    ...
    const intent = this.writeImmutable(`${prefix}-intent.json`, {
      schema_version: "b2c-000197-child-intent-v10", evidence_policy_version: V10_EVIDENCE_POLICY,
    ...
    discoverSecretsV10([stdout, stderr, child.error], this.secrets);
    const resultEntry = this.writeImmutable(`${prefix}-result.json`, {
      schema_version: "b2c-000197-child-result-v10", evidence_policy_version: V10_EVIDENCE_POLICY,
      formal_run_id: this.runId, sequence, stage: normalized, recorded_at_utc: this.now(),
    ...
    if (parser) {
      ...
      this.writeImmutable(`${prefix}-parse.json`, {
        schema_version: "b2c-000197-child-parse-v10", evidence_policy_version: V10_EVIDENCE_POLICY,
        formal_run_id: this.runId, sequence, stage: normalized, recorded_at_utc: this.now(),
    ...
    if (!allowFailure && (child.error || child.status !== 0 || child.signal != null)) throw new PhaseExecutionErrorV10(normalized, result);
    if (parseError && !allowParseFailure) throw new PhaseExecutionErrorV10(normalized, result, parseError);
    return result;
  }

  writeTerminal(kind, payload) {
    if (!["success", "failure"].includes(kind)) throw new Error("b2c-000197-v10-terminal-kind");
    discoverSecretsV10(payload, this.secrets);
    const artifact = this.writeImmutable(`${kind}-${this.runId}.json`, {
      schema_version: `b2c-000197-preliminary-${kind}-v10`, evidence_policy_version: V10_EVIDENCE_POLICY,
    ...
    const manifest = this.writeImmutable(`${kind}-${this.runId}.manifest.json`, {
      schema_version: `b2c-000197-preliminary-${kind}-manifest-v10`, evidence_policy_version: V10_EVIDENCE_POLICY,
...
function phaseFailure(stage, result) {
  if (result.error || result.status !== 0 || result.signal != null) return new PhaseExecutionErrorV10(stage, result);
  if (result.parseError) return new PhaseExecutionErrorV10(stage, result, result.parseError);
  return null;
}

export function runPhasedGateV10(recorder, { phases, cleanupPhases }) {
...
export function executeWithEvidenceV10({ evidenceRoot, operation, recorderOptions = {}, successPayload = {} }) {
  const recorder = new PreliminaryEvidenceRecorderV10({ evidenceRoot, ...recorderOptions });
  try {
...
export function parseTapSummaryV10(stdout, { expectedTests, expectedNames = [] }) {
  const count = (label) => { const matches = [...stdout.matchAll(new RegExp(`^# ${label} (\\d+)$`, "gmu"))];
    if (matches.length !== 1) throw new Error(`v10-tap-${label}`); return Number(matches[0][1]); };
  ...
  if (result.tests !== expectedTests || result.suites !== 0 || result.pass !== expectedTests || result.fail !== 0
      || result.cancelled !== 0 || result.skipped !== 0 || result.todo !== 0) throw new Error("v10-tap-count-drift");
  ...
  if (expectedNames.length && (actualNames.length !== expectedNames.length
      || actualNames.some((name, index) => name !== expectedNames[index]))) throw new Error("v10-tap-name-drift");
