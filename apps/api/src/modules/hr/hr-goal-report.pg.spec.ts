...
test("draft submit concurrent review resubmit and confirmation are atomic and immutable",{skip:!enabled},async()=>{
 const self=actor(employeeUser,selfPermissions),manager=actor(managerUser,parkPermissions);const draft=await service.createDraft(scope,self,{reportType:"daily",periodStart:"2026-08-25",periodEnd:"2026-08-25",completedWork:"完成阶段任务",hours:8,goalSuggestions:[{goalId:employeeGoal,proposedProgress:.9,suggestionSummary:"建议主管确认"}]}) as {id:string;status:string};assert.equal(draft.status,"draft");
 ...
 let row=(await db.query(`SELECT status FROM hr_work_report WHERE id=$1`,[draft.id]))[0];if(row.status==="returned"){await service.updateDraft(scope,self,draft.id,{reportType:"daily",periodStart:"2026-08-25",periodEnd:"2026-08-25",completedWork:"完成阶段任务并补证",hours:8,goalSuggestions:[{goalId:employeeGoal,proposedProgress:.95,suggestionSummary:"补证后建议"}]});await service.submit(scope,self,draft.id);await service.review(scope,manager,draft.id,{action:"confirmed"});}
