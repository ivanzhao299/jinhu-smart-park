...
AND (
  ...
  OR (tenant.contact_user_id is null AND user.create_by is null AND user.remark == "bootstrap-admin created")
)
```

The pointer is authoritative for tenants created through `TenantsService.create`. The exact remark plus null creator fallback is restricted to pointer-less tenants produced by the reviewed production bootstrap script; normal API-created users retain a creator even if their remark is later changed. Requiring an active current-park `TENANT_ADMIN` role prevents a stale pointer/remark from preserving identity after role removal and keeps park switching scoped to the target context.
