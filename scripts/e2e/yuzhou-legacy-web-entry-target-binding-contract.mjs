...
test("all 15 Yuzhou Web entries have a mapped target or an explicit gap",()=>{
  assert.deepEqual(verifyLegacyWebEntryTargetBinding(manifest,{root}),{ok:true,entries:15,mapped:15,gaps:0,roleMatrixVerified:false,compatibilityScoreContribution:0,productionImport:"HOLD"});
});
...
test("fabricated gaps and summary drift fail closed",()=>{
  const invented=clone(manifest);invented.entries[6].status="gap";invented.entries[6].reasonCode="TARGET_SELF_SERVICE_PASSWORD_CHANGE_NOT_IMPLEMENTED";invented.entries[6].targetEvidence=[];
