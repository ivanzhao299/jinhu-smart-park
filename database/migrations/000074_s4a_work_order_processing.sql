...
permissions(code, name, resource, action, api_path) AS (
  VALUES
    ('workorder:accept', '工单接单', 'biz.work_order', 'accept', '/api/v1/work-orders/:id/accept'),
    ('workorder:start', '开始处理工单', 'biz.work_order', 'start', '/api/v1/work-orders/:id/start'),
    ('workorder:wait_material', '工单待物料', 'biz.work_order', 'wait_material', '/api/v1/work-orders/:id/wait-material'),
    ('workorder:finish', '完成处理工单', 'biz.work_order', 'finish', '/api/v1/work-orders/:id/finish'),
    ('workorder:manage_all', '管理全部工单', 'biz.work_order', 'manage_all', '/api/v1/work-orders/*')
)
...
role_permissions(role_code, permission_code) AS (
  VALUES
    ...
    ('PROPERTY_MANAGER', 'workorder:accept'),
    ('PROPERTY_MANAGER', 'workorder:start'),
    ('PROPERTY_MANAGER', 'workorder:wait_material'),
    ('PROPERTY_MANAGER', 'workorder:finish'),
