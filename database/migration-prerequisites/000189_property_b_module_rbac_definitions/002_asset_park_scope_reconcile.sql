...
-- and 000200 from the canonical active biz_park scope.
--
-- This prerequisite is intentionally insert-only. Existing asset_park rows are
-- never re-enabled or overwritten; ambiguous or invalid active scopes fail
-- closed before the target migration runs.
...
BEGIN
  ...
  IF invalid_count <> 0 THEN
    RAISE EXCEPTION 'property-asset-park-scope-reconcile-preflight-failed'
      USING ERRCODE = '23514';
  END IF;
END;
$$;
...
BEGIN
  ...
  IF invalid_count <> 0 THEN
    RAISE EXCEPTION 'property-asset-park-scope-reconcile-postcondition-failed'
      USING ERRCODE = '23514';
  END IF;
END;
$$;
