...
- Bound technical handoff: `../handoff/2026-08-14-technical-closure-to-human-gate.md`，并由 PR #331 民宿闭环证据增量更新
- Current technical candidate: `619b8d20e891c74f69abcc9c908666034c37c648`（PR #331 merge 后 main）
- Kit provenance commit: `1f0dcafb95fbde57a1a8c7f6349393ca0b880691`（PR #290 merge）
- Current candidate evidence: CI run `32340203702` success；Deploy Production run `32340203683` success；health、Docker cleanup 与公开保护账号校验通过
- H0 frozen candidate: pending external coordinator acceptance of `619b8d20e891c74f69abcc9c908666034c37c648`；冻结前不得开始真人观察或宣称 `production_ready`。
...
| #271 / PR276 | 高风险动作开放与 fail-closed：民宿取消、退款、减免；住房审批、作废、退租、采购付款/转收费、退款、押金退还；字段权限/敏感字段掩码 | API / Permissions / Audit / Web | `../handoff/2026-08-14-technical-closure-to-human-gate.md` 中 #271；Codex Review follow-up；字段策略/高风险动作测试证据 | `HOMESTAY-FINANCE-*`、`HOMESTAY-FRONTDESK-*`、`HOUSING-APPROVER-*`、`FINANCE-APPROVER-*`、`PURCHASE-REQUESTER-*`、`PURCHASE-APPROVER-*`、`PAYMENT-STAFF-*`、`AUDITOR-03` | `rollout_ops.high_risk_feature_flag_*`、`rollout_ops.high_risk_production_enforce_approved`、`signoffs.finance`、`signoffs.security_privacy`、`release_owner_go.explicit_go_for_candidate_sha` | covered |
| #271 / #289 闭环范围 | 高风险动作逐项判定 ready / fail-closed / out-of-scope：已开放动作具备审批、权限、幂等、审计和终态约束；未满足合同的动作不得通过前端、API 或配置绕开 | API / Web / Release | Issue `#289` 由 PR `#331` 完成并关闭；最终 reviewed head `8012b854`、main `619b8d20`；main CI/Release Smoke、生产健康/清理、公开保护账号校验通过。PR #332 复审后新增 Issue `#333`：周转详情/操作的负责人范围与关联工单权威校验须在独立代码 PR 完成后重新绑定候选和技术证据。 | 现有四张民宿任务卡尚未覆盖凭证遗失、guest candidate 边界、退房后 charge 拒绝和周转关联工单；在 #333 修复、相应版本化任务卡和 H0 冻结完成前，不能将该增量标为真人覆盖。发现 fail-closed 入口可见或可执行仍为 P0/P1 stopship。 | `human_uat.no_open_stopship_defects`、`computed_contract.required_true_fields_for_ready_for_production_deploy`、`computed_contract.required_true_fields_for_production_ready`、`rollout_ops.all_high_risk_flags_match_freeze` | follow_up_fail_closed |
...
| PR283 | 新租户/新园区编码规则初始化、物业菜单授权、多园区资产 UI 刷新；影响住宿/民宿 H0 scope、资产房源创建、园区可达性和角色菜单验证 | Database / Seed / API / Web / RBAC | PR `#283` merge commit `7c5db355ee8e8903dfc63101cfc646eaea2821d8`；main candidate `5987b14526443ee638aa27134485b12b681ef5b8`；CI/deploy run 绑定 | `PARK-ADMIN-*`、`SHARED-PROPERTY-*`、`HOUSING-LEASING-*`、`HOMESTAY-FRONTDESK-*` 的 scope/menu/资产入口验证 | `scope_coverage.*`、`human_uat.accepted_h0_profile_checksum`、`human_uat.all_observation_rows_match_h0_profile_checksum`、`signoffs.business_shared_property`、`signoffs.technical_operations` | covered |
| PR285 | 技术移交：把 #270-#273/#284 的证据转为 PR192 真人 UAT 前置输入；形成合并前技术 handoff 基线 `5987b14526443ee638aa27134485b12b681ef5b8` | Docs / UAT | `../handoff/2026-08-14-technical-closure-to-human-gate.md` | 本 UAT 包的 H0 输入来源 | `human_uat.accepted_h0_handoff_*`、`human_uat.all_observation_rows_match_accepted_h0_handoff` | covered |
| PR286 | 真人 UAT 执行包与生产就绪门禁模板合并；把范围矩阵、任务卡、ledger hash、阈值、H0 handoff 和 readiness AND 门纳入 main | Docs / UAT / Release | PR `#286` merge commit `717d264275df7ba81b031413c211a4125ca432a5`；PR CI run `31781954682` success；main CI run `31782716720` success；Deploy Production run `31782716535` success；生产 `/health`、`/ready`、`/login` 探针 200 | 作为外部 H0/H1/H4/H5 模板；不计入真人样本；当前执行须使用 PR290 kit provenance 并把 H0 candidate 重新冻结到顶层 `619b8d20` | `kit_provenance_commit`、`technical.main_ci.head_sha_matches_candidate_sha`、`technical.production_deploy.deployed_sha_matches_candidate_sha`、`human_uat.accepted_h0_build_sha_matches_candidate_sha` | covered |
| PR290 | PR286 合并后的证据收口：保留 PR285/PR286 历史证据，新增 kit content manifest，并要求 H0 candidate/kit provenance 绑定实际 main 部署 SHA | Docs / UAT / Release | PR `#290` merge commit / kit provenance `1f0dcafb95fbde57a1a8c7f6349393ca0b880691`；当前 technical candidate 因后续 PR #331 更新为 `619b8d20e891c74f69abcc9c908666034c37c648`，其 main CI run `32340203702` 与 Deploy Production run `32340203683` 均成功 | 作为外部 H0/H1/H4/H5 模板修订，不计入真人样本；外部 coordinator 必须接受并冻结 `619b8d20` 后才可写入 observation/signoff ledgers | `scope_coverage.candidate_sha_matches_top_level_candidate`、`technical.main_ci.head_sha_matches_candidate_sha`、`technical.production_deploy.deployed_sha_matches_candidate_sha`、`human_uat.accepted_h0_build_sha_matches_candidate_sha` | covered |
...
| 住房交割/维修/附件 | 入住交割、退租财务交割、维修报修/处理、现场图片、文件权限和移动端 | API / Web / Files / Mobile | #271/#272/#273 技术证据 | `HOUSING-HANDOVER-*`、`REPAIR-STAFF-*`、`AUDITOR-03` | `human_uat.all_phone_required_tasks_observed_on_phone_390px_class`、`cleanup.residual_files`、`signoffs.security_privacy` | covered |
| 住房账单/财务 | 账单生成、重复生成冲突、收款、退款、减免、押金收退、金额上限、来源不可变 | API / DB / Web / Finance | #271/#272 技术证据；幂等/冲突/金额精度 E2E | `HOUSING-BILLING-*`、`CASHIER-*`、`FINANCE-APPROVER-*` | `signoffs.finance`、`human_uat.no_open_stopship_defects`、`rollout_ops.monitoring_verified_for_candidate_and_window` | covered |
| 住房采购 | 采购申请、审批、付款、退款/作废、转租客收费、已转收费后终态约束 | API / DB / Web / Approval | #271/#272 技术证据；purchase lifecycle/transfer 回归 | `PURCHASE-REQUESTER-*`、`PURCHASE-APPROVER-*`、`PAYMENT-STAFF-*`、`FINANCE-APPROVER-*`、`AUDITOR-*` | `signoffs.finance`、`signoffs.business_housing`、`rollout_ops.all_high_risk_flags_match_freeze` | covered |
