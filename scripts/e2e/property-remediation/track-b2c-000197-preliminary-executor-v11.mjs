...
export const V11_STATIC_EVIDENCE_ID = "b2c-000197-v11-static-candidate-unbound";
export const V11_RUN_ID = "b2c197_prelim_20260802f";
export const V11_EVIDENCE_POLICY = "b2c-000197-child-evidence-v11";
...
export function discoverSecretsV11(value, target = [], key = "") {
  if (value == null) return target;
  if (Buffer.isBuffer(value)) return discoverSecretsV11(value.toString("utf8"), target, key);
  if (Array.isArray(value)) {
    for (const entry of value) discoverSecretsV11(entry, target, ARG_CONTAINER.test(key) ? "" : key);
    return target;
  }
  if (typeof value === "object") {
    for (const [entryKey, entryValue] of Object.entries(value)) discoverSecretsV11(entryValue, target, entryKey);
    return target;
  }
  const text = String(value);
  if (SECRET_KEY.test(key) && !ARG_CONTAINER.test(key)) addSecret(target, text);
  for (const found of text.matchAll(DATABASE_URL)) {
    addSecret(target, found[0]);
    for (const userinfo of found[0].matchAll(URL_USERINFO)) {
      addSecret(target, userinfo[2]);
...
export function redactEvidenceTextV11(value, secrets = []) {
  let output = String(value ?? ""); discoverSecretsV11(output, secrets);
...
function stageName(value) {
  ...
  if (!stage) throw new Error("b2c-000197-v11-stage-required");
  return stage;
}

function serializeError(error, secrets) {
  if (!error) return null;
  return { name: redactEvidenceTextV11(error.name ?? "Error", secrets).slice(0, 100),
    message: redactEvidenceTextV11(error.message ?? String(error), secrets).slice(0, 500),
    code: error.code == null ? null : redactEvidenceTextV11(error.code, secrets).slice(0, 100) };
}

function assertSanitized(payload, secrets) {
  const encoded = JSON.stringify(payload);
  for (const secret of secrets) if (secret && encoded.includes(secret)) throw new Error("b2c-000197-v11-secret-leak");
  if (DATABASE_URL.test(encoded)) throw new Error("b2c-000197-v11-database-url-leak");
  DATABASE_URL.lastIndex = 0;
}

function environmentEnvelope(env, allowlist, secrets) {
  discoverSecretsV11(env, secrets);
  ...
  for (const entry of allowlist) {
    if (!entry || typeof entry.name !== "string" || !["value", "redacted"].includes(entry.persist)) {
      throw new Error("b2c-000197-v11-env-allowlist-malformed");
    }
    if (!(entry.name in env)) throw new Error(`b2c-000197-v11-env-allowlist-missing:${entry.name}`);
    ...
    } else recorded[entry.name] = redactEvidenceTextV11(value, secrets);
  }
  return { childEnv, recorded, redactedFields };
}

export class PhaseExecutionErrorV11 extends Error {
  constructor(stage, result, cause = null) {
    super(`b2c-000197-v11-phase-failed:${stage}:exit=${result?.status ?? "none"}:signal=${result?.signal ?? "none"}:${cause?.message ?? "child"}`);
    this.name = "PhaseExecutionErrorV11"; this.stage = stage; this.result = result; this.cause = cause;
  }
}

export class PreliminaryEvidenceRecorderV11 {
  constructor({ evidenceRoot, runId = V11_STATIC_EVIDENCE_ID, spawn = spawnSync, writeFile = writeFileSync,
    mkdir = mkdirSync, now = () => new Date().toISOString(), secrets = [] }) {
    if (![V11_STATIC_EVIDENCE_ID, V11_RUN_ID].includes(runId)) throw new Error("b2c-000197-v11-evidence-id-drift");
    ...
    if (realpathSync(this.root) !== this.root || !statSync(this.root).isDirectory()) throw new Error("v11-evidence-root");
  }

  writeImmutable(filename, payload, stage) {
    const path = resolve(this.root, filename);
    if (dirname(path) !== this.root || basename(path) !== filename) throw new Error(`v11-path-escape:${stage}`);
    discoverSecretsV11(payload, this.secrets);
    const sanitized = JSON.parse(redactEvidenceTextV11(JSON.stringify(payload), this.secrets));
    ...
    if ((observed.mode & 0o777) !== 0o444 || observed.size !== content.byteLength) throw new Error("v11-mode-size-drift");
  ...
  runChild({ stage, command, args = [], cwd, env = {}, envAllowlist = [], input = null,
    ...
    discoverSecretsV11({ command, args, env }, this.secrets);
    ...
    const intent = this.writeImmutable(`${prefix}-intent.json`, {
      schema_version: "b2c-000197-child-intent-v11", evidence_policy_version: V11_EVIDENCE_POLICY,
    ...
    let child;
    const spawnOptions = { cwd, env: environment.childEnv, encoding: null, maxBuffer: 64 * 1024 * 1024 };
    if (input != null) spawnOptions.input = input;
    try { child = this.spawn(command, args, spawnOptions); }
    ...
    discoverSecretsV11([stdout, stderr, child.error], this.secrets);
    const resultEntry = this.writeImmutable(`${prefix}-result.json`, {
      schema_version: "b2c-000197-child-result-v11", evidence_policy_version: V11_EVIDENCE_POLICY,
      formal_run_id: this.runId, sequence, stage: normalized, recorded_at_utc: this.now(),
    ...
    if (parser) {
      ...
      this.writeImmutable(`${prefix}-parse.json`, {
        schema_version: "b2c-000197-child-parse-v11", evidence_policy_version: V11_EVIDENCE_POLICY,
        formal_run_id: this.runId, sequence, stage: normalized, recorded_at_utc: this.now(),
    ...
    if (!allowFailure && (child.error || child.status !== 0 || child.signal != null)) throw new PhaseExecutionErrorV11(normalized, result);
    if (parseError && !allowParseFailure) throw new PhaseExecutionErrorV11(normalized, result, parseError);
    return result;
  }

  writeTerminal(kind, payload) {
    if (!["success", "failure"].includes(kind)) throw new Error("b2c-000197-v11-terminal-kind");
    discoverSecretsV11(payload, this.secrets);
    const artifact = this.writeImmutable(`${kind}-${this.runId}.json`, {
      schema_version: `b2c-000197-preliminary-${kind}-v11`, evidence_policy_version: V11_EVIDENCE_POLICY,
    ...
    const manifest = this.writeImmutable(`${kind}-${this.runId}.manifest.json`, {
      schema_version: `b2c-000197-preliminary-${kind}-manifest-v11`, evidence_policy_version: V11_EVIDENCE_POLICY,
...
function phaseFailure(stage, result) {
  if (result.error || result.status !== 0 || result.signal != null) return new PhaseExecutionErrorV11(stage, result);
  if (result.parseError) return new PhaseExecutionErrorV11(stage, result, result.parseError);
  return null;
}

export function runPhasedGateV11(recorder, { phases, cleanupPhases }) {
...
export function executeWithEvidenceV11({ evidenceRoot, operation, recorderOptions = {}, successPayload = {} }) {
  const recorder = new PreliminaryEvidenceRecorderV11({ evidenceRoot, ...recorderOptions });
  try {
    ...
    const childFailureSummary = error?.childSummary == null ? null : {
      stage: stageName(error.childSummary.stage ?? error?.stage ?? "top-level"),
      target: error.childSummary.target ?? null,
      boundary: error.childSummary.boundary ?? null,
      sqlstate: error.childSummary.sqlstate ?? null,
      expected_marker: error.childSummary.expected_marker ?? null,
      observed_markers: Array.isArray(error.childSummary.observed_markers)
        ? error.childSummary.observed_markers : [],
      exit_code: error.childSummary.exit_code ?? null,
      signal: error.childSummary.signal ?? null,
      sqlstate_valid: error.childSummary.sqlstate_valid === true,
      marker_valid: error.childSummary.marker_valid === true,
      child_valid: error.childSummary.child_valid === true,
      snapshot_checked: error.childSummary.snapshot_checked === true,
      snapshot_exact: error.childSummary.snapshot_checked === true
        ? error.childSummary.snapshot_exact === true : null,
    };
    recorder.writeTerminal("failure", { failure_stage: stageName(error?.stage ?? "top-level"),
      child_failure_summary: childFailureSummary,
      error: serializeError(error, recorder.secrets), cause: serializeError(error?.cause, recorder.secrets),
      run_id_reusable: false });
    throw error;
  }
}

export function parseTapSummaryV11(stdout, { expectedTests, expectedNames = [] }) {
  const count = (label) => { const matches = [...stdout.matchAll(new RegExp(`^# ${label} (\\d+)$`, "gmu"))];
    if (matches.length !== 1) throw new Error(`v11-tap-${label}`); return Number(matches[0][1]); };
  ...
  if (result.tests !== expectedTests || result.suites !== 0 || result.pass !== expectedTests || result.fail !== 0
      || result.cancelled !== 0 || result.skipped !== 0 || result.todo !== 0) throw new Error("v11-tap-count-drift");
  ...
  if (expectedNames.length && (actualNames.length !== expectedNames.length
      || actualNames.some((name, index) => name !== expectedNames[index]))) throw new Error("v11-tap-name-drift");
