...
本阶段只做 ST-2B-0 门禁确认和隔离数据集准备设计，不生成、不提交 `scripts/e2e/snapshots/first-release-api-snapshots.numeric.json`。

## 2. 背景

默认 `workorders.stats` schema snapshot 已稳定。默认快照不保存具体 numeric count，可避免写入型 e2e 后 stats 计数波动导致普通快照失败。

numeric 模式脚本能力已存在，并已具备防误用保护。numeric 模式可以显式启用，使用独立 baseline path，但 numeric baseline 尚未建立。
...
当前只完成门禁确认，不生成 numeric baseline。

下一步才是 ST-2B-1：在隔离固定数据集下生成 numeric baseline，并按本文规则单独审查、单独提交。
