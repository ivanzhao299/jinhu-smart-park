...
test("workforce decision snapshot is one scoped aggregate without employee identifiers",async()=>{
 const fixture=[{employee_total:8,active_headcount:6,byStatus:[{status:"active",count:6},{status:"departed",count:2}],byType:[{type:"full_time",count:7},{type:"part_time",count:1}],total:5,employee_count:4,historical_count:3,online_count:2,eventByType:[{eventType:"transfer",count:5}],eventByMonth:[{month:"2026-08",count:5}]}];
 const {calls,run}=service(fixture);
 assert.deepEqual(await run({from:"2026-01-01",to:"2026-12-31"}),{from:"2026-01-01",to:"2026-12-31",employeeTotal:8,activeHeadcount:6,byStatus:fixture[0]!.byStatus,byType:fixture[0]!.byType,employmentEvents:{total:5,employeeCount:4,historicalCount:3,onlineCount:2,byType:fixture[0]!.eventByType,byMonth:fixture[0]!.eventByMonth}});
 assert.equal(calls.length,1);assert.deepEqual(calls[0]!.params,[scope.tenantId,scope.parkId,"2026-01-01","2026-12-31"]);
 assert.match(calls[0]!.sql,/tenant_id=\$1 AND park_id=\$2 AND is_deleted=false/);assert.match(calls[0]!.sql,/migration_decision='accepted'/);assert.match(calls[0]!.sql,/count\(DISTINCT employee_id\)/);
 assert.doesNotMatch(calls[0]!.sql,/full_name|employee_code|work_mobile|work_email|reason|before_snapshot|after_snapshot/);
});
