...
export const ATTEMPT_ID = "b2c197_prelim_20260802f_loader_recovery_attempt04";
...
export const AUTHORITY_PATH = resolve(researchRoot,
  "b2c-000197-v11-ef-loader-recovery-authority-v4-20260802f.grammar");
export const DATABASE_GO_PATH = resolve(researchRoot,
  "b2c-000197-v11-ef-loader-recovery-independent-database-go-v4-20260802f.grammar");
export const QA_GO_PATH = resolve(researchRoot,
  "b2c-000197-v11-ef-loader-recovery-independent-qa-go-v4-20260802f.grammar");
export const TEST_RESULT_PATH = resolve(researchRoot,
  "b2c-000197-v11-ef-loader-recovery-test-result-v4-20260802f.grammar");
...
const AUTHORITY_KEYS = Object.freeze(["formal_run_id", "attempt_id", "execution_authorized",
  ...
  "database_go_path", "qa_go_path", "test_result_path", "test_result_raw_sha256",
  "v3_database_no_go_audit_raw_sha256", "v3_qa_no_go_audit_raw_sha256"]);
const DATABASE_GO_KEYS = Object.freeze(["formal_run_id", "attempt_id", "decision",
  "execution_authorized", "formal_go", "authority_raw_sha256", "runner_raw_sha256", "candidate_raw_sha256",
...
export function redactEvidenceTextV4(value) {
  return String(value ?? "").replace(URL_SECRET, "<redacted-database-url>")
    .replace(KEY_VALUE_SECRET, (_, key) => `${key}=<redacted>`);
}

export function parseStrictGrammarV4(value, { header, keys }) {
  const lines = String(value).split(/\r?\n/u);
  if (lines.at(-1) === "") lines.pop();
  if (lines.shift() !== header) throw new Error("b2c-000197-recovery-v4-grammar-header-drift");
  const allowed = new Set(keys);
  const parsed = new Map();
  for (const line of lines) {
    const separator = line.indexOf("\t");
    if (separator <= 0) throw new Error("b2c-000197-recovery-v4-grammar-line-malformed");
    const key = line.slice(0, separator);
    if (!allowed.has(key)) throw new Error(`b2c-000197-recovery-v4-grammar-unknown-key:${key}`);
    if (parsed.has(key)) throw new Error(`b2c-000197-recovery-v4-grammar-duplicate-key:${key}`);
    parsed.set(key, line.slice(separator + 1));
  }
  for (const key of keys) {
    if (!parsed.has(key)) throw new Error(`b2c-000197-recovery-v4-grammar-missing-key:${key}`);
  }
  return parsed;
}

function requireValue(map, key, expected, scope) {
  if (map.get(key) !== expected) throw new Error(`b2c-000197-recovery-v4-${scope}-drift:${key}`);
}

export function verifyAuthorizationEnvelopeV4({ authorityBytes, databaseGoBytes, qaGoBytes,
  runnerSha }) {
  const authoritySha = sha256(authorityBytes);
  const databaseGoSha = sha256(databaseGoBytes);
  const authority = parseStrictGrammarV4(authorityBytes, {
    header: "b2c-000197-v11-ef-loader-recovery-authority-v4-machine", keys: AUTHORITY_KEYS,
  });
  const databaseGo = parseStrictGrammarV4(databaseGoBytes, {
    header: "b2c-000197-v11-ef-loader-recovery-independent-database-go-v4",
    keys: DATABASE_GO_KEYS,
  });
  const qaGo = parseStrictGrammarV4(qaGoBytes, {
    header: "b2c-000197-v11-ef-loader-recovery-independent-qa-go-v4", keys: QA_GO_KEYS,
  });
  ...
  if (!/^[a-f0-9]{64}$/u.test(preflightSha)) {
    throw new Error("b2c-000197-recovery-v4-authority-drift:preflight_raw_sha256");
  }
  requireValue(authority, "database_go_path", DATABASE_GO_PATH, "authority");
  requireValue(authority, "qa_go_path", QA_GO_PATH, "authority");
  requireValue(authority, "test_result_path", TEST_RESULT_PATH, "authority");
  if (!/^[a-f0-9]{64}$/u.test(authority.get("test_result_raw_sha256"))) {
    throw new Error("b2c-000197-recovery-v4-authority-drift:test_result_raw_sha256");
  }
  requireValue(authority, "v3_database_no_go_audit_raw_sha256",
    "b2cc91297a88829d622142063ccb1b3420b245495dd02995a56311d7a3fcf6f5", "authority");
  requireValue(authority, "v3_qa_no_go_audit_raw_sha256",
    "c1b97284fc326e22731717dbe5edace52e78eae5e8645a74d80686362597d864", "authority");
  for (const [map, scope] of [[databaseGo, "database-go"], [qaGo, "qa-go"]]) {
    ...
    requireValue(map, "qa_go_path", QA_GO_PATH, scope);
    requireValue(map, "qa_go_schema",
      "b2c-000197-v11-ef-loader-recovery-independent-qa-go-v4", scope);
  }
...
export function atomicClaimEvidenceRootV4(evidenceRoot = EVIDENCE_ROOT,
  ...
  if (dependencies.realpathSync(evidenceRoot) !== evidenceRoot
    ...
    throw new Error("b2c-000197-recovery-v4-attempt-claim-identity-drift");
  }
  return evidenceRoot;
}

export class RecoveryEvidenceRecorderV4 {
  ...
  write(filename, payload) {
    ...
    if (dirname(path) !== this.root) throw new Error("b2c-000197-recovery-v4-evidence-path-escape");
    ...
    if (URL_SECRET.test(encoded) || KEY_VALUE_SECRET.test(encoded)) {
      ...
      throw new Error("b2c-000197-recovery-v4-evidence-secret-leak");
    }
    ...
    if ((observed.mode & 0o777) !== 0o444 || observed.size !== content.byteLength) {
      throw new Error("b2c-000197-recovery-v4-evidence-mode-or-size-drift");
    }
  ...
  child(stage, command, args, options, spawn = spawnSync, now = () => new Date().toISOString()) {
    ...
    const intent = this.write(`${prefix}-intent.json`, {
      schema_version: "b2c-000197-v11-ef-loader-recovery-child-intent-v4",
    ...
    const result = this.write(`${prefix}-result.json`, {
      schema_version: "b2c-000197-v11-ef-loader-recovery-child-result-v4",
      formal_run_id: FORMAL_RUN_ID, attempt_id: ATTEMPT_ID, sequence, stage,
      recorded_at_utc: now(), intent_raw_sha256: intent.raw_sha256,
      exit_code: child.status ?? null, signal: child.signal ?? null,
      spawn_error: child.error ? redactEvidenceTextV4(child.error.message ?? String(child.error)) : null,
      stdout: { bytes: stdout.byteLength, raw_sha256: sha256(stdout),
        redacted_utf8: redactEvidenceTextV4(stdout.toString("utf8")) },
      stderr: { bytes: stderr.byteLength, raw_sha256: sha256(stderr),
        redacted_utf8: redactEvidenceTextV4(stderr.toString("utf8")) },
    });
    return { ...child, stdout, stderr, result };
  }

  terminal(kind, payload) {
    if (this.terminalWritten) throw new Error("b2c-000197-recovery-v4-terminal-already-written");
    this.terminalWritten = true;
    const artifact = this.write(`${kind}-${ATTEMPT_ID}.json`, {
      schema_version: "b2c-000197-v11-ef-loader-recovery-terminal-v4",
    ...
    return this.write(`${kind}-${ATTEMPT_ID}.manifest.json`, {
      schema_version: "b2c-000197-v11-ef-loader-recovery-terminal-manifest-v4",
      formal_run_id: FORMAL_RUN_ID, attempt_id: ATTEMPT_ID,
...
function requireChildSuccess(child, stage) {
  if (child.error || child.status !== 0 || child.signal != null) {
    throw new Error(`b2c-000197-recovery-v4-child-failed:${stage}`);
  }
  return child.stdout.toString("utf8").trim();
}

export function validateContainerInspectionV4(value, target) {
  ...
  if (JSON.stringify(Object.keys(parsed).sort()) !== JSON.stringify(expectedKeys)) {
    throw new Error(`b2c-000197-recovery-v4-container-shape-drift:${target.key}`);
  }
  ...
  if (parsed.id !== target.containerId || parsed.image !== IMAGE_ID || parsed.name !== `/${target.container}`
    ...
    throw new Error(`b2c-000197-recovery-v4-container-identity-drift:${target.key}`);
  }
  return parsed;
}

export function validateEmptyDatabasePreflightV4(value, target) {
  const parsed = JSON.parse(value);
  if (parsed.database !== target.database || parsed.server_version !== "16.14"
    ...
    throw new Error(`b2c-000197-recovery-v4-database-preflight-drift:${target.key}`);
  }
  return parsed;
}

export function parseLoaderSuccessV4(value) {
  const trimmed = String(value).trim();
  if (!trimmed) throw new Error("b2c-000197-recovery-v4-loader-json-empty");
  let parsed;
  try { parsed = JSON.parse(trimmed); } catch {
    throw new Error("b2c-000197-recovery-v4-loader-json-malformed-or-extra");
  }
  if (JSON.stringify(Object.keys(parsed).sort()) !== JSON.stringify(["run_id", "status", "targets"])
    ...
    throw new Error("b2c-000197-recovery-v4-loader-json-semantic-drift");
  }
  return { status: "SUCCESS", run_id: FORMAL_RUN_ID, attempt_id: ATTEMPT_ID,
    targets: { e: TARGETS[0], f: TARGETS[1] } };
}

export function validatePostLoadDatabaseV4(value, target) {
  const parsed = JSON.parse(value);
  if (parsed.database !== target.database || parsed.server_version !== "16.14"
    ...
    throw new Error(`b2c-000197-recovery-v4-postload-drift:${target.key}`);
  }
  return parsed;
}
...
export function executeClaimedAttemptV4({ candidateBytes, evidenceRoot, spawn = spawnSync,
  now = () => new Date().toISOString() }) {
  const recorder = new RecoveryEvidenceRecorderV4(evidenceRoot);
  try {
    ...
    if (requireChildSuccess(version, "docker-version") !== DOCKER_VERSION) {
      throw new Error("b2c-000197-recovery-v4-docker-version-drift");
    }
    for (const target of TARGETS) {
      const inspect = recorder.child(`preflight-inspect-${target.key}`, DOCKER_PATH,
        ["inspect", target.container, "--format", INSPECT_FORMAT],
        { cwd: root, env: MINIMAL_ENV }, spawn, now);
      validateContainerInspectionV4(requireChildSuccess(inspect, `preflight-inspect-${target.key}`), target);
      ...
      validateEmptyDatabasePreflightV4(
    ...
    const loaderResult = parseLoaderSuccessV4(requireChildSuccess(loader, "loader"));
    const postchecks = [];
    for (const target of TARGETS) {
      ...
      postchecks.push(validatePostLoadDatabaseV4(
    ...
    if (!recorder.terminalWritten) recorder.terminal("failure", {
      ...
      failure_reason: redactEvidenceTextV4(error?.message ?? String(error)),
    });
    throw error;
  }
}
...
function verifyExecutable(path, expectedRealpath, expectedSha, label) {
  ...
  if (realpathSync(path) !== expectedRealpath || sha256(readFileSync(path)) !== expectedSha) {
    throw new Error(`b2c-000197-recovery-v4-${label}-identity-drift`);
  }
}

export function verifyFrozenInputsV4() {
  if (process.execPath !== NODE_PATH || process.version !== NODE_VERSION) {
    throw new Error("b2c-000197-recovery-v4-node-runtime-drift");
  }
  ...
  if (candidateBytes.byteLength !== CANDIDATE_BYTES || sha256(candidateBytes) !== CANDIDATE_SHA256) {
    throw new Error("b2c-000197-recovery-v4-candidate-drift");
  }
  const runnerSha = sha256(readFileSync(SELF_PATH));
  const authorization = verifyAuthorizationEnvelopeV4({
...
function staticCandidate() {
  return { status: "blocked-awaiting-independent-v4-database-and-qa-go",
...
if (realpathSync(process.argv[1] ?? SELF_PATH) === realpathSync(SELF_PATH)) {
  if (process.env.B2C_000197_V11_EF_RECOVERY_V4_EXECUTE === "1") {
    try {
      const verified = verifyFrozenInputsV4();
      atomicClaimEvidenceRootV4();
      executeClaimedAttemptV4({ candidateBytes: verified.candidateBytes, evidenceRoot: EVIDENCE_ROOT });
    } catch (error) {
      process.stderr.write(`${redactEvidenceTextV4(error?.stack ?? error?.message ?? String(error))}\n`);
      process.exitCode = 1;
    }
  } else {
