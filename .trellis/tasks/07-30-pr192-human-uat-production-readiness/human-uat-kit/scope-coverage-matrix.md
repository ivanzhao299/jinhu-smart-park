...
Bound technical candidate: pending PR `#290` merge and production deployment.
...
| #271 / #289 闭环范围 | 高风险动作逐项判定 ready / fail-closed / out-of-scope：已开放动作具备审批、权限、幂等、审计和终态约束；未满足合同的动作不得通过前端、API 或配置绕开 | API / Web / Release | Issue `#289` 由 PR `#331` 完成并关闭；最终 reviewed head `8012b854`、main `619b8d20`；Codex 无重大问题；main CI/Release Smoke、生产健康/清理、公开保护账号校验通过；自动建维修工单在 #328/PR331 中明确为缺少跨模块事务/唯一键合同前的 out-of-scope fail-closed 项 | 观察中若发现 fail-closed 入口可见或可执行，仍记 P0/P1 stopship；真人 UAT 不改变 readiness 判定 | `human_uat.no_open_stopship_defects`、`computed_contract.required_true_fields_for_ready_for_production_deploy`、`computed_contract.required_true_fields_for_production_ready`、`rollout_ops.all_high_risk_flags_match_freeze` | covered |
...
| PR286 | 真人 UAT 执行包与生产就绪门禁模板合并；把范围矩阵、任务卡、ledger hash、阈值、H0 handoff 和 readiness AND 门纳入 main | Docs / UAT / Release | PR `#286` merge commit `717d264275df7ba81b031413c211a4125ca432a5`；PR CI run `31781954682` success；main CI run `31782716720` success；Deploy Production run `31782716535` success；生产 `/health`、`/ready`、`/login` 探针 200 | 作为外部 H0/H1/H4/H5 模板；不计入真人样本；PR290 merge 后必须改用 PR290 的 main/deploy SHA | `kit_provenance_commit`、`technical.main_ci.head_sha_matches_candidate_sha`、`technical.production_deploy.deployed_sha_matches_candidate_sha`、`human_uat.accepted_h0_build_sha_matches_candidate_sha` | covered |
| PR290 | PR286 合并后的证据收口：保留 PR285/PR286 历史证据，新增 kit content manifest，并要求 H0 candidate/kit provenance 绑定 PR290 merge 后的实际 main 部署 SHA | Docs / UAT / Release | 本 PR merge 后填写 `current_human_uat_candidate_sha`、`kit_provenance_commit`、main CI run、Deploy Production run 和生产健康探针；不得提前填 PR 分支 SHA 或旧部署 SHA | 作为外部 H0/H1/H4/H5 模板修订；不计入真人样本 | `scope_coverage.candidate_sha_matches_top_level_candidate`、`technical.main_ci.head_sha_matches_candidate_sha`、`technical.production_deploy.deployed_sha_matches_candidate_sha`、`human_uat.accepted_h0_build_sha_matches_candidate_sha` | covered |
