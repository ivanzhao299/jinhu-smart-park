...
test("decision center is dedicated, aggregate-only, mobile-safe, and wired to its protected API",()=>{
 ...
 assert.match(page,/当前编制供需推演/);assert.match(page,/不猜测招聘到岗时间、离职意向或未配置岗位需求。/);assert.match(page,/人力需求缺口/);assert.match(page,/内部供给基线/);assert.match(page,/待归位供给/);assert.match(page,/人力资本概览/);assert.match(page,/岗位编制与在岗/);assert.match(page,/仅统计已启用岗位；未设置编制人数不作为零。/);assert.match(page,/snapshot\.staffing\.activeUnassignedHeadcount/);assert.match(page,/snapshot\.staffing\.overCapacityPositionCount/);
