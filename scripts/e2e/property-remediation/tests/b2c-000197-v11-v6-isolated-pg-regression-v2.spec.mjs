...
import {
  V11_V6_PG_V2_ENV_KEYS,
  V11_V6_PG_V2_REVIEW_PATHS,
  V11_V6_PG_V2_REVIEW_SCHEMAS,
  assertIndependentGoV2,
  assertResourceAuthorityV2,
  executeIsolatedPgRegressionV2,
  isolatedPgRegressionCandidateV2,
  parseExactGrammarV2,
  runAuthorizedRegressionV2,
} from "../track-b2c-000197-v11-v6-isolated-pg-regression-v2.mjs";

const sha256 = (value) => createHash("sha256").update(value).digest("hex");
const fixtureRoot = mkdtempSync("/tmp/b2c197-v11v6-v2-");
...
const target = Object.freeze({ runId: "b2c197_v11v6_isolated_attempt02", container: "jinhu-b2c197-v11v6-isolated-attempt02",
  containerId: "a".repeat(64), database: "jinhu_b2c197_v11v6_isolated_db02", volume: "b".repeat(64),
...
const bindings = { run_id: target.runId, candidate_authority_raw_sha256: "1".repeat(64),
  ...
  migration_000197_raw_sha256: "8".repeat(64), baseline_path: baselinePath, baseline_raw_sha256: target.baselineSha,
  resource_authority_path: resolve(fixtureRoot, "resource.grammar"), resource_authority_raw_sha256: "9".repeat(64) };
const reviewFields = (kind) => ({ ...bindings,
  ...
  decision: "GO", execution_authorized: "true", formal_go: "false", open_p0: "0", open_p1: "0", open_p2: "0",
  ...(kind === "qa" ? { database_review_raw_sha256: "a".repeat(64) } : {}) });

test("v2 candidate freezes review paths and schemas while execution remains false", () => {
  const candidate = isolatedPgRegressionCandidateV2();
  ...
  assert.deepEqual(candidate.required_environment_keys, [...V11_V6_PG_V2_ENV_KEYS]);
  assert.deepEqual(candidate.fixed_review_paths, V11_V6_PG_V2_REVIEW_PATHS);
  assert.deepEqual(candidate.review_schemas, V11_V6_PG_V2_REVIEW_SCHEMAS);
});

test("v2 database GO and one-way QA GO accept only exact acyclic zero-finding fields", () => {
  const database = reviewFields("database");
  assert.equal(assertIndependentGoV2(grammar(V11_V6_PG_V2_REVIEW_SCHEMAS.database, database),
    "database", bindings).decision, "GO");
  const qaBindings = { ...bindings, database_review_raw_sha256: "a".repeat(64) };
  const qa = reviewFields("qa");
  assert.equal(assertIndependentGoV2(grammar(V11_V6_PG_V2_REVIEW_SCHEMAS.qa, qa), "qa", qaBindings)
    .database_review_raw_sha256, "a".repeat(64));
  assert.throws(() => assertIndependentGoV2(grammar(V11_V6_PG_V2_REVIEW_SCHEMAS.qa, qa), "qa",
...
test("v2 resource authority accepts only exact dedicated anonymous PG16 target", () => {
  const fields = { run_id: target.runId, container: target.container, container_id: target.containerId,
    ...
    host_port_bindings: "0", status: "READY-NOT-AUTHORIZED" };
  const exact = grammar(V11_V6_PG_V2_REVIEW_SCHEMAS.resource, fields);
  assert.deepEqual(assertResourceAuthorityV2(exact, target.runId), target);
  for (const invalid of [
    exact.replace(/^status\tREADY-NOT-AUTHORIZED\n/mu, ""),
    `${exact}unknown\tx\n`,
    `${exact}status\tREADY-NOT-AUTHORIZED\n`,
  ...
  ]) assert.throws(() => assertResourceAuthorityV2(invalid, target.runId), /v11-v6-pg-v2-resource/u);
});

test("v2 GO intake rejects missing, duplicate, unknown, role, decision, P-count and SHA drift", () => {
  const fields = reviewFields("database"); const schema = V11_V6_PG_V2_REVIEW_SCHEMAS.database;
  ...
  ]) assert.throws(() => assertIndependentGoV2(invalid, "database", bindings), /v11-v6-pg-v2/u);
  assert.throws(() => parseExactGrammarV2(exact, "wrong-schema", new Set(Object.keys(fields)), "database"), /schema/u);
});

test("missing dynamic authority stops before claim and first child command", () => {
  const fake = fakeRunner();
  assert.throws(() => executeIsolatedPgRegressionV2({ env: { B2C_000197_V11_V6_PG_V2_EXECUTE: "1" },
    runCommand: fake.runCommand }), /input-drift/u);
  assert.equal(fake.calls.length, 0);
});

test("authorized v2 fake run writes immutable all-child success evidence and keeps SQL off argv", () => {
  const fake = fakeRunner(); const evidenceRoot = resolve(fixtureRoot, "success-evidence");
  const output = runAuthorizedRegressionV2({ authority, evidenceRoot, runCommand: fake.runCommand });
...
test("failure is redacted, immutable, non-reusable and never retries or cleans up", () => {
  const fake = fakeRunner({ sqlstate: "42703", stderrSecret: true });
  const evidenceRoot = resolve(fixtureRoot, "failure-evidence");
  assert.throws(() => runAuthorizedRegressionV2({ authority, evidenceRoot, runCommand: fake.runCommand }), /fault-drift/u);
  ...
  assert.match(persisted, /<redacted-secret>/u); assert.match(persisted, /<redacted-database-url>/u);
  ...
  assert.throws(() => runAuthorizedRegressionV2({ authority, evidenceRoot, runCommand: fake.runCommand }), /EEXIST/u);
...
test("strict identity, mount, no-port and PostgreSQL 16 checks fail closed", () => {
  for (const [name, options, pattern] of [
    ...
    assert.throws(() => runAuthorizedRegressionV2({ authority, evidenceRoot, runCommand: fake.runCommand }), pattern);
