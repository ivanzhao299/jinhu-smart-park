...
import {
  V11_V6_PG_V4_ENV_KEYS,
  V11_V6_PG_V4_REVIEW_PATHS,
  V11_V6_PG_V4_REVIEW_SCHEMAS,
  assertIndependentGoV4,
  assertResourceAuthorityV4,
  executeIsolatedPgRegressionV4,
  isolatedPgRegressionCandidateV4,
  parseExactGrammarV4,
  redactV4,
  runAuthorizedRegressionV4,
} from "../track-b2c-000197-v11-v6-isolated-pg-regression-v4.mjs";

const sha256 = (value) => createHash("sha256").update(value).digest("hex");
const fixtureRoot = mkdtempSync("/tmp/b2c197-v11v6-v4-");
...
const target = Object.freeze({ runId: "b2c197_v11v6_isolated_attempt04", container: "jinhu-b2c197-v11v6-isolated-attempt04",
  containerId: "a".repeat(64), database: "jinhu_b2c197_v11v6_isolated_db04", volume: "b".repeat(64),
...
test("v4 candidate freezes review paths and schemas while execution remains false", () => {
  const candidate = isolatedPgRegressionCandidateV4();
  ...
  assert.deepEqual(candidate.required_environment_keys, [...V11_V6_PG_V4_ENV_KEYS]);
  assert.deepEqual(candidate.fixed_review_paths, V11_V6_PG_V4_REVIEW_PATHS);
  assert.deepEqual(candidate.review_schemas, V11_V6_PG_V4_REVIEW_SCHEMAS);
});

test("v4 database GO and one-way QA GO accept only exact acyclic zero-finding fields", () => {
  const database = reviewFields("database");
  assert.equal(assertIndependentGoV4(grammar(V11_V6_PG_V4_REVIEW_SCHEMAS.database, database),
    "database", bindings).decision, "GO");
  const qaBindings = { ...bindings, database_review_raw_sha256: "a".repeat(64) };
  const qa = reviewFields("qa");
  assert.equal(assertIndependentGoV4(grammar(V11_V6_PG_V4_REVIEW_SCHEMAS.qa, qa), "qa", qaBindings)
    .database_review_raw_sha256, "a".repeat(64));
  assert.throws(() => assertIndependentGoV4(grammar(V11_V6_PG_V4_REVIEW_SCHEMAS.qa, qa), "qa",
...
test("v4 resource authority accepts only exact dedicated anonymous PG16 target", () => {
  ...
  const exact = grammar(V11_V6_PG_V4_REVIEW_SCHEMAS.resource, fields);
  assert.deepEqual(assertResourceAuthorityV4(exact, target.runId, resourceBindings), target);
  ...
  ]) assert.throws(() => assertResourceAuthorityV4(invalid, target.runId, resourceBindings), /v11-v6-pg-v4-resource/u);
});

test("v4 GO intake rejects missing, duplicate, unknown, role, decision, P-count and SHA drift", () => {
  const fields = reviewFields("database"); const schema = V11_V6_PG_V4_REVIEW_SCHEMAS.database;
  ...
  ]) assert.throws(() => assertIndependentGoV4(invalid, "database", bindings), /v11-v6-pg-v4/u);
  assert.throws(() => parseExactGrammarV4(exact, "wrong-schema", new Set(Object.keys(fields)), "database"), /schema/u);
});

test("missing dynamic authority stops before claim and first child command", () => {
  const fake = fakeRunner();
  assert.throws(() => executeIsolatedPgRegressionV4({ env: { B2C_000197_V11_V6_PG_V4_EXECUTE: "1" },
    runCommand: fake.runCommand }), /input-drift/u);
  assert.equal(fake.calls.length, 0);
});

test("authorized v4 fake run writes immutable all-child success evidence and keeps SQL off argv", () => {
  const fake = fakeRunner(); const evidenceRoot = resolve(fixtureRoot, "success-evidence");
  const output = runAuthorizedRegressionV4({ authority, evidenceRoot, runCommand: fake.runCommand });
...
test("psql output is suppressed, immutable, non-reusable and never retries or cleans up", () => {
  ...
  assert.throws(() => runAuthorizedRegressionV4({ authority, evidenceRoot, runCommand: fake.runCommand }), /fault-drift/u);
  ...
  assert.throws(() => runAuthorizedRegressionV4({ authority, evidenceRoot, runCommand: fake.runCommand }), /EEXIST/u);
...
test("structured redaction covers SQL, JSON, environment, URL and option secrets while benign argv survives", () => {
  const secretText = "PASSWORD 'sql-secret' {\"token\":\"json-secret\"} PGPASSWORD=env-secret "
    + "--password cli-secret postgresql://user:url-secret@db/app\nauthorization: Bearer-secret";
  const redacted = redactV4(secretText);
  ...
  runAuthorizedRegressionV4({ authority, evidenceRoot, runCommand: fake.runCommand });
...
test("authorization redaction consumes complete schemes and credentials with zero sentinel residue", () => {
  const sentinels = ["SENTINEL_BEARER_7f91", "SENTINEL_BASIC_6a22", "SENTINEL_JSON_11cc",
    "SENTINEL_TAB_89de", "SENTINEL_REPEAT_a501", "SENTINEL_REPEAT_b602"];
  const input = `Authorization: Bearer   ${sentinels[0]}\nAUTHORIZATION   Basic  ${sentinels[1]}\n`
    + `{"authorization":"Bearer ${sentinels[2]}"}\nauthorization:\tBearer\t${sentinels[3]}\n`
    + `Authorization: Bearer ${sentinels[4]}\nAuthorization: Basic ${sentinels[5]}`;
  const output = redactV4(input);
  for (const sentinel of sentinels) assert.equal(output.includes(sentinel), false);
  assert.equal((output.match(/<redacted-secret>/gu) ?? []).length, 6);
  assert.equal(output.replace(/authorization=<redacted-secret>|"authorization":"<redacted-secret>"|\s/giu, ""), "{}");
  const benign = ["docker", "inspect", "--format", "{{.Id}}", target.container];
  assert.deepEqual(benign.map((arg) => redactV4(arg)), benign);
});
...
test("strict identity, mount, no-port and PostgreSQL 16 checks fail closed", () => {
  for (const [name, options, pattern] of [
    ...
    assert.throws(() => runAuthorizedRegressionV4({ authority, evidenceRoot, runCommand: fake.runCommand }), pattern);
