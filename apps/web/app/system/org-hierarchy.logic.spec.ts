...
test("user page creates organization assignments atomically and guards stale catalogs", () => {
  ...
  assert.match(usersSource, /disabled=\{\(canAssignRoles && \(!editingUser \|\| roleOnlyEditing\) && !roleCatalogReady\) \|\| roleCatalogLoading \|\| \(!roleOnlyEditing && \(loginSettingsLoading \|\| orgCatalogLoading \|\| !formParkId\)\)\}/);
