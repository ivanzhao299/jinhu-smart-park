...
本文最初用于 E2-5B-3 设计确认。E2-5B-3A 已按本文完成删除 / 作废语义保护实施与回归补充；E2-5B-3B 已在保留 softDelete 语义的前提下接入 `IdempotencyInterceptor`，并补齐 replay / conflict / failed request retry 回归。

## 2. 当前接口现状

| 接口 | controller method | 当前是否 guard | 当前是否 interceptor | 当前 service 行为 | 当前状态保护 | 当前审计留痕 | 当前风险 |
|---|---|---|---|---|---|---|---|
| `DELETE /leasing/receivables/:id` | `LeasingReceivablesController.remove` -> `LeasingReceivablesService.softDelete` | 是，全局 `IdempotencyKeyGuard` 要求非公开写请求带 key | 是，E2-5B-3B 已接入 | 读取 scoped 应收后设置 `isDeleted=true`、`status=void`，并返回 `{ id }` | 阻止 `amountPaid > 0`、`amountWaived > 0`、`invoiceStatus != none`、paid / partial / waived / void 等状态以及 payment application 的应收删除 | controller 有 `AuditLog`；service 写应收状态日志，action 为 `delete` | `DELETE` 名称容易被误解为物理删除；真实幂等已保证 same key replay 不因 softDeleted 后不可见而变成 404 |
| `DELETE /leasing/payments/:id` | `LeasingPaymentsController.remove` -> `LeasingPaymentsService.softDelete` | 是，全局 `IdempotencyKeyGuard` 要求非公开写请求带 key | 是，E2-5B-3B 已接入 | 读取 scoped 收款后设置 `isDeleted=true`、`status=void`，并返回 `{ id }` | 阻止 void、partial、applied、`sumAppliedAmount > 0` 或存在 application 的收款删除 | controller 有 `AuditLog`；service 未见独立 payment status log | `DELETE` 名称容易被误解为物理删除；真实幂等已保证 same key replay 不因 softDeleted 后不可见而变成 404 |

当前两个接口都不是物理删除，而是“软删除 + 置为 void”。它们仍通过 HTTP `DELETE` 暴露，E2-5B-3B 已接入真实幂等 interceptor，但这不改变其业务语义，也不代表已实现冲销 / 反核销流程。
...
- E2-5B-3B 已通过 `IdempotencyInterceptor` 保证 same key + same request replay 返回第一次成功响应，不会因为 `is_deleted = false` 过滤而变成 404。
...
- E2-5B-3B 已在后续接入 `IdempotencyInterceptor`，E2-5B-3A 本身只负责业务语义保护。
...
- 已完成。当前保留 DELETE，并在 controller method 上接入 `IdempotencyInterceptor`。
- same key replay 返回第一次 `{ id }` 响应，不能因第二次查不到 `isDeleted=false` 记录而变成失败。
- same key + different query 通过 fingerprint 返回 `409 conflict`。

风险：

- DELETE 通常没有 body；本阶段使用 query 差异覆盖 conflict，因为当前 fingerprint 已包含 query。
- query 仅用于回归构造 conflict，不改变当前业务语义；如未来正式引入作废原因，应设计独立字段或 action 口径。

验收标准：

- missing key 返回 `400`。
- first request 成功软删除 / 作废。
- replay same key same request 返回第一次结果，不变成 `404`。
- same key + different query 返回 `409`。
- 对已发生财务活动对象的 failed request retry 仍返回业务错误，不能 replay 成功。
...
- 已作废对象 same key replay 行为明确，不变成 404。
- DELETE 幂等已覆盖 missing key / first / replay / query conflict / failed request retry。
...
- 是否可以直接接入幂等：E2-5B-3A 业务语义保护完成后，E2-5B-3B 已在保留 DELETE 口径下接入。
- 是否必须先补业务语义保护：已完成 E2-5B-3A，明确删除 / 作废边界和错误口径。
...
下一步不再需要针对这两个 DELETE 补基础幂等。推荐顺序是：

1. E2-5B-3C：对已发生财务活动记录设计作废 / 冲销 / 反核销专项流程。
2. 单独确认 payment softDelete 是否需要独立状态日志。
3. 继续将 `POST /leasing/receivables/generate-batch` 作为批量生成专项治理项。

当前可以把应收 / 收款 DELETE 视为“真实幂等已覆盖”，但不能把它们视为“完整财务作废 / 冲销流程已覆盖”。
