...
test("T3 attendance punch quarantine rollback is lab-only and removes audit records only",()=>{
  for(const value of ["ALLOW_YUZHOU_ROLLBACK","succeeded attendance quarantine batch not found","attendance quarantine rollback accounting drift","DELETE FROM migration_check","DELETE FROM migration_error","DELETE FROM migration_rollback_point","DELETE FROM migration_batch_item","DELETE FROM migration_batch","attendance quarantine wrote legacy mapping","auditResidual","productionImport','HOLD'"])assert.match(source,new RegExp(value.replace(/[.*+?^${}()|[\]\\]/g,"\\$&")));
