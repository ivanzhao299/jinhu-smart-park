...
run_baseline_checks() {
  ...
  check_positive "production seed baseline exists" "SELECT COUNT(*) FROM sys_tenant WHERE tenant_id = $(sql_quote "$TENANT_ID") AND status = 1 AND is_deleted = false;"
  check_positive "default tenant exists" "SELECT COUNT(*) FROM sys_tenant WHERE tenant_id = $(sql_quote "$TENANT_ID") AND status = 1 AND is_deleted = false;"
  check_positive "default park exists" "SELECT COUNT(*) FROM biz_park park JOIN sys_tenant tenant ON tenant.tenant_id = park.tenant_id AND tenant.status = 1 AND tenant.is_deleted = false WHERE park.tenant_id = $(sql_quote "$TENANT_ID") AND park.park_id = $(sql_quote "$PARK_ID") AND park.status = 1 AND park.is_deleted = false;"
