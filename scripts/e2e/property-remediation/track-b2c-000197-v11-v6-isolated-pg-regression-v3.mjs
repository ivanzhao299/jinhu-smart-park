...
const runnerSpecPath = resolve(root, "scripts/e2e/property-remediation/tests/b2c-000197-v11-v6-isolated-pg-regression-v3.spec.mjs");
const artifactPaths = Object.freeze({
  authority: resolve(research, "b2c-000197-v11-v6-pg-regression-v3-candidate-authority-20260802.grammar"),
  manifest: resolve(research, "b2c-000197-v11-v6-pg-regression-v3-candidate-manifest-20260802.json"),
  testRecord: resolve(research, "b2c-000197-v11-v6-pg-regression-v3-static-test-record-20260802.json"),
  handoff: resolve(research, "b2c-000197-v11-v6-pg-regression-v3-review-handoff-20260802.md"),
...
export const V11_V6_PG_V3_REVIEW_SCHEMAS = Object.freeze({
  database: "b2c-000197-v11-v6-pg-regression-v3-independent-database-review-v1",
  qa: "b2c-000197-v11-v6-pg-regression-v3-independent-qa-security-review-v1",
  resource: "b2c-000197-v11-v6-pg-regression-v3-resource-authority-v1",
});
export const V11_V6_PG_V3_REVIEW_PATHS = Object.freeze({
  database: resolve(research, "b2c-000197-v11-v6-pg-regression-v3-independent-database-review-20260802.grammar"),
  qa: resolve(research, "b2c-000197-v11-v6-pg-regression-v3-independent-qa-security-review-20260802.grammar"),
  resource: resolve(research, "b2c-000197-v11-v6-pg-regression-v3-resource-authority-20260802.grammar"),
});
export const V11_V6_PG_V3_ENV_KEYS = Object.freeze([
  "B2C_000197_V11_V6_PG_V3_EXECUTE", "B2C_000197_V11_V6_PG_V3_RUN_ID",
  "B2C_000197_V11_V6_PG_V3_RESOURCE_SHA", "B2C_000197_V11_V6_PG_V3_DATABASE_REVIEW_SHA",
  "B2C_000197_V11_V6_PG_V3_QA_REVIEW_SHA",
]);
...
export const redactV3 = (value) => String(value ?? "").replace(databaseUrl, "<redacted-database-url>")
...
function exactFile(path, hash, requireReadonly = true) {
  if (!path || !hex64.test(hash) || !existsSync(path) || lstatSync(path).isSymbolicLink()
    ...
    throw new Error(`v11-v6-pg-v3-input-drift:${path}`);
  }
  return readFileSync(path, "utf8");
}

function exactCurrentFile(path, requireReadonly = true) {
  if (!existsSync(path)) throw new Error(`v11-v6-pg-v3-input-drift:${path}`);
  return exactFile(path, sha256(readFileSync(path)), requireReadonly);
}

export function parseExactGrammarV3(text, schema, expectedNames, label) {
  const lines = String(text).trimEnd().split("\n");
  if (lines.shift() !== schema) throw new Error(`v11-v6-pg-v3-${label}-schema`);
  const fields = new Map();
  for (const line of lines) {
    const cells = line.split("\t");
    if (cells.length !== 2 || !expectedNames.has(cells[0]) || fields.has(cells[0])) {
      throw new Error(`v11-v6-pg-v3-${label}-field`);
    }
    fields.set(cells[0], cells[1]);
  }
  if (fields.size !== expectedNames.size) throw new Error(`v11-v6-pg-v3-${label}-missing`);
  return fields;
}

export function assertResourceAuthorityV3(text, runId, bindings) {
  ...
  const fields = parseExactGrammarV3(text, V11_V6_PG_V3_REVIEW_SCHEMAS.resource, expected, "resource");
  ...
  if (target.runId !== runId || !safeRunId.test(runId) || !safeContainer.test(target.container)
    ...
    throw new Error("v11-v6-pg-v3-resource-contract");
  }
...
export function assertIndependentGoV3(text, kind, bindings) {
  ...
  const fields = parseExactGrammarV3(text, V11_V6_PG_V3_REVIEW_SCHEMAS[kind], new Set(Object.keys(expected)), kind);
  for (const [name, value] of Object.entries(expected)) {
    if (fields.get(name) !== value) throw new Error(`v11-v6-pg-v3-${kind}-binding:${name}`);
  }
  return Object.fromEntries(fields);
}

export function intakeAuthoritiesV3(env = process.env) {
  if (env.B2C_000197_V11_V6_PG_V3_EXECUTE !== "1") throw new Error("v11-v6-pg-v3-not-authorized");
  const runId = env.B2C_000197_V11_V6_PG_V3_RUN_ID;
  ...
  const databaseSha = env.B2C_000197_V11_V6_PG_V3_DATABASE_REVIEW_SHA ?? "";
  const databaseText = exactFile(V11_V6_PG_V3_REVIEW_PATHS.database, databaseSha);
  const database = assertIndependentGoV3(databaseText, "database", bindings);
  const qaSha = env.B2C_000197_V11_V6_PG_V3_QA_REVIEW_SHA ?? "";
  const qaText = exactFile(V11_V6_PG_V3_REVIEW_PATHS.qa, qaSha);
  const qa = assertIndependentGoV3(qaText, "qa", { ...bindings, database_review_raw_sha256: databaseSha });
  const resourceSha = env.B2C_000197_V11_V6_PG_V3_RESOURCE_SHA ?? "";
  const resourceText = exactFile(V11_V6_PG_V3_REVIEW_PATHS.resource, resourceSha);
  const target = assertResourceAuthorityV3(resourceText, runId, { candidate_authority_raw_sha256:
...
class EvidenceV3 {
  ...
  child({ stage, command, args, input = "", runCommand, allowFailure = false }) {
    ...
    this.write(`${prefix}-${safe}-intent.json`, { schema_version: "b2c-000197-v11-v6-pg-v3-child-intent-v1",
    ...
    const safeOutput = (value) => psqlOutput ? "<suppressed:psql-output>" : redactV3(value);
    this.write(`${prefix}-${safe}-result.json`, { schema_version: "b2c-000197-v11-v6-pg-v3-child-result-v1",
      run_id: this.runId, stage: safe, exit_code: result.status ?? null, signal: result.signal ?? null,
      spawn_error: result.error ? { name: redactV3(result.error.name), message: redactV3(result.error.message) } : null,
    ...
    if (result.error || result.signal || (!allowFailure && result.status !== 0)) {
      const error = new Error(`v11-v6-pg-v3-child:${safe}`); error.stage = safe; throw error;
    }
    return result;
  }
  terminal(kind, payload) {
    const terminal = this.write(`${kind}-${this.runId}.json`, { schema_version: "b2c-000197-v11-v6-pg-v3-terminal-v1",
    ...
    const manifest = this.write(`${kind}-${this.runId}.manifest.json`, {
      schema_version: "b2c-000197-v11-v6-pg-v3-evidence-manifest-v1", run_id: this.runId,
...
function inspect(recorder, runCommand, target) {
  ...
  if (extra.length !== 0 || id !== target.containerId || name !== `/${target.container}` || running !== "true"
      ...
      || mounts[0]?.Destination !== "/var/lib/postgresql/data") throw new Error("v11-v6-pg-v3-identity-drift");
}
...
const snapshotSql = `SELECT json_build_object(
  ...
  'build_residue',to_regclass('public.uq_biz_property_approval_request_active_source_v3_build') IS NOT NULL)::text;`;
...
function assertAbsent(state) {
  if (state.history_primary !== null || state.history_mirror !== null || Number(state.approval_rows) !== 0
    ...
    throw new Error("v11-v6-pg-v3-snapshot-drift");
  }
}
function observe(result, entry, before, after) {
  ...
  if (result.status === 0 || result.status == null || result.signal || result.error || summary.sqlstate !== "P0001"
    ...
    const error = new Error(`v11-v6-pg-v3-fault-drift:${entry.boundary}`); error.stage = `fault-${entry.boundary}`; throw error;
  }
  return summary;
}

export function runAuthorizedRegressionV3({ authority, evidenceRoot, runCommand = commandResult }) {
  const recorder = new EvidenceV3(evidenceRoot, authority.target.runId);
  try {
    ...
    if (!/^16[0-9]{4}$/u.test(version)) throw new Error("v11-v6-pg-v3-version-drift");
    ...
    recorder.terminal("failure", { failure_stage: stageName(error.stage ?? "runner"),
      error: { name: redactV3(error.name), message: redactV3(error.message) } }); throw error;
  }
}

export function executeIsolatedPgRegressionV3({ env = process.env, runCommand = commandResult } = {}) {
  const authority = intakeAuthoritiesV3(env);
  const evidenceRoot = resolve(research, `b2c-000197-v11-v6-pg-regression-v3-evidence-${authority.target.runId}`);
  return runAuthorizedRegressionV3({ authority, evidenceRoot, runCommand });
}

export function isolatedPgRegressionCandidateV3() {
  return { status: "blocked-awaiting-independent-database-and-qa-go-and-resource-authority",
    execution_authorized: false, formal_go: false, fixed_review_paths: V11_V6_PG_V3_REVIEW_PATHS,
    review_schemas: V11_V6_PG_V3_REVIEW_SCHEMAS, required_environment_keys: [...V11_V6_PG_V3_ENV_KEYS],
    fixed_physical_targets: [], docker_or_database_command_executed: false };
}

if (fileURLToPath(import.meta.url) === resolve(process.argv[1] ?? "")) {
  const payload = process.env.B2C_000197_V11_V6_PG_V3_EXECUTE === "1"
    ? executeIsolatedPgRegressionV3() : isolatedPgRegressionCandidateV3();
