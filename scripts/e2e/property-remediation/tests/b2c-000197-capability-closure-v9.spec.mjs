...
import { formalOperationV9, formalStaticV9 } from "../track-b2c-000197-preliminary-orchestrator-v9.mjs";
...
test("formal executable callgraph invokes every hard capability on both targets", () => {
  ...
  const result=formalOperationV9({}, { inputs:mark("inputs"),authorities:mark("authorities",{reviews:2,drain:true}),
...
test("formal static child receives frozen mode as recorded benign env",()=>{const calls=[];formalStaticV9({runChild:(x)=>{calls.push(x);return{};}});
 const stage=calls.find((x)=>x.stage==="static-v9-orchestrator");assert.equal(stage.env.B2C_000197_V9_STATIC_MODE,"frozen");
 assert.deepEqual(stage.envAllowlist.find((x)=>x.name==="B2C_000197_V9_STATIC_MODE"),{name:"B2C_000197_V9_STATIC_MODE",persist:"value"});});

test("authoritative manifest closure excludes returned artifacts and matches declared local dependencies",()=>{
 const source=readFileSync(resolve(process.cwd(),"scripts/e2e/property-remediation/track-b2c-000197-preliminary-orchestrator-v9.mjs"),"utf8");
 ...
 for(const dependency of ["track-b2c-000197-failure-cases-v9.mjs","track-b2c-000197-preliminary-executor-v9.mjs",
   "track-b2c-000197-closure-resolver-v9.mjs","static-v9-capability","static-v9-recursive-closure",
