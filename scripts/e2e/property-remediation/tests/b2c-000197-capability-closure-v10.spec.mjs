...
import { formalOperationV10, formalStaticV10 } from "../track-b2c-000197-preliminary-orchestrator-v10.mjs";
...
test("formal executable callgraph invokes every hard capability on both targets", () => {
  ...
  const result=formalOperationV10({}, { inputs:mark("inputs"),authorities:mark("authorities",{reviews:2,drain:true}),
...
test("formal static child receives frozen mode as recorded benign env",()=>{const calls=[];formalStaticV10({runChild:(x)=>{calls.push(x);return{};}});
 const stage=calls.find((x)=>x.stage==="static-v10-orchestrator");assert.equal(stage.env.B2C_000197_V10_STATIC_MODE,"frozen");
 assert.deepEqual(stage.envAllowlist.find((x)=>x.name==="B2C_000197_V10_STATIC_MODE"),{name:"B2C_000197_V10_STATIC_MODE",persist:"value"});});

test("authoritative manifest closure excludes returned artifacts and matches declared local dependencies",()=>{
 const source=readFileSync(resolve(process.cwd(),"scripts/e2e/property-remediation/track-b2c-000197-preliminary-orchestrator-v10.mjs"),"utf8");
 ...
 for(const dependency of ["track-b2c-000197-failure-cases-v10.mjs","track-b2c-000197-preliminary-executor-v10.mjs",
   "track-b2c-000197-closure-resolver-v10.mjs","static-v10-capability","static-v10-recursive-closure",
