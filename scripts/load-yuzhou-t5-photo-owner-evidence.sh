...
SELECT set_config('yuzhou.t5_run',:'run',true),set_config('yuzhou.t5_db',:'db',true),set_config('yuzhou.t0_run',:'t0_run',true),set_config('yuzhou.t5_snapshot',:'snapshot',true);
DO $$BEGIN
 ...
 IF NOT EXISTS(SELECT 1 FROM migration_batch WHERE run_id=current_setting('yuzhou.t0_run') AND source_system='yuzhou-v10' AND source_snapshot_sha256=current_setting('yuzhou.t5_snapshot') AND target_database=current_database() AND status='succeeded') THEN RAISE EXCEPTION 'compatible succeeded T0 batch required'; END IF;
END$$;
...
CREATE TEMP TABLE protected_before AS SELECT (SELECT encode(digest(COALESCE(string_agg(to_jsonb(x)::text,'' ORDER BY x.id),''),'sha256'),'hex') FROM hr_employee x) employee_hash,(SELECT encode(digest(COALESCE(string_agg(to_jsonb(x)::text,'' ORDER BY x.id),''),'sha256'),'hex') FROM sys_file x) file_hash,(SELECT encode(digest(COALESCE(string_agg(to_jsonb(x)::text,'' ORDER BY x.id),''),'sha256'),'hex') FROM hr_employee_document x) document_hash,(SELECT encode(digest(COALESCE(string_agg(to_jsonb(x)::text,'' ORDER BY x.id),''),'sha256'),'hex') FROM hr_employee_compensation x) compensation_hash,(SELECT encode(digest(COALESCE(string_agg(to_jsonb(x)::text,'' ORDER BY x.id),''),'sha256'),'hex') FROM hr_payroll_run x) payroll_hash,(SELECT encode(digest(COALESCE(string_agg(to_jsonb(x)::text,'' ORDER BY x.id),''),'sha256'),'hex') FROM hr_payslip x) payslip_hash,(SELECT encode(digest(COALESCE(string_agg(to_jsonb(x)::text,'' ORDER BY x.id),''),'sha256'),'hex') FROM biz_user_message x) message_hash;
...
SELECT b.id,'T5_FILE_SOURCE_ACCOUNTING',to_jsonb(2155),to_jsonb(a.l+a.q),'{}'::jsonb,a.l+a.q=2155,encode(digest('T5_FILE_SOURCE_ACCOUNTING:2155','sha256'),'hex') FROM b CROSS JOIN a;
WITH b AS(SELECT id FROM migration_batch WHERE run_id=:'run'),protected_after AS(
  SELECT
    (SELECT encode(digest(COALESCE(string_agg(to_jsonb(x)::text,'' ORDER BY x.id),''),'sha256'),'hex') FROM hr_employee x) employee_hash,
    (SELECT encode(digest(COALESCE(string_agg(to_jsonb(x)::text,'' ORDER BY x.id),''),'sha256'),'hex') FROM sys_file x) file_hash,
    (SELECT encode(digest(COALESCE(string_agg(to_jsonb(x)::text,'' ORDER BY x.id),''),'sha256'),'hex') FROM hr_employee_document x) document_hash,
    ...
    (SELECT encode(digest(COALESCE(string_agg(to_jsonb(x)::text,'' ORDER BY x.id),''),'sha256'),'hex') FROM hr_payroll_run x) payroll_hash,
...
INSERT INTO migration_check(batch_id,check_code,expected_value,actual_value,tolerance,passed,evidence_sha256)
SELECT b.id,'T5_FILE_NO_BINARY_OR_LINK_WRITE',to_jsonb(p),to_jsonb(a),'{}'::jsonb,p=a,encode(digest(to_jsonb(p)::text,'sha256'),'hex') FROM b CROSS JOIN protected_before p CROSS JOIN protected_after a;
