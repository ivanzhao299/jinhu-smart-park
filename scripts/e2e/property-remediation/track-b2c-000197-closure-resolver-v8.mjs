...
export const FORMAL_CHILD_ENTRIES_V8 = Object.freeze([
  "scripts/e2e/property-remediation/tests/b2c-000197-preliminary-executor-v8.spec.mjs",
  "scripts/e2e/property-remediation/tests/b2c-000197-preliminary-orchestrator-v8.spec.mjs",
  "scripts/e2e/property-remediation/tests/b2c-000197-capability-closure-v8.spec.mjs",
  "scripts/e2e/property-remediation/tests/b2c-000197-recursive-closure-v8.spec.mjs",
  "scripts/e2e/property-remediation/tests/b2c-000197-frozen-closure-v8.spec.mjs",
...
export const TYPECHECK_GOVERNANCE_FILES_V8 = Object.freeze([
...
function tsconfig(root) {
  ...
  if (loaded.error) throw new Error(`v8-tsconfig-read:${JSON.stringify(loaded.error)}`);
  const parsed = ts.parseJsonConfigFileContent(loaded.config, ts.sys, dirname(path), { noEmit: true }, path);
  if (parsed.errors.length) throw new Error(`v8-tsconfig-parse:${JSON.stringify(parsed.errors)}`);
  return parsed;
}

export function compilerRepositoryFilesV8(rootInput = process.cwd()) {
...
export function parseTscListFilesV8(stdout, rootInput = process.cwd()) {
...
export function classifySpecifierV8(importerInput, specifier, rootInput = process.cwd()) {
...
export function resolveFormalExecutionClosureV8(rootInput = process.cwd()) {
  const root = realpathSync(resolve(rootInput)); const parsed = tsconfig(root);
  const repository = new Set([...compilerRepositoryFilesV8(root), ...FORMAL_CHILD_ENTRIES_V8,
    ...TYPECHECK_GOVERNANCE_FILES_V8].map((path) => normalize(root, realpathSync(resolve(root, path)))));
  ...
  if (unresolved.length) {
    throw new Error(`b2c-000197-v8-unresolved-repository-specifiers:${JSON.stringify(unresolved)}`);
  }
  ...
  return Object.freeze({ repositoryFiles: Object.freeze([...repository].sort()),
    compilerFiles: compilerRepositoryFilesV8(root), builtin: Object.freeze(classify(builtin)),
    external: Object.freeze(classify(external)), unresolved: Object.freeze([]) });
}

export function assertTscFileListMatchesV8(stdout, rootInput = process.cwd()) {
  const compiler = compilerRepositoryFilesV8(rootInput); const actual = parseTscListFilesV8(stdout, rootInput);
  if (JSON.stringify(compiler) !== JSON.stringify(actual)) {
    const expected = new Set(compiler); const observed = new Set(actual);
    throw new Error(`b2c-000197-v8-tsc-file-list-drift:${JSON.stringify({
