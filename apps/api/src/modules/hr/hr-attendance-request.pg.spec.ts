...
suite("HR attendance request PostgreSQL gate",()=>{
 ...
 before(async()=>{
  dataSource=new DataSource({type:"postgres",host:process.env.POSTGRES_HOST??"127.0.0.1",port:Number(process.env.POSTGRES_PORT??"5432"),database:process.env.POSTGRES_DB??"jinhu_smart_park",username:process.env.POSTGRES_USER??"jinhu",password:process.env.POSTGRES_PASSWORD,entities:[HrEmployeeEntity,HrApprovalRequestEntity,HrApprovalActionEntity,HrAttendanceRequestEntity]});
 ...
 it("keeps request, approval and overlap facts transactionally consistent",async()=>{
  ...
  assert.equal(created.status,"draft");assert.equal(created.durationMinutes,570);assert.equal(created.isSelf,true);
  ...
  assert.equal(reviewed.status,"approved");assert.equal(reviewed.isSelf,false);
