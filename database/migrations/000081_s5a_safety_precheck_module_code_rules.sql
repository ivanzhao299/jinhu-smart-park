...
VALUES (
  ...
  1,
  45,
  'S5-A safety module precheck seed'
)
...
SELECT
  ...
  safety_module.id,
  1,
  'S5-A safety plan-module precheck seed'
FROM target_plans
...
SELECT
  ...
  '{}'::jsonb,
  'enabled',
  'S5-A safety module enabled for Jinhu GROUP tenant'
FROM seed_scope
...
code_rules(entity_type, rule_code, rule_name, target_module, target_entity, prefix, pattern, date_pattern, sequence_length, reset_policy, separator, example_code, remark) AS (
  VALUES
    ('safety_inspect_point', 'SAFETY_INSPECT_POINT_CODE', '安全巡检点编码规则', 'safety', 'safety_inspect_point', 'SP-', '{PREFIX}{SEQ:6}', NULL, 6, 'none', '', 'SP-000001', 'S5-A safety inspect point code rule'),
    ('safety_inspect_template', 'SAFETY_INSPECT_TEMPLATE_CODE', '安全巡检模板编码规则', 'safety', 'safety_inspect_template', 'STPL-', '{PREFIX}{SEQ:6}', NULL, 6, 'none', '', 'STPL-000001', 'S5-A safety inspect template code rule'),
    ('safety_inspect_plan', 'SAFETY_INSPECT_PLAN_CODE', '安全巡检计划编码规则', 'safety', 'safety_inspect_plan', 'SPLAN-', '{PREFIX}{SEQ:6}', NULL, 6, 'none', '', 'SPLAN-000001', 'S5-A safety inspect plan code rule'),
    ('safety_inspect_task', 'SAFETY_INSPECT_TASK_CODE', '安全巡检任务编码规则', 'safety', 'safety_inspect_task', 'STASK-', '{PREFIX}{DATE:yyyyMM}-{SEQ:6}', 'yyyyMM', 6, 'monthly', '', 'STASK-202605-000001', 'S5-A safety inspect task code rule'),
    ('safety_hazard', 'SAFETY_HAZARD_CODE', '安全隐患编码规则', 'safety', 'safety_hazard', 'HZ-', '{PREFIX}{DATE:yyyyMM}-{SEQ:6}', 'yyyyMM', 6, 'monthly', '', 'HZ-202605-000001', 'S5-A safety hazard code rule'),
    ('safety_hazard_log', 'SAFETY_HAZARD_LOG_CODE', '安全隐患日志编码规则', 'safety', 'safety_hazard_log', 'HZL-', '{PREFIX}{DATE:yyyyMM}-{SEQ:6}', 'yyyyMM', 6, 'monthly', '', 'HZL-202605-000001', 'S5-A safety hazard log code rule')
)
