...
test("training reward rollback is run-bound, reverse ordered, and leaves no active mapping",()=>{
 ...
 assert.match(source,/DECLARE v_batch_id uuid/);
 assert.match(source,/WHERE m\.batch_id=v_batch_id AND m\.is_active AND m\.mapping_status='loaded'/);
 assert.doesNotMatch(source,/WHERE batch_id=batch_id/);
