...
- [ ] 获取最新 `origin/main`，记录 SHA 与 divergence。
- [ ] 启动本任务并提交只含本任务目录的 coordination SHA；核验其相对 `f19ab4d5`
  无业务/配置变化。
- [ ] 核验 snapshot、cutpoints、commit ancestry 和远端 ref 不碰撞。
- [ ] 生成 branch/ref manifest 与 SHA-256。
- [ ] 核验工作树只含既有 `.codex/config.toml`、`NUL`；不得暂存。
- [ ] 推送 immutable snapshot、coordination 与必要 compare refs，禁止 force。

Stop point：任何 ref 已存在且目标不同，立即停止，不覆盖。

## Phase 1：独立集成 worktree

- [ ] 从最新 `origin/main` 创建 `codex/pr192-main-integration`。
- [ ] 使用独立 worktree，不切换当前用户工作树。
- [ ] 以 `--no-ff --no-commit` 合并 coordination SHA。
- [ ] 输出冲突清单和逐文件裁决记录。
- [ ] 先运行冲突相关 targeted tests，再创建 merge commit。
