...
import { V7_TARGETS, assertAbsentV7, staticV7Candidate } from "../track-b2c-000197-preliminary-orchestrator-v7.mjs";

const source = readFileSync(resolve(process.cwd(),
  "scripts/e2e/property-remediation/track-b2c-000197-preliminary-orchestrator-v7.mjs"), "utf8");
...
test("logical v7 run ID is 02c while physical resources remain exact approved 02b identities", () => {
  assert.equal(staticV7Candidate({ mode: "unfrozen" }).formal_run_id, "b2c197_prelim_20260802c");
  assert.deepEqual(V7_TARGETS.map(({ container, containerId }) => [container, containerId]), [
...
test("absent requires numeric zero rows and exact old catalog", () => {
  assert.equal(assertAbsentV7(valid), valid);
  for (const drift of [{ approval_rows: 1 }, { approval_rows: "0" }, { history_primary: [] }, { history_mirror: [] },
    { indexdef: "drift" }, { predicate: "drift" }, { build_residue: true }]) assert.throws(() => assertAbsentV7({ ...valid, ...drift }));
});

test("static mode explicitly distinguishes pre-freeze and post-freeze contracts", () => {
  const mode = process.env.B2C_000197_V7_STATIC_MODE ?? "unfrozen";
  const value = staticV7Candidate({ mode });
  if (mode === "frozen") {
...
test("formal static and lifecycle Node test children all declare TAP reporter", () => {
  for (const marker of ["static-v7-evidence", "static-v7-orchestrator", "static-v7-closure", "static-v7-contract",
    "static-v7-lifecycle", "approval-named-tests"]) assert.match(source, new RegExp(marker));
