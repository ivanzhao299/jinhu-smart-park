...
docker exec -i "$PG_CONTAINER" psql -X -v ON_ERROR_STOP=1 -U jinhu -d "$TARGET_DATABASE" \
  ...
  -v events_sha="$EVENTS_SHA256" -v types_sha="$TYPES_SHA256" <<'SQL'
BEGIN;
...
WITH b AS (SELECT id FROM migration_batch WHERE run_id=:'run_id'), classified AS (
 SELECT s.*,e.id employee_id FROM stg_employment_event s
 LEFT JOIN hr_employee e ON e.tenant_id=:'tenant_id' AND e.park_id=:'park_id' AND e.employee_code=s.payload->'source'->>'employeeCode' AND e.is_deleted=false
)
...
SELECT b.id,'employment_event','dbo.readjust','load','running',6887,count(*) FILTER(WHERE employee_id IS NOT NULL),0,count(*) FILTER(WHERE employee_id IS NULL),:'events_sha',now()
...
SELECT :'tenant_id',:'park_id',e.id,s.payload->>'normalizedEventType',(s.payload->'source'->>'sourceEffectiveAt')::timestamp::date,
 ...
 CASE WHEN s.payload->>'migrationDecision'='needs_review' OR s.payload->'source'->>'legacyState'<>'1' THEN 'needs_review' ELSE 'accepted' END,
 true,'Migrated from Yuzhou V10; run='||:'run_id'
FROM stg_employment_event s JOIN hr_employee e ON e.tenant_id=:'tenant_id' AND e.park_id=:'park_id' AND e.employee_code=s.payload->'source'->>'employeeCode' AND e.is_deleted=false;
...
FROM stg_employment_event s CROSS JOIN b JOIN hr_employment_event ev ON ev.tenant_id=:'tenant_id' AND ev.park_id=:'park_id' AND ev.legacy_event_no=s.payload->'source'->>'legacyEventNo' AND ev.is_deleted=false;
...
SELECT b.id,item.id,'mapping','EMPLOYMENT_EVENT_EMPLOYEE_NOT_MAPPED',s.payload->>'sourceIdentitySha256',jsonb_build_object('rule','employee_mapping_missing'),true,false
FROM stg_employment_event s CROSS JOIN b JOIN item ON item.batch_id=b.id
LEFT JOIN hr_employee e ON e.tenant_id=:'tenant_id' AND e.park_id=:'park_id' AND e.employee_code=s.payload->'source'->>'employeeCode' AND e.is_deleted=false
WHERE e.id IS NULL;
...
SELECT b.id,'T1_EVENT_ACCOUNTING','6887'::jsonb,to_jsonb((SELECT count(*) FROM legacy_record_map WHERE batch_id=b.id AND target_table='hr_employment_event')+(SELECT count(*) FROM migration_error WHERE batch_id=b.id AND error_code='EMPLOYMENT_EVENT_EMPLOYEE_NOT_MAPPED')),'{}'::jsonb,
 (SELECT count(*) FROM legacy_record_map WHERE batch_id=b.id AND target_table='hr_employment_event')+(SELECT count(*) FROM migration_error WHERE batch_id=b.id AND error_code='EMPLOYMENT_EVENT_EMPLOYEE_NOT_MAPPED')=6887,encode(digest('T1_EVENT_ACCOUNTING:6887','sha256'),'hex') FROM b
...
UPDATE migration_batch SET phase='verify',status=CASE WHEN EXISTS(SELECT 1 FROM migration_check WHERE batch_id=migration_batch.id AND NOT passed) THEN 'failed' ELSE 'succeeded' END,
 counts=jsonb_build_object('eventsLoaded',(SELECT count(*) FROM legacy_record_map WHERE batch_id=migration_batch.id AND target_table='hr_employment_event'),'eventsQuarantined',(SELECT count(*) FROM migration_error WHERE batch_id=migration_batch.id),'eventsNeedsReview',(SELECT count(*) FROM hr_employment_event WHERE remark='Migrated from Yuzhou V10; run='||:'run_id' AND migration_decision='needs_review')),
