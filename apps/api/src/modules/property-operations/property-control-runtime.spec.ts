...
test("control DTOs and projections use camelCase and stable pagination", () => {
  ...
  assert.match(operationService, /executeApprovedModeTransition[\s\S]*unit\.usage_type AS "usageType"/);
  assert.match(operationService, /executeApprovedModeTransition[\s\S]*isUnitUsageAllowedForPropertyMode\(targetMode/);
  ...
  assert.match(operationService, /PROPERTY_MODE_UNIT_USAGE_ALLOWLIST\.long_rent/);
  assert.match(operationService, /unit\.usage_type IN \(:\.\.\.eligibleUsageTypes\)/);
  assert.match(operationService, /const lockedUnit = await manager\.getRepository\(UnitEntity\)\.findOne/);
  assert.match(operationService, /isUnitUsageAllowedForPropertyMode\(dto\.target_mode, lockedUnit\.usageType\)/);
  assert.match(operationService, /Unit usage is not allowed for target operating mode/);
  assert.match(service, /UNIT_USAGE_HOUSING/);
  assert.match(service, /unit\.usage_type IN \(:\.\.\.eligibleUsageTypes\)/);
  assert.match(service, /PROPERTY_MODE_UNIT_USAGE_ALLOWLIST\.long_rent/);
  ...
  assert.match(service, /executeApprovedForceRelease[\s\S]*unit\.usage_type=ANY\(\$4::smallint\[\]\)/);
  ...
  assert.match(unitsService, /assertUsageCompatibleWithPropertyModes/);
  assert.match(unitsService, /canonical_payload->>'targetMode'/);
  assert.match(unitsService, /isUnitUsageAllowedForPropertyMode\(mode, usageType\)/);
...
test("projected operation exposes office long-rent eligibility and target-aware blockers", async () => {
  ...
  const project = service["projectOperation"].bind(service);
