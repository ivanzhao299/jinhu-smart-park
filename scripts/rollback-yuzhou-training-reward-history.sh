...
DO $$DECLARE batch_id uuid; projected bigint; mapped bigint; BEGIN
 ...
 SELECT id INTO batch_id FROM migration_batch WHERE run_id=current_setting('yuzhou.training_reward_rollback') AND status='succeeded' AND target_database=current_database() FOR UPDATE;
 IF batch_id IS NULL THEN RAISE EXCEPTION 'rollbackable batch not found'; END IF;
 SELECT count(*) INTO projected FROM hr_legacy_training_reward_projection WHERE migration_batch_id=batch_id AND status='staged';
 SELECT count(*) INTO mapped FROM legacy_record_map WHERE batch_id=batch_id AND is_active AND mapping_status='loaded';
...
COMMIT;
SQL
docker exec "$PG" psql -X -A -t -U jinhu -d "$DB" -v run="$RUN_ID" -c "SELECT json_build_object('status','PASS','residualCount',(SELECT count(*) FROM hr_legacy_training_reward_projection p JOIN migration_batch b ON b.id=p.migration_batch_id WHERE b.run_id=:'run'),'productionImport','HOLD');"
