...
export function formatLightweightFirstProgress(progress, now = Date.now()) {
  ...
  return `[${bar}] ${String(progress.completedPercent).padStart(3, " ")}% phase=${progress.phase} status=${progress.status} elapsed=${Math.floor(elapsedMilliseconds / 60000)}m productionImport=HOLD`;
}
