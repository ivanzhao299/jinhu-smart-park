...
Business display names used as catalog labels must be normalized before validation and must contain
a readable non-numeric character. Reject blank and numeric-only names at the API boundary even when
the Web form also validates them; identifiers and codes belong in their dedicated fields.
