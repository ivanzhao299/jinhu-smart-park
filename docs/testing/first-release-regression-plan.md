...
- 当前覆盖范围：
  ...
  - 批量生成应收幂等与业务去重：missing key / first request / same key replay / same key different body conflict / same payload different key skipped / failed request retry；same key replay 返回首次 generated 结果，different key same payload 进入业务跳过
  - 应收查询
  ...
  - 应收删除 / 作废幂等：missing key / first request / replay / query conflict / failed request retry；未发生财务活动应收可 softDelete，已核销 / 有 application 应收删除被拒绝
  ...
  - 收款删除 / 作废幂等：missing key / first request / replay / query conflict / failed request retry；未核销收款可 softDelete，已核销 / 有 application 收款删除被拒绝
  - 收款查询
  ...
  - `POST /leasing/receivables/generate-batch` 已完成业务去重 / 事务语义保护和请求级 replay / conflict 回归；并发专项压测和 batch history / result 明细仍后置
- 对租赁链路数据依赖的说明：
...
- 暂缓范围：
  ...
  - 发票
  - 欠费账龄
  - 多合同批量生成的严格并发压测、batch history / result 明细和部分成功行级重试追踪
  - 浏览器 E2E
