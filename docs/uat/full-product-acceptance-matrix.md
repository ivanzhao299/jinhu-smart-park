...
| 住房出租 | 个人租客、住宅租约、周期费用、交割、报修工单、采购成本 | MVP 开发完成，菜单为“住房出租 → 住房运营 → `/housing`” | `uat_pending` | 未启用 | [专项验证证据](./housing-rental-mvp-evidence.md)：迁移、类型检查、单元测试、隔离库 SQL 回归、真实 API 全链路和桌面/390px 视觉验收已通过；待目标角色菜单/页面权限、模块启停、文件、数据范围和真实业务数据验收 |
