...
test("M6 Slice 1 adds no write route or historical schema mutation",()=>{
 ...
 assert.doesNotMatch(controller,/@(?:Post|Put|Delete)\("(?:attendance|insurance)/);
 assert.doesNotMatch(service,/\.save\([^)]*(?:Attendance|Insurance)|\.update\([^)]*(?:Attendance|Insurance)/);
...
test("attendance and insurance reads fail before response when required audit is unavailable",async()=>{
 ...
 const args=Array(31).fill({});args[22]={createQueryBuilder:()=>qb};args[23]={createQueryBuilder:()=>qb};args[24]={createQueryBuilder:()=>qb};args[25]={find:async()=>[]};args[30]={recordOperationRequired:async()=>{throw new Error("required audit unavailable");}};
...
test("empty authorized insurance scopes are audited before returning",async()=>{
 let audits=0;
 const args=Array(31).fill({});
 args[30]={recordOperationRequired:async()=>{audits+=1;}};
 const service=Reflect.construct(HrService,args) as HrService;
