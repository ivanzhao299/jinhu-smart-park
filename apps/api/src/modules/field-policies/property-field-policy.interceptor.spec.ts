...
function context(method: string, user: typeof actor | undefined) {
  return {
...
test("property field-policy interceptors project every authenticated GET with the owning module", async () => {
  ...
  assert.deepEqual(await firstValueFrom(homestay.intercept(context("GET", actor), { handle: () => of({ raw: true }) })), { projected: true });
