...
[ "$(docker inspect --format '{{ index .Config.Labels "com.docker.compose.project" }}' "$PG_CONTAINER" 2>/dev/null || true)" = "jinhu_hr_migration_lab" ] || fail "PostgreSQL container is not the migration lab"
