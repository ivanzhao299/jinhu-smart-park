...
import {
  PhaseExecutionErrorV8, V8_RUN_ID, executeWithEvidenceV8, parseTapSummaryV8, runPhasedGateV8,
} from "./track-b2c-000197-preliminary-executor-v8.mjs";
import { failureInjectionCasesV8 } from "./track-b2c-000197-failure-cases-v8.mjs";
import {
  resolveFormalExecutionClosureV8, TYPECHECK_GOVERNANCE_FILES_V8,
} from "./track-b2c-000197-closure-resolver-v8.mjs";
...
const manifestPath = resolve(research, "b2c-000197-preliminary-v8-input-manifest-20260802.grammar");
const handoffPath = resolve(research, "b2c-000197-preliminary-executor-v8-review-handoff-20260802.md");
...
const expected = Object.freeze({ migration: "a9b98ca82aa4dafc16535085184df838880ef27801f7cd4b225e1ca1a15af059",
  ...
  preflightArtifact: "f7f0576d318712c1115e34af5be2e9558fbc44f77f705a1247e80a934284cf7c",
  preflightManifest: "5de5ae05a3d2f55533c8bbcf2f3b2fabfb90b363bc961c24ce9017b86f9d8df6",
...
const STATIC_INPUTS_V8 = Object.freeze([
  ...
  [".trellis/tasks/07-30-pr192-b-domain-integrations/research/b2c-000197-v8-complete-preflight-evidence-b2c197_prelim_20260802c/success-b2c197_prelim_20260802c.json", expected.preflightArtifact],
  [".trellis/tasks/07-30-pr192-b-domain-integrations/research/b2c-000197-v8-complete-preflight-evidence-b2c197_prelim_20260802c/success-b2c197_prelim_20260802c.manifest.json", expected.preflightManifest],
...
let authoritativeCacheV8 = null;
let authoritativeRowsCacheV8 = null;
export function authoritativeFileRowsV8() {
  if (!authoritativeRowsCacheV8) {
    const closure = resolveFormalExecutionClosureV8(root); const compiler = new Set(closure.compilerFiles);
    const governance = new Set(TYPECHECK_GOVERNANCE_FILES_V8); const staticInputs = new Set(STATIC_INPUTS_V8.map(([path]) => path));
    authoritativeRowsCacheV8 = Object.freeze([...new Set([...closure.repositoryFiles, ...staticInputs])].sort().map((path) =>
      Object.freeze({ path, reason: staticInputs.has(path) ? (path.includes("v8-complete-preflight-evidence")
  ...
  return authoritativeRowsCacheV8;
}
export function authoritativeFilePathsV8() {
  authoritativeCacheV8 ??= Object.freeze(authoritativeFileRowsV8().map(({ path }) => path));
  return authoritativeCacheV8;
}

export const V8_TARGETS = Object.freeze([
...
function exact(path, hash) {
  if (!existsSync(path) || lstatSync(path).isSymbolicLink() || realpathSync(path) !== path
      || sha256(readFileSync(path)) !== hash) throw new Error(`b2c-000197-v8-input-drift:${path}`);
}

function inputs() {
  const prefix = readdirSync(resolve(root, "database/migrations")).filter((name) => name.startsWith("000197_"));
  if (prefix.length !== 1 || prefix[0] !== migrationFilename) throw new Error("b2c-000197-v8-prefix-drift");
  for (const [path, hash] of STATIC_INPUTS_V8) exact(resolve(root, path), hash);
}
...
function inspect(recorder, target) {
  ...
  if (value !== wanted) throw new Error(`b2c-000197-v8-resource:${target.key}`);
  return value;
}
...
function inspectNoHostPorts(recorder, target) {
  ...
  if (value !== '{}|{"5432/tcp":null}') throw new Error(`b2c-000197-v8-host-ports:${target.key}`);
...
function preflightHealth(recorder, target) {
  ...
  for (const [name, count] of Object.entries(value)) {
    if (count !== 0) throw new Error(`b2c-000197-v8-preflight-health:${target.key}:${name}:${count}`);
  }
  return value;
}
...
export function assertAbsentV8(value, target = { key: "unknown" }) {
  if (value.history_primary !== null || value.history_mirror !== null || value.approval_rows !== 0
    ...
    throw new Error(`b2c-000197-v8-not-dual-absent-empty:${target.key}`);
  }
  return value;
}
...
function history(recorder, target, status, error = null) {
  psql(recorder, target, `history-${target.key}-${status}`, `BEGIN;
    ...
    VALUES (${literal(migrationFilename)},${literal(expected.migration)},${literal(status)},clock_timestamp(),
      ...
      'b2c-000197-v8-executor','${V8_RUN_ID}') ON CONFLICT(filename) DO UPDATE SET checksum=EXCLUDED.checksum,
...
function migrate(recorder, target) {
  const before = snapshot(recorder, target, `before-${target.key}`); assertAbsentV8(before, target); history(recorder, target, "running");
  const result = psql(recorder, target, `migration-${target.key}`, readFileSync(migrationPath), { allowFailure: true });
  if (result.status !== 0 || result.signal || result.error) {
    history(recorder, target, "failed", `${result.stdout}${result.stderr}`); throw new PhaseExecutionErrorV8(`migration-${target.key}`, result);
  }
  ...
  if (after.approval_rows !== 0 || after.indexdef !== expected.newIndex || after.predicate !== expected.newPredicate
      || after.build_residue) throw new Error(`b2c-000197-v8-post-drift:${target.key}`);
  ...
  if (JSON.stringify(after) !== JSON.stringify(rerun)) throw new Error(`b2c-000197-v8-rerun:${target.key}`);
  return { before, after, rerun_exact: true };
}

function faults(recorder, target) {
  return failureInjectionCasesV8().map(({ name, boundary, prefix, assertion }) => {
    const before = snapshot(recorder, target, `fault-${target.key}-${name}-before`); const marker = `v8-injected-${name}`;
    ...
    if (result.status === 0 || !`${result.stdout}${result.stderr}`.includes(marker)
        || JSON.stringify(before) !== JSON.stringify(after)) throw new Error(`b2c-000197-v8-fault:${name}`);
...
function predicateMatrix(recorder, target) {
  const tenant = `v8-matrix-${target.key}-${V8_RUN_ID}`; const park = "p1";
  const cases = [
  ...
  if (result.active !== 7 || result.terminal !== 5 || result.total !== 12) {
    throw new Error(`b2c-000197-v8-predicate-matrix-drift:${target.key}`);
  }
  ...
  if (blocked.status === 0 || !`${blocked.stdout}${blocked.stderr}`.includes("23505")) {
    throw new Error(`b2c-000197-v8-active-duplicate-not-blocked:${target.key}`);
  }
  ...
  if (terminalCount !== 2) throw new Error(`b2c-000197-v8-terminal-duplicate-blocked:${target.key}`);
...
function approval(recorder, target) {
  ...
  if (!password || !ip) throw new Error("b2c-000197-v8-secret-ip");
  ...
  return runPhasedGateV8(recorder, { phases: [
...
function frozenManifest() {
  ...
  if (lines.shift() !== "b2c-000197-preliminary-input-manifest-v8") throw new Error("v8-manifest-schema");
  ...
  if (new Set(actualPaths).size !== actualPaths.length
      || JSON.stringify(actualRows) !== JSON.stringify(authoritativeFileRowsV8())) {
    throw new Error("v8-manifest-authoritative-closure");
  }
  for (const line of rows) {
    const [, relative, size, hash, reason] = line.split("\t"); const content = readFileSync(resolve(root, relative));
    if (!reason) throw new Error(`v8-manifest-reason:${relative}`);
    if (content.length !== Number(size) || sha256(content) !== hash) throw new Error(`v8-manifest-drift:${relative}`);
  }
...
function authorities() {
  ...
  for (const [key, authority] of [["A", "independent-database-reviewer"], ["B", "independent-qa-security-reviewer"]]) {
    const path = resolve(root, process.env[`B2C_000197_V8_REVIEW_${key}_PATH`] ?? "");
    exact(path, process.env[`B2C_000197_V8_REVIEW_${key}_SHA`]);
    const lines = readFileSync(path, "utf8").trimEnd().split("\n");
    if (lines.shift() !== "b2c-000197-preliminary-v8-independent-review-v1") throw new Error("v8-review-schema");
    const fields = new Map(lines.map((line) => { const at = line.indexOf("\t"); return [line.slice(0, at), line.slice(at + 1)]; }));
    for (const [name, value] of Object.entries({ formal_run_id: V8_RUN_ID, manifest_raw_sha256: manifest,
      handoff_raw_sha256: handoff, resource_authority_raw_sha256: expected.resource,
      executor_raw_sha256: sha256(readFileSync(new URL("./track-b2c-000197-preliminary-executor-v8.mjs", import.meta.url))),
      orchestrator_raw_sha256: sha256(readFileSync(new URL(import.meta.url))), reviewer_authority: authority, decision: "GO" })) {
      if (fields.get(name) !== value) throw new Error(`v8-review:${name}`);
    }
    result[key] = Object.fromEntries(fields);
  }
  const drainPath = resolve(root, process.env.B2C_000197_V8_OLD_WRITER_DRAIN_PATH ?? "");
  exact(drainPath, process.env.B2C_000197_V8_OLD_WRITER_DRAIN_SHA);
  const drainLines = readFileSync(drainPath, "utf8").trimEnd().split("\n");
  if (drainLines.shift() !== "b2c-000197-old-writer-drain-v8") throw new Error("v8-drain-schema");
  const drain = new Map(drainLines.map((line) => { const at = line.indexOf("\t"); return [line.slice(0, at), line.slice(at + 1)]; }));
  for (const [name, value] of Object.entries({ formal_run_id: V8_RUN_ID,
    ...
    if (drain.get(name) !== value) throw new Error(`v8-drain:${name}`);
  }
  result.drain_sha256 = process.env.B2C_000197_V8_OLD_WRITER_DRAIN_SHA;
  return result;
}

export function formalStaticV8(recorder) {
  const frozenEnv = childEnv({ B2C_000197_V8_STATIC_MODE: "frozen" });
  const frozenAllow = [...safeEnv(), { name: "B2C_000197_V8_STATIC_MODE", persist: "value" }];
  for (const [stage, cwd, args, count, env, envAllowlist, expectedNames = []] of [
    ["static-v8-evidence", root, ["--test-reporter=tap", "scripts/e2e/property-remediation/tests/b2c-000197-preliminary-executor-v8.spec.mjs"], 8, childEnv(), safeEnv()],
    ["static-v8-orchestrator", root, ["--test-reporter=tap", "scripts/e2e/property-remediation/tests/b2c-000197-preliminary-orchestrator-v8.spec.mjs"], 4, frozenEnv, frozenAllow],
    ["static-v8-capability", root, ["--test-reporter=tap", "scripts/e2e/property-remediation/tests/b2c-000197-capability-closure-v8.spec.mjs"], 3, childEnv(), safeEnv(), [
    ...
    ["static-v8-recursive-closure", root, ["--test-reporter=tap", "scripts/e2e/property-remediation/tests/b2c-000197-recursive-closure-v8.spec.mjs"], 3, childEnv(), safeEnv(), [
    ...
    ["static-v8-closure", root, ["--test-reporter=tap", "scripts/e2e/property-remediation/tests/b2c-000197-frozen-closure-v8.spec.mjs"], 3, childEnv(), safeEnv()],
    ["static-v8-contract", root, ["--test-reporter=tap", "scripts/e2e/property-remediation/tests/b2c-approval-index-forward-fix.spec.mjs"], 8, childEnv(), safeEnv()],
    ["static-v8-lifecycle", apiRoot, ["--test-reporter=tap", "--require", "ts-node/register",
    ...
    parser: (stdout) => parseTapSummaryV8(stdout, { expectedTests: count, expectedNames }) });
}

export function staticV8Candidate({ mode = "auto" } = {}) {
  inputs(); const frozen = existsSync(manifestPath) && existsSync(handoffPath);
  if (mode === "frozen") {
    if (!frozen) throw new Error("b2c-000197-v8-freeze-files-missing"); frozenManifest();
    return { status: "frozen-awaiting-independent-reviews", manifest_frozen: true, execution_authorized: false,
      live_execution: false, formal_run_id: V8_RUN_ID, fixture_run_id: fixtureRunId, runtime_v8: expected.runtime };
  }
  if (mode === "unfrozen") return { status: "unfrozen-v8-integrated", manifest_frozen: false,
    execution_authorized: false, live_execution: false, formal_run_id: V8_RUN_ID, fixture_run_id: fixtureRunId,
    runtime_v8: expected.runtime };
  return staticV8Candidate({ mode: frozen ? "frozen" : "unfrozen" });
}

export function executePreflightV8() {
  return executeWithEvidenceV8({ evidenceRoot: resolve(research, `b2c-000197-v8-complete-preflight-evidence-${V8_RUN_ID}`),
    operation: (recorder) => { inputs(); return V8_TARGETS.map((target) => { const identity = inspect(recorder, target);
      const state = snapshot(recorder, target, `preflight-${target.key}`); assertAbsentV8(state, target);
    ...
    successPayload: { scope: "c-and-d-v8-complete-read-only-preflight", execution_authorized: false,
      database_or_container_mutation_performed: false, resources_retained: ["c", "d"] } });
}

export function executeStaticGateV8() {
  return executeWithEvidenceV8({ evidenceRoot: resolve(research, `b2c-000197-v8-static-gate-evidence-${V8_RUN_ID}`),
    operation: (recorder) => { inputs(); frozenManifest(); formalStaticV8(recorder);
      return { formal_static_children: 7, capability_recorded: true, recursive_closure_recorded: true }; },
    successPayload: { scope: "v8-frozen-static-only", execution_authorized: false, live_execution: false,
      database_or_container_mutation_performed: false } });
}

export function executeFormalV8() {
  if (process.env.B2C_000197_PRELIMINARY_V8_RUN_ID !== V8_RUN_ID) throw new Error("v8-run-id-drift");
  return executeWithEvidenceV8({ evidenceRoot: resolve(research, `b2c-000197-formal-evidence-${V8_RUN_ID}`),
    operation: (recorder) => formalOperationV8(recorder),
    successPayload: { scope: "absent-path-c-and-d-preliminary-only", final_current: false, resources_retained: ["c", "d"] } });
}

export function formalOperationV8(recorder, overrides = {}) {
  const dependency = { inputs, authorities, formalStatic: formalStaticV8, inspect, snapshot,
    assertAbsent: assertAbsentV8, migrate, predicateMatrix, faults, approval, ...overrides };
  dependency.inputs(); const authority = dependency.authorities(); dependency.formalStatic(recorder);
  const preflight = V8_TARGETS.map((target) => { const identity = dependency.inspect(recorder, target);
  ...
  const results = V8_TARGETS.map((target) => ({ key: target.key,
  ...
  return { authority, preflight, results, approval: dependency.approval(recorder, V8_TARGETS[0]) };
}

if (fileURLToPath(import.meta.url) === resolve(process.argv[1] ?? "")) {
  if (process.env.B2C_000197_V8_PREFLIGHT === "1") process.stdout.write(`${JSON.stringify(executePreflightV8(), null, 2)}\n`);
  else if (process.env.B2C_000197_V8_STATIC_GATE === "1") process.stdout.write(`${JSON.stringify(executeStaticGateV8(), null, 2)}\n`);
  else if (process.env.B2C_000197_PRELIMINARY_V8_EXECUTE === "1") process.stdout.write(`${JSON.stringify(executeFormalV8(), null, 2)}\n`);
  else process.stdout.write(`${JSON.stringify(staticV8Candidate({ mode: process.env.B2C_000197_V8_STATIC_MODE ?? "auto" }), null, 2)}\n`);
}
