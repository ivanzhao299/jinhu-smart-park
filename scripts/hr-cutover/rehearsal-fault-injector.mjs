...
export const ALLOWED_REHEARSAL_FAULTS = Object.freeze([
  "VERIFY_FIXTURE_ROW_CHANGED",
  "REGISTERED_FILE_UNREADABLE"
]);

const DETECTOR_BY_FAULT = Object.freeze({
  VERIFY_FIXTURE_ROW_CHANGED: "RESTORE_FIXTURE_MISMATCH",
...
export function injectAllowlistedFault(options) {
  ...
  if (faultId === "VERIFY_FIXTURE_ROW_CHANGED") {
    assertCallback(options.mutateFixture, "mutateFixture");
    assertCallback(options.restoreFixture, "restoreFixture");
    assertCallback(options.detectFixture, "detectFixture");
    try {
      options.mutateFixture();
      try { options.detectFixture(); }
      catch (error) {
        if (error?.code === detectorCode || String(error?.message ?? "").includes(detectorCode) || String(error?.message ?? "").includes("mismatch")) detectorObserved = true;
        else detectorFailure = error;
      }
    } finally {
      try { options.restoreFixture(); } catch (error) { revertFailure = error; }
    }
  } else {
    ...
    try {
      ...
      try { options.detectFile(); }
        ...
        if (error?.code === detectorCode || String(error?.message ?? "").includes(detectorCode) || String(error?.message ?? "").includes("unreadable")) detectorObserved = true;
        else detectorFailure = error;
      }
    } finally {
