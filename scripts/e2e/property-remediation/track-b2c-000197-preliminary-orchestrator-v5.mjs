...
import {
  PhaseExecutionErrorV5, V5_RUN_ID, executeWithEvidenceV5, parseTapSummaryV5, runPhasedGateV5,
} from "./track-b2c-000197-preliminary-executor-v5.mjs";
...
const manifestPath = resolve(research, "b2c-000197-preliminary-v5-input-manifest-20260802.grammar");
const handoffPath = resolve(research, "b2c-000197-preliminary-executor-v5-review-handoff-20260802.md");
...
const expected = Object.freeze({ migration: "a9b98ca82aa4dafc16535085184df838880ef27801f7cd4b225e1ca1a15af059",
  ...
  preflightArtifact: "e6eea12c4ccf878d7c848da6771a1c850142ba44e1d191b469a3c420c8e5c4e2",
  preflightManifest: "381203fa10c484a02cbf054727cad3a425ac9554c0b174ddd575b0a542c58dc1",
  returnedDrain: "93fb2c36e3d44bfa32cb88e1a2c36489ae216371fa3335c3d18b0c702b58fa1a",
...
export const V5_TARGETS = Object.freeze([
...
function exact(path, hash) {
  if (!existsSync(path) || lstatSync(path).isSymbolicLink() || realpathSync(path) !== path
      || sha256(readFileSync(path)) !== hash) throw new Error(`b2c-000197-v5-input-drift:${path}`);
}

function inputs() {
  const prefix = readdirSync(resolve(root, "database/migrations")).filter((name) => name.startsWith("000197_"));
  if (prefix.length !== 1 || prefix[0] !== migrationFilename) throw new Error("b2c-000197-v5-prefix-drift");
  for (const [path, hash] of [
    [migrationPath, expected.migration],
    ...
    [resolve(research, `b2c-000197-v5-preflight-evidence-${V5_RUN_ID}/success-${V5_RUN_ID}.json`), expected.preflightArtifact],
    [resolve(research, `b2c-000197-v5-preflight-evidence-${V5_RUN_ID}/success-${V5_RUN_ID}.manifest.json`), expected.preflightManifest],
    [resolve(research, "b2c-000197-old-writer-drain-v4-returned-20260802c.grammar"), expected.returnedDrain],
  ]) exact(path, hash);
}
...
function inspect(recorder, target) {
  ...
  if (value !== wanted) throw new Error(`b2c-000197-v5-resource:${target.key}`);
  return value;
}
...
export function assertAbsentV5(value, target = { key: "unknown" }) {
  if (value.history_primary !== null || value.history_mirror !== null || value.approval_rows !== 0
    ...
    throw new Error(`b2c-000197-v5-not-dual-absent-empty:${target.key}`);
  }
  return value;
}
...
function history(recorder, target, status, error = null) {
  psql(recorder, target, `history-${target.key}-${status}`, `BEGIN;
    ...
    VALUES (${literal(migrationFilename)},${literal(expected.migration)},${literal(status)},clock_timestamp(),
      ...
      'b2c-000197-v5-executor','${V5_RUN_ID}') ON CONFLICT(filename) DO UPDATE SET checksum=EXCLUDED.checksum,
...
function migrate(recorder, target) {
  const before = snapshot(recorder, target, `before-${target.key}`); assertAbsentV5(before, target); history(recorder, target, "running");
  const result = psql(recorder, target, `migration-${target.key}`, readFileSync(migrationPath), { allowFailure: true });
  if (result.status !== 0 || result.signal || result.error) {
    history(recorder, target, "failed", `${result.stdout}${result.stderr}`); throw new PhaseExecutionErrorV5(`migration-${target.key}`, result);
  }
  ...
  if (after.approval_rows !== 0 || after.indexdef !== expected.newIndex || after.predicate !== expected.newPredicate
      || after.build_residue) throw new Error(`b2c-000197-v5-post-drift:${target.key}`);
  ...
  if (JSON.stringify(after) !== JSON.stringify(rerun)) throw new Error(`b2c-000197-v5-rerun:${target.key}`);
  return { before, after, rerun_exact: true };
}

function faults(recorder, target) {
  return failureInjectionCasesV5().map(({ name, boundary, prefix, assertion }) => {
    const before = snapshot(recorder, target, `fault-${target.key}-${name}-before`); const marker = `v5-injected-${name}`;
    ...
    if (result.status === 0 || !`${result.stdout}${result.stderr}`.includes(marker)
        || JSON.stringify(before) !== JSON.stringify(after)) throw new Error(`b2c-000197-v5-fault:${name}`);
...
function approval(recorder, target) {
  ...
  if (!password || !ip) throw new Error("b2c-000197-v5-secret-ip");
  ...
  return runPhasedGateV5(recorder, { phases: [
...
function frozenManifest() {
  ...
  if (lines.shift() !== "b2c-000197-preliminary-input-manifest-v5") throw new Error("v5-manifest-schema");
  for (const line of lines.filter((entry) => entry.startsWith("file\t"))) {
    ...
    if (content.length !== Number(size) || sha256(content) !== hash) throw new Error(`v5-manifest-drift:${relative}`);
  }
...
function authorities() {
  ...
  for (const [key, authority] of [["A", "independent-database-reviewer"], ["B", "independent-qa-security-reviewer"]]) {
    const path = resolve(root, process.env[`B2C_000197_V5_REVIEW_${key}_PATH`] ?? "");
    exact(path, process.env[`B2C_000197_V5_REVIEW_${key}_SHA`]);
    const lines = readFileSync(path, "utf8").trimEnd().split("\n");
    if (lines.shift() !== "b2c-000197-preliminary-v5-independent-review-v1") throw new Error("v5-review-schema");
    const fields = new Map(lines.map((line) => { const at = line.indexOf("\t"); return [line.slice(0, at), line.slice(at + 1)]; }));
    for (const [name, value] of Object.entries({ formal_run_id: V5_RUN_ID, manifest_raw_sha256: manifest,
      handoff_raw_sha256: handoff, resource_authority_raw_sha256: expected.resource,
      executor_raw_sha256: sha256(readFileSync(new URL("./track-b2c-000197-preliminary-executor-v5.mjs", import.meta.url))),
      orchestrator_raw_sha256: sha256(readFileSync(new URL(import.meta.url))), reviewer_authority: authority, decision: "GO" })) {
      if (fields.get(name) !== value) throw new Error(`v5-review:${name}`);
    }
    result[key] = Object.fromEntries(fields);
  }
  const drainPath = resolve(root, process.env.B2C_000197_V5_OLD_WRITER_DRAIN_PATH ?? "");
  exact(drainPath, process.env.B2C_000197_V5_OLD_WRITER_DRAIN_SHA);
  const drainLines = readFileSync(drainPath, "utf8").trimEnd().split("\n");
  if (drainLines.shift() !== "b2c-000197-old-writer-drain-v5") throw new Error("v5-drain-schema");
  const drain = new Map(drainLines.map((line) => { const at = line.indexOf("\t"); return [line.slice(0, at), line.slice(at + 1)]; }));
  for (const [name, value] of Object.entries({ formal_run_id: V5_RUN_ID,
    ...
    if (drain.get(name) !== value) throw new Error(`v5-drain:${name}`);
  }
  result.drain_sha256 = process.env.B2C_000197_V5_OLD_WRITER_DRAIN_SHA;
  return result;
}

function formalStatic(recorder) {
  for (const [stage, cwd, args, count] of [
    ["static-v5-evidence", root, ["--test-reporter=tap", "scripts/e2e/property-remediation/tests/b2c-000197-preliminary-executor-v5.spec.mjs"], 8],
    ["static-v5-orchestrator", root, ["--test-reporter=tap", "scripts/e2e/property-remediation/tests/b2c-000197-preliminary-orchestrator-v5.spec.mjs"], 4],
    ["static-v5-closure", root, ["--test-reporter=tap", "scripts/e2e/property-remediation/tests/b2c-000197-frozen-closure-v5.spec.mjs"], 3],
    ["static-v5-contract", root, ["--test-reporter=tap", "scripts/e2e/property-remediation/tests/b2c-approval-index-forward-fix.spec.mjs"], 8],
    ["static-v5-lifecycle", apiRoot, ["--test-reporter=tap", "--require", "ts-node/register",
      "src/modules/property-approvals/property-approval.port.pg-cli.spec.ts"], 4],
  ]) recorder.runChild({ stage, command: process.execPath, args, cwd, env: childEnv(), envAllowlist: safeEnv(),
    parser: (stdout) => parseTapSummaryV5(stdout, { expectedTests: count }) });
}

export function staticV5Candidate({ mode = "auto" } = {}) {
  inputs(); const frozen = existsSync(manifestPath) && existsSync(handoffPath);
  if (mode === "frozen") {
    if (!frozen) throw new Error("b2c-000197-v5-freeze-files-missing"); frozenManifest();
    return { status: "frozen-awaiting-independent-reviews", manifest_frozen: true, execution_authorized: false,
      live_execution: false, formal_run_id: V5_RUN_ID, fixture_run_id: fixtureRunId, runtime_v8: expected.runtime };
  }
  if (mode === "unfrozen") return { status: "unfrozen-v8-integrated", manifest_frozen: false,
    execution_authorized: false, live_execution: false, formal_run_id: V5_RUN_ID, fixture_run_id: fixtureRunId,
    runtime_v8: expected.runtime };
  return staticV5Candidate({ mode: frozen ? "frozen" : "unfrozen" });
}

export function executePreflightV5() {
  return executeWithEvidenceV5({ evidenceRoot: resolve(research, `b2c-000197-v5-preflight-evidence-${V5_RUN_ID}`),
    operation: (recorder) => { inputs(); return V5_TARGETS.map((target) => { const identity = inspect(recorder, target);
      const state = snapshot(recorder, target, `preflight-${target.key}`); assertAbsentV5(state, target);
      return { key: target.key, topology: target.topology, identity, state }; }); },
    successPayload: { scope: "c-and-d-v5-read-only-preflight", execution_authorized: false } });
}

export function executeFormalV5() {
  if (process.env.B2C_000197_PRELIMINARY_V5_RUN_ID !== V5_RUN_ID) throw new Error("v5-run-id-drift");
  return executeWithEvidenceV5({ evidenceRoot: resolve(research, `b2c-000197-formal-evidence-${V5_RUN_ID}`),
    operation: (recorder) => { inputs(); const authority = authorities(); formalStatic(recorder);
      const preflight = V5_TARGETS.map((target) => { const identity = inspect(recorder, target);
        const state = snapshot(recorder, target, `formal-preflight-${target.key}`); assertAbsentV5(state, target);
        return { identity, state }; });
      const results = V5_TARGETS.map((target) => ({ key: target.key, migration: migrate(recorder, target),
        failures: faults(recorder, target) }));
      return { authority, preflight, results, approval: approval(recorder, V5_TARGETS[0]) }; },
    successPayload: { scope: "absent-path-c-and-d-preliminary-only", final_current: false, resources_retained: ["c", "d"] } });
}

if (fileURLToPath(import.meta.url) === resolve(process.argv[1] ?? "")) {
  if (process.env.B2C_000197_V5_PREFLIGHT === "1") process.stdout.write(`${JSON.stringify(executePreflightV5(), null, 2)}\n`);
  else if (process.env.B2C_000197_PRELIMINARY_V5_EXECUTE === "1") process.stdout.write(`${JSON.stringify(executeFormalV5(), null, 2)}\n`);
  else process.stdout.write(`${JSON.stringify(staticV5Candidate({ mode: process.env.B2C_000197_V5_STATIC_MODE ?? "auto" }), null, 2)}\n`);
}
