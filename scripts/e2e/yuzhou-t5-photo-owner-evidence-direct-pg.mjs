...
`);
  ...
  assert.equal(psql(database, `SELECT count(*)||'|'||(SELECT count(*) FROM legacy_record_map WHERE batch_id=(SELECT id FROM migration_batch WHERE run_id='${runId}') AND is_active)||'|'||(SELECT count(*) FROM migration_check WHERE batch_id=(SELECT id FROM migration_batch WHERE run_id='${runId}') AND passed) FROM hr_legacy_t5_file_evidence;`), "2155|2155|2");
