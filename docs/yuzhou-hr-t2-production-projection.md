...
私有文件入口 `materialize-production-t2-decision-candidates.mjs --config <private-config>` 已实现：核对当前提交、工件文件字节、来源清单、T2 staging、已有类型/状态字典后调用组装器。已发布基线 `ee7a730c` 的真实运行生成 1,163 条候选（4 条可新增、1,159 条隔离），据此定位旧暂存格式缺失派生字段及中文协议标记不兼容。2026-09-06 本轮修复后的只读开发态组装核对得到 1,155 条可新增、8 条缺父合同隔离，原始行对象与文件哈希不变；读取约 5.2 MB，没有写业务数据或覆盖旧候选。该开发态结果没有重新签发生产 C/S/M 回执。**修复版仍须经过审查、合并发布、当前身份绑定与候选封存；生产导入、API/页面回读、业务签署及完整 T2 验收仍未完成。** `resolved` 不是审批证明，`READY_FOR_REVIEW` 不是可生产执行。

旧 staging 缺少全部 5 项派生字段时，本层先验证原始行哈希，再调用与 transform 共用的纯语义函数，在内存中补充日期推导期限、直接续签次数及三个决定标记；不修改源行、源文件、源键或来源哈希。只缺一部分字段，或现有派生值/标记与原日期及 `continuetimes` 不符，仍明确隔离，不自动修正。单条自报哈希不是来源真实性或审批证明。

期限保留原算法的首尾月份边界计数：年月差加上“结束日不早于开始日”时的一月，不改称普通整月时长。空日期边界保持空期限，空续签次数保留来源空值并按已有规则投影为零；非法日历、倒序范围和超出 PostgreSQL int4 的次数仍拒绝。含糊的旧期限/年数只存快照，单次签署日期不冒充签署历史。

未知/null 协议标记保持明确错误，不能为使导入通过而自动改成 false；由后续候选生成器记入差异/待处理台账。投影函数不自行删除、忽略或批准隔离行。

协议标记支持旧库精确中文 `是/否`，以及原有 boolean、数字 `1/0`、字符串 `"1"/"0"`；不做宽泛真假转换，不把空值、未知值或带空格变体当成否。原始编码及哈希保持不变。这里的全链测试指现代投影→候选→私有工件，不包括旧隔离 SQL loader：旧 loader 的 `='1'` 标记转换及单次日期填入首次/末次签署仍与现代语义不同，本轮不改动，也不得复用为生产写入途径。若后续仍使用旧 loader，应独立修复并验证，不可把本轮结果视为该路径已通过。

## 验证入口

`pnpm test:e2e:yuzhou-production-import-t2-artifact` 同时执行原有来源工件测试与新字段转换测试，沿用当前 PR CI 入口。新测试使用合成数据；目标模型校验不是数据库、API 或前端往返证明。

早期字段测试曾通过显式指定本地 PostgreSQL 容器，在 `BEGIN READ ONLY` 内执行固定字面量转换，证明金额和时间类型转换后的字段/哈希一致；不读取业务表，不写记录。本轮完整 T2 入口为 60 项通过、1 项可选 PostgreSQL 检查跳过，另有原转换脚本语义与映射依赖合同通过。未指定容器时该组明确跳过，不冒充本轮数据库验证。真实 writer 装载、关联、回读与回滚尚未执行。
