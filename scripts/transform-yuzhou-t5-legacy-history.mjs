...
const sourceTableFor={person_core:"person"};
...
for(const [name,count] of Object.entries(expected)){
  ...
  const rows=source.map(row=>{const value=key(row);if(value===null||value===undefined||String(value)==="")throw Error(`${name} missing stable key`);if(seen.has(String(value)))throw Error(`${name} duplicate stable key`);seen.add(String(value));
    ...
    return identity(sourceTableFor[name]??name,value,row,{domain:domainFor[name],employeeCode:employeeCodeFor[name]?String(row[employeeCodeFor[name]]??"").trim():null});
  });
  const file=`${name}.jsonl`; domains[name]={sourceObject:`dbo.${name==="photo"?"person.photo":sourceTableFor[name]??name}`,rows:rows.length,file,fileSha256:write(file,rows),objectStatus:name==="jch_1"?"absent":rows.length===0?"empty":"present"};
}
