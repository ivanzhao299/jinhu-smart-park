...
| #271 后续范围 | 后端审批、权限、幂等、审计、终态约束尚未完整的动作继续关闭，不得通过前端或配置绕开 | API / Web / Release | Issue/PR 的后续范围说明；高风险 manifest / capability fail-closed 证据 | 观察中若发现入口可见或可执行，记 P0/P1 stopship | `human_uat.no_open_stopship_defects`、`computed_contract.required_true_fields_for_ready_for_production_deploy`、`computed_contract.required_true_fields_for_production_ready`、`rollout_ops.all_high_risk_flags_match_freeze` | follow_up_fail_closed |
